...
try {
    ...
    if (gle && gle instanceof WebGLRenderingContext)
        ggl = true
} catch (e) {}
ggl = false;
if (!TouchEvent)
...
try {
    if (performance) {
        ...
        timeObj = performance;
        if (testing)
            console.log("timeObj set to performance")
    }
} catch (e) {}
var lfsx;
var lfsy;
...
function RectanglePacker(width, height, padding) {
    ...
    this.reset = function(width, height, padding) {
        while (this.mInsertedRectangles.length > 0)
            this.freeRectangle(this.mInsertedRectangles.pop());
        while (this.mFreeAreas.length > 0)
    ...
    this.packRectangles = function(sort) {
        if (sort)
            this.mInsertList.sort(numsort_width);
        while (this.mInsertList.length > 0) {
...
ctx.putImageData(map, 0, 0);
sadd = elem.toDataURL();
if (sadg.length > 32 && sadu.length > 32 && sadd.length > 32)
...
function mkBtn(elemid, prefix, iw, ih) {
    ...
    if (elem.style)
        if (elem.style.position) {
            if ((elem.style.position + "").toLowerCase() == "absolute")
                str = false;
            if ((elem.style.position + "").toLowerCase() == "fixed")
                str = false
        }
    if (str)
...
function hmos() {
    ...
    for (var i = mos.length - 1; i >= 0; i--) {
        ...
        if (wa)
            if (o.disabled)
                o.upi.style.opacity = 0;
            else
                o.upi.style.opacity = o.mof;
        wa = false;
        if (o.md) {
            if (o.mdf != 1) {
                dstf = true;
        ...
        if (wa)
            if (o.disabled)
                o.downi.style.opacity = 0;
            else
                o.downi.style.opacity = o.mdf
    }
    if (!dstf)
        whmos = false
}
function makeTextBtn(s, bh, ts, br, bgm) {
    ...
    st.lineHeight = bh + "px";
    if (!is_mobile)
        st.boxShadow = "0px 3px 20px rgba(0,0,0, .75)";
        ...
        try {
            if (o.src)
                if ((o.src + "").indexOf("mindscape.xyz") >= 0) {
                    ...
                    if ((o.src + "").indexOf("android") >= 0)
                        div.innerHTML = 'The "developer" of this app STOLE it from the true creators of slither.io. <a href="https://play.google.com/store/apps/details?id=air.com.hypah.io.slither">Tap here to download the real game!</a>';
                    else
            ...
            if (cluo.ptms.length > 0) {
                ...
                for (j = cluo.ptms.length - 1; j >= 0; j--)
                    if (cluo.ptms[j] < ptm)
                        ptm = cluo.ptms[j];
                if (testing)
                    console.log("cluster " + k + "   ping time: " + ptm + "   old ping time: " + optm);
                if (ptm < bcptm) {
                    bcptm = ptm;
                    bcluo = cluo
                }
...
function loginFade() {
    ...
    if (login_fr >= 1) {
        ...
        if (ggl)
            app.view.style.opacity = login_fr;
...
o.elem.onclick = function() {
    if (building_skin)
        if (!ending_build_skin) {
            ...
            var aa = "";
            if (build_segments.length > 0)
                aa = getBuildSkinData(false);
            else {
                var taa = "";
                try {
    ...
    if (!save_btn.disabled) {
        ...
        if (protocol_version >= 5) {
            ...
            for (var i = 0; i < msg.length; i++)
                ba[i + 2] = msg.charCodeAt(i);
            ws.send(ba)
        } else {
            var ba = new Uint8Array(1 + msg.length);
            ba[0] = 118;
            for (var i = 0; i < msg.length; i++)
                ba[i + 1] = msg.charCodeAt(i);
            ws.send(ba)
        }
...
function setMinimapSize(sz, force) {
    ...
    if (rad != mmrad || force) {
        ...
        if (real_sid > 0)
            if (!team_mode)
                sid_tf.textContent = "server " + real_sid;
            else
                sid_tf.textContent = "";
        else
            sid_tf.textContent = "";
...
function setSkin(o, cv, ca) {
    ...
    if (!o.cusk) {
        ...
        if (cv == 9)
            ...
            rbcs = [0, 1, 2, 3, 4, 5, 6, 7, 8];
        else if (cv == 16)
            rbcs = [15, 15, 15, 15, 15, 15, 15, 4, 4, 4, 4, 4, 4, 4];
        else if (cv == 17)
            rbcs = [9, 9, 9, 9, 9, 9, 9, 16, 16, 16, 16, 16, 16, 16];
        else if (cv == 18)
            rbcs = [7, 7, 7, 7, 7, 7, 7, 9, 9, 9, 9, 9, 9, 9];
        else if (cv == 19)
            rbcs = [9];
        else if (cv == 20)
            rbcs = [3, 3, 3, 3, 3, 0, 0, 0, 0, 0];
        else if (cv == 21)
            ...
            rbcs = [21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 22, 22, 22, 22, 22, 22, 22, 22, 22];
        else if (cv == 26)
            rbcs = [24];
        else if (cv == 27)
            rbcs = [25];
        else if (cv == 28)
            rbcs = [18, 18, 18, 18, 18, 18, 18, 25, 25, 25, 25, 25, 25, 25, 7, 7, 7, 7, 7, 7, 7];
        else if (cv == 29)
            rbcs = [11, 11, 4, 11, 11, 11, 11, 4, 11, 11];
        else if (cv == 30)
            rbcs = [10, 10, 19, 20, 10, 10, 20, 19];
        else if (cv == 31)
            rbcs = [10, 10];
        else if (cv == 32)
            rbcs = [20, 20];
            ...
            rbcs = [7, 7, 9, 13, 13, 9, 16, 16, 9, 12, 12, 9, 7, 7, 9, 16, 16, 9];
        else if (cv == 35)
            rbcs = [7, 7, 9, 9, 6, 6, 9, 9];
        else if (cv == 36)
            rbcs = [16, 16, 9, 9, 15, 15, 9, 9];
        else if (cv == 37)
            rbcs = [22];
        else if (cv == 38)
            rbcs = [18];
            ...
            rbcs = [2, 2, 2, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 3, 3, 5, 5, 5, 5, 5, 5, 5, 5, 7, 7, 7, 7, 7, 7, 7, 7];
        else if (cv == 43)
            rbcs = [28];
        else if (cv == 44)
            rbcs = [29];
        else if (cv == 45)
            rbcs = [7, 7, 7, 9, 9, 9, 9, 9, 9, 9, 9, 7, 7, 7];
        else if (cv == 46)
            rbcs = [7];
        else if (cv == 47)
            rbcs = [16, 16, 16, 18, 18, 18, 18, 18, 18, 18, 18, 18, 7, 7, 7, 7, 7, 7, 7, 7, 16, 16, 16, 16];
        else if (cv == 48)
            rbcs = [7];
        else if (cv == 49)
            rbcs = [23, 23, 23, 23, 23, 9, 9, 9, 9, 9, 9, 9, 9, 23, 23];
        else if (cv == 50)
            rbcs = [18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 16, 16, 16, 16, 16, 16, 16, 16, 7, 7, 7, 7, 7, 7, 7, 7];
        else if (cv == 51)
            rbcs = [7, 7, 7, 9, 9, 16, 16, 16, 16, 16, 16, 9, 9];
        else if (cv == 52)
            rbcs = [7, 7, 7, 7, 18, 18, 18, 18, 18, 18, 18, 18, 18, 7, 7, 7, 7, 7];
        else if (cv == 53)
            rbcs = [30];
        else if (cv == 54)
            rbcs = [31];
            ...
            rbcs = [30, 30, 30, 30, 30, 30, 35, 35, 35, 35, 35, 35, 33, 33, 33, 33, 33, 33, 31, 31, 31, 31, 31, 31, 32, 32, 32, 32, 32, 32, 34, 34, 34, 34, 34, 34];
        else if (cv == 62)
            rbcs = [17, 17, 17, 17, 17, 39, 39, 39, 39, 39];
        else if (cv == 63)
            rbcs = [7, 7, 7, 11, 11, 11];
        else if (cv == 64)
            rbcs = [16, 16, 11, 11];
        else if (cv == 65)
            rbcs = [4, 4, 4, 4, 9, 9, 9, 9];
        else
            cv = cv % 9;
        if (ggl)
            if (!rbcs)
...
function newSlither(id, xx, yy, cv, ang, pts, msl, custom_skin_uint8) {
    ...
    o.dead_amt = 0;
    o.alive_amt = 0;
    if (team_mode && slither && cv == slither.cv)
        slithers.push(o);
    else
...
function newPrey(id, xx, yy, rad, cv, dir, wang, ang, speed) {
    ...
    if (!ggl) {
        ...
        if (pr.cv2 < 0)
            pr.cv2 = 0;
        if (pr.cv2 >= pci.pr_imgs.length)
            pr.cv2 = pci.pr_imgs.length - 1
    }
    if (ggl) {
        var pri;
        var gpri;
...
for (p = 0; p < l; p += 4) {
    ...
    if (v <= 3)
        imgd[p + 3] = Math.max(0, Math.min(255, Math.round(v / 3 * 255)));
    else
        imgd[p + 3] = 255
}
ctx.putImageData(map, 0, 0);
...
for (var i = 0; i < rrs.length; i++) {
    ...
    for (var j = 0; j < jk; j++) {
        ...
        if (iioc && testing)
            if (kmcs.length >= 1)
                kmcs.push(kmcs[0]);
            else if (per_color_imgs.length >= 2)
...
var oef = function() {
    ...
    if (entering_code || ending_enter_code) {
        ...
        for (var i = 0; i < etcods.length; i++) {
            var v = etcods[i].v;
            if (etcdis[v].loaded)
                ctx.drawImage(etcdis[v].ii, i * 47, 0, 54, 67)
        }
        if (etcdis[11].loaded) {
    ...
    if (choosing_skin) {
        for (var i = slithers.length - 1; i >= 0; i--) {
            ...
            for (var j = o.pts.length - 1; j >= 0; j--)
                o.pts[j].yy = grd / 2 + Math.cos(j / 4 + fr / 19) * 15 * (1 - j / o.pts.length);
    ...
    if (connected) {
        ...
        if (vfrb > 0)
            if (flx_tg > 0) {
                ...
                for (qq = 1; qq <= ki; qq++) {
                    if (qq == ki)
                        flux_grd = flux_grds[flux_grd_pos];
                    flux_grds[flux_grd_pos] = real_flux_grd;
                    flux_grd_pos++;
                    if (flux_grd_pos >= flxc)
                        flux_grd_pos = 0
                }
            } else if (flx_tg == 0)
                flx_tg = -1;
        ...
        if (team_mode) {
            ...
            if (team1_eo.tg > 0 || team2_eo.tg > 0) {
                ...
                trf(kamala_ii, "scale(.5, .5) translateY(" + 2 * -(h2 - 22) + "px)")
            }
        }
    }
    if (slither != null)
            ...
            if (console && console.log) {
                ...
                for (var i = 0; i < rdpspc.length; i++) {
                    if (rdpspc[i] >= 0)
                        total += rdpspc[i];
                    if (pkpspc[i] >= 0)
                        total_packets += pkpspc[i]
                }
                for (var i = 0; i < rdpspc.length; i++)
                    if (rdpspc[i] >= 1 || pkpspc[i] >= 1)
                        s.push(String.fromCharCode(i) + ": " + pkpspc[i] + " packets, " + rdpspc[i] + " bytes (" + Math.round(rdpspc[i] / total * 1E3) / 10 + "%)");
        ...
        if (fo.eaten) {
            ...
            if (fo.fr != 1) {
                ...
                if (fo.fr >= 1) {
                    ...
                    fo.rad += (.5 * (1 - Math.cos(Math.PI * fo.rad)) - fo.rad) * .66
                }
                fo.lrrad = fo.rad
            }
            fo.rx = fo.xx;
...
window.gotServerVersion = function(server_version) {
    random_id = "";
    for (var i = 0; i < 27; i++)
        random_id += String.fromCharCode(65 + (Math.random() < .5 ? 0 : 32) + alpha_chars.charCodeAt(i) + Math.floor(Math.random() * 26));
...
var redraw = function() {
    ...
    if (ggl)
        for (var i = bgees.length - 1; i >= 0; i--) {
            ...
            bgee.tilePosition.y = hh / 2 / csc + bgy2 * bgh2 * o.sc
        }
    else if (bgp2) {
        ctx.save();
        ctx.fillStyle = bgp2;
                            ...
                            for (j = bp - 1; j >= 0; j--)
                                if (pbu[j] == 2) {
                                    ...
                                    if (j < 4) {
                                        ...
                                        ctx.drawImage(kfmc, -glsz, -glsz, 2 * glsz, 2 * glsz);
                                    ctx.restore()
                                }
                        } else
                            for (j = bp - 1; j >= 0; j--)
                        ...
                        for (j = fj; j >= 0; j--) {
                            ...
                            if (pbu[j] >= 1) {
                                ...
                                if (o.dead) {
                                    ...
                                    sd.y = sg.y;
                                    sd.visible = true;
                                    sd.alpha = d2 * falf * (.6 + .4 * Math.cos(j / 4 - 15 * o.dead_amt))
                                }
                            } else if (sgsv[j]) {
                                sg.visible = false;
                                sh.visible = false;
                        ...
                        for (j = bp - 1; j >= 0; j--)
                            if (pbu[j] >= 1) {
                                ...
                                if (j < 4) {
                                    ...
                                    ctx.drawImage(pci.kmcs[k], -gsc * lsz, -gsc * lsz, gsc * 2 * lsz, gsc * 2 * lsz);
                                if (fdhc && j < fdl) {
                                    fdam = 1 - j / fdl;
                                ...
                                for (j = bp - 1; j >= 0; j--)
                                    if (pbu[j] == 2) {
                                        ...
                                        if (j < 4) {
                                            ...
                                            ctx.drawImage(per_color_imgs[rbcs[j % rbl]].kfmc, -gsc * glsz, -gsc * glsz, gsc * 2 * glsz, gsc * 2 * glsz);
                                        ctx.restore()
                                    }
                                ctx.restore()
                            }
                    ...
                    for (j = bp - 1; j >= 0; j--)
                        if (pbu[j] >= 1) {
                            ...
                            if (j < 4) {
                                ...
                                ctx.drawImage(kmcs[k], -gsc * lsz, -gsc * lsz, gsc * 2 * lsz, gsc * 2 * lsz);
                            ctx.restore()
                        }
                    if (o.tsp > o.fsp)
                        if (high_quality || gla > 0) {
                            ...
                            for (j = bp - 1; j >= 0; j--)
                                if (pbu[j] == 2) {
                                    ...
                                    if (j < 4) {
                                        ...
                                        ctx.drawImage(opci.kfmc, -gsc * glsz, -gsc * glsz, gsc * 2 * glsz, gsc * 2 * glsz);
                                    ctx.restore()
                                }
                            ctx.restore()
                        }
                ...
                if (o.antenna) {
                    ...
                    if (bp >= 2 && (ax >= apx1 && ay >= apy1 && ax <= apx2 && ay <= apy2)) {
                        ...
                        for (var j = 1; j <= fj; j++) {
                            ...
                            tx = o.atx[j] - xx;
                            ty = o.aty[j] - yy;
                            if (tx >= -4 && ty >= -4 && tx < 4 && ty < 4)
                                ang = at2lt[ty * 32 + 128 << 8 | tx * 32 + 128];
                            else if (tx >= -8 && ty >= -8 && tx < 8 && ty < 8)
                                ang = at2lt[ty * 16 + 128 << 8 | tx * 16 + 128];
                            else if (tx >= -16 && ty >= -16 && tx < 16 && ty < 16)
                                ang = at2lt[ty * 8 + 128 << 8 | tx * 8 + 128];
                            else if (tx >= -127 && ty >= -127 && tx < 127 && ty < 127)
                                ang = at2lt[ty + 128 << 8 | tx + 128];
                            else
                        ...
                        if (o.abrot) {
                            ...
                            ctx.drawImage(o.bulb, (o.atx[fj] - view_xx + o.blbx * o.bsc * o.sc) * gsc, (o.aty[fj] - view_yy + o.blby * o.bsc * o.sc) * gsc, o.blbw * o.bsc * m, o.blbh * o.bsc * m);
                        if (o.apbs) {
                            ctx.globalAlpha = .5 * a;
                            ctx.lineWidth = 3 * m;
                            ctx.stroke();
                        ...
                        for (j = bp - 1; j >= 0; j--)
                            if (pbu[j] == 2) {
                                ...
                                if (j < 4) {
                                    ...
                                    ctx.drawImage(kdmc, -dsz, -dsz, 2 * dsz, 2 * dsz);
                                ctx.restore()
                            }
                        ctx.restore()
                    }
            ...
            if (o.one_eye) {
                ...
                if (ggl) {
                    ...
                    if (o.dead_amt == 0)
                        ...
                        o.ebi.alpha = Math.sqrt(1 - o.dead_amt);
                    o.ebi.scale.x = o.ebi.scale.y = esz / 64
                } else {
                    if (o.dead_amt == 0)
                        ctx.globalAlpha = 1;
                    else
                        ctx.globalAlpha = Math.sqrt(1 - o.dead_amt);
                ...
                if (ggl) {
                    ...
                    ctx.drawImage(o.epi, 0, 0, o.epiw, o.epih, mww2 + (ex + hx - esz / 2 - view_xx) * gsc, mhh2 + (ey + hy - esz / 2 - view_yy) * gsc, esz * gsc, esz * gsc)
            } else {
                if (!o.eac)
                    drawEyes(o, ctx, fang, ssc, 1, 1);
                if (o.accessory != -1) {
                    var ed = o.ed * ssc;
                    var j = o.accessory;
                    if (j >= 0 && j < a_ct) {
    ...
    if (team_mode) {
        ...
        for (var i = slithers.length - 1; i >= 0; i--) {
            ...
            if (o.iiv)
                if (!o.eac)
                    drawEyes(o, ctx, o.ehang, o.sc, .4, .75)
        }
        ctx.restore()
    }
    if (ggl) {
            ...
            for (i = 1; i <= 2; i++) {
                ...
                if (i == 1)
                    for (j = 2E3; j >= -2E3; j -= 100) {
                        ...
                        yy = grd + Math.sin(view_ang + j / k) * (flux_grd + 4E3);
                        ctx.lineTo(mww2 + (xx - view_xx) * gsc, mhh2 + (yy - view_yy) * gsc)
                    }
                if (i == 1) {
                    ctx.closePath();
                ...
                if (playing) {
                    ...
                    if (team_mode) {
                        ...
                        if (team1_score > 0 || team2_score > 0)
                            f_htm += '<span style="font-size: 14px;"><span style="opacity: .4;">' + t1str + ': </span><span style="opacity: .9; font-weight: bold; color: #FF4040;">' + team1_score + "</span></span>";
                        f_htm += "<br><br>";
                        if (team1_score > 0 || team2_score > 0)
                            f_htm += '<span style="font-size: 14px;"><span style="opacity: .4;">' + t2str + ': </span><span style="opacity: .9; font-weight: bold; color: #8090FF;">' + team2_score + "</span></span>";
                        f_htm += "<br><br>";
                        ...
                        f_htm += '<br><br><span style="font-size: 14px;"><span style="opacity: .4;">' + t3str + ': </span><span style="opacity: .9; font-weight: bold; color:' + other_cs + ';">' + slither.kill_count + "</span></span>";
                        ...
                        f_htm += '<BR><span style="opacity: .3;">' + rstr + ': </span><span style="opacity: .35;">' + rank + '</span><span style="opacity: .3;"> ' + ofstr + ' </span><span style="opacity: .35;">' + slither_count + "</span>"
                    }
                    lbf.innerHTML = f_htm
                }
    ctx.restore()
...
function drawEyes(o, ctx, fang, ssc, a, a2) {
    ...
    if (ggl) {
        ...
        if (o.dead_amt == 0)
            lefteye.alpha = o.eca * a;
        else
            lefteye.alpha = o.eca * Math.sqrt(1 - o.dead_amt) * a;
        ...
        if (o.dead_amt == 0)
            ctx.globalAlpha = o.eca * a;
        else
            ctx.globalAlpha = o.eca * Math.sqrt(1 - o.dead_amt) * a;
        ctx.beginPath();
    ...
    if (ggl) {
        ...
        if (o.dead_amt == 0)
            leftpupil.alpha = o.ppa * a2;
        else
            leftpupil.alpha = o.ppa * Math.sqrt(1 - o.dead_amt) * a2;
        leftpupil.x = ex + hx - view_xx;
        leftpupil.y = ey + hy - view_yy;
        leftpupil.scale.x = leftpupil.scale.y = o.pr * ssc / 21
    } else {
        if (o.dead_amt == 0)
            ctx.globalAlpha = o.ppa * a2;
        else
            ctx.globalAlpha = o.ppa * Math.sqrt(1 - o.dead_amt) * a2;
        ctx.fillStyle = o.ppc;
        ctx.beginPath();
    ...
    if (ggl) {
        ...
        if (o.dead_amt == 0)
            righteye.alpha = o.eca * a;
        else
            righteye.alpha = o.eca * Math.sqrt(1 - o.dead_amt) * a;
    ...
    if (ggl) {
        ...
        if (o.dead_amt == 0)
            rightpupil.alpha = o.ppa * a2;
        else
            rightpupil.alpha = o.ppa * Math.sqrt(1 - o.dead_amt) * a2;
...
for (var i = 0; i < tpfa.length; i++)
    tpfa[i] = Math.random() * 32;
var pfd;
if (testing) {
...
function connect() {
    if (waiting_for_sos)
        if (sos_ready_after_mtm >= 0 && timeObj.now() > sos_ready_after_mtm)
            waiting_for_sos = false;
        else if (sos_loaded_at_mtm >= 0 && timeObj.now() - sos_loaded_at_mtm > 7E3)
            waiting_for_sos = false;
        else
            return;
    resetGame();
    ...
    ws.onmessage = function(e) {
        ...
        if (want_seq) {
            ...
            if (seq - 1 != lseq)
                if (seq != 0)
                    if (testing)
                        console.log("sequence error! " + seq + " != " + lseq);
            lseq = seq
        } else if (want_etm_s) {
            lptm = cptm;
            cptm = timeObj.now();
    ...
    window.gotPacket = function(a) {
        ...
        if (cmd == "a") {
            ...
            for (var i = 0; i < flxc; i++)
                flux_grds[i] = flux_grd;
            team_mode = false;
            if (m < alen) {
                var game_mode = a[m];
                m++;
            ...
            var o;
            var id = a[m] << 8 | a[m + 1];
            if (protocol_version >= 14 && (cmd == "d" || cmd == "7" || dlen <= 2 && (cmd == "e" || cmd == "E" || cmd == "3" || cmd == "4" || cmd == "5")))
                o = slither;
            else {
                var id = a[m] << 8 | a[m + 1];
                m += 2;
            ...
            if (testing) {
                ...
                if (xx != o.lpo_xx || yy != o.lpo_yy) {
                    ...
                    if (Math.abs(dx) > 1 || Math.abs(dy) > 1) {
                        ...
                        if (o == slither)
                            console.log("invalid new point delta for self: " + dx + " " + dy + " (" + o.lpo_cmd + ", prev " + o.lpo_prev_cmd + ") (sct " + o.sct + " " + sct_c + ") " + o.lpo_dd);
                        else
                            console.log("invalid new point delta for " + o.nk + ": " + dx + " " + dy + " (" + o.lpo_cmd + ", prev " + o.lpo_prev_cmd + ") (sct " + o.sct + " " + sct_c + ") " + o.lpo_dd)
                    }
                }
            }
            ...
            if (protocol_version >= 15)
                if (cmd == "G" || cmd == "N" || cmd == "=" && dlen == 6 || cmd == "+" && dlen == 9)
                    o = slither;
                else {
                    var id = a[m] << 8 | a[m + 1];
                    m += 2;
            ...
            else if (cmd == "g" && dlen == 4 || cmd == "G" && dlen == 2 || cmd == "n" && dlen == 7 || cmd == "N" && dlen == 5)
                o = slither;
            else {
                var id = a[m] << 8 | a[m + 1];
                m += 2;
            ...
            if (slither_count > biggest_slither_count)
            ...
            if (a[m] == 2) {
                ...
                if (lang == "de")
                    fstr = "Deine endg\u00fcltige L\u00e4nge war";
                else if (lang == "fr")
                    fstr = "Votre longueur finale \u00e9tait de";
                else if (lang == "pt")
                    fstr = "Seu comprimento final foi de";
                var exc = "";
                if (final_score > 1E3)
                    exc = "!";
            ...
            if (victory_score > 0) {
                var h = "";
                if (victory_message.length > 0)
                    h += "<span style='font-size:17px;'><b><i><span style='opacity: .5;'>&quot;</span>" + victory_message + "<span style='opacity: .5;'>&quot;</span></i></b></span><BR><div style='height: 5px;'></div>";
                if (victory_nick.length > 0) {
                    if (victory_message.length > 0)
                        h += "<i><span style='opacity: .5;'>- </span><span style='opacity: .75;'><b>" + victory_nick + "</b></span><span style='opacity: .5;'>, today's longest</span></i>";
                    else
                        h = "<i><span style='opacity: .5;'>Today's longest was </span><span style='opacity: .75;'><b>" + victory_nick + "</b></span></i>";
                    ...
                    h += "<i><span style='opacity: .5;'>Today's longest: </span><span style='opacity: .75;'><b>" + victory_score + "</b></span></i>";
                vcm.innerHTML = h
            }
        } else if (cmd == "p") {
            if (testing)
                console.log("ping: " + Math.round(timeObj.now() - lpstm));
            wfpr = false;
            if (lagging) {
                etm *= lag_mult;
                lagging = false
            ...
            if (alen == 9) {
                ...
                m += 4;
                team_score_pos++;
                if (team_score_pos >= team1_scores.length)
                    team_score_pos = 0;
            ...
            var o = os["s" + id];
            if (o)
                o.kill_count = a[m] << 16 | a[m + 1] << 8 | a[m + 2]
        } else if (cmd == "z") {
            ...
            for (var i = 0; i < a.length; i++)
                dd.push(("00" + (a[i] + 0).toString(16)).substr(-2));
            console.log(dd.join(" "))
        }
    }
    ;
...
function deleteLastCodeDigit() {
    if (entering_code)
        if (!checking_code)
            if (etcods.length > 0) {
                if (etcods[etcods.length - 1].v == 10)
...
function reposSkinStuff() {
    ...
    if (building_skin)
        skodiv.style.top = Math.round(hh / 2 + 120 + 94 * skoboym) + "px";
    else if (selecting_cosmetic)
        skodiv.style.top = Math.round(hh / 2 + 120 + 94 * secosoym) + "px";
    else
    ...
    if (!playing)
        if (dead_mtm == -1) {
            ...
            if (!ever_showed_entercode) {
                ...
                for (var i = etcobs.length - 1; i >= 0; i--)
                    if (i == 10)
                        etcobs[i].ii.src = "./s/codedel.png";
                    else
                        etcobs[i].ii.src = "./s/code" + i + ".png";
...
function showLogo(done) {
    ...
    if (lts[lts.length - 1].mwig == 0 && lga == 1 && lgss == 1 && ncka == 1) {
        ...
        if (done || ncka != 1) {
            ...
            if (ncka >= 1)
                ncka = 1;
            nick_holder.style.opacity = Math.min(1, ncka * 6);
            if (!is_mobile)
                smh.style.opacity = Math.max(0, Math.min(1, (ncka - .05) * 5));
            if (ncka >= .01)
                playh.style.opacity = Math.min(1, (ncka - .01) * 5)
        }
                                ...
                                for (var j = clus.length - 1; j >= 0; j--) {
                                    ...
                                    if (cluo)
                                        if (cluo.ps == this) {
                                            ...
                                            if (testing)
                                                console.log(" ping time for cluster " + j + ": " + ptm);
                                            if (waiting_for_sos)
                                                if (sos_ready_after_mtm == -1)
                                                    sos_ready_after_mtm = timeObj.now() + 2667;
                                            if (testing)
                                                if (cluo.ptms.length == 0)
                                                    console.log("first ping response for " + j + " at " + timeObj.now());
                                            cluo.ptms.push(ptm);
...
function reposEnterCode() {
    ...
    etco_sp_ii.style.left = Math.round(ww / 2 - 100 / 2) + "px";
    ...
    for (var i = etcobs.length - 1; i >= 0; i--) {
        ...
        o.ii.style.top = Math.round(hh / 2 + 100 * sc - 135 / 2 + yy * sc * (135 + 20)) + "px";
...
function showServers() {
    ...
    if (bso)
        csid = bso.sid;
    for (var i = svl_divs.length - 1; i >= 0; i--)
        svli.removeChild(svl_divs[i]);
    svl_divs = [];
    svlo = null;
    recalcPtms();
    if (csid == -1)
    ...
    for (var i = 0; i < sos.length; i++)
        if (sos[i].active || sos[i] == bso || sos[i] == fbso)
            tsos.push(sos[i]);
    svli.className = "nsi";
    var dx = -1;
    var dy = 0;
    ...
    for (var i = 0; i < tsos.length; i++) {
        ...
        if (k == ping_rs.length - 1) {
            ...
            var gg = ping_gs[k] + (ping_gs[k + 1] - ping_gs[k]) * v;
...
window.partycityia = function() {
    ...
    if (testing)
        ...
        a.href = "https://www.partycity.com/slither-party-supplies?extcmp=dsp|banner|slither.io|slitherpartysupplies";
    a.target = "_blank";
    a.appendChild(sn);
    v.appendChild(a);
    window.ut_sn = sn;
...
if (is_ios || is_android) {
    ...
    if (is_ios)
        ...
        if (is_amazon)
            mba.href = "http://www.amazon.com/Lowtech-Enterprises-slither-io/dp/B01E312TYQ/";
        else
            mba.href = "https://play.google.com/store/apps/details?id=air.com.hypah.io.slither"
}
var app;
var root = null;
var nmlo;
    ...
    (async () => {
        ...
        for (var j = 0; j < texture_sheets.length; j++) {
            ...
            for (var i = textures.length - 1; i >= 0; i--) {
                var txu = textures[i];
                if (txu.sheet == j)
                    subtextures.push(txu)
            }
            for (var i = subtextures.length - 1; i >= 0; i--) {
                var txu = subtextures[i];
                packer.insertRectangle(txu.cc.width, txu.cc.height, i)
            }
            ...
            for (var i = subtextures.length - 1; i >= 0; i--) {
                ...
                if (ti == -1) {
                    if (testing)
                        console.log("lost a rectangle for subtexture " + i)
                } else {
        ...
        try {
            ...
            bgi = await PIXI.Assets.load("./bg54.jpg")
        } catch (e) {}
        for (var i = 0; i >= 0; i--) {
            if (i == 0)
                bgee = new PIXI.TilingSprite(bgi,app.screen.width,app.screen.height);
            else
                bgee = new PIXI.TilingSprite(bgi2,app.screen.width,app.screen.height);
            bgee.anchor.set(.5, .5);
            var o = {};
            o.bgee = bgee;
            o.sp = 1 - i * .2;
        ...
        for (var i = 0; i < bgees.length; i++)
            root.addChild(bgees[i].bgee);
