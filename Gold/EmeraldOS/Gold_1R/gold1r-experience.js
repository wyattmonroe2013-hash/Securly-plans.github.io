...
/* EmeraldOS Gold 1R user experience package.
...
(function EmeraldOSGold1RExperience(){
  if(window.__EMERALDOS_GOLD_1R_EXPERIENCE__)return;
  window.__EMERALDOS_GOLD_1R_EXPERIENCE__=true;

  const VERSION="1R",PREFIX="gold1g_";
  const ACCOUNT_KEY=PREFIX+"account_history_1r";
  const GAME_KEY=PREFIX+"game_data_1r";
  const CAL_PREF_KEY=PREFIX+"calendar_preferences_1r";
  const EVENTS_KEY=PREFIX+"events";
  const UPDATE_HISTORY_KEY=PREFIX+"update_history_1r";
  const $=id=>document.getElementById(id);
  const qa=(selector,root=document)=>[...root.querySelectorAll(selector)];
  const api=()=>window.Gold1R||window.Gold50||window.Gold1G;
  ...
  const notify=(title,body,app="Gold 1R")=>api()?.notify?.(title,body,app);
  ...
  function historyRows(items,limit=40){return items.slice(0,limit).map(item=>`<tr><td>${new Date(item.time||Date.now()).toLocaleString()}</td><td>${esc(item.category||item.type||"Account")}</td><td><b>${esc(item.title||item.type||"Account event")}</b><br><small>${esc(formatDetails(item))}</small></td><td><span class="gold1r-log-status ${item.outcome==="failed"?"failed":"success"}">${esc(item.outcome||"recorded")}</span></td></tr>`).join("")}
  ...
  function changePassword(){
    const html=`<div class="app-shell gold1r-password"><div class="app-body"><div class="gold1r-page-title"><div><h1>Change password</h1><p>Update the password for ${esc(userName())}. The current password is required.</p></div><span class="gold1r-security-shield">Secure</span></div><form id="passwordChangeForm"><label>Current password<input id="passwordCurrent" class="field" type="password" autocomplete="current-password" required></label><label>New password<input id="passwordNew" class="field" type="password" autocomplete="new-password" minlength="8" required></label><label>Confirm new password<input id="passwordConfirm" class="field" type="password" autocomplete="new-password" minlength="8" required></label><div class="gold1r-password-meter"><span id="passwordMeter"></span></div><small id="passwordHint" class="muted">Use at least 8 characters. A longer passphrase is recommended.</small><div class="actions"><button id="passwordSubmit" class="button primary" type="submit">Change password</button><button id="passwordCancel" class="button" type="button">Cancel</button></div><p id="passwordMessage" role="status"></p></form></div></div>`;
    ...
    form.onsubmit=async event=>{event.preventDefault();message.textContent="Changing password…";const button=win.querySelector("#passwordSubmit");button.disabled=true;try{if(next.value!==confirmInput.value)throw new Error("New passwords do not match.");await window.GoldAuth.changePassword(userName(),current.value,next.value);message.className="gold1r-success-message";message.textContent="Password changed successfully.";notify("Password changed","Your EmeraldOS account password was updated.","Accounts");saveVM();setTimeout(()=>closeWindow(win),900)}catch(error){window.GoldAuth?.recordAccountEvent?.("password-change",{username:userName(),title:"Password change attempt",outcome:"failed",reason:error.message});message.className="gold1r-error-message";message.textContent=error.message||"Password could not be changed.";button.disabled=false;current.focus();current.select()}};
    setTimeout(()=>current.focus(),30);return win;
  }
  window.Gold1RAccountUI={summaryHTML,settingsHistoryHTML,openHistory,changePassword,exportHistory,clearHistory,record};
  ...
  function calendarToolbar(){return `<div class="app-toolbar gold1r-calendar-toolbar"><button id="calendarPrev" class="button">‹</button><button id="calendarToday" class="button">Today</button><button id="calendarNext" class="button">›</button><b id="calendarHeading"></b><span class="spacer"></span><div class="gold1r-segment"><button data-calendar-view="month">Month</button><button data-calendar-view="week">Week</button><button data-calendar-view="agenda">Agenda</button></div><button id="calendarExport" class="button">Export</button><button id="calendarNew" class="button primary">New event</button></div>`}
  function eventChip(event){return `<button class="gold1r-event-chip" draggable="true" data-calendar-event="${esc(event.id)}" style="--event-color:${esc(event.color)}" title="${esc(event.title)}${event.location?` · ${esc(event.location)}`:""}"><span>${event.allDay?"All day":esc(event.startTime)}</span><b>${esc(event.title)}</b></button>`}
  function monthHTML(anchor){
    ...
    for(let index=0;index<42;index++){const date=new Date(start);date.setDate(start.getDate()+index);const iso=dateISO(date),items=sortEvents(events.filter(item=>item.date===iso));days.push(`<div class="gold1r-calendar-day ${date.getMonth()!==anchor.getMonth()?"outside":""} ${iso===dateISO(new Date())?"today":""} ${iso===calendarState.selected?"selected":""}" data-calendar-date="${iso}" tabindex="0"><div class="gold1r-day-number"><button data-calendar-new-date="${iso}">${date.getDate()}</button>${items.length>3?`<small>+${items.length-3}</small>`:""}</div><div class="gold1r-day-events">${items.slice(0,3).map(eventChip).join("")}</div></div>`)}
    return `<div class="gold1r-calendar-weekdays">${["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(day=>`<b>${day}</b>`).join("")}</div><div class="gold1r-calendar-month">${days.join("")}</div>`;
  }
  function weekHTML(anchor){
    const start=new Date(anchor);start.setDate(start.getDate()-start.getDay());const events=calendarEvents();
    return `<div class="gold1r-calendar-week">${Array.from({length:7},(_,index)=>{const date=new Date(start);date.setDate(start.getDate()+index);const iso=dateISO(date),items=sortEvents(events.filter(item=>item.date===iso));return `<section class="gold1r-week-column ${iso===dateISO(new Date())?"today":""}" data-calendar-date="${iso}"><header><b>${date.toLocaleDateString(undefined,{weekday:"short"})}</b><button data-calendar-new-date="${iso}">${date.getDate()}</button></header><div>${items.map(eventChip).join("")||'<p class="muted">No events</p>'}</div></section>`}).join("")}</div>`;
  }
  ...
  function openInteractiveCalendar(options={}){
    ...
    const html=`<div class="app-shell gold1r-calendar">${calendarToolbar()}<div class="app-body"><div id="calendarView"></div></div></div>`;
  ...
  function exportCalendar(){
    ...
    saveText(`EmeraldOS-Gold-${VERSION}-Calendar.ics`,`BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//Emerald Systems//EmeraldOS Gold 1R//EN\r\n${body}\r\nEND:VCALENDAR\r\n`,`text/calendar`);
  }
  function checkCalendarReminders(){
    const current=Date.now(),seen=read(PREFIX+"calendar_reminders_seen_1r",{}),today=dateISO(new Date());let changed=false;
    calendarEvents().forEach(event=>{if(event.allDay)return;const when=eventDateTime(event).getTime()-event.reminder*60000,key=`${event.id}:${event.date}:${event.startTime}`;if(current>=when&&current<=when+60000&&!seen[key]){seen[key]=now();changed=true;notify(event.title,event.location?`${event.startTime} · ${event.location}`:`Starts at ${event.startTime}`,"Calendar")}});Object.keys(seen).forEach(key=>{if(!key.includes(today)&&Date.parse(seen[key])<Date.now()-7*86400000){delete seen[key];changed=true}});if(changed)write(PREFIX+"calendar_reminders_seen_1r",seen);
  }
  ...
  function gameCard(id,name,desc,accent){const stats=gameData()[id]||{};return `<button class="gold1r-game-card" data-game="${id}" style="--game-accent:${accent}"><img src="app-logos/${id}.svg" alt=""><span><b>${esc(name)}</b><small>${esc(desc)}</small><em>${id==="snake"?`Best score: ${stats.best||0}`:id==="minesweeper"?`Wins: ${stats.wins||0}${stats.bestTime!=null?` · Best: ${stats.bestTime}s`:""}`:`Wins: ${stats.wins||0}${stats.bestMoves!=null?` · Best: ${stats.bestMoves} moves`:""}`}</em></span></button>`}
  function openGameCenter(){const html=`<div class="app-shell gold1r-game-center"><div class="app-body"><div class="gold1r-page-title"><div><h1>Emerald Games</h1><p>Built-in games that save scores and preferences with your Gold cloud VM.</p></div><span class="gold1r-status-pill">Game Mode ready</span></div><div class="gold1r-game-grid">${gameCard("snake","Emerald Snake","Guide the emerald, collect gems, and beat your high score.","#107c10")}${gameCard("minesweeper","Emerald Mines","Clear a classic minefield with flags and a running timer.","#d83b01")}${gameCard("memory","Memory Match","Match every pair in as few moves as possible.","#5c2d91")}</div><section class="card"><h2>Game settings</h2><p>Enable Game Mode in Settings to reduce animations while games are open. Scores are included in migration and backup data.</p><button class="button" data-open-settings="gaming">Open Gaming settings</button></section></div></div>`;const win=openWindow("games","Emerald Games",html,{width:920,height:620});win.querySelectorAll("[data-game]").forEach(button=>button.onclick=()=>api().openApp(button.dataset.game));win.querySelector("[data-open-settings]").onclick=()=>api().openSettings?.("system");return win}
  function openSnake(){
    const html=`<div class="app-shell gold1r-game"><div class="app-toolbar"><button id="snakeStart" class="button primary">Start</button><button id="snakePause" class="button">Pause</button><span>Score: <b id="snakeScore">0</b></span><span>Best: <b id="snakeBest">${gameData().snake.best||0}</b></span><span class="spacer"></span><small>Arrow keys or WASD</small></div><div class="app-body gold1r-game-stage"><canvas id="snakeCanvas" width="600" height="420" tabindex="0"></canvas><p id="snakeStatus">Press Start to play.</p></div></div>`;
  ...
  function openMinesweeper(){
    const html=`<div class="app-shell gold1r-game"><div class="app-toolbar"><button id="mineNew" class="button primary">New game</button><span>Mines: <b id="mineCount">10</b></span><span>Time: <b id="mineTime">0</b>s</span><span>Best: <b id="mineBest">${gameData().minesweeper.bestTime??"—"}</b></span></div><div class="app-body gold1r-game-stage"><div id="mineBoard" class="gold1r-mine-board" aria-label="Minesweeper board"></div><p id="mineStatus">Reveal every safe tile. Right-click to place a flag.</p></div></div>`;
  ...
  function openMemory(){
    const symbols=["◆","●","▲","■","★","✦","♥","☀"];const html=`<div class="app-shell gold1r-game"><div class="app-toolbar"><button id="memoryNew" class="button primary">New game</button><span>Moves: <b id="memoryMoves">0</b></span><span>Time: <b id="memoryTime">0</b>s</span><span>Best: <b id="memoryBest">${gameData().memory.bestMoves??"—"}</b></span></div><div class="app-body gold1r-game-stage"><div id="memoryBoard" class="gold1r-memory-board"></div><p id="memoryStatus">Match all eight pairs.</p></div></div>`;
  ...
  function patchAPI(){Object.assign(window.Gold1R||{}, {openCalendar:openInteractiveCalendar,openAccountHistory:openHistory,changePassword,openGameCenter,openPasswordChange:changePassword,gameData,calendarEvents,recordAccountEvent:record});window.Gold1R=window.Gold1R||api()}
  function init(){
    if(!api()?.APPS){setTimeout(init,80);return}
    registerApps();patchAPI();record("vm-start",{title:"EmeraldOS Gold 1R started",outcome:"success",shellV2:new URLSearchParams(location.search).get("elsusShell")==="2"});
