...
/* EmeraldOS Gold 1M cloud continuity and migration gate.
   ...
   Gold 1M to save over the shared VM. This module never writes the shared
   system/emeraldGoldLatest release pointer. */
(function EmeraldOSGold1MContinuity(){
  if(window.__EMERALDOS_GOLD_1M_CONTINUITY__)return;
  window.__EMERALDOS_GOLD_1M_CONTINUITY__=true;

  const VERSION="1M",PREFIX="gold1g_",REPORT_KEY=PREFIX+"migration_1m_cloud_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1M continuity local write failed",key,error);return false}};
  const helper=()=>window.Gold1MLocalMigration||{};
  ...
  const knownVersions=["1H","1I","1J","1K","1L","1M"];
  const knownSchemas=new Set([
    ...
    "gold1k-split-v1","gold1k-split-v2","gold1l-split-v1","gold1l-split-v2","gold1m-split-v1","gold1m-split-v2"
  ]);
  ...
  const targetCategories=[...baseCategories,"registry_1m","registry_1m_history","clipboard_history_1m","startup_apps_1m","update_preferences_1m","update_history_1m","focus_sessions_1m"];
  ...
  function localKeyFor(category){
    if(category==="registry_1m")return PREFIX+"registry_1m";
    if(category==="registry_1m_history")return PREFIX+"registry_1m_history";
    if(category==="clipboard_history_1m")return PREFIX+"clipboard_history_1m";
    if(category==="startup_apps_1m")return PREFIX+"startup_apps_1m";
    if(category==="update_preferences_1m")return PREFIX+"update_preferences_1m";
    if(category==="update_history_1m")return PREFIX+"update_history_1m";
    if(category==="focus_sessions_1m")return PREFIX+"focus_sessions_1m";
  ...
  function mergeCategory(category,sources){
    ...
    const arrayCategory=["tickets","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","sticky_notes","voice_recordings","registry_1m_history","clipboard_history_1m","startup_apps_1m","update_history_1m"].includes(category);
  ...
  async function collectOrphanedChunks(fb,vmUser=currentUser()){
    ...
    }catch(error){console.debug("Gold 1M orphaned chunk recovery unavailable",error?.message)}
  }
  ...
  async function archiveSourceCategory(fb,version,category,value,vmUser=currentUser()){
    ...
    const id=`${safeVersion}__${category}`,json=JSON.stringify(value??null),base={version:safeVersion,category,updatedAt:now(),source:"Gold 1M pre-migration archive"};
  ...
  async function collectCloudSources(fb){
    ...
    for(const vmUser of userCandidates()){
      ...
      }catch(error){console.debug("Gold 1M VM summary source skipped",vmUser,error?.message)}
      ...
      if(summary){
        if(knownSchemas.has(summary.schema)){
          for(const category of summary.categories||[]){
            ...
            }catch(error){console.warn("Gold 1M category migration skipped",category,error)}
          }
        }else{
      ...
      }catch(error){console.debug("Gold 1M category scan unavailable",vmUser,error?.message)}
      ...
      }catch(error){console.debug("Gold 1M version backup scan unavailable",vmUser,error?.message)}
  ...
  function refreshDesktop(){
    try{window.Gold50?.setPrefs?.(read(PREFIX+"prefs",{}));window.Gold50?.saveFiles?.(read(PREFIX+"files",[]));window.Gold50?.renderDesktop?.();window.Gold50?.renderStartMenu?.();window.Gold50?.renderNotificationBadge?.()}catch(error){console.debug("Gold 1M UI refresh after migration skipped",error?.message)}
  }

  function openMigrationCenter(){
    const report=read(REPORT_KEY,{stage:"not run"}),local=read(PREFIX+"migration_1m_local_report",{}),files=read(PREFIX+"files",[]);
  ...
  async function runMigration(show=false){
    if(window.__GOLD1M_MIGRATION_RUNNING__)return window.__GOLD1M_MIGRATION_PROMISE__;
    window.__GOLD1M_MIGRATION_RUNNING__=true;
    sourceMap.clear();collectLocalSources();
    const promise=(async()=>{
      let cloudInfo={summary:null,sourceVersion:"none"},cloudError=null;
      try{const fb=await withTimeout(import("./firebase.js"),20000,"Firebase migration connection timed out");if(fb.db)cloudInfo=await withTimeout(collectCloudSources(fb),30000,"Cloud migration scan timed out");else cloudError="Firebase is not configured"}catch(error){cloudError=error.message;console.warn("Gold 1M cloud migration continued locally",error)}
      ...
      if(cloudTemporarilyUnverified){
        ...
        window.__GOLD1M_MIGRATION_BLOCK_SAVES__=true;window.__GOLD1M_MIGRATION_COMPLETE__=false;
        localStorage.setItem(PREFIX+"cloud_status","migration waiting for cloud verification");
        if(show)window.Gold1M?.notify?.("Cloud migration not verified","Gold 1M kept cloud autosave blocked so older files cannot be overwritten. Reconnect and run the migration scan again.","Migration & Continuity");
        return report;
      }
      report.autosaveUnblockedAt=now();report.autosaveStillBlocked=false;
      write(REPORT_KEY,report);refreshDesktop();
      window.__GOLD1M_MIGRATION_BLOCK_SAVES__=false;window.__GOLD1M_MIGRATION_COMPLETE__=true;
      try{window.__GOLD1M_MIGRATION_COMMITTING__=true;await window.Gold1M?.saveWorkspaceNow?.(false)}catch(error){console.warn("Gold 1M post-migration save deferred",error)}finally{window.__GOLD1M_MIGRATION_COMMITTING__=false}
      const noticeKey=PREFIX+"migration_1m_notice_shown";
      if(show||localStorage.getItem(noticeKey)!=="true"){
        window.Gold1M?.notify?.("Migration complete",`${report.filesAfter||0} files and all available VM categories were merged before cloud autosave was enabled.`,"Migration & Continuity");
        localStorage.setItem(noticeKey,"true");
      }
      return report;
    })().finally(()=>{window.__GOLD1M_MIGRATION_RUNNING__=false});
    window.__GOLD1M_MIGRATION_PROMISE__=promise;return promise;
  }

  function install(){
    registerMigrationApp();
    const api=window.Gold1M||{};api.runMigration=runMigration;api.openMigrationCenter=openMigrationCenter;api.migrationReport=()=>read(REPORT_KEY,null);window.Gold1M=api;
    if(window.Gold50){window.Gold50.openMigrationCenter=openMigrationCenter;window.Gold50.runMigration=runMigration}
    runMigration(false);
    window.addEventListener("online",()=>{if(window.__GOLD1M_MIGRATION_BLOCK_SAVES__===true)setTimeout(()=>runMigration(false),1000)});
    setInterval(()=>{if(window.__GOLD1M_MIGRATION_BLOCK_SAVES__===true&&navigator.onLine)runMigration(false)},60000);
  }
