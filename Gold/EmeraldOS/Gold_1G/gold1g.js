"use strict";
/* EmeraldOS Gold 1G - redesigned quota-safe cloud VM build */
(function(){
...
function gold1gSmallManifest(data){
  const d=data||{};
  const count=a=>Array.isArray(a)?a.length:0;
  return {
    product:d.product||'EmeraldOS Gold',
    version:d.activeVersion||d.version||BUILD.version,
    folder:d.activeFolder||d.folder||'Gold_1G',
    user:d.user||username(),
    savedAt:d.lastSave||d.savedAt||now(),
    cloudPath:d.cloudPath||'emeraldOSUsers/{username}/goldVM/current',
    localOnly:true,
    note:'Compact local pointer only. Full VM state is saved to Firebase goldVM/current when available to prevent browser storage quota crashes.',
    counts:{
      files:count(d.files),tickets:count(d.tickets),notes:count(d.notes),tasks:count(d.tasks),
      contacts:count(d.contacts),notifications:count(d.notifications),userApps:count(d.userApps||d.apps)
    }
  };
}
function gold1gStorageCleanupForQuota(){
  try{
    const removable=[];
    for(let i=0;i<localStorage.length;i++){
      const k=localStorage.key(i)||'';
      if(/^(gold1[abcdefg]_|gold1[0-9]+_)?workspace$/i.test(k)) removable.push(k);
      if(/gold1[abcdefg]_workspace/i.test(k)) removable.push(k);
      if(k==='emeraldGoldShell_liveState'||k==='emeraldGoldShell_lastSnapshot') removable.push(k);
      if(/^emeraldGoldShell_.*Snapshot/i.test(k)) removable.push(k);
    }
    [...new Set(removable)].forEach(k=>{try{localStorage.removeItem(k)}catch{}});
  }catch(error){console.warn('Gold 1G storage cleanup skipped',error)}
}
function write(k,v){
  const serialized=JSON.stringify(v);
  try{localStorage.setItem(k,serialized);}
  catch(error){
    const quota=error&&(error.name==='QuotaExceededError'||String(error.message||'').toLowerCase().includes('quota'));
    if(quota){
      gold1gStorageCleanupForQuota();
      if(String(k).includes('workspace')){
        try{localStorage.setItem(PREFIX+'workspace_manifest',JSON.stringify(gold1gSmallManifest(v)));}catch{}
        console.warn('Gold 1G skipped oversized local workspace cache; cloud VM save will continue.',error);
        return v;
      }
      try{localStorage.setItem(k,serialized);}
      catch(secondError){console.warn('Gold 1G localStorage write skipped for '+k,secondError)}
      return v;
    }
    console.warn('Gold 1G localStorage write failed for '+k,error);
  }
  return v;
}
...
let saveTimer=null;function saveWorkspaceDebounced(){clearTimeout(saveTimer);saveTimer=setTimeout(()=>saveWorkspaceNow(false),700)}async function saveWorkspaceNow(show=true){const data={prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs(),savedAt:now(),build:BUILD.name};gold1gWriteWorkspacePointer(data);if(fb?.setDoc&&fb?.doc&&fb?.db){try{await fb.setDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'goldVM','current'),data,{merge:true})}catch(e){console.warn('cloud save failed',e)}}if(show)notify('Workspace saved','Gold 1G VM save completed. Full state saves to cloud when Firebase is available; local storage keeps only a compact pointer.','Restore Center')}
...
function gold1gWindowState(){
  try{return [...document.querySelectorAll('.window')].map(w=>({
    id:w.id,title:w.dataset.title||'',left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,
    minimized:w.classList.contains('minimized'),maximized:w.classList.contains('max')
  }));}catch{return []}
}
function gold1gLocalState(){
  const localKeys={};
  [
    'setup_done','update_setup_done','staff_session','cloud_status','cloud_last_save',
    'desktopFilePins','startPins','taskbarPins','desktopPositions','project_board',
    'store_reports','staff_logs','safe_mode','safemode'
  ].forEach(k=>{try{const v=localStorage.getItem(PREFIX+k);if(v!==null)localKeys[PREFIX+k]=v;}catch{}});
  return {
    product:'EmeraldOS Gold',
    version:BUILD.version,
    folder:'Gold_1G',
    ...
    workspace:{openWindows:gold1gWindowState()},
    localKeys
  };
}
function gold1gWriteWorkspacePointer(data){
  try{localStorage.removeItem(PREFIX+'workspace')}catch{}
  try{localStorage.removeItem('emeraldGoldShell_liveState')}catch{}
  return write(PREFIX+'workspace_manifest',gold1gSmallManifest(data));
}
async function gold1gSaveCloudVMState(show=true){
  ...
  data.lastSave = now();
  gold1gWriteWorkspacePointer(data);
  try{
    const api = fb || await initFirebase();
...
async function gold1gRestoreCloudVMState(show=true){
  ...
  if(!data) data = read(PREFIX+'workspace', null); // legacy full local fallback only; Gold 1G now uses workspace_manifest locally
  if(data){
    if(data.localKeys) Object.entries(data.localKeys).forEach(([k,v])=>{ if(v!==null && v!==undefined){try{localStorage.setItem(k,String(v));}catch(error){console.warn('Gold 1G skipped restoring local key '+k,error)}} });
