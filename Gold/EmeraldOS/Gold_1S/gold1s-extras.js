...
========================================================= */
(function EmeraldOSGold1SExtras(){
  if(window.__EMERALDOS_GOLD_1S_EXTRAS__) return;
  window.__EMERALDOS_GOLD_1S_EXTRAS__=true;

  const VERSION="1S";
  const FOLDER="Gold_1S";
  const UPDATE_SERVICE="Original E.L.S.U.S.";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1s";
  const HISTORY_KEY=PREFIX+"update_history_1s";
  const SNOOZE_KEY=PREFIX+"update_snooze_1s";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1s";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1s";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1s";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1s";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1S update check used local manifest cache",error?.message||error);
    }
  ...
  async function beginUserUpdate(manifest){
    ...
    try{
      if(window.Gold1S?.requestVersionChange){await window.Gold1S.requestVersionChange(manifest);return true}
      if(window.Gold1S?.saveWorkspaceNow)await window.Gold1S.saveWorkspaceNow(false);
      window.Gold1S?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1S live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1S live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1S?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1S?.openUpdateCenter?.(currentManifest);
  ...
  function patchPublicAPI(){
    window.Gold1S=window.Gold1S||api()||{};
    Object.assign(window.Gold1S,{elsusMode:"original",checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1S","Update checks, Focus Sessions, and Quick Settings loaded.","System");
  }
