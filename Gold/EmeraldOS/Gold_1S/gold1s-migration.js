...
/* EmeraldOS Gold 1S local continuity migration.
   ...
   asynchronous cloud migration in gold1s-continuity.js has completed. */
(function EmeraldOSGold1SLocalMigration(){
  if(window.__EMERALDOS_GOLD_1S_LOCAL_MIGRATION__) return;
  window.__EMERALDOS_GOLD_1S_LOCAL_MIGRATION__=true;
  window.__GOLD1S_MIGRATION_BLOCK_SAVES__=true;

  const PREFIX="gold1g_";
  const REPORT_KEY=PREFIX+"migration_1s_local_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1S local migration write failed",key,error);return false}};
  ...
  const fileKeys=[
    "gold1g_files","gold1h_files","gold1i_files","gold1j_files","gold1k_files","gold1l_files","gold1m_files","gold1n_files","gold1o_files","gold1p_files","gold1q_files","gold1r_files","gold1s_files",
    "gold1g_workspace","gold1h_workspace","gold1i_workspace","gold1j_workspace","gold1k_workspace","gold1l_workspace","gold1m_workspace","gold1n_workspace","gold1o_workspace","gold1p_workspace","gold1q_workspace","gold1r_workspace","gold1s_workspace",
  ...
  const previousSession=readMaybe(PREFIX+"session_state_1s")||readMaybe(PREFIX+"session_state_1r")||readMaybe(PREFIX+"session_state_1q")||readMaybe(PREFIX+"session_state_1p")||readMaybe(PREFIX+"session_state_1o");
  ...
  if(previousSession!==undefined)write(PREFIX+"session_state_1s",previousSession);
  else if(previousMeta?.openWindows)write(PREFIX+"session_state_1s",{version:"1O",updatedAt:previousMeta.savedAt||now(),windows:previousMeta.openWindows,openApps:[]});
  const previousNotificationPrefs=readMaybe(PREFIX+"notification_preferences_1s")||readMaybe(PREFIX+"notification_preferences_1r")||readMaybe(PREFIX+"notification_preferences_1q")||readMaybe(PREFIX+"notification_preferences_1p")||readMaybe(PREFIX+"notification_preferences_1o");
  if(previousNotificationPrefs!==undefined)write(PREFIX+"notification_preferences_1s",previousNotificationPrefs);
  const previousHomePreference=readMaybe(PREFIX+"home_1s_at_login")??readMaybe(PREFIX+"home_1r_at_login")??readMaybe(PREFIX+"home_1q_at_login")??readMaybe(PREFIX+"home_1p_at_login")??readMaybe(PREFIX+"home_1o_at_login")??readMaybe(PREFIX+"welcome_1n_at_login");
  if(previousHomePreference!==undefined)write(PREFIX+"home_1s_at_login",previousHomePreference);

  const categories=["prefs","folders","mail","notes","tasks","events","contacts","tickets","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings","feedback_1s","routines_1s","accessibility_1s","home_1s_at_login","notification_preferences_1s","session_state_1s","account_history_1s","game_data_1s","calendar_preferences_1s","guide_preferences_1s","radio_stations_1s","radio_preferences_1s","file_tombstones","file_system_metadata","file_recovery_catalog","file_operations_1s"];
  const prefixes=["gold1a_","gold1b_","gold1c_","gold1d_","gold1e_","gold1f_","gold1h_","gold1i_","gold1j_","gold1k_","gold1l_","gold1m_","gold1n_","gold1o_","gold1p_","gold1q_","gold1r_","gold1s_","gold1g_"];
  ...
  const sourceVersions=["1h","1i","1j","1k","1l","1m","1n","1o","1p","1q","1r","1s"];
  versionedFamilies.forEach(family=>{
    ...
    if(merged!==undefined)write(`${PREFIX}${family}_1s`,merged);
  });
  ...
  window.Gold1SLocalMigration={report,collectFiles,mergeFiles,deepMerge,mergeArray,normalizeFile};
})();
