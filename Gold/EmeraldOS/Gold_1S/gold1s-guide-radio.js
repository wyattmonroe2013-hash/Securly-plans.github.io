"use strict";
(function EmeraldOSGold1SGuideAndRadio(){
  if(window.__EMERALDOS_GOLD_1S_GUIDE_RADIO__)return;
  window.__EMERALDOS_GOLD_1S_GUIDE_RADIO__=true;
  const VERSION="1S",PREFIX="gold1g_",GUIDE_PREFS=PREFIX+"guide_preferences_1s",RADIO_STATIONS=PREFIX+"radio_stations_1s",RADIO_PREFS=PREFIX+"radio_preferences_1s",PIN_KEY=PREFIX+"guide_default_pins_1s";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return value}catch(error){console.warn("Gold 1S data write failed",key,error);return value}};
  const now=()=>new Date().toISOString();
  const core=()=>window.Gold50||window.Gold1S;
  const notify=(title,body,app="Complete Guide")=>core()?.notify?.(title,body,app);
  const saveVM=()=>{if(window.__GOLD1S_MIGRATION_BLOCK_SAVES__===true)return;setTimeout(()=>window.Gold1S?.saveWorkspaceNow?.(false),0)};
  ...
  const topics=[
    ...
    {id:"files",title:"Files, folders, and universal compatibility",group:"Files and data",keywords:"files folders explorer compatibility universal id extensions old versions",summary:"Create, organize, open, and migrate files across Gold versions.",steps:[
      ...
      "Create files with File Explorer or Gold Office. Gold 1S normalizes records to the universal Gold file format while retaining legacy IDs and source metadata.",
    ...
    {id:"quick",title:"Quick Settings and notifications",group:"Settings",keywords:"quick settings brightness volume focus assist night light wifi notifications action center",summary:"Control common settings without opening the full Settings app.",steps:[
      ...
      "Action Center stores notifications even when startup calm mode suppresses banners.",
  ...
  function renderTopic(topic){return `<article class="gold1s-guide-article"><div class="gold1s-guide-heading"><div><small>${esc(topic.group)}</small><h1>${esc(topic.title)}</h1><p>${esc(topic.summary)}</p></div><button class="button" id="guideFavorite">${guidePrefs().favorites.includes(topic.id)?"Remove favorite":"Add favorite"}</button></div><ol>${topic.steps.map(step=>`<li>${esc(step)}</li>`).join("")}</ol>${topic.links?.length?`<section class="card"><h2>Open related tools</h2><div class="button-row">${topic.links.map(([id,label])=>`<button class="button" data-guide-app="${esc(id)}">${esc(label)}</button>`).join("")}</div></section>`:""}</article>`}
  function openGuide(startTopic){
    const initial=topicById(startTopic||guidePrefs().lastTopic);
    const html=`<div class="app-shell gold1s-guide"><div class="app-toolbar"><input id="guideSearch" class="field" placeholder="Search the complete guide" aria-label="Search the complete guide"><button id="guideHome" class="button">Getting started</button><button id="guideExport" class="button">Export guide index</button></div><div class="gold1s-guide-layout"><nav id="guideNav" aria-label="Guide topics"></nav><main id="guideContent"></main></div></div>`;
    ...
    search.oninput=()=>renderNav(search.value);search.onkeydown=e=>{if(e.key==="Enter")nav.querySelector("[data-guide-topic]")?.click()};win.querySelector("#guideHome").onclick=()=>show("start");win.querySelector("#guideExport").onclick=()=>core().saveText?.("EmeraldOS-Gold-1S-Complete-Guide-Index.txt",topics.map(t=>`${t.group} - ${t.title}\n${t.summary}\n${t.steps.map((s,i)=>`${i+1}. ${s}`).join("\n")}\n`).join("\n"));renderNav();show(selected);return win;
  }
  ...
  function init(){
    ...
    ensureGames();
    Object.assign(window.Gold1S||core(),{openGuide,openCompleteGuide:openGuide,openRadio,guideTopics:topics,radioStations:stations});
