"use strict";
/* EmeraldOS Gold 1D - independent logos, staff-only tools, live remote desktop, EmeraldOS Gold UX polish */
(function(){
const PREFIX="gold1d_";
const BUILD={name:"EmeraldOS Gold 1D",version:"1D",cloudPath:"goldVM/current",bios:"Emerald BIOS A9",shellManaged:true,folder:"Gold_1D"};

/* EmeraldOS Gold 1D local data migration from Gold 1A.
   Cloud VM data remains version-independent under emeraldOSUsers/{username}/goldVM/current. */
function migrateGold1ALocalData(){
  try{
    if(localStorage.getItem('gold1d_migrated_from_gold1a')==='true') return;
    const oldPrefix='gold1a_', newPrefix='gold1d_';
    ...
    localStorage.setItem('gold1d_migrated_from_gold1a','true');
  }catch(e){console.warn('Gold 1D migration skipped', e)}
}
...
async function initFirebase(){try{fb=await import("./firebase.js");return fb}catch(e){console.warn("Gold 1D Firebase unavailable",e);return null}}
...
function defaultFiles(){return[
 {id:uid(),name:"Welcome to EmeraldOS Gold 1D.edoc",type:"doc",folder:"Documents",star:true,content:"<h1>Welcome to EmeraldOS Gold 1D</h1><p>This version improves setup, support, Staff Edition, Emerald BIOS, apps, logos, settings, and EmeraldOS Gold usability.</p>",created:now(),updated:now(),trash:false},
...
function mail(){return read(PREFIX+"mail",{inbox:[{id:uid(),from:"system@gold.mail",to:mailAddress(),subject:"Welcome to Gold Mail",body:"Gold Mail is ready in EmeraldOS Gold 1D.",read:false,time:now()}],sent:[],drafts:[],trash:[]})}function saveMail(m){write(PREFIX+"mail",m);renderNotifBadge();saveWorkspaceDebounced()}
function notes(){return read(PREFIX+"notes",[])}function saveNotes(n){write(PREFIX+"notes",n);saveWorkspaceDebounced()}
function tasks(){return read(PREFIX+"tasks",[{id:uid(),text:"Explore EmeraldOS Gold 1D",done:false,priority:"Normal",due:""}])}function saveTasks(t){write(PREFIX+"tasks",t);saveWorkspaceDebounced()}
...
APPS.push(
 {id:"help",name:"Get Help",label:"GH",color:"blue",group:"Support",desc:"Gold 1D help, tips, guides, and shortcuts.",open:openHelp},
...
function openBIOSOptions(){openWindow("bios","BIOS Options",`<h2>Emerald BIOS A9</h2><div class="bios-embed"><p>Emerald BIOS options are available at startup using F12.</p><div class="toolbar"><button class="btn primary" onclick="location.href='bios.html'">Open full BIOS</button><button class="btn" onclick="localStorage.setItem('gold1d_safemode','true');location.reload()">Boot Safe Mode</button><button class="btn" onclick="localStorage.removeItem('gold1d_setup_done');location.reload()">Run Setup</button><button class="btn" onclick="location.href='staff.html'">Staff Edition</button></div></div>`,{width:720,height:420})}
...
function openSetup(){
  ...
  function setupHero(){return `<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>Welcome to EmeraldOS Gold 1D</h2><p class="muted">Set up your Gold virtual machine, personalize your desktop, choose your apps, and connect support tools.</p></div></div><div class="setup-checklist"><span>Account protected</span><span>Workspace continuity</span><span>Emerald support ready</span></div></div>`}
...
function finishSetup(){localStorage.setItem(PREFIX+'setup_done','true');$("setupWizard")?.classList.add('hidden');notify('Setup complete','EmeraldOS Gold 1D is ready.','Setup')}
...
function openNotepad(){openWindow('notepad','Notepad',`<h2>Notepad</h2><textarea id="np" class="field" rows="14">${esc(read(PREFIX+'notepad',''))}</textarea><button class="btn primary" onclick="localStorage.setItem('gold1d_notepad',JSON.stringify(np.value));Gold50.notify('Saved','Notepad saved','Notepad')">Save</button><button class="btn" onclick="Gold50.saveText('notepad.txt',np.value)">Export</button>`,{width:620,height:520})}
...
function openCalendar(){openWindow('calendar','Calendar',`<h2>Calendar</h2><p>${new Date().toDateString()}</p><textarea class="field" rows="8" onchange="localStorage.setItem('gold1d_calendar',JSON.stringify(this.value))">${esc(read(PREFIX+'calendar',''))}</textarea>`,{width:550,height:420})}
...
function openSecurity(){openWindow('security','Security Center',`<h2>Security Center</h2><div class="grid3"><div class="card"><h3>Safe Mode</h3><p>${localStorage.getItem(PREFIX+'safemode')==='true'?'Enabled':'Disabled'}</p><button class="btn" onclick="localStorage.setItem('gold1d_safemode','true');location.reload()">Enable</button><button class="btn" onclick="localStorage.removeItem('gold1d_safemode');location.reload()">Disable</button></div><div class="card"><h3>Blocked Users</h3><p>${read(PREFIX+'blocked',[]).length}</p></div><div class="card"><h3>Staff Session</h3><p>${isStaff()?'Active':'Not active'}</p></div></div>`,{width:760,height:480})}
...
function officeNew(type){
  ...
  if(type==='sheet')content=JSON.stringify([['Item','Amount','Notes'],['Apps','30','Gold 1D'],['Tickets','5','Support'],['Total','=SUM(B2:B3)','']]);
...
function gold1dUserApps(){return read(PREFIX+'user_apps',[])}
...
let saveTimer=null;function saveWorkspaceDebounced(){clearTimeout(saveTimer);saveTimer=setTimeout(()=>saveWorkspaceNow(false),700)}async function saveWorkspaceNow(show=true){const data={prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs(),savedAt:now(),build:BUILD.name};write(PREFIX+'workspace',data);if(fb?.setDoc&&fb?.doc&&fb?.db){try{await fb.setDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'),data)}catch(e){console.warn('cloud save failed',e)}}if(show)notify('Workspace saved','Gold 1D workspace saved locally and attempted cloud sync.','Restore Center')}
async function restoreWorkspace(show=true){let data=read(PREFIX+'workspace',null);if(fb?.getDoc&&fb?.doc&&fb?.db){try{const snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'));if(snap.exists())data=snap.data()}catch(e){console.warn('cloud restore failed',e)}}if(data){['prefs','files','tickets','mail','notes','tasks','contacts','notifications'].forEach(k=>{if(data[k])write(PREFIX+k,data[k])});applyPrefs();renderDesktop();renderNotifBadge();if(show)notify('Workspace restored','Gold 1D workspace restored.','Restore Center')}}function exportWorkspace(){saveText('emeraldos-gold-8.0-workspace.json',JSON.stringify({prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs()},null,2),'application/json')}function importWorkspacePrompt(){const txt=prompt('Paste workspace JSON');if(!txt)return;try{const data=JSON.parse(txt);Object.keys(data).forEach(k=>write(PREFIX+k,data[k]));location.reload()}catch{alert('Invalid JSON')}}function resetGold(){if(confirm('Reset local EmeraldOS Gold 1D data?')){Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)).forEach(k=>localStorage.removeItem(k));location.reload()}}
...
========================================================= */
Object.assign(BUILD,{name:"EmeraldOS Gold 1D",version:"1D",cloudPath:"goldVM/current",bios:"Emerald BIOS A9"});
...
function applyPrefs(){const p=prefs();document.body.classList.toggle("dark",p.theme==="dark");document.body.classList.toggle("highcontrast",p.theme==="highcontrast");document.body.classList.toggle("bigtext",!!p.bigText);document.body.classList.toggle("no-motion",!!p.reducedMotion);document.body.classList.toggle("gold1d-solid-bg",!!p.useBackgroundColor);document.body.classList.toggle("gold1d-readable",!!p.readableFont);document.body.classList.toggle("gold1d-focus-ring",!!p.focusRings);document.body.classList.toggle("gold1d-large-cursor",!!p.largeCursor);document.body.classList.toggle("gold1d-underlined",!!p.underlineControls);document.body.classList.toggle("gold1d-extra-contrast",!!p.highContrastBorders);document.body.classList.toggle("gold1d-taskbar-top",p.taskbarPosition==="top");document.body.classList.toggle("gold1d-taskbar-compact",p.taskbarSize==="compact");document.body.classList.remove("bg-gold-bloom","bg-blue-window","bg-emerald-field","bg-slate-pro","custom-bg","gradient-aurora","gradient-skyline","gradient-emerald-pro","gradient-midnight","gradient-sunrise","gradient-ribbon","gradient-calm");if(p.customBackground){document.body.classList.add("custom-bg");document.body.style.setProperty("--customBg",`url(${p.customBackground})`);document.body.style.setProperty("--bgMode",p.bgMode||"cover");document.body.style.setProperty("--bgPosition",p.bgPosition||"center");document.body.style.setProperty("--bgRepeat",p.bgRepeat||"no-repeat")}else if(p.useBackgroundColor){document.body.style.setProperty("--gold1dBackground",p.backgroundColor||"#e9f3ff")}else{document.body.classList.add("gradient-"+(p.gradient||"aurora"));document.body.style.removeProperty("--customBg")}document.documentElement.style.setProperty("--accent",p.accent||"#0078d4");document.documentElement.style.setProperty("--accent2",p.secondaryAccent||"#0f7b4d");document.documentElement.style.setProperty("--gold70Secondary",p.secondaryAccent||"#0f7b4d");document.documentElement.style.setProperty("--winRadius",p.rounding==="none"?"0":p.rounding==="large"?"14px":p.rounding==="pill"?"20px":"6px");document.documentElement.style.setProperty("--gold1dTaskOpacity",String((Number(p.taskbarOpacity)||92)/100));document.documentElement.style.setProperty("--gold1dPanelOpacity",String((Number(p.panelOpacity)||94)/100));document.documentElement.style.setProperty("--gold1dMica",(Number(p.micaBlur)||0)+"px");document.documentElement.style.setProperty("--gold1dStartWidth",(Number(p.startWidth)||860)+"px");document.documentElement.style.setProperty("--gold1dTextScale",String(Number(p.textScale)||1));document.documentElement.style.setProperty("--gold1dBackground",p.backgroundColor||"#e9f3ff");document.documentElement.style.setProperty("--font",p.fontFamily||"Segoe UI");document.documentElement.style.setProperty("--gold1dIconScale",p.iconSize==="small"?".86":p.iconSize==="large"?"1.16":p.iconSize==="xl"?"1.32":"1");document.documentElement.style.setProperty("--gold1dDesktopGap",p.density==="compact"?"8px":p.density==="spacious"?"24px":"16px");const search=$("searchBtn"),widgets=$("widgetsBtn"),staff=$("staffBtn");if(search)search.style.display=p.taskbarSearch?"":"none";if(widgets)widgets.style.display=p.taskbarWidgets?"":"none";if(staff)staff.style.display=p.taskbarStaff?"":"none";}

function gold1dLocalState(){const localKeys={};Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)||["loggedIn","username","role","role2","userId"].includes(k)).forEach(k=>localKeys[k]=localStorage.getItem(k));return{build:BUILD.name,version:BUILD.version,savedAt:now(),user:username(),prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs(),userApps:read(PREFIX+'user_apps',[]),remoteSessions:read(PREFIX+'remote_sessions',[]),emergencyLogs:read(PREFIX+'emergency_logs',[]),snapshots:read(PREFIX+'vm_snapshots',[]),openWindows:[...document.querySelectorAll('.window')].map(w=>({id:w.id,title:w.dataset.title,left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,max:w.classList.contains('max'),min:w.classList.contains('minimized')})),localKeys};}
async function gold1dCloudWrite(data){if(!prefs().cloudSync)return false;if(!fb)await initFirebase();if(fb?.setDoc&&fb?.doc&&fb?.db){try{await fb.setDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'),data);await fb.setDoc(fb.doc(fb.db,'emeraldOSGoldVMStates',username()),data);localStorage.setItem(PREFIX+'cloud_last_save',now());localStorage.setItem(PREFIX+'cloud_status','saved');return true}catch(e){console.warn('Gold 8 cloud save failed',e);localStorage.setItem(PREFIX+'cloud_status','save failed: '+(e.message||e));return false}}localStorage.setItem(PREFIX+'cloud_status','firebase unavailable');return false;}
async function gold1dCloudRead(){if(!fb)await initFirebase();if(fb?.getDoc&&fb?.doc&&fb?.db){try{let snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'));if(snap.exists()){localStorage.setItem(PREFIX+'cloud_status','restored');return snap.data()}snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSGoldVMStates',username()));if(snap.exists()){localStorage.setItem(PREFIX+'cloud_status','restored');return snap.data()}}catch(e){console.warn('Gold 8 cloud restore failed',e);localStorage.setItem(PREFIX+'cloud_status','restore failed: '+(e.message||e))}}return null;}
...
async function saveWorkspaceNow(show=true){const data=gold1dLocalState();write(PREFIX+'workspace',data);const cloud=await gold1dCloudWrite(data);if(show)notify('Gold VM saved',cloud?'Everything was saved to the cloud VM state.':'Saved locally. Cloud sync will retry when Firebase is available.','Gold VM');return data;}
async function restoreWorkspace(show=true){let data=null;if(prefs().restoreOnLogin!==false)data=await gold1dCloudRead();if(!data)data=read(PREFIX+'workspace',null);if(data){if(data.localKeys)Object.entries(data.localKeys).forEach(([k,v])=>{if(v!==null&&v!==undefined)localStorage.setItem(k,String(v))});['prefs','files','tickets','mail','notes','tasks','contacts','notifications'].forEach(k=>{if(data[k])write(PREFIX+k,data[k])});if(data.userApps)write(PREFIX+'user_apps',data.userApps);if(data.remoteSessions)write(PREFIX+'remote_sessions',data.remoteSessions);if(data.emergencyLogs)write(PREFIX+'emergency_logs',data.emergencyLogs);if(data.snapshots)write(PREFIX+'vm_snapshots',data.snapshots);applyPrefs();renderDesktop();renderNotifBadge();if(show)notify('Gold VM restored','Your apps, files, settings, support history, and customizations were restored.','Gold VM')}else if(show)notify('No VM state found','Use setup or restore a backup to begin.','Gold VM')}
function exportWorkspace(){saveText('emeraldos-gold-8.0-cloud-vm-backup.json',JSON.stringify(gold1dLocalState(),null,2),'application/json')}
function resetGold(){if(confirm('Reset this EmeraldOS Gold 1D VM on this browser? Cloud data is not deleted.')){Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)).forEach(k=>localStorage.removeItem(k));location.reload()}}
function gold1dCloudStatusHTML(){return `<div class="grid4"><div class="card quick-card"><h3>Cloud sync</h3><p>${esc(localStorage.getItem(PREFIX+'cloud_status')||'ready')}</p></div><div class="card quick-card"><h3>Last save</h3><p>${esc(localStorage.getItem(PREFIX+'cloud_last_save')||'not yet')}</p></div><div class="card quick-card"><h3>VM path</h3><p>emeraldOSUsers/${esc(username())}/goldVM/current</p></div><div class="card quick-card"><h3>Tracked data</h3><p>apps, files, settings, tickets, mail, and workspace</p></div></div>`}
...
/* Gold 1D boot fix: these Gold 8 handlers are scoped inside seedGold50, so expose and install them when seedGold50 runs. */
  try{
    ...
    document.body.classList.add('gold1d','gold1d-shell');
    installGold80Apps();
    applyPrefs();
    if(!window.__gold1dAutoSaveInstalled){
      window.__gold1dAutoSaveInstalled=true;
      setInterval(()=>{const n=Number(prefs().autosaveInterval)||15;if(prefs().cloudSync!==false)saveWorkspaceNow(false)},15000);
    }
  }catch(e){console.error('Gold 1D handler install failed',e)}



/* EmeraldOS Gold 1D shell-managed update integration */
function gold1dShellManifest(){return read('emeraldGoldShell_latest',{latestVersion:'1D',folder:'Gold_1D',entry:'OS.html',releaseTitle:'EmeraldOS Gold 1D'});}
function gold1dVMState(){
  let cloudState={};
  try{cloudState=gold1dLocalState?gold1dLocalState():{};}catch(e){cloudState={};}
  return {
...
function gold1dPostState(reason='heartbeat'){
  try{
    if(window.parent&&window.parent!==window){
      window.parent.postMessage({type:'emeraldos-gold-live-state',reason,state:gold1dVMState()},'*');
    }
  }catch(e){console.warn('Gold 1D shell state post failed',e);}
}
function gold1dAskShellUpdateCheck(){try{window.parent?.postMessage({type:'emeraldos-gold-check-update',from:'Gold_1D',version:BUILD.version},'*');notify('Update check requested','The EmeraldOS Gold Shell is checking Firebase for the latest build.','Gold Update Center')}catch(e){notify('Update check failed',e.message,'Gold Update Center')}}
function openGold1AUpdateCenter(){
  const m=gold1dShellManifest();
  const active=localStorage.getItem('emeraldGoldShell_activeVersion')||BUILD.version;
  openWindow('updateshell','Gold Update Center',`<div class="goldshell-header"><div><h2>Gold Update Center</h2><p class="muted">Shell-managed updates keep your Gold VM data separate from version folders.</p></div><button class="btn primary" onclick="Gold50.gold1dAskShellUpdateCheck()">Check for updates</button></div><div class="grid2"><div class="card"><h3>Running build</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1D</p></div><div class="card"><h3>Latest Firebase pointer</h3><p>${esc(m.latestVersion||'Unknown')}</p><p class="muted">${esc(m.folder||'No folder')} / ${esc(m.entry||'OS.html')}</p></div><div class="card"><h3>Update behavior</h3><p>Gold Shell reads <code>system/emeraldGoldLatest</code>, saves a VM snapshot, then loads the folder Firebase points to.</p></div><div class="card"><h3>VM data</h3><p>Preferences, files, tickets, mail, apps, support history, and workspace state stay in the cloud VM profile.</p></div></div><h3>Release note</h3><p>${esc(m.summary||m.releaseTitle||'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.saveWorkspaceNow(true);Gold50.gold1dPostState('manual-save')">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html'">Return to Gold Shell</button></div>`,{width:980,height:680});
}
function installGold100ShellFeatures(){
  ...
  window.addEventListener('message',ev=>{const d=ev.data||{};if(d.type==='emeraldos-gold-latest'){write('emeraldGoldShell_latest',d.latest||{});notify('Update pointer refreshed',`Latest: ${(d.latest&&d.latest.latestVersion)||'unknown'}`,'Gold Update Center')}if(d.type==='emeraldos-gold-shell-save-request'){gold1dPostState('shell-save-request')}});
  window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{gold1dPostState('boot');setInterval(()=>gold1dPostState('heartbeat'),8000)},1700));
}
...
function applyGold50Visuals(){
  const p=prefs();
  document.body.classList.toggle('gold1d-solid-bg',!!p.useBackgroundColor);
  document.body.classList.toggle('gold1d-readable',!!p.readableFont);
  document.body.classList.toggle('gold1d-focus-ring',!!p.focusRing);
  document.body.classList.toggle('gold1d-large-cursor',!!p.largeCursor);
  document.body.classList.toggle('gold1d-links-underlined',!!p.underlineLinks);
  document.body.classList.toggle('gold1d-extra-contrast',!!p.extraContrast);
  document.body.classList.toggle('gold1d-less-blur',p.windowTransparency===false);
  document.body.classList.toggle('gold1d-compact-start',p.startStyle==='compact');
  document.body.classList.remove('gold1d-filter-none','gold1d-filter-warm','gold1d-filter-cool','gold1d-filter-gray');
  document.body.classList.add('gold1d-filter-'+(p.colorFilter||'none'));
  document.documentElement.style.setProperty('--gold1dBackgroundColor',p.backgroundColor||'#edf2f8');
  document.documentElement.style.setProperty('--gold1dTextScale',((Number(p.textScale)||100)/100).toString());
  document.documentElement.style.setProperty('--gold1dTaskbarHeight',p.taskbarSize==='large'?'56px':p.taskbarSize==='compact'?'40px':'48px');
}
...
function settings50Page(tab){const p=prefs();
 ...
 if(tab==='update')return `<h2>Update & Security</h2><div class="grid2"><div class="card"><h3>EmeraldOS Gold 1D</h3><p>Development, accessibility, personalization, and EmeraldOS Gold user-experience update.</p></div><div class="card"><h3>Safe Mode</h3><button class="btn" onclick="Gold50.openApp('safemode')">Open Safe Mode</button></div></div>`;
...
function getGold50Packages(){return gold1dUserApps()}
...
window.addEventListener('message',e=>{const d=e.data||{};if(d.source!=='gold1d-package')return;if(d.type==='notify')notify(d.title||'Package App',d.message||'',d.app||'Package App');if(d.type==='export')saveText(d.name||'package-export.txt',d.text||'');if(d.type==='open')openApp(d.id||'settings')});
...
function applyGold70Visuals(){
  const p=prefs();
  document.body.classList.add('gold1d');
  ...
  document.documentElement.style.setProperty('--gold1dSecondary',p.secondaryAccent||'#0f7b4d');
  document.documentElement.style.setProperty('--gold1dTaskOpacity',String((Number(p.taskbarOpacity)||92)/100));
  document.documentElement.style.setProperty('--gold1dStartWidth',(Number(p.startWidth)||860)+'px');
  document.documentElement.style.setProperty('--gold1dIconScale',p.iconSize==='large'?'1.14':p.iconSize==='small'?'.88':'1');
  document.documentElement.style.setProperty('--gold1dDesktopGap',p.density==='compact'?'9px':p.density==='spacious'?'24px':'16px');
  document.documentElement.style.setProperty('--gold1dMica',(Number(p.micaBlur)||18)+'px');
}
...
function settings60Page(tab){const p=prefs();
  ...
  if(tab==='privacy')return `<h2>Privacy</h2><label class="setting-line"><span>Focus Assist</span><input type="checkbox" ${p.focusAssist?'checked':''} onchange="Gold50.setGold70Pref('focusAssist',this.checked)"></label><label class="setting-line"><span>Notifications</span><input type="checkbox" ${p.notifications!==false?'checked':''} onchange="Gold50.setGold70Pref('notifications',this.checked)"></label><label class="setting-line"><span>Remote assistance requires visible user consent</span><input type="checkbox" checked disabled></label><button class="btn" onclick="localStorage.removeItem('${PREFIX}search_history');Gold50.notify('Privacy','Search history cleared','Settings')">Clear local search history</button>`;
  if(tab==='update')return `<h2>Update & Security</h2><div class="grid2"><div class="card"><h3>EmeraldOS Gold 1D</h3><p>Personalization, Staff resources, Support, BIOS A9, and EmeraldOS Gold-style user friendliness.</p></div><div class="card"><h3>Safe Mode</h3><button class="btn" onclick="Gold50.openApp('safemode')">Open Safe Mode</button></div></div>`;
...
function openStaffReports60(){if(!isStaff()){location.href='staff.html';return}const open=tickets().filter(t=>t.status!=='Resolved').length,res=tickets().filter(t=>t.status==='Resolved').length,remote=remoteSessions().length;openWindow('staffreports','Staff Reports',`<h2>Staff Reports</h2><div class="grid4"><div class="card"><h3>Open tickets</h3><p>${open}</p></div><div class="card"><h3>Resolved</h3><p>${res}</p></div><div class="card"><h3>Remote sessions</h3><p>${remote}</p></div><div class="card"><h3>Staff logs</h3><p>${read(PREFIX+'staff_logs',[]).length}</p></div></div><div class="toolbar"><button class="btn primary" onclick="Gold50.saveText('gold1d-tickets.json',JSON.stringify(Gold50.tickets?Gold50.tickets():[],null,2),'application/json')">Export Tickets</button><button class="btn" onclick="Gold50.staffLogs()">Open Staff Logs</button></div><h3>Recent logs</h3>${read(PREFIX+'staff_logs',[]).slice(0,8).map(l=>`<div class="card"><b>${esc(l.type)}</b><p>${esc(l.note)}</p><small>${new Date(l.time).toLocaleString()}</small></div>`).join('')||'<p class="muted">No logs.</p>'}`,{width:920,height:680})}
...
function applyGold70SupportVisuals(){document.body.classList.add('gold1d-goldshell');const p=prefs();document.documentElement.style.setProperty('--gold1dTileOpacity',p.tileOpacity||'.94');document.documentElement.style.setProperty('--gold1dPanelRadius',p.goldshellRadius||'8px')}
...
/* EmeraldOS Gold 1D rebuilt update-system fallback.
   Some Gold 1D update helpers are created inside the seeded boot scope.
...
function gold1dRebuiltSnapshot(){
  return {
    product:'EmeraldOS Gold',
    version:BUILD.version,
    folder:'Gold_1D',
...
function gold1dRebuiltLocalManifest(){
  return read('emeraldGoldShell_latest',{latestVersion:'1D',build:'1D',folder:'Gold_1D',entry:'OS.html',releaseTitle:'EmeraldOS Gold 1D',summary:'EmeraldOS Gold update-system build.'});
}
async function gold1dRebuiltFetchLatest(){
  try{
    const fbm = await import('./firebase.js');
    const snap = await fbm.getDoc(fbm.doc(fbm.db,'system','emeraldGoldLatest'));
    const latest = snap.exists()?Object.assign(gold1dRebuiltLocalManifest(),snap.data()):gold1dRebuiltLocalManifest();
    write('emeraldGoldShell_latest',latest);
    return latest;
  }catch(e){
    console.warn('Gold 1D update check Firebase unavailable',e);
    return gold1dRebuiltLocalManifest();
  }
}
function gold1dRebuiltSaveBeforeUpdate(){
  try{saveWorkspaceNow(false)}catch(e){console.warn(e)}
  try{write('emeraldGoldShell_liveState',gold1dRebuiltSnapshot())}catch(e){console.warn(e)}
}
async function gold1dRebuiltCheckForUpdates(){
  const latest = await gold1dRebuiltFetchLatest();
  const current = BUILD.version;
  const folder = latest.folder || 'Gold_1D';
  const version = latest.latestVersion || latest.build || '1B';
  const message = version===current && folder==='Gold_1D'
...
function gold1dRebuiltApplyUpdate(){
  gold1dRebuiltSaveBeforeUpdate();
  localStorage.setItem('emeraldGoldShell_forceCheck','true');
  localStorage.setItem('emeraldGoldShell_returnedFromVersion','Gold_1D');
  location.href='../gold-shell.html?force=1';
}
function openGold1AUpdateCenterRebuilt(manifest){
  const m=manifest||gold1dRebuiltLocalManifest();
  const version=m.latestVersion||m.build||'1B';
  const folder=m.folder||'Gold_1D';
  const entry=m.entry||'OS.html';
  const currentFolder='Gold_1D';
  const isDifferent=(version!==BUILD.version)||(folder!==currentFolder);
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">EmeraldOS Gold uses a root updater that checks Firebase, saves the VM, and boots the configured version folder. The updater is not visible after boot.</p></div><button class="btn primary" onclick="Gold50.gold1dCheckForUpdates()">Check now</button></div><div class="grid2"><div class="card"><h3>Current VM</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: ${currentFolder}</p></div><div class="card"><h3>Configured latest</h3><p>${esc(version)}</p><p class="muted">${esc(folder)} / ${esc(entry)}</p></div><div class="card"><h3>Status</h3><p>${isDifferent?'An update or folder change is available.':'No newer folder is currently configured.'}</p></div><div class="card"><h3>VM continuity</h3><p>Before applying an update, Gold saves a workspace snapshot so your files, settings, apps, tickets, and desktop state can continue.</p></div></div><h3>Release note</h3><p>${esc(m.summary||m.releaseTitle||'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.gold1dSaveUpdateSnapshot()">Save VM snapshot</button><button class="btn primary" onclick="Gold50.gold1dApplyUpdate()">Restart through System Update</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button></div>`,{width:980,height:680});
}
Object.assign(window.Gold50,{
  openGold1AUpdateCenter:(typeof openGold1AUpdateCenter==='function'?openGold1AUpdateCenter:openGold1AUpdateCenterRebuilt),
  gold1dAskShellUpdateCheck:(typeof gold1dAskShellUpdateCheck==='function'?gold1dAskShellUpdateCheck:gold1dRebuiltCheckForUpdates),
  gold1dCheckForUpdates:gold1dRebuiltCheckForUpdates,
  gold1dApplyUpdate:gold1dRebuiltApplyUpdate,
  gold1dSaveUpdateSnapshot:()=>{gold1dRebuiltSaveBeforeUpdate();notify('VM snapshot saved','EmeraldOS Gold saved a local update snapshot.','System Update')},
  gold1dPostState:(typeof gold1dPostState==='function'?gold1dPostState:()=>write('emeraldGoldShell_liveState',gold1dRebuiltSnapshot())),
  gold1dVMState:(typeof gold1dVMState==='function'?gold1dVMState:gold1dRebuiltSnapshot),
...
try{const updateApp=APPS.find(a=>a.id==='updateshell');const updateDef={id:'updateshell',name:'System Update',label:'UP',color:'blue',group:'System',desc:'Check Firebase for the latest EmeraldOS Gold folder and restart through the updater.',open:window.Gold50.openGold1AUpdateCenter};if(updateApp)Object.assign(updateApp,updateDef);else APPS.push(updateDef);}catch(e){console.warn('Gold 1D update app install failed',e)}
try{enhanceSettings70()}catch(e){console.warn('Gold 1D settings enhancer failed',e)}


/* Gold 1D boot fix: final exposure is installed inside seedGold50 so boot never references scoped handlers too early. */
function gold1dInstallLiveHandlers(){return true;}
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{if(window.Gold50?.openCloudSyncCenter80){document.body.classList.add('gold1d','gold1d-shell');}}catch(e){console.warn(e)}},120));


/* EmeraldOS Gold 1D first-run Firestore rules installer.
...
function gold1dRulesText(){return `rules_version = '2';
...
`;}
function gold1dRulesStatus(){return read(PREFIX+'rules_status',{ran:false,status:'not_run',message:'Rules installer has not run yet.'});}
function gold1dWriteRulesStatus(status){write(PREFIX+'rules_status',status);return status;}
async function gold1dStageFirestoreRules(){
  const status={ran:true,status:'staging',version:'1D',time:now(),message:'Preparing EmeraldOS Gold 1D rules.'};
  gold1dWriteRulesStatus(status);
  let fbm=null;
  try{fbm=await import('./firebase.js');}catch(e){status.status='local_only';status.message='Firebase module unavailable. Rules saved locally only.';status.error=e.message;gold1dWriteRulesStatus(status);return status;}
  const payload={product:'EmeraldOS Gold',version:'1D',rulesVersion:'gold10-default',rules:gold1dRulesText(),createdBy:username(),createdAt:now(),status:'pending_deployment',note:'Generated automatically by EmeraldOS Gold 1D first boot. Deploy from Firebase Console/CLI or a secured deployer endpoint.'};
  try{
...
async function gold1dTrySecureRulesDeploy(){
  ...
  const body={product:'EmeraldOS Gold',version:'1D',rules:gold1dRulesText(),requestedBy:username(),time:now()};
...
async function gold1dAutoRulesInstall(silent=false){
  if(localStorage.getItem(PREFIX+'rules_install_ran')==='true') return gold1dRulesStatus();
  localStorage.setItem(PREFIX+'rules_install_ran','true');
  const staged=await gold1dStageFirestoreRules();
  try{
    const deployed=await gold1dTrySecureRulesDeploy();
    if(deployed && (deployed.ok||deployed.deployed)){
      staged.status='deployed';staged.message='Rules were deployed through the configured secure deployer.';staged.deployerResult=deployed;gold1dWriteRulesStatus(staged);
    }
  }catch(e){staged.deployerError=e.message;gold1dWriteRulesStatus(staged);}
  if(!silent){notify('Rules installer finished',gold1dRulesStatus().message,'Security Rules');}
  return gold1dRulesStatus();
}
function openGoldRulesInstaller100(){
  const s=gold1dRulesStatus();
  openWindow('rulesinstaller','Security Rules Installer',`<div class="goldshell-header"><div><h2>Security Rules Installer</h2><p class="muted">Gold 1D prepares the recommended Firestore rules on first boot. Direct deployment requires Firebase Console/CLI or a secure deployer endpoint.</p></div><button class="btn primary" onclick="Gold50.gold1dAutoRulesInstall(false)">Run installer</button></div><div class="grid2"><div class="card"><h3>Status</h3><p>${esc(s.status)}</p><p class="muted">${esc(s.message||'')}</p></div><div class="card"><h3>Secure deployer</h3><p>Optional. Set <code>emeraldGoldRulesDeployerUrl</code> and <code>emeraldGoldRulesDeployerKey</code> in localStorage for automatic deployment through your own backend.</p></div></div><h3>Rules</h3><textarea class="field mono" rows="18">${esc(gold1dRulesText())}</textarea><div class="toolbar"><button class="btn" onclick="navigator.clipboard?.writeText(Gold50.gold1dRulesText())">Copy rules</button><button class="btn" onclick="location.href='rules-installer.html'">Open full rules page</button></div>`,{width:1080,height:760});
}
function installGold100RulesInstaller(){
  try{
    const old=APPS.find(a=>a.id==='rulesinstaller');
    const app={id:'rulesinstaller',name:'Security Rules Installer',label:'RI',color:'red',group:'System',desc:'Stages Gold 1D Firestore rules and deploys through an optional secure backend.',staffOnly:true,open:openGoldRulesInstaller100};
  ...
  }catch(e){console.warn('Gold 1D rules app install failed',e)}
  /* Rules installer is manual in redesigned Gold 1D. */
}
installGold100RulesInstaller();
Object.assign(window.Gold50,{openGoldRulesInstaller100,gold1dAutoRulesInstall,gold1dRulesText,gold1dRulesStatus,gold1dStageFirestoreRules,gold1dTrySecureRulesDeploy});



/* EmeraldOS Gold 1D redesigned manual update architecture.
   The root shell checks Firebase, but each user chooses when to move their VM to a newer folder.
   Gold_1D can publish its own latest pointer when manually booted once by staff. */
const GOLD1A_LATEST_MANIFEST = {
  ...
  enabled: true,
  setupMode: "continue",
  releaseTitle: "EmeraldOS Gold 1D",
...
function gold1dCurrentManifest(){ return {...GOLD1A_LATEST_MANIFEST}; }
function gold1dLatestCache(){ return read('emeraldGoldShell_latest', GOLD1A_LATEST_MANIFEST); }
function gold1dIsNewerManifest(latest){
  if(!latest) return false;
  const latestVersion = String(latest.latestVersion || latest.build || '');
  const latestFolder = String(latest.folder || '');
  return latestVersion !== BUILD.version || latestFolder !== 'Gold_1D';
}
async function gold1dFetchLatestManifest(){
  try{
    const fbm = await import('./firebase.js');
    const snap = await fbm.getDoc(fbm.doc(fbm.db, 'system', 'emeraldGoldLatest'));
    ...
    console.warn('Gold 1D could not read update manifest', error);
    return gold1dLatestCache();
  }
}
function gold1dUpdateSnapshot(){
  const data = {
    product:'EmeraldOS Gold',
    version:BUILD.version,
    folder:'Gold_1D',
...
async function gold1dCloudWrite(path, value){
  try{ const fbm = await import('./firebase.js'); await fbm.setDoc(fbm.doc(fbm.db, ...path), value, {merge:true}); return true; }
  catch(error){ console.warn('Gold 1D cloud write failed', path, error); return false; }
}
async function gold1dSaveManualUpdateSnapshot(){
  const snap = gold1dUpdateSnapshot();
  const user = username();
  await gold1dCloudWrite(['emeraldOSUsers', user, 'goldVM', 'current'], {
    activeVersion: BUILD.version,
    activeFolder: 'Gold_1D',
    entry: 'OS.html',
    lastSave: now(),
    cloudSync: true
  });
  await gold1dCloudWrite(['emeraldOSUsers', user, 'goldVMSnapshots', `manual_update_${Date.now()}`], snap);
  notify('VM snapshot saved','Your EmeraldOS Gold VM state was saved before update.','System Update');
  return snap;
}
async function gold1dApplyManualUpdate(){
  const latest = await gold1dFetchLatestManifest();
  if(!latest || !latest.folder){ alert('No update target is configured.'); return; }
  await gold1dSaveManualUpdateSnapshot();
...
function gold1dSkipUpdate(version){
...
function gold1dRemindLater(version){
...
function gold1dUpdatePromptHTML(latest){
  const version = esc(latest.latestVersion || latest.build || 'Unknown');
  const folder = esc(latest.folder || '');
  return `<div class="update-prompt"><h2>EmeraldOS Gold update available</h2><p>A newer EmeraldOS Gold version is available for your cloud VM.</p><div class="grid2"><div class="card"><h3>Current</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1D</p></div><div class="card"><h3>Available</h3><p>EmeraldOS Gold ${version}</p><p class="muted">Folder: ${folder}</p></div></div><h3>Release note</h3><p>${esc(latest.summary || latest.releaseTitle || 'No release note was provided.')}</p><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1dApplyManualUpdate()">Update this VM now</button><button class="btn" onclick="Gold50.gold1dRemindLater('${version}')">Remind me later</button><button class="btn" onclick="Gold50.gold1dSkipUpdate('${version}')">Skip this version</button><button class="btn" onclick="Gold50.openGold1AUpdateCenter()">Open System Update</button></div><p class="muted">Updates are manual per user. Your VM keeps running this version until you choose to update.</p></div>`;
}
function gold1dOpenUpdatePrompt(latest){
  openWindow('updateprompt','EmeraldOS Gold Update Available',gold1dUpdatePromptHTML(latest),{width:860,height:590});
}
async function gold1dCheckLoginUpdateNotice(){
  const latest = await gold1dFetchLatestManifest();
  const latestVersion = String(latest.latestVersion || latest.build || '');
  ...
  const newer = gold1dIsNewerManifest(latest);
  write('emeraldGold_updateAvailable', {available:newer, latest, active:gold1dCurrentManifest(), checkedAt:now()});
  if(newer && skipped !== latestVersion && !(laterVersion === latestVersion && Date.now() < laterUntil)){
    notify('EmeraldOS Gold update available',`Version ${latestVersion} is ready. You can update now or stay on Gold 1D.`, 'System Update');
    setTimeout(()=>gold1dOpenUpdatePrompt(latest), 700);
  }
}
function openGold1AUpdateCenter(latestManifest){
  const latest = latestManifest || gold1dLatestCache();
  const newer = gold1dIsNewerManifest(latest);
  const version = latest.latestVersion || latest.build || 'Unknown';
  const folder = latest.folder || '';
  const active = read('emeraldGoldShell_activeManifest', gold1dCurrentManifest());
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">EmeraldOS Gold checks Firebase for the newest approved version. Updates are manual for each user, so your VM only changes when you choose Update.</p></div><button class="btn primary" onclick="Gold50.gold1dCheckForUpdatesManual()">Check now</button></div><div class="grid2"><div class="card"><h3>This VM is running</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1D</p></div><div class="card"><h3>Latest configured build</h3><p>EmeraldOS Gold ${esc(version)}</p><p class="muted">${esc(folder)} / ${esc(latest.entry || 'OS.html')}</p></div><div class="card"><h3>User update mode</h3><p>${newer?'A newer version is available.':'This VM is on the latest configured version.'}</p><p class="muted">The shell will not move you to a new folder until you accept the update.</p></div><div class="card"><h3>Release publisher</h3><p>When a new version folder is uploaded, boot that version manually once to publish its Firebase pointer.</p><button class="btn" onclick="Gold50.gold1dPublishLatestManifest(true)">Publish Gold 1D as latest</button></div></div><h3>Release note</h3><p>${esc(latest.summary || latest.releaseTitle || 'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.gold1dSaveManualUpdateSnapshot()">Save VM snapshot</button><button class="btn primary" ${newer?'':'disabled'} onclick="Gold50.gold1dApplyManualUpdate()">Update this VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="Gold50.gold1dResetUpdateChoice()">Reset update choice</button></div>`,{width:1020,height:720});
}
async function gold1dCheckForUpdatesManual(){
  const latest = await gold1dFetchLatestManifest();
  openGold1AUpdateCenter(latest);
  if(gold1dIsNewerManifest(latest)) gold1dOpenUpdatePrompt(latest);
  else notify('System Update','This VM is running the latest configured EmeraldOS Gold version.','System Update');
}
function gold1dResetUpdateChoice(){
...
async function gold1dPublishLatestManifest(showResult=false){
  ...
  try{
    ...
    if(showResult || manualBoot) notify('Release pointer published','system/emeraldGoldLatest now points to Gold_1D.','System Update');
    return {ok:true};
  }catch(error){
    console.warn('Could not publish Gold 1D latest pointer', error);
    write(PREFIX+'latest_manifest_publish_error', {time:now(), error:error.message, manifest:GOLD1A_LATEST_MANIFEST});
    if(showResult || manualBoot) notify('Release pointer not published', 'Firebase rules may require staff write access. Use Gold_1D/OS.html?publishLatest=1 or staff credentials.', 'System Update');
    return {ok:false, error:error.message};
  }
}
function gold1dInstallManualUpdateSystem(){
  try{
    ...
    if(old) Object.assign(old, app); else APPS.push(app);
  }catch(error){ console.warn('Gold 1D manual update app install failed', error); }
}
gold1dInstallManualUpdateSystem();
Object.assign(window.Gold50, {
  openGold1AUpdateCenter,
  gold1dCheckForUpdatesManual,
  gold1dApplyManualUpdate,
  gold1dSaveManualUpdateSnapshot,
  gold1dResetUpdateChoice,
  gold1dSkipUpdate,
  gold1dRemindLater,
  gold1dPublishLatestManifest,
  gold1dFetchLatestManifest,
  gold1dCurrentManifest
});
window.addEventListener('DOMContentLoaded',()=>{
  setTimeout(()=>gold1dPublishLatestManifest(false), 1800);
  setTimeout(()=>gold1dCheckLoginUpdateNotice(), 2600);
});
...
/* EmeraldOS Gold 1D OS-folder-only update helpers */
function gold1dLatestManifest(){
  return {
    product:'EmeraldOS Gold', latestVersion:'1D', build:'1D', folder:'Gold_1D', entry:'OS.html',
...
async function gold1dPublishLatest(showResult=true){
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
    const {doc,setDoc}=api;
    const manifest=gold1dLatestManifest();
    await setDoc(doc(api.db,'system','emeraldGoldLatest'), manifest, {merge:true});
    localStorage.setItem('gold1d_published_latest','true');
    if(showResult) notify('System Update','Gold 1D was published as the latest available build.','System Update');
    return true;
  }catch(e){
    console.warn('Gold 1D publish latest failed', e);
    if(showResult) notify('System Update','Could not publish latest build. Check Firebase rules/access.','System Update');
    return false;
  }
}
function openGold1AUpdateCenter(){
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">Gold 1D is an OS folder only. The shell stays outside this folder and only routes updates when users choose to update.</p></div><button class="btn primary" onclick="Gold50.gold1dCheckForUpdatesManual?.()||Gold50.gold1dCheckForUpdates?.()">Check Firebase</button></div><div class="grid2"><div class="card"><h3>This folder</h3><p>EmeraldOS Gold 1D</p><p class="muted">Gold_1D / OS.html</p></div><div class="card"><h3>Publish flow</h3><p>Boot Gold_1D manually once with <code>?publishLatest=1</code>, or press publish here, to set <code>system/emeraldGoldLatest</code>.</p></div><div class="card"><h3>User updates</h3><p>Each user updates manually. The shell notifies them on login and only changes their active folder after they accept.</p></div><div class="card"><h3>Continuity</h3><p>VM data remains stored under <code>goldVM/current</code>, independent of the version folder.</p></div></div><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1dPublishLatest(true)">Publish Gold 1D as latest</button><button class="btn" onclick="Gold50.saveWorkspaceNow(true)">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html?applyUpdate=1'">Restart through shell</button></div>`,{width:1040,height:700});
}
function gold1dInstallUpdateApp(){
  try{
  ...
  }catch(e){console.warn('Gold 1D update app install failed',e)}
}
gold1dInstallUpdateApp();
Object.assign(window.Gold50||{}, {gold1dLatestManifest,gold1dPublishLatest,openGold1AUpdateCenter});
window.Gold1A=window.Gold50; window.Gold1D=window.Gold50;
if(new URLSearchParams(location.search).has('publishLatest')) setTimeout(()=>gold1dPublishLatest(true), 1200);


/* =========================================================
   EMERALDOS GOLD 1D - E.L.S.U.S. COMPATIBILITY + UX PATCH
...
const GOLD1A_RELEASE_MANIFEST={
  ...
  channel:'stable', status:'stable', required:false, enabled:true, setupMode:'continue',
  releaseTitle:'EmeraldOS Gold 1D',
  summary:'Gold 1D is an OS-folder-only E.L.S.U.S.-compatible release built from Gold 11.0. It improves the EmeraldOS Gold desktop experience, adds a curated non-duplicate app catalog, improves customization safety, and keeps VM data version-independent.',
  migrationFrom:['11.0','10.0','9.0','8.0','8.0.1'], migrationId:'gold11-to-gold1d-elsus',
  minShellVersion:'1.0', rollbackFolder:'Gold_11.0', rollbackVersion:'11.0', releasedAt:new Date().toISOString()
};
function gold1dClamp(n,min,max,fall){n=Number(n);if(!Number.isFinite(n))n=fall;return Math.max(min,Math.min(max,n));}
function gold1dCSSColor(v,fall){v=String(v||'').trim();return /^#[0-9a-f]{3,8}$/i.test(v)?v:fall;}
function gold1dCurrentManifest(){return {...GOLD1A_RELEASE_MANIFEST};}
async function gold1dPublishLatest(showResult=true){
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
...
function gold1dInstallCuratedApps(){
  ...
  unique.forEach(gold1dAddOrUpdateApp);
  ...
  pref.backgroundColor=pref.backgroundColor||'#eaf3ff'; pref.panelOpacity=gold1dClamp(pref.panelOpacity,70,100,94); pref.taskbarOpacity=gold1dClamp(pref.taskbarOpacity,60,100,92);
  pref.startWidth=gold1dClamp(pref.startWidth,560,1100,880); pref.micaBlur=gold1dClamp(pref.micaBlur,0,30,18);
  write(PREFIX+'prefs',pref);
}
function gold1dApplyCustomizationSafeCss(){
  const p=prefs(); const root=document.documentElement;
  document.body.classList.add('gold1d','gold1d-elsus');
  document.body.classList.toggle('gold1d-solid-bg',!!p.useBackgroundColor && !p.customBackground);
  document.body.classList.toggle('gold1d-taskbar-top',p.taskbarPosition==='top');
  root.style.setProperty('--gold1dBgColor',gold1dCSSColor(p.backgroundColor,'#eaf3ff'));
  root.style.setProperty('--gold1dAccent',gold1dCSSColor(p.accent,'#0078d4'));
  root.style.setProperty('--gold1dAccent2',gold1dCSSColor(p.secondaryAccent,'#0f7b4d'));
  root.style.setProperty('--accent',gold1dCSSColor(p.accent,'#0078d4'));
  root.style.setProperty('--accent2',gold1dCSSColor(p.secondaryAccent,'#0f7b4d'));
  root.style.setProperty('--gold1dPanelOpacity',String(gold1dClamp(p.panelOpacity,70,100,94)/100));
  root.style.setProperty('--gold1dTaskOpacity',String(gold1dClamp(p.taskbarOpacity,60,100,92)/100));
  root.style.setProperty('--gold1dMica',gold1dClamp(p.micaBlur,0,30,18)+'px');
  root.style.setProperty('--gold1dStartWidth',gold1dClamp(p.startWidth,560,1100,880)+'px');
  root.style.setProperty('--gold1dIconScale',p.iconSize==='xl'?'1.28':p.iconSize==='large'?'1.14':p.iconSize==='small'?'.88':'1');
  root.style.setProperty('--gold1dDesktopGap',p.density==='compact'?'9px':p.density==='spacious'?'24px':'16px');
  root.style.setProperty('--winRadius',p.rounding==='none'?'0':p.rounding==='large'?'14px':p.rounding==='pill'?'18px':'8px');
}
function gold1dWireCustomizationSafety(){
  const oldSetGold70=window.Gold50?.setGold70Pref;
  if(oldSetGold70&&!oldSetGold70.gold1dWrapped){
    const wrapped=function(k,v){oldSetGold70(k,v);gold1dApplyCustomizationSafeCss();try{renderDesktop()}catch{}};wrapped.gold1dWrapped=true;window.Gold50.setGold70Pref=wrapped;
  }
  const oldSetPrefs=window.Gold50?.setPrefs;
  if(oldSetPrefs&&!oldSetPrefs.gold1dWrapped){
    const wrapped=function(p){oldSetPrefs(p);gold1dApplyCustomizationSafeCss();try{renderDesktop()}catch{}};wrapped.gold1dWrapped=true;window.Gold50.setPrefs=wrapped;
  }
}
function openGold1AWelcome(){
  openWindow('emeraldbrand','EmeraldOS Gold 1D',`<div class="welcome70"><div class="gold1d-hero"><div class="gold-logo-xl">G</div><div><h1>EmeraldOS Gold 1D</h1><p>OS-folder-only E.L.S.U.S. release with a cleaner EmeraldOS Gold desktop, curated app catalog, safer customization, and cloud VM continuity.</p></div></div><div class="grid3"><div class="card"><h3>Customize everything</h3><p>Theme Studio controls background color, wallpaper, accents, taskbar, panels, blur, rounding, desktop spacing, and accessibility.</p><button class="btn primary" onclick="Gold50.openApp('themestudio')">Open Theme Studio</button></div><div class="card"><h3>Cloud VM</h3><p>Your files, settings, apps, tickets, mail, notes, tasks, and workspace stay separate from the version folder.</p><button class="btn" onclick="Gold50.openApp('cloudsync')">Open Cloud Sync</button></div><div class="card"><h3>E.L.S.U.S.</h3><p>The updater shell prompts users and only changes their active version when they choose to update.</p><button class="btn" onclick="Gold50.openApp('updateshell')">System Update</button></div></div></div>`,{width:940,height:620});
}
function gold1dInstallBootPolish(){
  gold1dInstallCuratedApps();
  gold1dApplyCustomizationSafeCss();
  gold1dWireCustomizationSafety();
  gold1dAddOrUpdateApp({id:'emeraldbrand',name:'EmeraldOS Gold Welcome',label:'EG',color:'gold',group:'Core',purpose:'Welcome',desc:'Overview of Gold 1D, E.L.S.U.S., customization, and cloud VM continuity.',open:openGold1AWelcome});
...
window.Gold1A=window.Gold50; window.Gold1D=window.Gold50;
window.addEventListener('DOMContentLoaded',()=>setTimeout(gold1dInstallBootPolish,120));
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{ if(new URLSearchParams(location.search).has('publishLatest')) gold1dPublishLatest(true); },1500));


/* =========================================================
   EMERALDOS GOLD 1D UPDATE SETUP + CLASSIC RIGHT-CLICK MENU
========================================================= */
const GOLD1D_ESSENTIAL_DESKTOP=['explorer','office','mail','support','settings','updateshell'];
function gold1dDesktopPins(){return read(PREFIX+'desktopFilePins',[])}
function gold1dSaveDesktopPins(arr){write(PREFIX+'desktopFilePins',[...new Set(arr)].filter(Boolean));saveWorkspaceDebounced();try{renderDesktop()}catch{}}
function gold1dStartPins(){return read(PREFIX+'startPins',['explorer','office','support','settings','updateshell'])}
function gold1dTaskbarPins(){return read(PREFIX+'taskbarPins',['explorer','office','support'])}
function gold1dSetStartPins(arr){write(PREFIX+'startPins',[...new Set(arr)].filter(Boolean));try{renderStartMenu()}catch{};saveWorkspaceDebounced()}
function gold1dSetTaskbarPins(arr){write(PREFIX+'taskbarPins',[...new Set(arr)].filter(Boolean));try{renderTaskApps()}catch{};saveWorkspaceDebounced()}
function gold1dPin(arr,id){return [...new Set([...(arr||[]),id])].filter(Boolean)}
function gold1dUnpin(arr,id){return (arr||[]).filter(x=>x!==id)}
function gold1dApplyEssentialDesktop(){const p=prefs();p.desktopApps=[...GOLD1D_ESSENTIAL_DESKTOP];write(PREFIX+'prefs',p);applyPrefs();renderDesktop();renderStartMenu();notify('Desktop cleaned','Only essential apps are pinned to the desktop. All apps are still in Start and Search.','Setup')}
function gold1dPinAppToDesktop(id){const p=prefs();p.desktopApps=gold1dPin(p.desktopApps||[],id);write(PREFIX+'prefs',p);applyPrefs();renderDesktop();notify('Pinned to desktop',appById(id)?.name||id,'Desktop')}
function gold1dRemoveAppFromDesktop(id){const p=prefs();p.desktopApps=gold1dUnpin(p.desktopApps||[],id);write(PREFIX+'prefs',p);applyPrefs();renderDesktop();notify('Removed from desktop',appById(id)?.name||id,'Desktop')}
function gold1dPinAppToStart(id){gold1dSetStartPins(gold1dPin(gold1dStartPins(),id));notify('Pinned to Start',appById(id)?.name||id,'Start')}
function gold1dUnpinAppFromStart(id){gold1dSetStartPins(gold1dUnpin(gold1dStartPins(),id));notify('Unpinned from Start',appById(id)?.name||id,'Start')}
function gold1dPinAppToTaskbar(id){gold1dSetTaskbarPins(gold1dPin(gold1dTaskbarPins(),id));notify('Pinned to taskbar',appById(id)?.name||id,'Taskbar')}
function gold1dUnpinAppFromTaskbar(id){gold1dSetTaskbarPins(gold1dUnpin(gold1dTaskbarPins(),id));notify('Unpinned from taskbar',appById(id)?.name||id,'Taskbar')}
function gold1dPinFileToDesktop(fid){if(!files().some(f=>f.id===fid)){notify('File not found','That file could not be pinned.','Desktop');return}gold1dSaveDesktopPins(gold1dPin(gold1dDesktopPins(),fid));notify('Pinned file to desktop',files().find(f=>f.id===fid)?.name||fid,'Gold Explorer')}
function gold1dUnpinFileFromDesktop(fid){gold1dSaveDesktopPins(gold1dUnpin(gold1dDesktopPins(),fid));notify('Unpinned file from desktop',files().find(f=>f.id===fid)?.name||fid,'Desktop')}
function gold1dFileDesktopIcon(f){return `<button class="desktop-icon desktop-file ${prefs().showLabels?'':'no-label'}" data-fileid="${esc(f.id)}" ondblclick="Gold50.openFile('${esc(f.id)}')" title="${esc(f.name)}"><div class="file-desktop-icon">${esc(fileIcon(f.type))}</div><span>${esc(f.name)}</span></button>`}
try{
  const gold1dOriginalRenderDesktop=renderDesktop;
  renderDesktop=function(){
    ...
    const pinIds=gold1dDesktopPins();
    const fileHTML=pinIds.map(fid=>files().find(f=>f.id===fid&&!f.trash)).filter(Boolean).map(gold1dFileDesktopIcon).join('');
    d.innerHTML=appHTML+fileHTML;
  };
}catch(e){console.warn('Gold 1D desktop override skipped',e)}
try{
  const gold1dOriginalRenderTaskApps=renderTaskApps;
  renderTaskApps=function(){
    const bar=$('taskApps'); if(!bar) return;
    const pinHTML=gold1dTaskbarPins().map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff()).map(a=>`<button class="taskbar-app pinned" data-appid="${esc(a.id)}" title="Pinned: ${esc(a.name)}" onclick="Gold50.openApp('${esc(a.id)}')">${appIcon(a,true)}<span>${esc(a.name)}</span></button>`).join('');
...
}catch(e){console.warn('Gold 1D taskbar override skipped',e)}
try{
  renderStartMenu=function(){
    const sm=$('startMenu'); if(!sm) return;
    const apps=visibleApps(); const groups=[...new Set(apps.map(a=>a.group))];
    const pins=gold1dStartPins().map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff());
...
}catch(e){console.warn('Gold 1D Start override skipped',e)}
function gold1dContextMenu(items,x,y){
...
function gold1dDesktopContext(e){
  ...
  if(fileEl){
    ...
    return gold1dContextMenu([
      {id:'open',label:'Open',action:()=>openFile(fid)},
      {id:'unpin',label:'Unpin file from desktop',action:()=>gold1dUnpinFileFromDesktop(fid)},
  ...
  if(appEl){
    ...
    return gold1dContextMenu([
      ...
      {id:'start',label:gold1dStartPins().includes(id)?'Unpin from Start':'Pin to Start',action:()=>gold1dStartPins().includes(id)?gold1dUnpinAppFromStart(id):gold1dPinAppToStart(id)},
      {id:'taskbar',label:gold1dTaskbarPins().includes(id)?'Unpin from taskbar':'Pin to taskbar',action:()=>gold1dTaskbarPins().includes(id)?gold1dUnpinAppFromTaskbar(id):gold1dPinAppToTaskbar(id)},
      {id:'desktop',label:(p.desktopApps||[]).includes(id)?'Remove from desktop':'Pin to desktop',action:()=>((prefs().desktopApps||[]).includes(id)?gold1dRemoveAppFromDesktop(id):gold1dPinAppToDesktop(id))},
      'sep',
  ...
  return gold1dContextMenu([
    ...
    'sep',
    {id:'clean',label:'Use essential desktop layout',action:()=>gold1dApplyEssentialDesktop()},
    {id:'settings',label:'Settings',action:()=>openSettings()}
  ],e.clientX,e.clientY);
}
function openDesktopCustomizer1B(){
  const p=prefs(); const pinnedFiles=gold1dDesktopPins(); const sPins=gold1dStartPins(); const tPins=gold1dTaskbarPins();
  openWindow('desktopcustomizer','Desktop Customizer',`<div class="goldshell-header"><div><h2>Desktop Customizer</h2><p class="muted">Classic EmeraldOS right-click tools for desktop apps, Start pins, taskbar pins, and pinned files.</p></div><button class="btn primary" onclick="Gold50.gold1dApplyEssentialDesktop()">Essential desktop</button></div><div class="grid2"><div class="card"><h3>Apps</h3><div class="purpose-map compact">${visibleApps().map(a=>`<div class="card mini"><h4>${appIcon(a,true)} ${esc(a.name)}</h4><div class="toolbar"><button class="btn small" onclick="Gold50.${(p.desktopApps||[]).includes(a.id)?'gold1dRemoveAppFromDesktop':'gold1dPinAppToDesktop'}('${esc(a.id)}')">${(p.desktopApps||[]).includes(a.id)?'Remove desktop':'Desktop'}</button><button class="btn small" onclick="Gold50.${sPins.includes(a.id)?'gold1dUnpinAppFromStart':'gold1dPinAppToStart'}('${esc(a.id)}')">${sPins.includes(a.id)?'Unpin Start':'Start'}</button><button class="btn small" onclick="Gold50.${tPins.includes(a.id)?'gold1dUnpinAppFromTaskbar':'gold1dPinAppToTaskbar'}('${esc(a.id)}')">${tPins.includes(a.id)?'Unpin taskbar':'Taskbar'}</button></div></div>`).join('')}</div></div><div class="card"><h3>Files pinned to desktop</h3><p class="muted">Pin important files from your Gold VM to the desktop without moving them.</p>${files().filter(f=>!f.trash).slice(0,80).map(f=>`<div class="settings-row"><div><b>${esc(f.name)}</b><p class="muted">${esc(f.folder)} · ${esc(f.type)}</p></div><button class="btn small" onclick="Gold50.${pinnedFiles.includes(f.id)?'gold1dUnpinFileFromDesktop':'gold1dPinFileToDesktop'}('${esc(f.id)}')">${pinnedFiles.includes(f.id)?'Unpin':'Pin file'}</button></div>`).join('')||'<p class="muted">No files yet.</p>'}</div></div>`,{width:1100,height:720})
}
function openUpdateSetup1B(){
  const box=$('setupWizard'); if(!box) return;
  let step=0;
  const steps=[
    ()=>`<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>EmeraldOS Gold 1D Update Setup</h2><p class="muted">Your VM was updated through E.L.S.U.S. This short setup keeps your files and settings, then helps you choose the new desktop layout.</p></div></div><div class="setup-checklist"><span>VM preserved</span><span>Desktop controls added</span><span>Classic right-click returned</span></div></div>`,
    ()=>`<h2>Choose your updated desktop</h2><p class="muted">Gold 1D keeps only essential apps on the desktop by default. Everything else stays available in Start and Search.</p><div class="grid3"><button class="tile-button" onclick="Gold50.gold1dApplyEssentialDesktop()">Use essential desktop</button><button class="tile-button" onclick="Gold50.openDesktopCustomizer1B()">Customize pins</button><button class="tile-button" onclick="Gold50.openApp('themestudio')">Customize style</button></div><h3>Essential desktop apps</h3><div class="grid3">${GOLD1D_ESSENTIAL_DESKTOP.map(id=>appById(id)).filter(Boolean).map(a=>`<div class="card">${appIcon(a,true)} <b>${esc(a.name)}</b><p class="muted">${esc(a.desc)}</p></div>`).join('')}</div>`,
    ...
    ()=>`<h2>You're ready</h2><p class="muted">Gold 1D is ready. Your VM data is still managed by E.L.S.U.S. and your existing data has been kept.</p><div class="grid3"><button class="tile-button" onclick="Gold50.openApp('updateshell')">System Update</button><button class="tile-button" onclick="Gold50.openApp('desktopcustomizer')">Desktop Customizer</button><button class="tile-button" onclick="Gold50.openApp('support')">Support Center</button></div>`
  ];
  function render(){box.classList.remove('hidden');box.innerHTML=`<div class="setup-card setup-card-rebuilt update-setup"><div class="setup-titlebar"><b>Gold 1D Update Setup</b><button class="btn small" onclick="Gold50.finishUpdateSetup1B()">Skip</button></div><div class="setup-steps">${steps.map((_,i)=>`<div class="setup-dot ${i<=step?'active':''}"></div>`).join('')}</div><div class="setup-body">${steps[step]()}</div><div class="toolbar setup-actions"><button class="btn" ${step===0?'disabled':''} onclick="Gold50.updateSetupStep1B(-1)">Back</button><span class="muted">Step ${step+1} of ${steps.length}</span><button class="btn primary" onclick="Gold50.updateSetupStep1B(1)">${step===steps.length-1?'Finish':'Next'}</button></div></div>`}
...
function finishUpdateSetup1B(){localStorage.setItem(PREFIX+'update_setup_done','true');$('setupWizard')?.classList.add('hidden');notify('Update setup complete','EmeraldOS Gold 1D is ready.','Setup')}
function gold1dInstallUpdateSetup(){
  const migrated=localStorage.getItem('gold1d_migrated_from_gold1a')==='true'||localStorage.getItem('gold1a_setup_done')==='true';
...
function gold1dInstallContextMenus(){document.addEventListener('contextmenu',gold1dDesktopContext);document.addEventListener('keydown',e=>{if(e.shiftKey&&e.key==='F10'){e.preventDefault();const d=$('desktop');const r=d?.getBoundingClientRect();gold1dContextMenu([{id:'customize',label:'Open Desktop Customizer',action:()=>openDesktopCustomizer1B()},{id:'refresh',label:'Refresh desktop',action:()=>renderDesktop()}],(r?.left||20)+80,(r?.top||20)+80)}})}
gold1dAddOrUpdateApp({id:'desktopcustomizer',name:'Desktop Customizer',label:'DC',color:'blue',group:'Customization',purpose:'Desktop pin management',desc:'Classic desktop right-click tools for app pins, taskbar pins, Start pins, and pinned files.',open:openDesktopCustomizer1B});
Object.assign(window.Gold50||{}, {gold1dApplyEssentialDesktop,gold1dPinAppToDesktop,gold1dRemoveAppFromDesktop,gold1dPinAppToStart,gold1dUnpinAppFromStart,gold1dPinAppToTaskbar,gold1dUnpinAppFromTaskbar,gold1dPinFileToDesktop,gold1dUnpinFileFromDesktop,openDesktopCustomizer1B,openUpdateSetup1B,finishUpdateSetup1B});
window.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{try{gold1dInstallContextMenus();renderDesktop();renderStartMenu();renderTaskApps();gold1dInstallUpdateSetup();}catch(e){console.warn('Gold 1D polish failed',e)}},650)});


/* =========================================================
   EMERALDOS GOLD 1D - ADVANCED DESKTOP, ROLLBACK, CRASH UX,
   AND STAFF-ONLY E.L.S.U.S. PUBLISHER MANAGER
========================================================= */
const GOLD1D_PUBLISHER_PIN = "093013";
const GOLD1D_ROLLBACK_TARGETS = [
...
function gold1dValidColor(value, fallback){
  value=String(value||"").trim();
  return /^#[0-9a-f]{3,8}$/i.test(value)?value:fallback;
}
function gold1dNumber(value,min,max,fallback){
  value=Number(value);
...
try{
  const gold1dBaseDefaultPrefs = defaultPrefs;
  defaultPrefs = function(){
    const base = gold1dBaseDefaultPrefs();
    return {
      ...base,
...
}catch(e){console.warn("Gold 1D default preference extension skipped",e)}

try{
  const gold1dBaseApplyPrefs = applyPrefs;
  applyPrefs = function(){
    gold1dBaseApplyPrefs();
    const p=prefs();
    const root=document.documentElement;
    root.style.setProperty("--gold1dTaskbarColor", gold1dValidColor(p.taskbarColor,"#f6f9fd"));
    root.style.setProperty("--gold1dTaskbarText", gold1dValidColor(p.taskbarTextColor,"#111827"));
    root.style.setProperty("--gold1dStartColor", gold1dValidColor(p.startMenuColor,"#ffffff"));
    root.style.setProperty("--gold1dGridSize", gold1dNumber(p.desktopGridSize,64,150,92)+"px");
    document.body.classList.toggle("gold1d-free-desktop", p.desktopObjectMode!=="grid");
    document.body.classList.toggle("gold1d-grid-desktop", p.desktopObjectMode==="grid");
    document.body.classList.toggle("gold1d-trash-hidden", p.trashOnDesktop===false);
  };
}catch(e){console.warn("Gold 1D visual preference extension skipped",e)}

function gold1dDesktopPositions(){return read(PREFIX+"desktopPositions",{})}
function gold1dSaveDesktopPositions(map){write(PREFIX+"desktopPositions",map||{});saveWorkspaceDebounced()}
function gold1dDesktopItemKey(type,id){return type+":"+id}
function gold1dDefaultPosition(i){
  const p=prefs(), grid=gold1dNumber(p.desktopGridSize,64,150,92);
  const cols=Math.max(1,Math.floor((window.innerWidth-40)/grid));
  return {x:18+(i%cols)*grid, y:18+Math.floor(i/cols)*(grid+18)};
}
function gold1dPositionStyle(key,index){
  const pos=gold1dDesktopPositions()[key] || gold1dDefaultPosition(index);
  const x=gold1dNumber(pos.x,0,Math.max(0,innerWidth-92),18);
  const y=gold1dNumber(pos.y,0,Math.max(0,innerHeight-120),18);
  return `left:${x}px;top:${y}px;`;
}
function gold1dSnap(v){
  const p=prefs();
  if(!p.desktopSnapToGrid) return v;
  const grid=gold1dNumber(p.desktopGridSize,64,150,92);
  return Math.round(v/grid)*grid+18;
}
function gold1dTrashIconHTML(index){
  const key=gold1dDesktopItemKey('system','trash');
  return `<button class="desktop-icon desktop-trash ${prefs().showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-trash="true" style="${gold1dPositionStyle(key,index)}" ondblclick="Gold50.openTrashCan1D()" title="Trash"><div class="trash-can-icon">♻</div><span>Trash</span></button>`;
}
function openTrashCan1D(){
  try{ openExplorer(); setTimeout(()=>Gold50.explorerView('trash'),80); }
  catch(e){ gold1dFileOrAppFailure('Trash','Trash failed to load.',e,'trash'); }
}
...
try{
  renderDesktop=function(){
    const d=$('desktop'); if(!d) return;
    const p=prefs();
    d.classList.add('gold1d-movable-desktop');
    d.style.gap=p.density==='compact'?'8px':p.density==='spacious'?'22px':'16px';
    ...
    const appHTML=(p.desktopApps||[]).map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff()).map(a=>{
      const key=gold1dDesktopItemKey('app',a.id);
      const html=`<button class="desktop-icon ${p.showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-appid="${esc(a.id)}" style="width:${width};${gold1dPositionStyle(key,i++)}" ondblclick="Gold50.openApp('${esc(a.id)}')" title="${esc(a.desc)}">${appIcon(a)}<span>${esc(a.name)}</span></button>`;
      return html;
    }).join('');
    const fileHTML=gold1dDesktopPins().map(fid=>files().find(f=>f.id===fid&&!f.trash)).filter(Boolean).map(f=>{
      const key=gold1dDesktopItemKey('file',f.id);
      return `<button class="desktop-icon desktop-file ${p.showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-fileid="${esc(f.id)}" style="width:${width};${gold1dPositionStyle(key,i++)}" ondblclick="Gold50.openFile('${esc(f.id)}')" title="${esc(f.name)}"><div class="file-desktop-icon">${esc(fileIcon(f.type))}</div><span>${esc(f.name)}</span></button>`;
    }).join('');
    const trashHTML=p.trashOnDesktop===false?'':gold1dTrashIconHTML(i++);
    d.innerHTML=appHTML+fileHTML+trashHTML;
  };
}catch(e){console.warn('Gold 1D desktop renderer failed to install',e)}

function gold1dInstallDesktopDrag(){
  let state=null;
  ...
  document.addEventListener('pointerup',e=>{
    ...
    state.icon.style.left=x+'px'; state.icon.style.top=y+'px';
    const map=gold1dDesktopPositions(); map[state.key]={x,y,updatedAt:now()}; gold1dSaveDesktopPositions(map);
...
function gold1dCenterPopup(title,message,level='warning',actions=[]){
  if(prefs().centerFailurePopups===false) return notify(title,message,'EmeraldOS Gold');
  document.querySelectorAll('.gold1d-center-popup').forEach(x=>x.remove());
  const d=document.createElement('div');
  d.className='gold1d-center-popup '+level;
...
function gold1dFileOrAppFailure(name,message,error,kind='app'){
  console.warn('Gold 1D load failure',name,error);
  notify('Load problem',`${name} refused to load.`,'EmeraldOS Gold');
  gold1dCenterPopup(`${kind==='file'?'File':'Application'} refused to load`,`${message} ${error?.message||''}`.trim(),'warning',[
...
function gold1dBSOD(reason='EmeraldOS Gold stopped responding.',code='GOLD_RUNTIME_FAULT'){
  if(document.querySelector('.gold1d-bsod')) return;
  const d=document.createElement('div');
  d.className='gold1d-bsod';
  d.innerHTML=`<div><h1>:(</h1><h2>EmeraldOS Gold ran into a problem.</h2><p>${esc(reason)}</p><p class="bsod-code">Stop code: ${esc(code)}</p><div class="bsod-progress"><span></span></div><div class="toolbar"><button onclick="location.reload()">Restart EmeraldOS Gold</button><button onclick="localStorage.setItem('${PREFIX}safemode','true');location.reload()">Restart in Safe Mode</button><button onclick="this.closest('.gold1d-bsod').remove();Gold50.openApp('support')">Open Support</button></div></div>`;
  document.body.appendChild(d);
}

try{
  const gold1dOriginalOpenApp = window.Gold50?.openApp || openApp;
  function gold1dSafeOpenApp(id){
    try{
      const app=appById(id);
      if(!app) throw new Error('The app is not installed or is hidden.');
      return gold1dOriginalOpenApp(id);
    }catch(e){gold1dFileOrAppFailure(id,'EmeraldOS Gold could not start this app.',e,'app');return false;}
  }
  window.Gold50.openApp=gold1dSafeOpenApp;
  openApp=gold1dSafeOpenApp;
}catch(e){console.warn('Gold 1D app failure wrapper skipped',e)}
try{
  const gold1dOriginalOpenFile = window.Gold50?.openFile || openFile;
  function gold1dSafeOpenFile(fid){
    try{
      ...
      return gold1dOriginalOpenFile(fid);
    }catch(e){gold1dFileOrAppFailure(fid,'EmeraldOS Gold could not open this file.',e,'file');return false;}
  }
  window.Gold50.openFile=gold1dSafeOpenFile;
  openFile=gold1dSafeOpenFile;
}catch(e){console.warn('Gold 1D file failure wrapper skipped',e)}

function gold1dReleaseManifest(){
  return {
    product:'EmeraldOS Gold', latestVersion:'1D', build:'1D', folder:'Gold_1D', entry:'OS.html',
    channel:'stable', status:'stable', required:false, enabled:true, setupMode:'migrationIntro',
    releaseTitle:'EmeraldOS Gold 1D',
    summary:'Gold 1D adds deeper customization, movable desktop objects, Trash, rollback tools, centered load-failure popups, crash recovery screens, and staff/PIN-gated E.L.S.U.S. publishing.',
    migrationFrom:['1B','1A','11.0'], migrationId:'gold1b-to-gold1d-desktop-rollback',
    minShellVersion:'1.0', rollbackFolder:'Gold_1B', rollbackVersion:'1B', releasedAt:new Date().toISOString()
  };
}
function gold1dPublisherUnlocked(){return localStorage.getItem(PREFIX+'publisher_pin_unlocked')==='true'}
async function gold1dRequirePublisherAccess(show=true){
  if(!isStaff()){
    if(show) gold1dCenterPopup('Staff verification required','Update publishing is only available after signing in through Gold Staff Edition.','danger',[{label:'Open Staff Edition',primary:true,action:()=>location.href='staff.html'}]);
    return false;
  }
  if(gold1dPublisherUnlocked()) return true;
  const pin=prompt('Update Publisher Manager PIN');
  if(pin!==GOLD1D_PUBLISHER_PIN){
    if(show) gold1dCenterPopup('Publisher PIN rejected','The Update Publisher Manager PIN was incorrect.','danger');
...
async function gold1dPublishLatest(showResult=true){
  if(!(await gold1dRequirePublisherAccess(showResult))) return false;
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
    const manifest=gold1dReleaseManifest();
    ...
    if(showResult) gold1dCenterPopup('E.L.S.U.S. published','system/emeraldGoldLatest now points to Gold_1D. Users will still update manually when prompted.','success');
    return true;
  }catch(e){
    console.warn('Gold 1D publish failed',e);
    if(showResult) gold1dCenterPopup('Publish failed','Firebase rejected the publish request. Check Firestore rules and staff write permissions.','danger',[{label:'Open Diagnostics',action:()=>Gold50.openApp('diagnostics')}]);
    return false;
  }
}

function openELSUSUpdateCenter1D(){
  const latest=read('emeraldGoldShell_latest',gold1dReleaseManifest());
  const update=read('emeraldGold_updateAvailable',{});
  openWindow('updateshell','E.L.S.U.S. System Update',`<div class="goldshell-header"><div><h2>E.L.S.U.S. System Update</h2><p class="muted">The update shell stays separate. This app shows update status and lets users restart through E.L.S.U.S. Publishing is staff-only.</p></div><button class="btn primary" onclick="location.href='../gold-shell.html?manualCheck=1'">Check through shell</button></div><div class="grid2"><div class="card"><h3>This VM folder</h3><p>EmeraldOS Gold 1D</p><p class="muted">Gold_1D / OS.html</p></div><div class="card"><h3>Latest known folder</h3><p>${esc(latest.latestVersion||'Unknown')}</p><p class="muted">${esc(latest.folder||'No folder')} / ${esc(latest.entry||'OS.html')}</p></div><div class="card"><h3>User-controlled updates</h3><p>Each user updates manually. Their active folder changes only after accepting the update prompt.</p></div><div class="card"><h3>Rollback</h3><p>Rollback can help recover from a bad update, but may cause app or file instability.</p><button class="btn" onclick="Gold50.openApp('rollback')">Open Rollback</button></div></div><h3>Actions</h3><div class="toolbar"><button class="btn primary" onclick="Gold50.saveWorkspaceNow(true)">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html?manualCheck=1'">Restart through E.L.S.U.S.</button>${isStaff()?'<button class="btn danger" onclick="Gold50.openUpdatePublisherManager1D()">Update Publisher Manager</button>':''}</div><p class="muted">Last shell check: ${esc(update.checkedAt||'Not checked in this browser')}</p>`,{width:1080,height:740});
}
function openUpdatePublisherManager1D(){
  if(!isStaff()){location.href='staff.html';return}
  gold1dRequirePublisherAccess(true).then(ok=>{
    if(!ok)return;
    const hist=read(PREFIX+'publish_history',[]);
    openWindow('updatepublisher','Update Publisher Manager',`<div class="goldshell-header"><div><h2>Update Publisher Manager</h2><p class="muted">Staff-only E.L.S.U.S. publishing. A publisher PIN is required every session.</p></div><button class="btn danger" onclick="localStorage.removeItem('${PREFIX}publisher_pin_unlocked');Gold50.notify('Publisher locked','PIN access was cleared.','E.L.S.U.S.')">Lock publisher</button></div><div class="grid2"><div class="card"><h3>Publish Gold 1D</h3><p>Writes <code>system/emeraldGoldLatest</code> to point users to <code>Gold_1D/OS.html</code>. Users must still accept the update manually.</p><button class="btn primary" onclick="Gold50.gold1dPublishLatest(true)">Publish Gold 1D as latest</button></div><div class="card"><h3>Rollback target</h3><p>Default rollback: Gold 1B. Changing the public latest version should only be done if the current latest build breaks boot.</p><button class="btn" onclick="Gold50.gold1dPublishRollbackLatest('Gold_1B','1B')">Publish Gold 1B as emergency latest</button></div><div class="card"><h3>PIN status</h3><p>${gold1dPublisherUnlocked()?'Unlocked':'Locked'}</p></div><div class="card"><h3>Staff session</h3><p>${esc(username())}</p><p class="muted">Role: ${esc(currentStaff()?.role||localStorage.getItem('role')||'staff')}</p></div></div><h3>Recent publishing</h3>${hist.map(h=>`<div class="card"><b>${esc(h.manifest?.releaseTitle||'Publish')}</b><p class="muted">${esc(h.time)} · ${esc(h.user)}</p></div>`).join('')||'<p class="muted">No publish history on this device.</p>'}`,{width:1080,height:760});
  });
}
async function gold1dPublishRollbackLatest(folder='Gold_1B',version='1B'){
  if(!(await gold1dRequirePublisherAccess(true)))return false;
  ...
  try{
    ...
    const manifest={...gold1dReleaseManifest(), latestVersion:version, build:version, folder, releaseTitle:`EmeraldOS Gold ${version} Emergency Rollback`, summary:'Emergency rollback published by staff through Gold 1D Update Publisher Manager.', rollbackFolder:'Gold_1D', rollbackVersion:'1D', releasedAt:new Date().toISOString()};
    await api.setDoc(api.doc(api.db,'system','emeraldGoldLatest'),manifest,{merge:true});
    gold1dCenterPopup('Emergency rollback published',`E.L.S.U.S. now points to ${folder}.`,'success');
    return true;
  }catch(e){gold1dCenterPopup('Rollback publish failed',e.message||'Firebase rejected the rollback publish.','danger');return false;}
}

function openRollbackCenter1D(){
  openWindow('rollback','Version Rollback',`<div class="goldshell-header"><div><h2>Version Rollback</h2><p class="muted">Rollback changes only this user's selected E.L.S.U.S. folder unless staff publishes an emergency rollback.</p></div><button class="btn primary" onclick="Gold50.saveVMSnapshot()">Create snapshot</button></div><div class="danger-card"><h3>Rollback warning</h3><p>Rolling back can cause instability with newer applications, files, settings, or VM data. Create a snapshot first and only rollback if a new version is not working.</p></div><div class="grid2">${GOLD1D_ROLLBACK_TARGETS.map(t=>`<div class="card"><h3>${esc(t.title)}</h3><p class="muted">${esc(t.folder)} / ${esc(t.entry)}</p><button class="btn" onclick="Gold50.gold1dUserRollback('${esc(t.folder)}','${esc(t.version)}')">Rollback my VM to ${esc(t.version)}</button></div>`).join('')}</div>`,{width:1000,height:720});
}
function gold1dUserRollback(folder,version){
  ...
  const manifest={product:'EmeraldOS Gold',latestVersion:version,build:version,folder,entry:'OS.html',channel:'stable',status:'rollback',required:false,enabled:true,setupMode:'continue',releaseTitle:'EmeraldOS Gold '+version,summary:'User selected rollback through Gold 1D.',rollbackFrom:'Gold_1D',rolledBackAt:now()};
  ...
  gold1dCenterPopup('Rollback selected','E.L.S.U.S. will restart into '+folder+'.','warning',[{label:'Restart now',primary:true,action:()=>location.href='../gold-shell.html?rollback=1'}]);
}

function openThemeStudio1D(){
  const p=prefs();
  openWindow('themestudio','Theme Studio',`<h2>Theme Studio</h2><p class="muted">Gold 1D adds safer deep customization with protected CSS variables.</p><div class="grid2"><div class="card"><h3>Colors</h3><label>Accent<input type="color" class="field" value="${esc(p.accent||'#0078d4')}" oninput="Gold50.setPrefs({accent:this.value})"></label><label>Secondary accent<input type="color" class="field" value="${esc(p.secondaryAccent||'#0f7b4d')}" oninput="Gold50.setPrefs({secondaryAccent:this.value})"></label><label>Taskbar color<input type="color" class="field" value="${esc(p.taskbarColor||'#f6f9fd')}" oninput="Gold50.gold1dSetPref('taskbarColor',this.value)"></label><label>Taskbar text<input type="color" class="field" value="${esc(p.taskbarTextColor||'#111827')}" oninput="Gold50.gold1dSetPref('taskbarTextColor',this.value)"></label><label>Start menu color<input type="color" class="field" value="${esc(p.startMenuColor||'#ffffff')}" oninput="Gold50.gold1dSetPref('startMenuColor',this.value)"></label></div><div class="card"><h3>Desktop objects</h3><label>Desktop mode<select class="field" onchange="Gold50.gold1dSetPref('desktopObjectMode',this.value)"><option value="free" ${p.desktopObjectMode!=='grid'?'selected':''}>Free move</option><option value="grid" ${p.desktopObjectMode==='grid'?'selected':''}>Grid layout</option></select></label><label><input type="checkbox" ${p.desktopSnapToGrid!==false?'checked':''} onchange="Gold50.gold1dSetPref('desktopSnapToGrid',this.checked)"> Snap objects to grid</label><label>Grid size<input type="range" min="64" max="150" value="${esc(p.desktopGridSize||92)}" oninput="Gold50.gold1dSetPref('desktopGridSize',this.value)"></label><label><input type="checkbox" ${p.trashOnDesktop!==false?'checked':''} onchange="Gold50.gold1dSetPref('trashOnDesktop',this.checked)"> Show Trash on desktop</label><button class="btn" onclick="Gold50.gold1dResetDesktopPositions()">Reset icon positions</button></div><div class="card"><h3>Background</h3><label>Background color<input type="color" class="field" value="${esc(p.backgroundColor||'#eaf3ff')}" oninput="Gold50.setGold70Pref?.('backgroundColor',this.value);Gold50.setGold70Pref?.('useBackgroundColor',true);Gold50.gold1dSetPref('backgroundColor',this.value)"></label><label>Background URL<input class="field" value="${esc(p.customBackground||'')}" onchange="Gold50.setPrefs({customBackground:this.value})"></label><input type="file" class="field" accept="image/*" onchange="Gold50.loadBackgroundFile(this)"></div><div class="card"><h3>Reliability</h3><label><input type="checkbox" ${p.centerFailurePopups!==false?'checked':''} onchange="Gold50.gold1dSetPref('centerFailurePopups',this.checked)"> Center failure popups</label><label><input type="checkbox" ${p.bsodOnCriticalErrors!==false?'checked':''} onchange="Gold50.gold1dSetPref('bsodOnCriticalErrors',this.checked)"> Crash screen for critical errors</label><button class="btn danger" onclick="Gold50.gold1dBSOD('Manual crash screen test.','MANUAL_TEST')">Test crash screen</button></div></div>`,{width:1080,height:760});
}
function gold1dSetPref(key,value){const p={};p[key]=value;setPrefs(p)}
function gold1dResetDesktopPositions(){localStorage.removeItem(PREFIX+'desktopPositions');renderDesktop();notify('Desktop reset','Desktop object positions were reset.','Desktop')}

function gold1dInstallTrashContext(){
  document.addEventListener('contextmenu',e=>{
    const trash=e.target.closest('[data-trash]'); if(!trash)return;
    e.preventDefault(); e.stopPropagation();
    gold1dContextMenu([
      {id:'open',label:'Open Trash',action:()=>openTrashCan1D()},
      ...
      {id:'hide',label:'Hide Trash icon',action:()=>gold1dSetPref('trashOnDesktop',false)}
    ],e.clientX,e.clientY);
  },true);
}
...
window.addEventListener('error',e=>{
  try{
    ...
    gold1dCenterPopup('EmeraldOS Gold warning',msg,'warning',[{label:'Open Diagnostics',action:()=>Gold50.openApp('diagnostics')}]);
    if(prefs().bsodOnCriticalErrors && /ReferenceError|SyntaxError|not defined|Cannot read|Cannot set/.test(msg)) setTimeout(()=>gold1dBSOD(msg,'GOLD_UNHANDLED_ERROR'),250);
  }catch{}
});
...
function gold1dInstallApps(){
  gold1dAddOrUpdateApp({id:'updateshell',name:'E.L.S.U.S. System Update',label:'UP',color:'blue',group:'System',purpose:'User update status',desc:'Check update status, save the VM, restart through E.L.S.U.S., and open rollback tools.',open:openELSUSUpdateCenter1D});
  gold1dAddOrUpdateApp({id:'updatepublisher',name:'Update Publisher Manager',label:'PM',color:'red',group:'Staff',purpose:'Staff update publishing',desc:'Staff-only and PIN-gated E.L.S.U.S. publishing controls.',staffOnly:true,open:openUpdatePublisherManager1D});
  gold1dAddOrUpdateApp({id:'rollback',name:'Version Rollback',label:'RB',color:'orange',group:'System',purpose:'Rollback',desc:'Rollback this VM to a previous EmeraldOS Gold folder with instability warnings.',open:openRollbackCenter1D});
  gold1dAddOrUpdateApp({id:'crashrecovery',name:'Crash Recovery',label:'CR',color:'blue',group:'Support',purpose:'Reliability',desc:'Test and recover from EmeraldOS Gold crash screens and load failures.',open:()=>openWindow('crashrecovery','Crash Recovery',`<h2>Crash Recovery</h2><p class="muted">Use these tools if an app or file refuses to load.</p><div class="grid3"><button class="tile-button" onclick="Gold50.gold1dBSOD('Manual crash screen test.','MANUAL_TEST')">Test crash screen</button><button class="tile-button" onclick="Gold50.openApp('diagnostics')">Diagnostics</button><button class="tile-button" onclick="localStorage.setItem('${PREFIX}safemode','true');location.reload()">Restart in Safe Mode</button></div>`,{width:780,height:520})});
  try{const theme=APPS.find(a=>a.id==='themestudio'); if(theme) theme.open=openThemeStudio1D;}catch{}
}

Object.assign(window.Gold50||{}, {
  openTrashCan1D,gold1dCenterPopup,gold1dBSOD,gold1dPublishLatest,gold1dRequirePublisherAccess,
  openELSUSUpdateCenter1D,openUpdatePublisherManager1D,gold1dPublishRollbackLatest,openRollbackCenter1D,gold1dUserRollback,
  openThemeStudio1D,gold1dSetPref,gold1dResetDesktopPositions
});
window.Gold1D=window.Gold50;
if(new URLSearchParams(location.search).has('publishLatest')) setTimeout(()=>gold1dPublishLatest(true), 1600);
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1dInstallApps();gold1dInstallDesktopDrag();gold1dInstallTrashContext();applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1D feature install failed',e)}},900));


/* =========================================================
   EMERALDOS GOLD 1D FINAL UX, LANGUAGE, ACCESSIBILITY,
   STAFF DIRECT CONTROL, AND SAFE CUSTOMIZATION LAYER
========================================================= */
const gold1dBaseDefaultPrefs = defaultPrefs;
defaultPrefs = function(){
  return {
    ...gold1dBaseDefaultPrefs(),
    language:"en",
    taskbarColor:"#f6f9fd",
...
applyPrefs = function(){
  ...
  const p=prefs();
  const root=document.documentElement;
  root.style.setProperty('--gold1dTaskbarColor',p.taskbarColor||'#f6f9fd');
  root.style.setProperty('--gold1dTaskbarText',p.taskbarTextColor||'#111827');
  root.style.setProperty('--gold1dStartColor',p.startMenuColor||'#ffffff');
  root.style.setProperty('--gold1dBackgroundColor',p.backgroundColor||'#eaf3ff');
  root.style.setProperty('--gold1dPanelOpacity',String(Math.max(.55,Math.min(1,Number(p.panelOpacity||.96)))));
  root.style.setProperty('--gold1dMenuOpacity',String(Math.max(.55,Math.min(1,Number(p.menuOpacity||.96)))));
  document.documentElement.lang=p.language==='es'?'es':'en';
  document.body.dataset.language=p.language==='es'?'es':'en';
  document.body.classList.toggle('gold1d-readable',!!p.readableFont);
  document.body.classList.toggle('gold1d-hit-large',!!p.largerHitTargets);
  document.body.classList.toggle('gold1d-underline-controls',!!p.underlineControls);
  document.body.classList.toggle('gold1d-extra-contrast',!!p.highContrastBorders);
  document.body.classList.remove('gold1d-filter-warm','gold1d-filter-cool','gold1d-filter-gray');
  if(p.colorFilter&&p.colorFilter!=='none') document.body.classList.add('gold1d-filter-'+p.colorFilter);
...
function gold1dMarkProcessed(cmd,session){
...
const gold1dBaseProcessRemoteCommand = processRemoteCommand;
processRemoteCommand = function(cmd,session){
  ...
  if(!directTypes.includes(t)) return gold1dBaseProcessRemoteCommand(cmd,session);
  if(!gold1dMarkProcessed(cmd,session)) return;
  try{
...
const gold1dBaseSnapshot = remoteDesktopSnapshot;
remoteDesktopSnapshot = function(){
  const snap=gold1dBaseSnapshot();
...
function gold1dInstallFinalApps(){
  gold1dAddOrUpdateApp({id:'language',name:'Language & Region',label:'LR',color:'teal',group:'Settings',purpose:'Language',desc:'Switch EmeraldOS Gold between English and Spanish core shell language.',open:openLanguageRegion1D});
  gold1dAddOrUpdateApp({id:'accessibility',name:'Accessibility Center',label:'AC',color:'blue',group:'Settings',purpose:'Accessibility',desc:'Readable text, contrast, reduced motion, focus rings, and language access.',open:openAccessibility1D});
  gold1dAddOrUpdateApp({id:'staffresources',name:'Staff Resources',label:'SR',color:'purple',group:'Staff',purpose:'Staff resources',desc:'Staff-only support procedures, E.L.S.U.S. notes, and remote support policy.',staffOnly:true,open:openStaffResources1D});
  try{const t=APPS.find(a=>a.id==='themestudio'); if(t)t.open=openThemeStudio1DPlus;}catch{}
  try{const s=APPS.find(a=>a.id==='staff'); if(s)s.staffOnly=true;}catch{}
}
Object.assign(window.Gold50||{}, {openLanguageRegion1D,openAccessibility1D,openThemeStudio1DPlus,openStaffResources1D,gold1dT});
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1dInstallFinalApps();applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1D final layer failed',e)}},1050));
