...
========================================================= */
(function EmeraldOSGold1JFeatures(){
  if(window.__EMERALDOS_GOLD_1J_FEATURES__) return;
  window.__EMERALDOS_GOLD_1J_FEATURES__=true;
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1J local write failed",key,error);return false}};
  ...
  const api=()=>window.Gold50||window.Gold1J;
  const notify=(title,body,app="EmeraldOS Gold 1J")=>api()?.notify?.(title,body,app)||console.info(`[${app}] ${title}: ${body}`);
  ...
  function setCustomLogo(appId,source){
    ...
    window.Gold1J?.saveWorkspaceNow?.(false);
  }
  ...
  function logoMarkup(app,size=32){
    ...
    return `<span class="gold1j-logo-wrap" style="width:${size}px;height:${size}px"><img class="gold1j-custom-logo" src="${esc(logoFor(app))}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="fallback-icon" style="display:none">${fallback}</span></span>`;
  }
  ...
  function sandboxUserApp(app,{preview=false,hashVerified=false}={}){
    ...
    const html=`<div class="gold1j-user-app-host"><iframe id="${frameId}" sandbox="allow-scripts allow-forms allow-modals allow-downloads" title="${esc(app.name)}"></iframe><div class="gold1j-sandbox-note">${preview?"Developer preview":"Staff-verified sandbox"} · ${esc(app.owner)}</div></div>`;
  ...
  function initShellBridge(){
    ...
    window.addEventListener("message",async event=>{
      ...
      if(type.includes("save")){await window.Gold1J?.saveWorkspaceNow?.(false);sendShell("elsus:save-complete",{savedAt:now()});}
  ...
  async function loadLatestManifest(){
    ...
    }catch(error){console.debug("Gold 1J update lookup used shell/local cache",error)}
    return cached||{latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",releaseTitle:"EmeraldOS Gold 1J",summary:"Current release"};
  }
  ...
  async function requestVersionChange(manifest){
    await window.Gold1J?.saveWorkspaceNow?.(false);
  ...
  async function openEnhancedUpdateCenter(){
    ...
    win.querySelector("#updateSave").onclick=()=>window.Gold1J?.saveWorkspaceNow?.(true);
  ...
  function openShellIntegration(){
    ...
    win.querySelector("#shellExit").onclick=()=>location.href="../gold-shell.html?force=1&from=Gold_1J";
    return win;
  }
  ...
  function applyLiveControl(value,{source="cloud"}={}){
    ...
    document.body.classList.toggle("gold1j-maintenance",liveControl.maintenanceMode);
    document.body.classList.toggle("gold1j-support-only",liveControl.supportOnlyMode);
    document.body.classList.toggle("gold1j-live-focus",liveControl.forceFocusAssist);
  ...
  async function listenLiveStaffControl(){
    ...
    }catch(error){console.debug("Gold 1J live control is using local fallback",error)}
  }
  ...
  async function renderStaffCenter(win,tab="overview"){
    ...
    content.innerHTML='<div class="gold1j-loading">Loading Staff Center…</div>';
  ...
  function wireStaffContent(win,tab,data){
    ...
    if(tab==="elsus"){
      ...
      win.querySelector("#staffPublisherOpen")?.addEventListener("click",()=>window.Gold1J?.openPublisherManager?.());
      win.querySelector("#staffSaveVM")?.addEventListener("click",()=>window.Gold1J?.saveWorkspaceNow?.(true));
    }
  }
  ...
  function openAppInspection(app){
    const risks=analyzeCode(app.code);return openWindow(`inspect_${app.id}`,`Inspect: ${app.name}`,`<div class="app-body"><h1>${esc(app.name)}</h1><p>${esc(app.description)}</p><div class="gold1j-settings-cards"><div class="card"><b>Owner</b><p>${esc(app.owner)}</p></div><div class="card"><b>Status</b><p>${esc(statusLabel(app.verificationStatus))}</p></div><div class="card"><b>Capabilities</b><p>${esc(app.requestedPermissions.join(", ")||"None")}</p></div><div class="card"><b>Review indicators</b><p>${esc(risks.join(", ")||"None")}</p></div></div><h2>Source code</h2><pre class="gold1j-source-view">${esc(app.code)}</pre></div>`,{width:980,height:700,singleton:false});
  }
  ...
  async function sendSessionHeartbeat(){
    ...
    try{const fb=await import("./firebase.js");if(fb.db)await fb.setDoc(fb.doc(fb.db,SESSION_COLLECTION,normalizeUser(userName())),{...record,lastSeen:fb.serverTimestamp()},{merge:true})}catch(error){console.debug("Gold 1J session heartbeat cloud write skipped",error.message)}
  }
  ...
  const SETTINGS_SECTIONS=[
    ...
    ["shell","E.L.S.U.S. Shell","In-OS shell integration"]
  ];
  ...
  function openEnhancedSettings(section="system"){
    ...
    const html=`<div class="settings-layout gold1j-settings"><aside class="settings-nav"><h2>Settings</h2><input id="settingsSearch" class="field" placeholder="Find a setting">${sections.map(([id,label,description])=>`<button class="${section===id?"active":""}" data-settings-section="${id}" data-setting-words="${esc((label+" "+description).toLowerCase())}"><b>${esc(label)}</b><small>${esc(description)}</small></button>`).join("")}</aside><main class="settings-content">${content}</main></div>`;
  ...
  function wireEnhancedSettings(win,section){
    ...
    win.querySelectorAll("[data-open-tool]").forEach(button=>button.onclick=()=>({clipboard:window.Gold1J?.openClipboardHistory,shell:openShellIntegration}[button.dataset.openTool]?.()));
    ...
    win.querySelector("#openRootShell")?.addEventListener("click",()=>location.href="../gold-shell.html?force=1&from=Gold_1J");
  }
  function applyExtendedPrefs(){
    const p=prefs(),body=document.body,taskbar=$("taskbar"),search=$("searchBtn");
    body.classList.toggle("gold1j-night-light",Boolean(p.nightLight));body.classList.toggle("gold1j-no-transparency",p.transparency===false);body.classList.toggle("gold1j-touch",Boolean(p.touchMode));body.classList.toggle("gold1j-game-mode",Boolean(p.gameMode));body.classList.toggle("gold1j-large-targets",Boolean(p.largePointerTargets));body.classList.toggle("no-motion",Boolean(p.reducedMotion||p.gameMode&&p.disableAnimationsInGameMode));body.classList.toggle("taskbar-top",p.taskbarPosition==="top");
  ...
  function applyVirtualDesktopVisibility(){
    qa(".window").forEach(win=>{const desktop=Number(win.dataset.virtualDesktop||1);win.classList.toggle("virtual-hidden",desktop!==currentDesktop);const task=document.querySelector(`[data-window="${CSS.escape(win.id)}"],#gold1j_task_${CSS.escape(win.id)}`);if(task)task.hidden=desktop!==currentDesktop;});
  ...
  function enhanceTaskViewDesktops(){
    ...
    let bar=panel.querySelector(".gold1j-desktop-bar");if(!bar){bar=document.createElement("div");bar.className="gold1j-desktop-bar";panel.prepend(bar)}
  ...
  function saveStickyNotes(notes){write(STICKY_KEY,notes);window.Gold1J?.saveWorkspaceNow?.(false)}
  function openStickyNotes(){
    const notes=stickyNotes();const html=`<div class="app-shell"><div class="app-toolbar"><button id="stickyNew" class="button primary">New note</button></div><div class="app-body"><div class="gold1j-sticky-grid">${notes.map(note=>`<article class="gold1j-sticky" style="--note-color:${esc(note.color||"#fff2ab")}"><textarea data-sticky-text="${esc(note.id)}">${esc(note.text)}</textarea><div><input type="color" data-sticky-color="${esc(note.id)}" value="${esc(note.color||"#fff2ab")}"><button data-sticky-delete="${esc(note.id)}">Delete</button></div></article>`).join("")||'<p>No sticky notes yet.</p>'}</div></div></div>`;
    ...
    win.querySelectorAll("[data-sticky-color]").forEach(input=>input.oninput=()=>{const list=stickyNotes(),item=list.find(note=>note.id===input.dataset.stickyColor);if(item){item.color=input.value;saveStickyNotes(list);input.closest(".gold1j-sticky").style.setProperty("--note-color",input.value)}});
  ...
  function patchOpenApp(){
    ...
    window.Gold1J=window.Gold1J||core;
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1J","Shell integration, Staff Center, verified user apps, custom logos, and Windows 10 features are ready.","System");
  }
