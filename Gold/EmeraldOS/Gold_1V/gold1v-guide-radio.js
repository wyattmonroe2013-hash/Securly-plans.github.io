"use strict";
(function EmeraldOSGold1VGuideAndRadio(){
  if(window.__EMERALDOS_GOLD_1V_GUIDE_RADIO__)return;
  window.__EMERALDOS_GOLD_1V_GUIDE_RADIO__=true;
  const VERSION="1V",PREFIX="gold1g_",GUIDE_PREFS=PREFIX+"guide_preferences_1v",RADIO_STATIONS=PREFIX+"radio_stations_1v",RADIO_PREFS=PREFIX+"radio_preferences_1v",PIN_KEY=PREFIX+"guide_default_pins_1v";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return value}catch(error){console.warn("Gold 1V data write failed",key,error);return value}};
  const now=()=>new Date().toISOString();
  const core=()=>window.Gold50||window.Gold1V;
  const notify=(title,body,app="Complete Guide")=>core()?.notify?.(title,body,app);
  const saveVM=()=>{if(window.__GOLD1V_MIGRATION_BLOCK_SAVES__===true)return;setTimeout(()=>window.Gold1V?.saveWorkspaceNow?.(false),0)};
  ...
  const topics=[
    ...
    {id:"files",title:"Files, folders, and universal compatibility",group:"Files and data",keywords:"files folders explorer compatibility universal id extensions old versions",summary:"Create, organize, open, and migrate files across Gold versions.",steps:[
      ...
      "Create files with File Explorer or Gold Office. Gold 1V normalizes records to the universal Gold file format while retaining legacy IDs and source metadata.",
    ...
    {id:"privacysharing",title:"Privacy, separate user VMs, and file sharing",group:"Files and data",keywords:"privacy separate users share file permissions owner access cloud isolation",summary:"Keep every account private and share only selected files.",steps:[
      "Gold 1V keys cloud storage to the authenticated account ID and marks VM categories private to that owner.",
    ...
    {id:"application-editor-compatibility",title:"Application Editor iframe compatibility",group:"Applications",keywords:"iframe sandbox same origin cache CacheStorage application editor",summary:"Run iframe-based user apps with the Gold 1V compatibility sandbox.",steps:[
      "Gold 1V Application Editor wrappers include allow-same-origin in addition to allow-scripts, forms, modals, and downloads.",
  ...
  function renderTopic(topic){return `<article class="gold1v-guide-article"><div class="gold1v-guide-heading"><div><small>${esc(topic.group)}</small><h1>${esc(topic.title)}</h1><p>${esc(topic.summary)}</p></div><button class="button" id="guideFavorite">${guidePrefs().favorites.includes(topic.id)?"Remove favorite":"Add favorite"}</button></div><ol>${topic.steps.map(step=>`<li>${esc(step)}</li>`).join("")}</ol>${topic.links?.length?`<section class="card"><h2>Open related tools</h2><div class="button-row">${topic.links.map(([id,label])=>`<button class="button" data-guide-app="${esc(id)}">${esc(label)}</button>`).join("")}</div></section>`:""}</article>`}
  function openGuide(startTopic){
    const initial=topicById(startTopic||guidePrefs().lastTopic);
    const html=`<div class="app-shell gold1v-guide"><div class="app-toolbar"><input id="guideSearch" class="field" placeholder="Search the complete guide" aria-label="Search the complete guide"><button id="guideHome" class="button">Getting started</button><button id="guideExport" class="button">Export guide index</button></div><div class="gold1v-guide-layout"><nav id="guideNav" aria-label="Guide topics"></nav><main id="guideContent"></main></div></div>`;
    ...
    search.oninput=()=>renderNav(search.value);search.onkeydown=e=>{if(e.key==="Enter")nav.querySelector("[data-guide-topic]")?.click()};win.querySelector("#guideHome").onclick=()=>show("start");win.querySelector("#guideExport").onclick=()=>core().saveText?.("EmeraldOS-Gold-1V-Complete-Guide-Index.txt",topics.map(t=>`${t.group} - ${t.title}\n${t.summary}\n${t.steps.map((s,i)=>`${i+1}. ${s}`).join("\n")}\n`).join("\n"));renderNav();show(selected);return win;
  }
  ...
  function init(){
    ...
    ensureGames();
    Object.assign(window.Gold1V||core(),{openGuide,openCompleteGuide:openGuide,openRadio,guideTopics:topics,radioStations:stations});
