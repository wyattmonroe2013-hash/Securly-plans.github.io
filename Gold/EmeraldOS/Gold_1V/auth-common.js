...
window.GoldAuth = (() => {
  ...
  const ACCOUNT_HISTORY_KEY = "gold1g_account_history_1v";
  ...
  async function findCloudUser(username) {
    ...
    // Gold 1V compatible with both layouts.
    for (const collectionName of collections) {
  ...
  async function beginSession(user) {
    ...
    await window.Gold1VUserIsolation?.switchToUser?.(userId, username);
  ...
  return {
    ...
    recordAccountEvent,
    changePassword,
    localIsolationStatus: () => window.Gold1VUserIsolation?.status?.() || null
  };
})();
