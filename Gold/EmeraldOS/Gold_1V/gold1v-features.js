...
========================================================= */
(function EmeraldOSGold1VFeatures(){
  if(window.__EMERALDOS_GOLD_1V_FEATURES__) return;
  window.__EMERALDOS_GOLD_1V_FEATURES__=true;
  ...
  const notify=(title,body,app="EmeraldOS Gold 1V")=>api()?.notify?.(title,body,app)||console.info(`[${app}] ${title}: ${body}`);
  ...
  function staffSession(){
    for(const key of [PREFIX+"staff_session","gold1v_staff_session","gold1j_staff_session"]){
  ...
  function setCustomLogo(appId,source){
    ...
    window.Gold1V?.saveWorkspaceNow?.(false);
  }
  ...
  function sandboxUserApp(app,{preview=false,hashVerified=false}={}){
    ...
    const handler=event=>{
      ...
      if(type==="cache-warning"){
        console.warn(`Gold 1V user app cache operation skipped (${payload.operation||"unknown"})`,payload.message||"");
  ...
  async function loadLatestManifest(){
    ...
    }catch(error){console.debug("Gold 1V update lookup used local cache",error?.message||error)}
    return cached||{latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",releaseTitle:"EmeraldOS Gold 1V",summary:"Current release"};
  }
  ...
  async function requestVersionChange(manifest){
    await window.Gold1V?.saveWorkspaceNow?.(false);
  ...
  async function openEnhancedUpdateCenter(){
    ...
    win.querySelector("#updateSave").onclick=()=>window.Gold1V?.saveWorkspaceNow?.(true);
    ...
    win.querySelector("#updatePublisher")?.addEventListener("click",()=>window.Gold1V?.openPublisherManager?.());
    return win;
  }
  function openELSUSStatus(){
    const latest=elsusState.latestManifest||read("emeraldGoldShell_latest",null);
    const html=`<div class="app-body"><h1>E.L.S.U.S.</h1><p>Gold 1V uses the original E.L.S.U.S. implementation from Gold 1I.</p><div class="gold1p-settings-cards"><div class="card"><h3>Routing</h3><p>Save VM → write pending manifest → open root <code>gold-shell.html</code></p></div><div class="card"><h3>Cloud VM</h3><p><code>emeraldOSUsers/${esc(userName())}/goldVM/current</code></p></div><div class="card"><h3>Latest manifest</h3><p>${esc(latest?.releaseTitle||"Not checked yet")}</p></div><div class="card"><h3>Publishing</h3><p>Manual Staff Edition and publisher-PIN workflow only.</p></div></div><div class="actions"><button id="elsusCheck" class="button primary">Check for updates</button><button id="elsusSave" class="button">Save VM</button><button id="elsusRoot" class="button">Open root E.L.S.U.S.</button></div><p class="muted">Gold 1V uses the direct, original E.L.S.U.S. route and requires no additional shell patch.</p></div>`;
    ...
    win.querySelector("#elsusSave").onclick=()=>window.Gold1V?.saveWorkspaceNow?.(true);
  ...
  async function listenLiveStaffControl(){
    ...
    }catch(error){console.debug("Gold 1V live control is using local fallback",error)}
  }
  ...
  function wireStaffContent(win,tab,data){
    ...
    if(tab==="elsus"){
      ...
      win.querySelector("#staffPublisherOpen")?.addEventListener("click",()=>window.Gold1V?.openPublisherManager?.());
      win.querySelector("#staffSaveVM")?.addEventListener("click",()=>window.Gold1V?.saveWorkspaceNow?.(true));
    }
  }
  ...
  async function sendSessionHeartbeat(){
    ...
    try{const fb=await import("./firebase.js");if(fb.db)await fb.setDoc(fb.doc(fb.db,SESSION_COLLECTION,normalizeUser(userName())),{...record,lastSeen:fb.serverTimestamp()},{merge:true})}catch(error){console.debug("Gold 1V session heartbeat cloud write skipped",error.message)}
  }
  ...
  function settingsContent(section){
    ...
    if(section==="shellv2")return `<h1>E.L.S.U.S. Shell V2</h1><p>Shell V2 can identify the user’s current Gold route before opening this version’s own login page. Gold 1V remains compatible with the original shell.</p><div class="gold1p-settings-cards"><div class="card"><h3>Two-step login</h3><p>The shell identifies the VM; Gold 1V still authenticates the operating-system session.</p></div><div class="card"><h3>Data isolation</h3><p>The shell reads routing metadata and does not replace files or settings.</p></div><div class="card"><h3>Compatibility test</h3><p>Review routing parameters, version metadata, and migration safeguards.</p></div></div><button class="button primary" data-open-app="shellv2test">Open Shell V2 Compatibility</button>`;
  ...
  function wireEnhancedSettings(win,section){
    ...
    win.querySelectorAll("[data-open-tool]").forEach(button=>button.onclick=()=>({clipboard:window.Gold1V?.openClipboardHistory,elsus:openELSUSStatus}[button.dataset.openTool]?.()));
  ...
  function saveStickyNotes(notes){write(STICKY_KEY,notes);window.Gold1V?.saveWorkspaceNow?.(false)}
  ...
  function patchOpenApp(){
    ...
    window.Gold1V=window.Gold1V||core;
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1V","E.L.S.U.S., Staff Center, verified user apps, custom logos, and Windows 10 features are ready.","System");
  }
