...
========================================================= */
(function EmeraldOSGold1VExtras(){
  if(window.__EMERALDOS_GOLD_1V_EXTRAS__) return;
  window.__EMERALDOS_GOLD_1V_EXTRAS__=true;

  const VERSION="1V";
  const FOLDER="Gold_1V";
  const UPDATE_SERVICE="Original E.L.S.U.S.";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1v";
  const HISTORY_KEY=PREFIX+"update_history_1v";
  const SNOOZE_KEY=PREFIX+"update_snooze_1v";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1v";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1v";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1v";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1v";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1V update check used local manifest cache",error?.message||error);
    }
  ...
  async function beginUserUpdate(manifest){
    ...
    try{
      if(window.Gold1V?.requestVersionChange){await window.Gold1V.requestVersionChange(manifest);return true}
      if(window.Gold1V?.saveWorkspaceNow)await window.Gold1V.saveWorkspaceNow(false);
      window.Gold1V?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1V live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1V live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1V?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1V?.openUpdateCenter?.(currentManifest);
  ...
  function patchPublicAPI(){
    window.Gold1V=window.Gold1V||api()||{};
    Object.assign(window.Gold1V,{elsusMode:"original",checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1V","Update checks, Focus Sessions, and Quick Settings loaded.","System");
  }
