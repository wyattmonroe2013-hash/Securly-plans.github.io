"use strict";
/* EmeraldOS Gold 1U user-experience, layout, and E.L.S.U.S. Shell V2 test tools. */
(function EmeraldOSGold1UImprovements(){
  if(window.__EMERALDOS_GOLD_1U_IMPROVEMENTS__) return;
  window.__EMERALDOS_GOLD_1U_IMPROVEMENTS__=true;
  ...
  function migrationStatus(){
    const report=read(PREFIX+"migration_1u_cloud_report",null)||read(PREFIX+"migration_1u_local_report",null)||{};
    return {
      complete:window.__GOLD1U_MIGRATION_COMPLETE__===true||report.completed===true,
      blocked:window.__GOLD1U_MIGRATION_BLOCK_SAVES__===true,
  ...
  function runShellV2Tests(){
    ...
    const checks=[
      ...
      {name:"Version login retained",pass:localStorage.getItem("loggedIn")==="true"||localStorage.getItem(PREFIX+"loggedIn")==="true",detail:"Gold 1U performed its own login after shell routing."},
      {name:"Cloud route metadata available",pass:Boolean(userRoute?.folder||read(PREFIX+"workspace_meta_1u",{})?.activeFolder||window.EMERALDOS_GOLD_FOLDER),detail:userRoute?.folder?`Cached Shell V2 route: ${userRoute.folder}.`:`Gold route metadata reports ${FOLDER}.`},
      {name:"Migration gate",pass:window.__GOLD1U_MIGRATION_BLOCK_SAVES__!==true,detail:window.__GOLD1U_MIGRATION_BLOCK_SAVES__===true?"Cloud autosave is correctly blocked until migration validation completes.":"Migration is complete and cloud saving is available."},
  ...
  function openShellV2Test(){
    ...
    const rows=result.checks.map(item=>`<tr><td><span class="gold1p-test-dot ${item.pass?"pass":"warn"}"></span>${esc(item.name)}</td><td>${item.pass?"Pass":"Review"}</td><td>${esc(item.detail)}</td></tr>`).join("");
    const shell=result.shell;
    const html=`<div class="app-shell gold1p-shell-test"><div class="app-toolbar"><button id="shellTestRun" class="button primary">Run tests again</button><button id="shellTestSave" class="button">Save VM route</button><button id="shellTestExport" class="button">Export report</button><button id="shellTestReturn" class="button">Return to Shell V2</button></div><div class="app-body"><div class="gold1p-page-heading"><div><h1>E.L.S.U.S. Shell V2 Compatibility</h1><p>Tests the optional Shell V2 routing layer while keeping Gold 1U compatible with the original E.L.S.U.S. shell.</p></div><span class="gold1p-status-pill ${result.allPassed?"good":"review"}">${result.passed}/${result.total} passed</span></div><div class="grid3"><div class="card"><b>Launch mode</b><h2>${shell.managed?"Shell V2":"Direct / original shell"}</h2></div><div class="card"><b>Gold route</b><h2>${VERSION}</h2><p>${FOLDER}</p></div><div class="card"><b>Migration gate</b><h2>${window.__GOLD1U_MIGRATION_BLOCK_SAVES__===true?"Protected":"Ready"}</h2></div></div><div class="gold1p-table-wrap"><table><thead><tr><th>Test</th><th>Result</th><th>Details</th></tr></thead><tbody>${rows}</tbody></table></div><section class="card"><h2>What Shell V2 is allowed to do</h2><p>Shell V2 verifies the EmeraldOS account only to locate the user’s current version. It then opens <code>${FOLDER}/index.html</code>, where Gold 1U performs its own normal login. Shell V2 does not replace files, settings, or VM data.</p></section><p class="muted">A review result can be expected when Gold 1U was opened directly instead of through Shell V2. It does not mean the operating system failed.</p></div></div>`;
    ...
    win.querySelector("#shellTestExport").onclick=()=>downloadJSON("EmeraldOS-Gold-1U-Shell-V2-Test.json",runShellV2Tests());
  ...
  function openWelcomeCenter(){
    ...
    const html=`<div class="app-shell gold1p-welcome"><div class="app-toolbar"><button class="button primary" data-welcome-app="explorer">Open Files</button><button class="button" data-welcome-app="settings">Settings</button><button class="button" data-welcome-app="migrationcenter">Migration status</button><button class="button" data-welcome-app="shellv2test">Shell V2 test</button></div><div class="app-body"><div class="gold1p-hero"><div><p class="eyebrow">EmeraldOS Gold 1U</p><h1>Welcome back, ${esc(username())}</h1><p>Your files, settings, preferences, applications, and earlier Gold file formats remain part of the same version-independent VM.</p></div><div class="gold1p-hero-mark">G</div></div><div class="grid3"><div class="card"><b>Migration</b><h2>${migration.complete?"Complete":"Checking"}</h2><p>${migration.files} files available</p></div><div class="card"><b>Cloud autosave</b><h2>${migration.blocked?"Protected":"Ready"}</h2><p>${migration.blocked?"Waiting for validation":"Continuity gate passed"}</p></div><div class="card"><b>Shell support</b><h2>Original + V2</h2><p>Compatible routing</p></div></div><section class="card"><div class="gold1p-section-title"><h2>Continue where you left off</h2><button id="welcomeRefresh" class="button">Refresh</button></div><div class="gold1p-recent-list">${recent.map(file=>`<button data-recent-file="${esc(file.id)}"><b>${esc(file.name)}</b><small>${esc(file.folder||"Documents")} · ${new Date(file.updated||file.created||Date.now()).toLocaleString()}</small></button>`).join("")||"<p>No recent files yet.</p>"}</div></section><section class="card"><h2>Gold 1U improvements</h2><div class="gold1p-feature-grid"><div><b>Safer continuity</b><p>Gold 1U includes Gold 1N and earlier schemas and file formats in migration scans.</p></div><div><b>Cleaner applications</b><p>Settings, tables, cards, toolbars, and narrow windows resize without clashing.</p></div><div><b>Shell V2 testing</b><p>Verify account routing without replacing the version’s own login.</p></div><div><b>User controls</b><p>Gold Home, Command Palette, My Workspace, and Feedback Hub make common actions easier to find.</p></div></div></section><label class="gold1p-checkbox"><input id="welcomeAtLogin" type="checkbox" ${read(PREFIX+"home_1u_at_login",true)?"checked":""}> Show Gold Home after Gold 1U sign-in</label></div></div>`;
    ...
    win.querySelector("#welcomeAtLogin").onchange=event=>write(PREFIX+"home_1u_at_login",event.target.checked);
    return win;
  }

  function feedbackItems(){const items=read(PREFIX+"feedback_1u",[]);return Array.isArray(items)?items:[]}
  function saveFeedback(items){write(PREFIX+"feedback_1u",items.slice(0,100));api()?.saveWorkspaceNow?.(false)}
  function openFeedbackHub(){
    const items=feedbackItems();
    const html=`<div class="app-shell gold1p-feedback"><div class="app-toolbar"><button id="feedbackNew" class="button primary">Save feedback</button><button id="feedbackExport" class="button">Export all</button><button id="feedbackClear" class="button danger">Clear local feedback</button></div><div class="app-body"><h1>Feedback Hub</h1><p>Record usability issues and ideas while testing Gold 1U. Entries remain in your VM and can be exported for development.</p><div class="gold1p-feedback-form"><label>Category<select id="feedbackCategory" class="field"><option>Problem</option><option>Suggestion</option><option>Shell V2 test</option><option>File compatibility</option><option>Accessibility</option></select></label><label>Title<input id="feedbackTitle" class="field" maxlength="120" placeholder="Describe the issue briefly"></label><label>Details<textarea id="feedbackDetails" class="field" rows="6" placeholder="What happened, and what did you expect?"></textarea></label></div><h2>Saved feedback</h2><div class="gold1p-feedback-list">${items.map(item=>`<article class="card"><div><b>${esc(item.title)}</b><span>${esc(item.category)}</span></div><p>${esc(item.details)}</p><small>${new Date(item.createdAt).toLocaleString()}</small></article>`).join("")||"<p class=\"muted\">No feedback has been saved.</p>"}</div></div></div>`;
    ...
    win.querySelector("#feedbackExport").onclick=()=>downloadJSON("EmeraldOS-Gold-1U-Feedback.json",feedbackItems());
  ...
  function registerApps(){
    ...
    add({id:"welcome",name:"Welcome Center",label:"WC",color:"#0078d7",group:"System",desc:"Quick access, recent files, and Gold 1U status.",open:openWelcomeCenter});
  ...
  function patchOpenApp(){
    const core=api();if(!core||core.__gold1pImprovementPatch)return;
    core.__gold1pImprovementPatch=true;
    ...
    window.Gold50=core;window.Gold1U=window.Gold1U||core;
    Object.assign(window.Gold1U,{openWelcomeCenter,openShellV2Test,runShellV2Tests,openFeedbackHub,shellV2Parameters:shellParameters});
  }

  function installResponsiveShell(){
    const update=()=>document.body.classList.toggle("gold1p-compact-shell",innerWidth<980);
    update();window.addEventListener("resize",update,{passive:true});
  }

  function maybeOpenWelcome(){
    if(read(PREFIX+"home_1u_at_login",true)!==true)return;
    if(sessionStorage.getItem("gold1p_home_opened")==="true")return;
    sessionStorage.setItem("gold1p_home_opened","true");
  ...
  function init(){
    ...
    api().notify?.("EmeraldOS Gold 1U","User improvements, application layout fixes, and Shell V2 compatibility testing are ready.","System");
  }
