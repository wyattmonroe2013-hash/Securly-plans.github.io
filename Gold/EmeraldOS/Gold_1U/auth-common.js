...
window.GoldAuth = (() => {
  ...
  const ACCOUNT_HISTORY_KEY = "gold1g_account_history_1u";
  ...
  async function findCloudUser(username) {
    ...
    // Gold 1U compatible with both layouts.
    for (const collectionName of collections) {
  ...
  async function beginSession(user) {
    ...
    await window.Gold1UUserIsolation?.switchToUser?.(userId, username);
  ...
  return {
    ...
    recordAccountEvent,
    changePassword,
    localIsolationStatus: () => window.Gold1UUserIsolation?.status?.() || null
  };
})();
