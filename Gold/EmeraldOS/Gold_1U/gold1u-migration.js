...
/* EmeraldOS Gold 1U local continuity migration.
   ...
   asynchronous cloud migration in gold1u-continuity.js has completed. */
(function EmeraldOSGold1ULocalMigration(){
  if(window.__EMERALDOS_GOLD_1U_LOCAL_MIGRATION__) return;
  window.__EMERALDOS_GOLD_1U_LOCAL_MIGRATION__=true;
  window.__GOLD1U_MIGRATION_BLOCK_SAVES__=true;

  const PREFIX="gold1g_";
  const REPORT_KEY=PREFIX+"migration_1u_local_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1U local migration write failed",key,error);return false}};
  ...
  const fileKeys=[
    "gold1g_files","gold1h_files","gold1i_files","gold1j_files","gold1k_files","gold1l_files","gold1m_files","gold1n_files","gold1o_files","gold1p_files","gold1q_files","gold1r_files","gold1s_files","gold1t_files","gold1u_files",
    "gold1g_workspace","gold1h_workspace","gold1i_workspace","gold1j_workspace","gold1k_workspace","gold1l_workspace","gold1m_workspace","gold1n_workspace","gold1o_workspace","gold1p_workspace","gold1q_workspace","gold1r_workspace","gold1s_workspace","gold1t_workspace","gold1u_workspace",
  ...
  const previousSession=readMaybe(PREFIX+"session_state_1u")||readMaybe(PREFIX+"session_state_1t")||readMaybe(PREFIX+"session_state_1s")||readMaybe(PREFIX+"session_state_1r")||readMaybe(PREFIX+"session_state_1q")||readMaybe(PREFIX+"session_state_1p")||readMaybe(PREFIX+"session_state_1o");
  ...
  if(previousSession!==undefined)write(PREFIX+"session_state_1u",previousSession);
  else if(previousMeta?.openWindows)write(PREFIX+"session_state_1u",{version:"1O",updatedAt:previousMeta.savedAt||now(),windows:previousMeta.openWindows,openApps:[]});
  const previousNotificationPrefs=readMaybe(PREFIX+"notification_preferences_1u")||readMaybe(PREFIX+"notification_preferences_1t")||readMaybe(PREFIX+"notification_preferences_1s")||readMaybe(PREFIX+"notification_preferences_1r")||readMaybe(PREFIX+"notification_preferences_1q")||readMaybe(PREFIX+"notification_preferences_1p")||readMaybe(PREFIX+"notification_preferences_1o");
  if(previousNotificationPrefs!==undefined)write(PREFIX+"notification_preferences_1u",previousNotificationPrefs);
  const previousHomePreference=readMaybe(PREFIX+"home_1u_at_login")??readMaybe(PREFIX+"home_1t_at_login")??readMaybe(PREFIX+"home_1s_at_login")??readMaybe(PREFIX+"home_1r_at_login")??readMaybe(PREFIX+"home_1q_at_login")??readMaybe(PREFIX+"home_1p_at_login")??readMaybe(PREFIX+"home_1o_at_login")??readMaybe(PREFIX+"welcome_1n_at_login");
  if(previousHomePreference!==undefined)write(PREFIX+"home_1u_at_login",previousHomePreference);

  const categories=["prefs","folders","mail","notes","tasks","events","contacts","tickets","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings","feedback_1u","routines_1u","accessibility_1u","home_1u_at_login","notification_preferences_1u","session_state_1u","account_history_1u","game_data_1u","calendar_preferences_1u","guide_preferences_1u","radio_stations_1u","radio_preferences_1u","file_tombstones","file_system_metadata","file_recovery_catalog","file_operations_1u","notification_app_permissions_1u","file_shares_1u","shared_files_cache_1u","privacy_audit_1u"];
  const prefixes=["gold1a_","gold1b_","gold1c_","gold1d_","gold1e_","gold1f_","gold1h_","gold1i_","gold1j_","gold1k_","gold1l_","gold1m_","gold1n_","gold1o_","gold1p_","gold1q_","gold1r_","gold1s_","gold1t_","gold1u_","gold1g_"];
  ...
  const sourceVersions=["1h","1i","1j","1k","1l","1m","1n","1o","1p","1q","1r","1s","1t","1u"];
  versionedFamilies.forEach(family=>{
    ...
    if(merged!==undefined)write(`${PREFIX}${family}_1u`,merged);
  });
  ...
  window.Gold1ULocalMigration={report,collectFiles,mergeFiles,deepMerge,mergeArray,normalizeFile};
})();
