...
========================================================= */
(function EmeraldOSGold1UFeatures(){
  if(window.__EMERALDOS_GOLD_1U_FEATURES__) return;
  window.__EMERALDOS_GOLD_1U_FEATURES__=true;
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1U local write failed",key,error);return false}};
  ...
  const notify=(title,body,app="EmeraldOS Gold 1U")=>api()?.notify?.(title,body,app)||console.info(`[${app}] ${title}: ${body}`);
  ...
  function staffSession(){
    for(const key of [PREFIX+"staff_session","gold1u_staff_session","gold1j_staff_session"]){
  ...
  function sandboxUserApp(app,{preview=false,hashVerified=false}={}){
    ...
    const html=`<div class="gold1p-user-app-host"><iframe id="${frameId}" sandbox="allow-scripts allow-forms allow-modals allow-downloads allow-same-origin" title="${esc(app.name)}"></iframe><div class="gold1p-sandbox-note">${preview?"Developer preview":"Staff-verified sandbox"} · ${esc(app.owner)}</div></div>`;
    ...
    iframe.srcdoc=`<!doctype html><html><head><meta charset="utf-8"><style>html,body{margin:0;height:100%;font-family:Segoe UI,Arial,sans-serif;background:#fff;color:#111}*{box-sizing:border-box}button,input,select,textarea{font:inherit}</style></head><body><div id="app"></div><script>const TOKEN=${JSON.stringify(token)};let DATA=${initial};const send=(type,payload={})=>parent.postMessage({source:'emerald-gold-user-app',token:TOKEN,type,payload},'*');const cacheWarning=(operation,error)=>{console.warn('[EmeraldOS Gold 1U] Cache API '+operation+' skipped:',error);send('cache-warning',{operation,message:String(error?.message||error||'Cache access unavailable')})};const emptyResponse=()=>undefined;const createNoopCache=()=>({match:async()=>emptyResponse(),matchAll:async()=>[],add:async()=>false,addAll:async()=>false,put:async()=>false,delete:async()=>false,keys:async()=>[]});const guardCache=cache=>({match:async(...a)=>{try{return await cache.match(...a)}catch(e){cacheWarning('match',e);return emptyResponse()}},matchAll:async(...a)=>{try{return await cache.matchAll(...a)}catch(e){cacheWarning('matchAll',e);return[]}},add:async(...a)=>{try{await cache.add(...a);return true}catch(e){cacheWarning('add',e);return false}},addAll:async(...a)=>{try{await cache.addAll(...a);return true}catch(e){cacheWarning('addAll',e);return false}},put:async(...a)=>{try{await cache.put(...a);return true}catch(e){cacheWarning('put',e);return false}},delete:async(...a)=>{try{return await cache.delete(...a)}catch(e){cacheWarning('cache.delete',e);return false}},keys:async(...a)=>{try{return await cache.keys(...a)}catch(e){cacheWarning('cache.keys',e);return[]}}});const SAFE_CACHES={open:async name=>{try{if(!globalThis.caches?.open)throw new Error('CacheStorage is unavailable in this browser context.');return guardCache(await globalThis.caches.open(String(name)))}catch(e){cacheWarning('open',e);return createNoopCache()}},has:async name=>{try{return globalThis.caches?.has?await globalThis.caches.has(String(name)):false}catch(e){cacheWarning('has',e);return false}},delete:async name=>{try{return globalThis.caches?.delete?await globalThis.caches.delete(String(name)):false}catch(e){cacheWarning('delete',e);return false}},keys:async()=>{try{return globalThis.caches?.keys?await globalThis.caches.keys():[]}catch(e){cacheWarning('keys',e);return[]}},match:async(...a)=>{try{return globalThis.caches?.match?await globalThis.caches.match(...a):emptyResponse()}catch(e){cacheWarning('match',e);return emptyResponse()}}};const api={setTitle:t=>send('title',{title:String(t)}),setContent:h=>document.getElementById('app').innerHTML=String(h),notify:(t,b)=>send('notify',{title:String(t),body:String(b)}),save:(k,v)=>{DATA[k]=v;send('save',{key:String(k),value:v})},load:(k,f=null)=>Object.prototype.hasOwnProperty.call(DATA,k)?DATA[k]:f,on:(selector,event,fn)=>document.querySelector(selector)?.addEventListener(event,fn),close:()=>send('close'),cache:SAFE_CACHES,download:(name,text,type='text/plain')=>{const a=document.createElement('a');a.download=String(name);a.href=URL.createObjectURL(new Blob([text],{type}));a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}};try{Function('api','caches','"use strict";\\n'+${code})(api,SAFE_CACHES)}catch(error){document.getElementById('app').innerHTML='<div style="padding:24px"><h2>Application error</h2><pre style="white-space:pre-wrap"></pre></div>';document.querySelector('pre').textContent=error.stack||error.message;send('error',{message:error.message})}<\/script></body></html>`
    const handler=event=>{
      ...
      if(type==="cache-warning"){
        console.warn(`Gold 1U user app cache operation skipped (${payload.operation||"unknown"})`,payload.message||"");
        if(!win.dataset.cacheWarningShown){
          win.dataset.cacheWarningShown="true";
          notify("Application cache unavailable",`${app.name} will continue without its optional browser cache.` ,app.name);
        }
      }
    };
  ...
  async function loadLatestManifest(){
    ...
    }catch(error){console.debug("Gold 1U update lookup used local cache",error?.message||error)}
    return cached||{latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",releaseTitle:"EmeraldOS Gold 1U",summary:"Current release"};
  }
  ...
  async function openEnhancedUpdateCenter(){
    ...
    win.querySelector("#updateSave").onclick=()=>window.Gold1U?.saveWorkspaceNow?.(true);
    ...
    win.querySelector("#updatePublisher")?.addEventListener("click",()=>window.Gold1U?.openPublisherManager?.());
    return win;
  }
  function openELSUSStatus(){
    const latest=elsusState.latestManifest||read("emeraldGoldShell_latest",null);
    const html=`<div class="app-body"><h1>E.L.S.U.S.</h1><p>Gold 1U uses the original E.L.S.U.S. implementation from Gold 1I.</p><div class="gold1p-settings-cards"><div class="card"><h3>Routing</h3><p>Save VM → write pending manifest → open root <code>gold-shell.html</code></p></div><div class="card"><h3>Cloud VM</h3><p><code>emeraldOSUsers/${esc(userName())}/goldVM/current</code></p></div><div class="card"><h3>Latest manifest</h3><p>${esc(latest?.releaseTitle||"Not checked yet")}</p></div><div class="card"><h3>Publishing</h3><p>Manual Staff Edition and publisher-PIN workflow only.</p></div></div><div class="actions"><button id="elsusCheck" class="button primary">Check for updates</button><button id="elsusSave" class="button">Save VM</button><button id="elsusRoot" class="button">Open root E.L.S.U.S.</button></div><p class="muted">Gold 1U uses the direct, original E.L.S.U.S. route and requires no additional shell patch.</p></div>`;
    ...
    win.querySelector("#elsusSave").onclick=()=>window.Gold1U?.saveWorkspaceNow?.(true);
  ...
  async function listenLiveStaffControl(){
    ...
    }catch(error){console.debug("Gold 1U live control is using local fallback",error)}
  }
  ...
  function wireStaffContent(win,tab,data){
    ...
    if(tab==="elsus"){
      ...
      win.querySelector("#staffPublisherOpen")?.addEventListener("click",()=>window.Gold1U?.openPublisherManager?.());
      win.querySelector("#staffSaveVM")?.addEventListener("click",()=>window.Gold1U?.saveWorkspaceNow?.(true));
    }
  }
  ...
  async function sendSessionHeartbeat(){
    ...
    try{const fb=await import("./firebase.js");if(fb.db)await fb.setDoc(fb.doc(fb.db,SESSION_COLLECTION,normalizeUser(userName())),{...record,lastSeen:fb.serverTimestamp()},{merge:true})}catch(error){console.debug("Gold 1U session heartbeat cloud write skipped",error.message)}
  }
  ...
  function settingsContent(section){
    ...
    if(section==="privacy")return `<h1>Privacy</h1><div class="gold1p-settings-cards"><div class="card"><h3>User isolation</h3><p>Files, settings, and preferences are separated by authenticated account ID in cloud and local VM storage.</p><button class="button primary" data-open-app="privacycenter">Open Privacy & User Isolation</button></div><div class="card"><h3>Specific-file sharing</h3><p>Your VM is private unless you explicitly share selected file records.</p><button class="button" data-open-app="filesharing">Open Files & Sharing</button></div><div class="card"><h3>Notification permissions</h3><p>Only updates, support, security, and emergency notices are enabled by default.</p><button class="button" data-open-app="notificationpermissions">Manage Can-Notify permissions</button></div></div>${settingToggle("setClipboardHistory","Clipboard history","Store copied text in this user's Gold VM.",p.clipboardHistory!==false)}${settingToggle("setDiagnostics","Optional diagnostics","Include non-sensitive environment details in support tickets.",p.optionalDiagnostics!==false)}${settingToggle("setSessionHeartbeat","Live session heartbeat","Report this Gold VM to Staff Center while signed in.",p.sessionHeartbeat!==false)}<div class="actions"><button class="button" data-open-tool="clipboard">Clipboard history</button><button class="button" id="clearLocalCaches">Clear replaceable caches</button></div>`;
    ...
    if(section==="shellv2")return `<h1>E.L.S.U.S. Shell V2</h1><p>Shell V2 can identify the user’s current Gold route before opening this version’s own login page. Gold 1U remains compatible with the original shell.</p><div class="gold1p-settings-cards"><div class="card"><h3>Two-step login</h3><p>The shell identifies the VM; Gold 1U still authenticates the operating-system session.</p></div><div class="card"><h3>Data isolation</h3><p>The shell reads routing metadata and does not replace files or settings.</p></div><div class="card"><h3>Compatibility test</h3><p>Review routing parameters, version metadata, and migration safeguards.</p></div></div><button class="button primary" data-open-app="shellv2test">Open Shell V2 Compatibility</button>`;
  ...
  function wireEnhancedSettings(win,section){
    ...
    win.querySelectorAll("[data-open-tool]").forEach(button=>button.onclick=()=>({clipboard:window.Gold1U?.openClipboardHistory,elsus:openELSUSStatus}[button.dataset.openTool]?.()));
  ...
  function saveStickyNotes(notes){write(STICKY_KEY,notes);window.Gold1U?.saveWorkspaceNow?.(false)}
  ...
  function patchOpenApp(){
    ...
    window.Gold1U=window.Gold1U||core;
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1U","E.L.S.U.S., Staff Center, verified user apps, custom logos, and Windows 10 features are ready.","System");
  }
