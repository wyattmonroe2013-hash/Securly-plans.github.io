...
========================================================= */
(function EmeraldOSGold1UExtras(){
  if(window.__EMERALDOS_GOLD_1U_EXTRAS__) return;
  window.__EMERALDOS_GOLD_1U_EXTRAS__=true;

  const VERSION="1U";
  const FOLDER="Gold_1U";
  const UPDATE_SERVICE="Original E.L.S.U.S.";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1u";
  const HISTORY_KEY=PREFIX+"update_history_1u";
  const SNOOZE_KEY=PREFIX+"update_snooze_1u";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1u";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1u";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1u";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1u";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1U update check used local manifest cache",error?.message||error);
    }
  ...
  async function beginUserUpdate(manifest){
    ...
    try{
      if(window.Gold1U?.requestVersionChange){await window.Gold1U.requestVersionChange(manifest);return true}
      if(window.Gold1U?.saveWorkspaceNow)await window.Gold1U.saveWorkspaceNow(false);
      window.Gold1U?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1U live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1U live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1U?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1U?.openUpdateCenter?.(currentManifest);
  ...
  function patchPublicAPI(){
    ...
    Object.assign(window.Gold1U,{elsusMode:"original",checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1U","Update checks, Focus Sessions, and Quick Settings loaded.","System");
  }
