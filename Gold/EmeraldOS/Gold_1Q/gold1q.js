...
(function EmeraldOSGold1Q(){
  ...
  function taskManagerHTML(tab="processes"){
    return `<div class="gold1p-app gold1p-taskmgr">
      ...
      <div class="gold1p-statusbar"><span>${processRows().length} apps and windows</span><span>EmeraldOS Gold 1Q</span><span class="gold1p-muted">Virtual process manager</span></div>
    </div>`;
  }
  ...
  function openControlItem(id){
    ...
    if(id==="network")return openSimplePanel("Network and Internet",`<h2>Network status</h2><p>${navigator.onLine?"Connected to the Internet":"Offline"}</p><div class="gold1p-control-grid"><button class="gold1p-control-item" onclick="Gold1Q.runCommand('settings')"><span class="icon">⌁</span><span><b>Network settings</b><small>Open EmeraldOS Settings.</small></span></button></div>`);
  ...
  function openPublisherManager(){
    ...
    const html=`<div class="gold1p-app" style="padding:20px;overflow:auto"><h1>Update Publisher Manager</h1><p>Publish <b>EmeraldOS Gold 1Q</b> to <code>system/emeraldGoldLatest</code>.</p><div class="gold1p-performance-card"><b>Security status</b><p>${unlocked?"Publisher PIN accepted for this staff session.":"Locked. Enter the publisher PIN."}</p>${unlocked?"":'<label>Publisher PIN <input id="gold1pPublisherPin" type="password" class="gold1p-field"></label> <button id="gold1pUnlockPublisher" class="gold1p-button">Unlock</button>'}</div><div class="gold1p-performance-card"><b>Release target</b><p>Gold_1Q / OS.html</p><p>Release title: EmeraldOS Gold 1Q</p></div><div style="display:flex;gap:8px;margin-top:18px"><button id="gold1pPublishVersion" class="gold1p-button primary" ${unlocked?"":"disabled"}>Publish this Version</button><button id="gold1pExportManifest" class="gold1p-button">Export manifest</button></div><p class="gold1p-muted">Gold 1Q never publishes from a query string or during first boot.</p></div>`;
    ...
    $("gold1pExportManifest")?.addEventListener("click",()=>saveText("FIREBASE_EMERALDGOLDLATEST_1Q.json",JSON.stringify(releaseManifest(),null,2),"application/json"));return win;
  }
  ...
  function customToolButtons(){return `
    <button class="gold1p-start-tile" onclick="Gold1Q.openRegistryEditor()"><span>▥</span><b>Registry Editor</b></button>
    <button class="gold1p-start-tile" onclick="Gold1Q.openTaskManager()"><span>▤</span><b>Task Manager</b></button>
    <button class="gold1p-start-tile" onclick="Gold1Q.openControlPanel()"><span>⚙</span><b>Control Panel</b></button>
    <button class="gold1p-start-tile" onclick="Gold1Q.openRun()"><span>▦</span><b>Run</b></button>
    <button class="gold1p-start-tile" onclick="Gold1Q.openClipboardHistory()"><span>▧</span><b>Clipboard</b></button>
    ...
    ${isStaff()?'<button class="gold1p-start-tile" onclick="Gold1Q.openPublisherManager()"><span>UP</span><b>Publisher</b></button>':''}` }
  ...
  function maybeShowUpdateSetup(){
    ...
    const overlay=document.createElement("section");overlay.id="gold1pUpdateSetup";overlay.className="gold1p-update-setup";overlay.innerHTML=`<div class="gold1p-update-card"><h1>Welcome to EmeraldOS Gold 1Q</h1><p>Your existing E.L.S.U.S. cloud VM is being preserved while Gold 1Q restores your previous workspace quietly and improves everyday usability.</p><ul><li>Quiet startup with one consolidated status banner</li><li>Cloud workspace resume and safer session continuity</li><li>Backup &amp; Sync with safe merge restore</li><li>File Compatibility Center for previous Gold file versions</li><li>System Health and Accessibility Center</li><li>Original E.L.S.U.S. and Shell V2 compatibility</li><li>Staff Center, verified user apps, custom logos, and less intrusive update prompts</li><li>Settings, toolbar, table, card, and narrow-window collision fixes</li></ul><p>Files, folders, settings, preferences, Registry data, applications, mail, and all available VM categories are merged and validated before cloud autosave can begin.</p><div class="actions"><button class="gold1p-button" onclick="document.getElementById('gold1pUpdateSetup').remove()">Remind me later</button><button class="gold1p-button primary" onclick="Gold1Q.finishUpdateSetup()">Continue</button></div></div>`;document.body.appendChild(overlay);
  }
  ...
  function branding(){
    document.title="EmeraldOS Gold 1Q";document.body.classList.add("gold1p","gold1p-shell");document.body.dataset.build=VERSION;
