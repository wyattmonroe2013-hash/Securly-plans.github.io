...
/* EmeraldOS Gold 1Q preboot compatibility layer.
...
(function installGold1QPreboot(){
  if(window.__EMERALDOS_GOLD_1Q_PREBOOT__) return;
  window.__EMERALDOS_GOLD_1Q_PREBOOT__=true;
  window.EMERALDOS_GOLD_VERSION="1Q";
  window.EMERALDOS_GOLD_FOLDER="Gold_1Q";

  const WORKSPACE_KEY="gold1g_workspace";
  const META_KEY="gold1g_workspace_meta_1q";
  ...
  /* Gold 1G installs a monolithic 15-second autosave inside its private scope.
     Redirect only that recognizable timer to the split Gold 1Q saver. */
  window.setInterval=function gold1IIntervalRouter(callback,delay,...args){
    ...
    if(Number(delay)===15000 && /saveWorkspaceNow/.test(source) && /cloudSync/.test(source)){
      return nativeSetInterval(function(){
        try{
          if(window.Gold1Q?.saveWorkspaceNow) window.Gold1Q.saveWorkspaceNow(false);
          else if(window.Gold50?.saveWorkspaceNow) window.Gold50.saveWorkspaceNow(false);
        }catch(error){console.warn("Gold 1Q autosave failed",error)}
      },30000);
    }
  ...
  }catch(error){console.warn("Gold 1Q preboot cleanup skipped",error)}
  ...
  Storage.prototype.setItem=function gold1IQuotaSafeSetItem(key,value){
    ...
    if(storage===localStorage && stringKey===WORKSPACE_KEY && byteLength(stringValue)>MAX_RECOVERY_BYTES){
      try{
        ...
        console.warn("Gold 1Q could not retain the compact local recovery cache",error);
        return;
      }
    }
    ...
    try{
      ...
      try{return nativeSetItem.call(storage,stringKey,stringValue)}catch(retryError){
        if(storage===localStorage && stringKey===WORKSPACE_KEY){
          ...
          console.warn("Gold 1Q skipped an oversized recovery-cache write after storage cleanup",retryError);
          return;
        }
        throw retryError;
      }
