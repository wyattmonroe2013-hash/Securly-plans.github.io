...
========================================================= */
(function EmeraldOSGold1N(){
  if(window.__EMERALDOS_GOLD_1N_LOADED__) return;
  window.__EMERALDOS_GOLD_1N_LOADED__=true;

  const VERSION="1N";
  const FOLDER="Gold_1N";
  const PREFIX="gold1g_";
  const PUBLISHER_PIN="093013";
  const REGISTRY_KEY=PREFIX+"registry_1n";
  const REGISTRY_HISTORY_KEY=PREFIX+"registry_1n_history";
  const CLIPBOARD_KEY=PREFIX+"clipboard_history_1n";
  const STARTUP_KEY=PREFIX+"startup_apps_1n";
  const CLOUD_CATEGORIES=["prefs","files","folders","tickets","mail","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","registry_1n","registry_1n_history","clipboard_history_1n","startup_apps_1n","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings","update_preferences_1n","update_history_1n","focus_sessions_1n"];
  ...
  const writeJSON=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1N local write failed",key,error);return false}};
  ...
  function notify(title,body,app="EmeraldOS Gold 1N"){
    try{
  ...
  function isStaff(){
    /* Gold 1N protected tools require a Staff Edition session that passed
       both EmeraldOS role verification and Emerald Mail verification. */
    for(const key of [PREFIX+"staff_session","gold1n_staff_session"]){
  ...
  function focusAnyWindow(win){
    ...
    const button=$("gold1n_task_"+win.id);
    qa(".gold1n-task-button").forEach(b=>b.classList.remove("active"));
    if(button) button.classList.add("active");
  }

  function taskButtonFor(win){
    const taskApps=$("taskApps");
    if(!taskApps || $("gold1n_task_"+win.id)) return;
    const button=document.createElement("button");
    button.id="gold1n_task_"+win.id;
    button.className="gold1n-task-button active";
    button.title=win.dataset.title||"EmeraldOS Gold 1N";
  ...
  function saveCustomWindowState(){
    const state=qa(".gold1n-window").map(w=>({
      id:w.id,title:w.dataset.title,left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,
      minimized:w.classList.contains("minimized"),maximized:w.classList.contains("gold1n-maximized")
    }));
  ...
  function open1IWindow(id,title,html,options={}){
    ...
    win.className="window gold1n-window";
  ...
  function wireCustomWindow(win){
    ...
    title?.addEventListener("pointerdown",e=>{
      if(e.target.closest("button")||win.classList.contains("gold1n-maximized")) return;
    ...
    win.querySelector("[data-g1h-resize]")?.addEventListener("pointerdown",e=>{
      if(win.classList.contains("gold1n-maximized")) return;
  ...
  function minimizeWindow(id){
    ...
    $("gold1n_task_"+id)?.classList.remove("active");
    saveCustomWindowState();
  }

  function maximizeWindow(id){
    const win=$(id);if(!win)return;
    if(win.classList.contains("gold1n-maximized")){
      const restore=readJSON(PREFIX+"restore_"+id,null);
      win.classList.remove("gold1n-maximized","gold1n-snapped");
      ...
      win.classList.add("gold1n-maximized");
    }
  ...
  function closeWindow(id){
    const win=$(id);if(!win)return;
    $("gold1n_task_"+id)?.remove();
    customWindows.delete(id);win.remove();saveCustomWindowState();
  }

  function snapWindow(win,zone){
    if(!win)return;
    if(win.classList.contains("gold1n-maximized")) win.classList.remove("gold1n-maximized");
    ...
    win.classList.add("gold1n-snapped");
    const map={
  ...
  function showSnapPreview(x,y){
    ...
    let preview=$("gold1nSnapPreview");
    if(!zone){preview?.remove();return}
    if(!preview){preview=document.createElement("div");preview.id="gold1nSnapPreview";preview.className="gold1n-snap-preview";document.body.appendChild(preview)}
  ...
  function hideSnapPreview(){$("gold1nSnapPreview")?.remove()}
  ...
  function registryTreeHTML(node,path=[],depth=0){
    return registryChildren(node).map(name=>{
      ...
      return `<div class="gold1n-tree-row ${selected?"selected":""}" data-reg-path="${esc(pathToken(next))}" style="padding-left:${depth*15}px">
        <span class="gold1n-tree-caret" data-reg-toggle="${esc(pathToken(next))}">${hasChildren?(expanded?"▼":"▶"):""}</span>
        <span class="gold1n-tree-folder">▰</span><span>${esc(name)}</span>
  ...
  function registryHTML(){
    ...
    return `<div class="gold1n-app gold1n-registry">
      <div class="gold1n-commandbar">
        ...
        <input id="gold1nRegistryPath" class="gold1n-field gold1n-reg-path gold1n-code" value="${esc(pathText(registrySelection))}" aria-label="Registry path">
        ${locked?'<span class="gold1n-reg-lock">Read-only without Staff Edition</span>':""}
      </div>
      <div class="gold1n-reg-body">
        <div class="gold1n-reg-tree">${registryTreeHTML(data)}</div>
        <div class="gold1n-reg-values">
          <table class="gold1n-reg-table"><thead><tr><th style="width:32%">Name</th><th style="width:19%">Type</th><th>Data</th></tr></thead>
          <tbody>${Object.entries(values).map(([name,value])=>`<tr data-reg-value="${esc(encodeURIComponent(name))}" class="${registrySelectedValue===name?"selected":""}"><td>${esc(name)}</td><td>${esc(value.type||"REG_SZ")}</td><td>${esc(registryValueDisplay(value))}</td></tr>`).join("")||'<tr><td colspan="3" class="gold1n-empty">This key has no values.</td></tr>'}</tbody></table>
        </div>
      </div>
      <div class="gold1n-statusbar"><span>${esc(pathText(registrySelection))}</span><span>${Object.keys(values).length} value(s)</span><span>${locked?"Protected hive":"Editable"}</span></div>
    </div>`;
  }
  ...
  function wireRegistryUI(win){
    ...
    const pathInput=$("gold1nRegistryPath");pathInput?.addEventListener("keydown",e=>{if(e.key==="Enter")navigateRegistry(pathInput.value)});
  }
  ...
  function applyRegistryEffects(data=registryData()){
    ...
    document.body.classList.toggle("gold1n-no-transparency",registryValueAt(data,personal,"Transparency",true)===false);
  ...
  function taskManagerHTML(tab="processes"){
    return `<div class="gold1n-app gold1n-taskmgr">
      <div class="gold1n-tabs"><button class="${tab==="processes"?"active":""}" data-task-tab="processes">Processes</button><button class="${tab==="performance"?"active":""}" data-task-tab="performance">Performance</button><button class="${tab==="startup"?"active":""}" data-task-tab="startup">Startup</button><button class="${tab==="details"?"active":""}" data-task-tab="details">Details</button></div>
      <div class="gold1n-taskmgr-content">${taskManagerPage(tab)}</div>
      <div class="gold1n-statusbar"><span>${processRows().length} apps and windows</span><span>EmeraldOS Gold 1N</span><span class="gold1n-muted">Virtual process manager</span></div>
    </div>`;
  }
  function taskManagerPage(tab){
    if(tab==="performance"){
      ...
      return `<div class="gold1n-performance-grid"><div class="gold1n-performance-card"><h3>CPU</h3><h1>${Math.min(94,8+processRows().length*4)}%</h1><div class="gold1n-meter"><span style="width:${Math.min(94,8+processRows().length*4)}%"></span></div><p class="gold1n-muted">Browser-hosted virtual CPU activity estimate.</p></div><div class="gold1n-performance-card"><h3>Memory</h3><h1>${percent}%</h1><div class="gold1n-meter"><span style="width:${percent}%"></span></div><p>${(localBytes/1024/1024).toFixed(2)} MB local VM storage in use.</p></div><div class="gold1n-performance-card"><h3>Cloud VM</h3><h2>${esc(localStorage.getItem(PREFIX+"cloud_status")||"Ready")}</h2><p>Path: emeraldOSUsers/${esc(userName())}/goldVM/current</p></div><div class="gold1n-performance-card"><h3>Uptime</h3><h2>${formatUptime(performance.now())}</h2><p>${navigator.onLine?"Network connected":"Offline"}</p></div></div>`;
    }
    if(tab==="startup"){
      ...
      return `<table class="gold1n-process-table"><thead><tr><th>Name</th><th>Status</th><th>Startup impact</th><th></th></tr></thead><tbody>${startup.map((a,i)=>`<tr><td>${esc(a.name)}</td><td>${a.enabled?"Enabled":"Disabled"}</td><td>${esc(a.impact||"Not measured")}</td><td><button class="gold1n-button" data-startup-toggle="${i}">${a.enabled?"Disable":"Enable"}</button></td></tr>`).join("")}</tbody></table>`;
    }
    const rows=processRows();
    return `<table class="gold1n-process-table"><thead><tr><th>Name</th><th>Status</th><th>App ID</th><th>Memory</th><th></th></tr></thead><tbody>${rows.map(p=>`<tr><td>${esc(p.title)}</td><td>${esc(p.status)}</td><td class="gold1n-code">${esc(p.app)}</td><td>${p.memory} MB</td><td><button class="gold1n-button" data-end-task="${esc(p.id)}">End task</button></td></tr>`).join("")||'<tr><td colspan="5" class="gold1n-empty">No applications are running.</td></tr>'}</tbody></table>`;
  }
  ...
  function controlPanelHTML(){return `<div class="gold1n-app gold1n-control-shell"><div class="gold1n-control-header"><h2>Control Panel</h2><p class="gold1n-muted">Adjust your computer's settings.</p><input id="gold1nControlSearch" class="gold1n-field" style="width:min(420px,100%)" placeholder="Search Control Panel"></div><div class="gold1n-control-grid">${CONTROL_ITEMS.map(item=>`<button class="gold1n-control-item" data-control="${item[3]}" data-control-text="${esc((item[0]+" "+item[1]).toLowerCase())}"><span class="icon">${item[2]}</span><span><b>${esc(item[0])}</b><small>${esc(item[1])}</small></span></button>`).join("")}</div></div>`}
  ...
  function wireControlPanel(win){
    ...
    $("gold1nControlSearch")?.addEventListener("input",e=>{const q=e.target.value.toLowerCase();win.querySelectorAll("[data-control]").forEach(b=>b.style.display=b.dataset.controlText.includes(q)?"":"none")});
  }
  function openControlItem(id){
    ...
    if(id==="network")return openSimplePanel("Network and Internet",`<h2>Network status</h2><p>${navigator.onLine?"Connected to the Internet":"Offline"}</p><div class="gold1n-control-grid"><button class="gold1n-control-item" onclick="Gold1N.runCommand('settings')"><span class="icon">⌁</span><span><b>Network settings</b><small>Open EmeraldOS Settings.</small></span></button></div>`);
    if(id==="hardware")return base("settings");
  }
  function openSimplePanel(id,html){return open1IWindow(id.toLowerCase().replace(/\W/g,"_"),id,`<div class="gold1n-app" style="padding:18px;overflow:auto">${html}</div>`,{width:760,height:520,icon:"⚙",refresh:true})}
  function openSystemInformation(){
    ...
    return openSimplePanel("System",`<h1>EmeraldOS Gold 1N</h1><p>Emerald Systems virtual operating environment</p><hr><div class="gold1n-control-grid"><div class="gold1n-performance-card"><b>Edition</b><p>EmeraldOS Gold 1N</p></div><div class="gold1n-performance-card"><b>User</b><p>${esc(userName())}</p></div><div class="gold1n-performance-card"><b>Browser platform</b><p>${esc(navigator.platform||"Web platform")}</p></div><div class="gold1n-performance-card"><b>Display</b><p>${innerWidth} × ${innerHeight} @ ${devicePixelRatio||1}x</p></div><div class="gold1n-performance-card"><b>Local VM storage</b><p>${(storage/1024/1024).toFixed(2)} MB</p></div><div class="gold1n-performance-card"><b>Cloud VM</b><p>emeraldOSUsers/${esc(userName())}/goldVM/current</p></div></div>`);
  }

  /* ------------------------- Run and Clipboard ------------------------- */
  function openRunDialog(){
    const html=`<div class="gold1n-app"><div class="gold1n-run"><div class="gold1n-run-icon">▦</div><div><p>Type the name of a program, folder, document, or EmeraldOS command, and Gold will open it for you.</p><div class="gold1n-run-row"><label for="gold1nRunInput">Open:</label><input id="gold1nRunInput" class="gold1n-field" autocomplete="off"></div></div></div><div class="gold1n-run-buttons"><button class="gold1n-button primary" id="gold1nRunOK">OK</button><button class="gold1n-button" onclick="Gold1N.closeWin('win_g1h_run')">Cancel</button><button class="gold1n-button" onclick="Gold1N.runCommand('explorer')">Browse...</button></div></div>`;
    const win=open1IWindow("run","Run",html,{width:520,height:255,icon:"▦",refresh:true});
    const input=$("gold1nRunInput");setTimeout(()=>input?.focus(),0);input?.addEventListener("keydown",e=>{if(e.key==="Enter")runCommand(input.value)});$("gold1nRunOK")?.addEventListener("click",()=>runCommand(input?.value||""));return win;
  }
  ...
  function openClipboardHistory(){
    ...
    const html=`<div class="gold1n-app"><div class="gold1n-commandbar"><b>Clipboard</b><span class="gold1n-muted">Win + V</span><span style="flex:1"></span><button id="gold1nClipboardClear">Clear all</button></div><div class="gold1n-clipboard-list">${items.map(item=>`<div class="gold1n-clipboard-item" data-clip-id="${esc(item.id)}" title="Click to copy">${esc(item.text)}</div>`).join("")||'<div class="gold1n-empty"><h2>Your clipboard is empty</h2><p>Copy text to add it to clipboard history.</p></div>'}</div></div>`;
    ...
    $("gold1nClipboardClear")?.addEventListener("click",()=>{writeJSON(CLIPBOARD_KEY,[]);openClipboardHistory()});return win;
  }
  ...
  function calendarHTML(date=new Date()){
    ...
    const labels=["Su","Mo","Tu","We","Th","Fr","Sa"].map(d=>`<span class="gold1n-muted">${d}</span>`);
    ...
    return `<div class="gold1n-calendar-head"><b>${date.toLocaleString(undefined,{month:"long",year:"numeric"})}</b><span>Today</span></div><div class="gold1n-calendar-grid">${labels.join("")}${days.join("")}</div>`;
  }
  function toggleClockFlyout(){
    const existing=$("gold1nClockFlyout");if(existing){existing.remove();return}
    const date=new Date(),fly=document.createElement("section");fly.id="gold1nClockFlyout";fly.className="gold1n-clock-flyout";
    fly.innerHTML=`<div class="gold1n-clock-time">${date.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div><div class="gold1n-clock-date">${date.toLocaleDateString([],{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</div>${calendarHTML(date)}`;
  ...
  async function fetchLatestManifest(){
    ...
    }catch(error){console.warn("Gold 1N update check used local manifest",error);return fallback}
  }
  async function openUpdateCenter(latestManifest=null){
    ...
    const html=`<div class="gold1n-app" style="padding:20px;overflow:auto"><div style="display:flex;align-items:start;justify-content:space-between;gap:12px"><div><h1>System Update</h1><p class="gold1n-muted">E.L.S.U.S. changes this VM only after you choose to update.</p></div><button id="gold1nCheckUpdates" class="gold1n-button primary">Check for updates</button></div><div class="gold1n-performance-grid"><div class="gold1n-performance-card"><h3>This VM</h3><p><b>EmeraldOS Gold 1N</b></p><p>Gold_1N / OS.html</p></div><div class="gold1n-performance-card"><h3>Latest configured release</h3><p><b>${esc(latest.releaseTitle||`EmeraldOS Gold ${latestVersion}`)}</b></p><p>${esc(latest.folder||"")} / ${esc(latest.entry||"OS.html")}</p></div><div class="gold1n-performance-card"><h3>Status</h3><p>${newer?"A newer release is available.":"This VM is on the latest configured release."}</p></div><div class="gold1n-performance-card"><h3>VM continuity</h3><p>Files, apps, settings, registry, and workspace data remain in the version-independent Gold VM.</p></div></div><h3>Release information</h3><p>${esc(latest.summary||"No release information is available.")}</p><div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:18px"><button id="gold1nSaveBeforeUpdate" class="gold1n-button">Save cloud VM</button><button id="gold1nApplyUpdate" class="gold1n-button primary" ${newer?"":"disabled"}>Update this VM now</button><button class="gold1n-button" onclick="Gold1N.openPublisherManager()">Update Publisher Manager</button></div></div>`;
    const win=open1IWindow("updateshell","System Update",html,{width:980,height:680,icon:"↻",refresh:true});
    $("gold1nCheckUpdates")?.addEventListener("click",async()=>openUpdateCenter(await fetchLatestManifest()));
    $("gold1nSaveBeforeUpdate")?.addEventListener("click",()=>saveWorkspaceNow1I(true));
    $("gold1nApplyUpdate")?.addEventListener("click",()=>applyConfiguredUpdate(latest));return win;
  }
  async function applyConfiguredUpdate(manifest){
    const latestVersion=manifest?.latestVersion||manifest?.build||"";
    if(versionRank(latestVersion)<=versionRank(VERSION))return notify("No newer update","EmeraldOS Gold 1N is already current.","System Update");
  ...
  /* ------------------------- Publisher ------------------------- */
  function releaseManifest(){return {product:"EmeraldOS Gold",latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",channel:"stable",status:"stable",required:false,enabled:true,setupMode:"manualUpdateSetup",releaseTitle:"EmeraldOS Gold 1N",activeUpdateNotifications:true,elsusMode:"original",summary:"EmeraldOS Gold 1N keeps the Gold 1K Windows 10-inspired look and features while restoring the original E.L.S.U.S. implementation used in Gold 1I.",migrationFrom:["1K","1J","1I","1H","1G","1F","1E","1D","1C","1B","1A"],migrationId:"gold-1k-to-1m-original-elsus-migration",minShellVersion:"1.0",rollbackFolder:"Gold_1K",rollbackVersion:"1K",releasedAt:"2026-07-13T00:00:00.000Z"}}
  ...
  function openPublisherManager(){
    if(!isStaff()){
      ...
      return openSimplePanel("Staff verification required",`<h2>Update Publisher Manager</h2><p>Publishing is available only from a verified Gold Staff Edition session.</p><button class="gold1n-button primary" onclick="location.href='staff.html'">Open Gold Staff Edition</button>`);
    }
    const unlocked=publisherUnlocked();
    const html=`<div class="gold1n-app" style="padding:20px;overflow:auto"><h1>Update Publisher Manager</h1><p>Publish <b>EmeraldOS Gold 1N</b> to <code>system/emeraldGoldLatest</code>.</p><div class="gold1n-performance-card"><b>Security status</b><p>${unlocked?"Publisher PIN accepted for this staff session.":"Locked. Enter the publisher PIN."}</p>${unlocked?"":'<label>Publisher PIN <input id="gold1nPublisherPin" type="password" class="gold1n-field"></label> <button id="gold1nUnlockPublisher" class="gold1n-button">Unlock</button>'}</div><div class="gold1n-performance-card"><b>Release target</b><p>Gold_1N / OS.html</p><p>Release title: EmeraldOS Gold 1N</p></div><div style="display:flex;gap:8px;margin-top:18px"><button id="gold1nPublishVersion" class="gold1n-button primary" ${unlocked?"":"disabled"}>Publish this Version</button><button id="gold1nExportManifest" class="gold1n-button">Export manifest</button></div><p class="gold1n-muted">Gold 1N never publishes from a query string or during first boot.</p></div>`;
    const win=open1IWindow("publisher","Update Publisher Manager",html,{width:720,height:540,icon:"UP",refresh:true});
    $("gold1nUnlockPublisher")?.addEventListener("click",()=>unlockPublisher($("gold1nPublisherPin")?.value||""));
    $("gold1nPublishVersion")?.addEventListener("click",()=>{publisherClickAuthorizedUntil=Date.now()+3000;publishLatest()});
    $("gold1nExportManifest")?.addEventListener("click",()=>saveText("FIREBASE_EMERALDGOLDLATEST_1N.json",JSON.stringify(releaseManifest(),null,2),"application/json"));return win;
  }
  ...
  async function publishLatest(){
    ...
    if(Date.now()>publisherClickAuthorizedUntil){
      notify("Publish button required","Open Update Publisher Manager and click Publish this Version. Gold 1N cannot publish from startup, a URL, or a console call.","System Update");
      return false;
    }
    publisherClickAuthorizedUntil=0;
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"system","emeraldGoldLatest"),releaseManifest(),{merge:true});writeJSON("emeraldGoldShell_latest",releaseManifest());localStorage.setItem(PREFIX+"latest_manifest_published_1n","true");notify("E.L.S.U.S. updated","system/emeraldGoldLatest now points to Gold_1N.","System Update");return true}catch(error){console.warn(error);notify("Publish failed",error.message,"System Update");return false}
  }

  /* ------------------------- Split Cloud VM ------------------------- */
  function localCategoryValue(category){
    if(category==="registry_1n")return registryData();
    if(category==="registry_1n_history")return readJSON(REGISTRY_HISTORY_KEY,[]);
    if(category==="clipboard_history_1n")return clipboardHistory();
    if(category==="startup_apps_1n")return readJSON(STARTUP_KEY,[]);
  ...
  function currentWindowState(){return qa(".window").map(w=>({id:w.id,title:w.dataset.title||"",left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,minimized:w.classList.contains("minimized"),maximized:w.classList.contains("max")||w.classList.contains("gold1n-maximized")})).slice(0,80)}
  async function saveWorkspaceNow1I(show=true){
    const summary={product:"EmeraldOS Gold",version:VERSION,build:VERSION,activeVersion:VERSION,activeFolder:FOLDER,schema:"gold1n-split-v2",user:userName(),savedAt:now(),categories:CLOUD_CATEGORIES,openWindows:currentWindowState(),cloudPath:"emeraldOSUsers/{username}/goldVM/current"};
    if(window.__GOLD1N_MIGRATION_BLOCK_SAVES__===true && window.__GOLD1N_MIGRATION_COMMITTING__!==true){
      writeJSON(PREFIX+"workspace_meta_1n",{...summary,saveDeferred:true,deferredReason:"migration-in-progress"});
      if(show)notify("Save deferred","Gold 1N is still merging data from previous versions. Cloud save will run automatically after migration completes.","Migration & Continuity");
      return summary;
    }
    writeJSON(PREFIX+"workspace_meta_1n",summary);
    try{
      const fbm=await import("./firebase.js");
      ...
      await fbm.setDoc(fbm.doc(fbm.db,"emeraldOSUsers",vmUserId(),"goldVM","current"),{...summary,categoryIndex:results,migrationValidated:true,migrationReportKey:"migration_1n_cloud_report"},{merge:true});
      ...
      if(show)notify("Cloud VM saved","Gold 1N saved the complete split, quota-safe VM state to the cloud.","Gold VM");return summary;
    }catch(error){
      console.warn("Gold 1N split cloud save failed",error);localStorage.setItem(PREFIX+"cloud_status","save failed: "+error.message);if(show)notify("Saved locally","Cloud sync will retry when Firebase is available.","Gold VM");return summary;
    }
  }
  function canonicalCloudCategory(category){
    return ({
      registry_1h:"registry_1n",registry_1i:"registry_1n",registry_1j:"registry_1n",registry_1k:"registry_1n",registry_1l:"registry_1n",registry_1m:"registry_1n",registry_1n:"registry_1n",
      registry_1h_history:"registry_1n_history",registry_1i_history:"registry_1n_history",registry_1j_history:"registry_1n_history",registry_1k_history:"registry_1n_history",registry_1l_history:"registry_1n_history",registry_1m_history:"registry_1n_history",registry_1n_history:"registry_1n_history",
      clipboard_history_1h:"clipboard_history_1n",clipboard_history_1i:"clipboard_history_1n",clipboard_history_1j:"clipboard_history_1n",clipboard_history_1k:"clipboard_history_1n",clipboard_history_1l:"clipboard_history_1n",clipboard_history_1m:"clipboard_history_1n",clipboard_history_1n:"clipboard_history_1n",
      startup_apps_1h:"startup_apps_1n",startup_apps_1i:"startup_apps_1n",startup_apps_1j:"startup_apps_1n",startup_apps_1k:"startup_apps_1n",startup_apps_1l:"startup_apps_1n",startup_apps_1m:"startup_apps_1n",startup_apps_1n:"startup_apps_1n",
      update_preferences_1k:"update_preferences_1n",update_preferences_1l:"update_preferences_1n",update_preferences_1m:"update_preferences_1n",update_preferences_1n:"update_preferences_1n",
      update_history_1k:"update_history_1n",update_history_1l:"update_history_1n",update_history_1m:"update_history_1n",update_history_1n:"update_history_1n",
      focus_sessions_1k:"focus_sessions_1n",focus_sessions_1l:"focus_sessions_1n",focus_sessions_1m:"focus_sessions_1n",focus_sessions_1n:"focus_sessions_1n"
    })[category]||category;
  }
  ...
  function mergeMigratedFiles(...groups){
    ...
    groups.flat().filter(Boolean).forEach(raw=>{
      ...
      if(sameId)f.id=`${f.id}_1n_${Math.random().toString(36).slice(2,8)}`;
  ...
  async function readLegacyCloudFiles(fbm){
    ...
    for(const path of paths){try{const snap=await fbm.getDoc(fbm.doc(fbm.db,...path));if(snap.exists())groups.push(collectMigratedFiles(snap.data()))}catch(error){console.debug("Gold 1N legacy cloud source skipped",path.join("/"),error?.message)}}
    return groups.flat();
  }
  async function restoreWorkspace1I(show=true){
    try{
      ...
      if(summary){
        const splitSchemas=new Set(["gold1h-split-v1","gold1h-split-v2","gold1i-split-v1","gold1i-split-v2","gold1j-split-v1","gold1j-split-v2","gold1k-split-v1","gold1k-split-v2","gold1l-split-v1","gold1l-split-v2","gold1m-split-v1","gold1m-split-v2","gold1n-split-v1","gold1n-split-v2"]);
        if(splitSchemas.has(summary.schema)){
          for(const sourceCategory of summary.categories||CLOUD_CATEGORIES){
            ...
            if(category==="files")fileGroups.push(collectMigratedFiles(value));
              ...
              const key=category==="registry_1n"?REGISTRY_KEY:category==="clipboard_history_1n"?CLIPBOARD_KEY:category==="startup_apps_1n"?STARTUP_KEY:PREFIX+category;
              writeJSON(key,value);
            }
          }
        }else{
          fileGroups.push(collectMigratedFiles(summary));
          const legacyMap={prefs:"prefs",mail:"mail",notes:"notes",tasks:"tasks",events:"events",contacts:"contacts",tickets:"tickets",notifications:"notifications",user_apps:"user_apps",remote_sessions:"remote_sessions",emergency_logs:"emergency_logs",vm_snapshots:"vm_snapshots",registry_1h:"registry_1n",registry_1i:"registry_1n",registry_1j:"registry_1n",registry_1k:"registry_1n",registry_1l:"registry_1n",registry_1m:"registry_1n",registry_1n:"registry_1n",registry_1h_history:"registry_1n_history",registry_1i_history:"registry_1n_history",registry_1j_history:"registry_1n_history",registry_1k_history:"registry_1n_history",registry_1l_history:"registry_1n_history",registry_1m_history:"registry_1n_history",registry_1n_history:"registry_1n_history",clipboard_history_1h:"clipboard_history_1n",clipboard_history_1i:"clipboard_history_1n",clipboard_history_1j:"clipboard_history_1n",clipboard_history_1k:"clipboard_history_1n",clipboard_history_1l:"clipboard_history_1n",clipboard_history_1m:"clipboard_history_1n",clipboard_history_1n:"clipboard_history_1n",startup_apps_1h:"startup_apps_1n",startup_apps_1i:"startup_apps_1n",startup_apps_1j:"startup_apps_1n",startup_apps_1k:"startup_apps_1n",startup_apps_1l:"startup_apps_1n",startup_apps_1m:"startup_apps_1n",startup_apps_1n:"startup_apps_1n",update_preferences_1k:"update_preferences_1n",update_preferences_1l:"update_preferences_1n",update_preferences_1m:"update_preferences_1n",update_history_1k:"update_history_1n",update_history_1l:"update_history_1n",update_history_1m:"update_history_1n",focus_sessions_1k:"focus_sessions_1n",focus_sessions_1l:"focus_sessions_1n",focus_sessions_1m:"focus_sessions_1n"};
          Object.entries(legacyMap).forEach(([source,category])=>{if(summary[source]===undefined)return;const key=category==="registry_1n"?REGISTRY_KEY:category==="clipboard_history_1n"?CLIPBOARD_KEY:category==="startup_apps_1n"?STARTUP_KEY:PREFIX+category;writeJSON(key,summary[source])});
        }
      }
      ...
      writeJSON(PREFIX+"migration_1n_cloud_report",{completed:true,completedAt:now(),sourceSchema:summary?.schema||"none",filesBefore:Array.isArray(localFiles)?localFiles.length:0,filesAfter:mergedFiles.length,firestoreReleasePointerChanged:false});
      try{window.Gold50?.setPrefs?.(readJSON(PREFIX+"prefs",{}));window.Gold50?.saveFiles?.(mergedFiles)}catch{}
      if(show)notify("Cloud VM migration complete",`${mergedFiles.length} files are available in Gold 1N. The E.L.S.U.S. release pointer was not changed.`,"Gold VM");
      return summary||{schema:"legacy-migration",files:mergedFiles.length};
    }catch(error){console.warn("Gold 1N cloud restore failed",error);if(show)notify("Restore failed",error.message,"Gold VM");return null}
  }
  ...
  function customToolButtons(){return `
    <button class="gold1n-start-tile" onclick="Gold1N.openRegistryEditor()"><span>▥</span><b>Registry Editor</b></button>
    <button class="gold1n-start-tile" onclick="Gold1N.openTaskManager()"><span>▤</span><b>Task Manager</b></button>
    <button class="gold1n-start-tile" onclick="Gold1N.openControlPanel()"><span>⚙</span><b>Control Panel</b></button>
    <button class="gold1n-start-tile" onclick="Gold1N.openRun()"><span>▦</span><b>Run</b></button>
    <button class="gold1n-start-tile" onclick="Gold1N.openClipboardHistory()"><span>▧</span><b>Clipboard</b></button>
    <button class="gold1n-start-tile" onclick="Gold50.openApp('converter')"><span>⇄</span><b>Converter</b></button>
    <button class="gold1n-start-tile" onclick="Gold50.openApp('storage')"><span>◫</span><b>Storage</b></button>
    ${isStaff()?'<button class="gold1n-start-tile" onclick="Gold1N.openPublisherManager()"><span>UP</span><b>Publisher</b></button>':''}` }
  function enhanceStartMenu(){
    ...
    if(!tileColumn||tileColumn.querySelector(".gold1n-start-tools-inline"))return;
    startEnhancing=true;
    const section=document.createElement("section");
    section.className="gold1n-start-tools-inline";
    section.innerHTML=`<h3>System tools</h3><div class="gold1n-start-tool-grid">${customToolButtons()}</div>`;
    tileColumn.appendChild(section);
    startEnhancing=false;
  }
  function enhanceActionCenter(){
    const panel=$("actionCenter");if(!panel||actionEnhancing||panel.querySelector(".gold1n-quick-actions"))return;
    actionEnhancing=true;
    panel.insertAdjacentHTML("afterbegin",`<div class="gold1n-quick-actions"><button class="gold1n-quick-action" data-quick="focus"><span>☾</span>Focus assist</button><button class="gold1n-quick-action" data-quick="night"><span>◐</span>Night light</button><button class="gold1n-quick-action" data-quick="settings"><span>⚙</span>All settings</button><button class="gold1n-quick-action" data-quick="save"><span>☁</span>Cloud save</button><button class="gold1n-quick-action" data-quick="safe"><span>!</span>Safe mode</button><button class="gold1n-quick-action" data-quick="registry"><span>▥</span>Registry</button><button class="gold1n-quick-action" data-quick="task"><span>▤</span>Task Manager</button><button class="gold1n-quick-action" data-quick="control"><span>▦</span>Control Panel</button></div>`);
  ...
  function quickAction(action,button){
    if(action==="focus"){document.body.classList.toggle("gold1n-focus");button.classList.toggle("active");return}
    if(action==="night"){document.body.classList.toggle("gold1n-night");button.classList.toggle("active");document.body.style.filter=document.body.classList.contains("gold1n-night")?"sepia(.22) brightness(.92)":"";return}
    ...
    if(action==="safe"&&confirm("Restart EmeraldOS Gold 1N in Safe Mode?")){localStorage.setItem(PREFIX+"safemode","true");location.reload()}
  }
  function enhanceSearch(){
    const panel=$("searchPanel");if(!panel||panel.querySelector(".gold1n-search-tools"))return;
    const box=document.createElement("div");box.className="gold1n-search-tools";box.innerHTML=`<h3>System tools</h3><div class="gold1n-start-tool-grid">${customToolButtons()}</div>`;panel.appendChild(box);
  }
  function enhanceDesktop(){
    ...
    tools.forEach(tool=>{if(desktop.querySelector(`[data-gold1n-shortcut="${tool.id}"]`))return;const button=document.createElement("button");button.className="desktop-icon gold1n-system-shortcut";button.dataset.gold1nShortcut=tool.id;button.innerHTML=`<span class="app-icon">${tool.icon}</span><span>${esc(tool.name)}</span>`;button.addEventListener("dblclick",tool.open);button.addEventListener("click",()=>{qa(".desktop-icon").forEach(x=>x.classList.remove("selected"));button.classList.add("selected")});desktop.appendChild(button)});
  }
  function showPowerMenu(){
    open1IWindow("power","Power",`<div class="gold1n-app" style="padding:20px"><h2>Power</h2><div class="gold1n-control-grid"><button class="gold1n-control-item" onclick="location.reload()"><span class="icon">↻</span><span><b>Restart</b><small>Restart EmeraldOS Gold 1N.</small></span></button><button class="gold1n-control-item" onclick="Gold1N.saveWorkspaceNow(true)"><span class="icon">☁</span><span><b>Save cloud VM</b><small>Save before leaving.</small></span></button><button class="gold1n-control-item" onclick="Gold50.logoutGold?.()"><span class="icon">⇥</span><span><b>Sign out</b><small>Close this user session.</small></span></button></div></div>`,{width:540,height:400,icon:"⏻",refresh:true});
  }
  function showDesktop(){
    const windows=qa(".window");const anyVisible=windows.some(w=>!w.classList.contains("minimized"));windows.forEach(w=>w.classList.toggle("minimized",anyVisible));qa(".gold1n-task-button").forEach(b=>b.classList.toggle("active",!anyVisible));
  }
  ...
  function showAltTab(){
    ...
    let overlay=$("gold1nAltTab");if(!overlay){overlay=document.createElement("div");overlay.id="gold1nAltTab";overlay.className="gold1n-alt-tab";document.body.appendChild(overlay)}
    overlay.innerHTML=`<div class="gold1n-alt-tab-inner">${altTabWindows.map((w,i)=>`<div class="gold1n-alt-tab-item ${i===altTabIndex?"active":""}"><span>${esc(w.dataset.icon||"▦")}</span><b>${esc(w.dataset.title||"Application")}</b><small>${w.classList.contains("minimized")?"Minimized":"Running"}</small></div>`).join("")}</div>`;
  }
  function commitAltTab(){
    $("gold1nAltTab")?.remove();const win=altTabWindows[altTabIndex];if(win){win.classList.remove("minimized");focusAnyWindow(win)}
  }
  ...
  /* ------------------------- Update setup ------------------------- */
  function updateSetupKey(){return `emeraldos_gold_1n_update_setup_done_${normalizeUser(userName())}`}
  ...
  async function finishUpdateSetup(){
    ...
    $("gold1nUpdateSetup")?.remove();
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"emeraldOSUsers",vmUserId(),"goldVM","setup"),{firstBootCompleted:true,firstBootScope:"global-elsus-user",lastCompletedVersion:VERSION,lastUpdateSetupVersion:VERSION,lastSetupType:"update",licenseAccepted:true,licenseVersion:"Gold_1N",folder:FOLDER,updatedAt:now()},{merge:true})}catch(error){console.warn("Gold 1N update setup cloud save failed",error)}
    notify("EmeraldOS Gold 1N is ready","Your existing cloud VM data and preferences were preserved.","Setup");
  }
  function maybeShowUpdateSetup(){
    ...
    const overlay=document.createElement("section");overlay.id="gold1nUpdateSetup";overlay.className="gold1n-update-setup";overlay.innerHTML=`<div class="gold1n-update-card"><h1>Welcome to EmeraldOS Gold 1N</h1><p>This update keeps your existing E.L.S.U.S. cloud VM and introduces a more familiar, user-friendly desktop.</p><ul><li>Protected migration before cloud autosave can begin</li><li>Gold 1M and earlier split-schema migration</li><li>Previous Gold file-version compatibility</li><li>Welcome Center and Feedback Hub</li><li>Shell V2 compatibility testing with the version login retained</li><li>Settings, toolbar, card, table, and narrow-window collision fixes</li></ul><p>Gold 1N merges files, folders, settings, preferences, registry data, apps, mail, and other VM categories before it is allowed to save over the shared cloud VM.</p><div class="actions"><button class="gold1n-button" onclick="document.getElementById('gold1nUpdateSetup').remove()">Remind me later</button><button class="gold1n-button primary" onclick="Gold1N.finishUpdateSetup()">Continue</button></div></div>`;document.body.appendChild(overlay);
  }
  ...
  function patchGoldAPI(){
    ...
    if(!api.__gold1nOriginalOpenApp)api.__gold1nOriginalOpenApp=api.openApp?.bind(api);
  ...
  function installObserver(){
    const observer=new MutationObserver(()=>{clearTimeout(window.__gold1nRefreshTimer);window.__gold1nRefreshTimer=setTimeout(refreshEnhancements,40)});observer.observe(document.body,{childList:true,subtree:true});
  }
  function branding(){
    document.title="EmeraldOS Gold 1N";document.body.classList.add("gold1n","gold1n-shell");document.body.dataset.build=VERSION;
    qa("h1,h2,p,span").forEach(el=>{if(el.children.length===0&&el.textContent?.includes("EmeraldOS Gold 1G"))el.textContent=el.textContent.replaceAll("EmeraldOS Gold 1G","EmeraldOS Gold 1N")});
  }
  ...
  function init(){
    ...
    setInterval(()=>{if(window.__GOLD1N_MIGRATION_BLOCK_SAVES__!==true)saveWorkspaceNow1I(false)},60000);
    window.addEventListener("beforeunload",()=>{writeJSON(PREFIX+"workspace_meta_1n",{version:VERSION,folder:FOLDER,savedAt:now(),openWindows:currentWindowState()})});
