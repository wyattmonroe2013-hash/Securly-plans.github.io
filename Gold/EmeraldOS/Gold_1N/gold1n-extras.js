...
========================================================= */
(function EmeraldOSGold1NExtras(){
  if(window.__EMERALDOS_GOLD_1N_EXTRAS__) return;
  window.__EMERALDOS_GOLD_1N_EXTRAS__=true;

  const VERSION="1N";
  const FOLDER="Gold_1N";
  const UPDATE_SERVICE="Original E.L.S.U.S.";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1n";
  const HISTORY_KEY=PREFIX+"update_history_1n";
  const SNOOZE_KEY=PREFIX+"update_snooze_1n";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1n";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1n";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1n";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1n";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1N update check used local manifest cache",error?.message||error);
    }
  ...
  function updateButton(){
    let button=$("gold1nUpdateStatusBtn");
    if(button)return button;
    const taskbar=$("taskbar"),anchor=$("quickSettingsBtn")||$("shellStatusBtn");
    if(!taskbar||!anchor)return null;
    button=document.createElement("button");
    button.id="gold1nUpdateStatusBtn";
    button.className="task-btn gold1n-update-status hidden";
    button.title="System Update";
    button.setAttribute("aria-label","System Update");
    button.innerHTML='<span aria-hidden="true">↻</span><span class="gold1n-update-dot" aria-hidden="true"></span>';
  ...
  function removeUpdatePrompt(){
    $("gold1nActiveUpdatePrompt")?.remove();
    document.body.classList.remove("gold1n-update-prompt-open");
  }
  function showUpdatePrompt(manifest,force=false){
    ...
    const firstSetup=$("setupWizard"),updateSetup=$("gold1nUpdateSetup");
    ...
    prompt.id="gold1nActiveUpdatePrompt";
    prompt.className=`gold1n-active-update-prompt${manifest.required?" required":""}`;
    prompt.setAttribute("role","dialog");
    prompt.setAttribute("aria-labelledby","gold1nUpdatePromptTitle");
    const version=manifestVersion(manifest),title=manifest.releaseTitle||`EmeraldOS Gold ${version}`;
    prompt.innerHTML=`
      <div class="gold1n-update-prompt-head">
        <span class="gold1n-update-prompt-icon" aria-hidden="true">↻</span>
        <div><b id="gold1nUpdatePromptTitle">${esc(title)} is ready</b><small>E.L.S.U.S. update</small></div>
        <button id="gold1nUpdatePromptClose" aria-label="Close update notification">×</button>
      </div>
      ...
      <div class="gold1n-update-prompt-meta"><span>Current: ${VERSION}</span><span>Available: ${esc(version)}</span><span>${manifest.required?"Required update":"Optional update"}</span></div>
      <div class="gold1n-update-prompt-actions">
        <button id="gold1nUpdateNow" class="button primary">Update now</button>
        <button id="gold1nUpdateDetails" class="button">View details</button>
        <button id="gold1nUpdateLater" class="button">Later</button>
      </div>`;
    document.body.appendChild(prompt);
    document.body.classList.add("gold1n-update-prompt-open");
    localStorage.setItem(LAST_PROMPT_KEY,now());
    addHistory("prompt",manifest,"The active update prompt was displayed.");
    $("gold1nUpdateNow").onclick=()=>beginUserUpdate(manifest);
    $("gold1nUpdateDetails").onclick=()=>{removeUpdatePrompt();openUpdateStatus(manifest)};
    $("gold1nUpdateLater").onclick=()=>{snooze();removeUpdatePrompt();addHistory("snoozed",manifest,`Snoozed for ${updatePrefs().snoozeHours} hours.`);notify("Update reminder snoozed",`Gold will remind you about ${title} later.`,"System Update")};
    $("gold1nUpdatePromptClose").onclick=()=>{if(!manifest.required)snooze(1);removeUpdatePrompt()};
    return true;
  }
  ...
  async function beginUserUpdate(manifest){
    ...
    try{
      if(window.Gold1N?.requestVersionChange){await window.Gold1N.requestVersionChange(manifest);return true}
      if(window.Gold1N?.saveWorkspaceNow)await window.Gold1N.saveWorkspaceNow(false);
      window.Gold1N?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1N live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1N live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1N?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1N?.openUpdateCenter?.(currentManifest);
  ...
  function quickSettingsPanel(){
    let panel=$("gold1nQuickSettingsPanel");
  ...
  function openFocusSessions(){
    ...
    const html=`<div class="app-shell"><div class="app-toolbar"><b>Focus Sessions</b><span style="flex:1"></span><button id="focusSettings" class="button">Settings</button></div><div class="app-body gold1n-focus-app"><div class="gold1n-focus-clock" data-focus-time>${formatFocusTime(focusRemaining)}</div><h2 data-focus-state>${focusRunning?"Focusing":"Ready"}</h2><p>Use a distraction-free timer with Focus Assist and Gold notifications.</p><div class="gold1n-focus-controls"><button id="focusStart" class="button primary">${focusRunning?"Pause":"Start"}</button><button id="focusReset" class="button">Reset</button></div><div class="gold1n-focus-presets"><button data-focus-minutes="15">15 minutes</button><button data-focus-minutes="25">25 minutes</button><button data-focus-minutes="45">45 minutes</button><button data-focus-minutes="60">60 minutes</button></div><label class="gold1n-focus-option"><input id="focusAssistDuringSession" type="checkbox" checked> Turn on Focus Assist while the timer is running</label></div></div>`;
  ...
  function installEvents(){
    ...
    document.addEventListener("click",event=>{if(!event.target.closest("#gold1nQuickSettingsPanel")&&!event.target.closest("#quickSettingsBtn"))$("gold1nQuickSettingsPanel")?.remove()});
  ...
  function patchPublicAPI(){
    window.Gold1N=window.Gold1N||api()||{};
    Object.assign(window.Gold1N,{elsusMode:"original",checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1N","Active update notifications, Focus Sessions, and Quick Settings are ready.","System");
  }
