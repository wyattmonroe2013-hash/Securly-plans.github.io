...
/* EmeraldOS Gold 1N local continuity migration.
   ...
   asynchronous cloud migration in gold1n-continuity.js has completed. */
(function EmeraldOSGold1NLocalMigration(){
  if(window.__EMERALDOS_GOLD_1N_LOCAL_MIGRATION__) return;
  window.__EMERALDOS_GOLD_1N_LOCAL_MIGRATION__=true;
  window.__GOLD1N_MIGRATION_BLOCK_SAVES__=true;

  const PREFIX="gold1g_";
  const REPORT_KEY=PREFIX+"migration_1n_local_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1N local migration write failed",key,error);return false}};
  ...
  function normalizeFile(raw,folderHint="Documents"){
    ...
    return {...raw,id:String(raw.id||raw.fileId||uid()),name,type,folder,content:String(content??""),created:raw.created||raw.createdAt||now(),updated:raw.updated||raw.updatedAt||now(),trash:Boolean(raw.trash||raw.deleted),migratedTo:"1N"};
  }
  ...
  function mergeFiles(...groups){
    ...
    groups.flat(Infinity).filter(Boolean).forEach(raw=>{
      ...
      if(sameId)file.id=`${file.id}_1n_${Math.random().toString(36).slice(2,8)}`;
  ...
  const fileKeys=[
    "gold1g_files","gold1h_files","gold1i_files","gold1j_files","gold1k_files","gold1l_files","gold1m_files","gold1n_files",
    "gold1g_workspace","gold1h_workspace","gold1i_workspace","gold1j_workspace","gold1k_workspace","gold1l_workspace","gold1m_workspace","gold1n_workspace",
  ...
  const categories=["prefs","folders","mail","notes","tasks","events","contacts","tickets","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings"];
  const prefixes=["gold1a_","gold1b_","gold1c_","gold1d_","gold1e_","gold1f_","gold1h_","gold1i_","gold1j_","gold1k_","gold1l_","gold1m_","gold1n_","gold1g_"];
  ...
  const sourceVersions=["1h","1i","1j","1k","1l","1m"];
  versionedFamilies.forEach(family=>{
    ...
    if(merged!==undefined)write(`${PREFIX}${family}_1n`,merged);
  });
  ...
  window.Gold1NLocalMigration={report,collectFiles,mergeFiles,deepMerge,mergeArray,normalizeFile};
})();
