...
/* EmeraldOS Gold 1N cloud continuity and migration gate.
   ...
   Gold 1N to save over the shared VM. This module never writes the shared
   system/emeraldGoldLatest release pointer. */
(function EmeraldOSGold1NContinuity(){
  if(window.__EMERALDOS_GOLD_1N_CONTINUITY__)return;
  window.__EMERALDOS_GOLD_1N_CONTINUITY__=true;

  const VERSION="1N",PREFIX="gold1g_",REPORT_KEY=PREFIX+"migration_1n_cloud_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1N continuity local write failed",key,error);return false}};
  const helper=()=>window.Gold1NLocalMigration||{};
  ...
  const knownVersions=["1H","1I","1J","1K","1L","1M","1N"];
  ...
  const baseCategories=["prefs","files","folders","tickets","mail","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings"];
  const targetCategories=[...baseCategories,"registry_1n","registry_1n_history","clipboard_history_1n","startup_apps_1n","update_preferences_1n","update_history_1n","focus_sessions_1n"];
  ...
  function localKeyFor(category){
    if(category==="registry_1n")return PREFIX+"registry_1n";
    if(category==="registry_1n_history")return PREFIX+"registry_1n_history";
    if(category==="clipboard_history_1n")return PREFIX+"clipboard_history_1n";
    if(category==="startup_apps_1n")return PREFIX+"startup_apps_1n";
    if(category==="update_preferences_1n")return PREFIX+"update_preferences_1n";
    if(category==="update_history_1n")return PREFIX+"update_history_1n";
    if(category==="focus_sessions_1n")return PREFIX+"focus_sessions_1n";
  ...
  function mergeCategory(category,sources){
    ...
    const arrayCategory=["tickets","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","sticky_notes","voice_recordings","registry_1n_history","clipboard_history_1n","startup_apps_1n","update_history_1n"].includes(category);
  ...
  async function collectOrphanedChunks(fb,vmUser=currentUser()){
    ...
    }catch(error){console.debug("Gold 1N orphaned chunk recovery unavailable",error?.message)}
  }
  ...
  async function archiveSourceCategory(fb,version,category,value,vmUser=currentUser()){
    ...
    const id=`${safeVersion}__${category}`,json=JSON.stringify(value??null),base={version:safeVersion,category,updatedAt:now(),source:"Gold 1N pre-migration archive"};
  ...
  async function collectCloudSources(fb){
    ...
    for(const vmUser of userCandidates()){
      ...
      }catch(error){console.debug("Gold 1N VM summary source skipped",vmUser,error?.message)}
      ...
      if(summary){
        if(knownSchemas.has(summary.schema)){
          for(const category of summary.categories||[]){
            ...
            }catch(error){console.warn("Gold 1N category migration skipped",category,error)}
          }
        }else{
      ...
      }catch(error){console.debug("Gold 1N category scan unavailable",vmUser,error?.message)}
      ...
      }catch(error){console.debug("Gold 1N version backup scan unavailable",vmUser,error?.message)}
  ...
  function refreshDesktop(){
    try{window.Gold50?.setPrefs?.(read(PREFIX+"prefs",{}));window.Gold50?.saveFiles?.(read(PREFIX+"files",[]));window.Gold50?.renderDesktop?.();window.Gold50?.renderStartMenu?.();window.Gold50?.renderNotificationBadge?.()}catch(error){console.debug("Gold 1N UI refresh after migration skipped",error?.message)}
  }

  function openMigrationCenter(){
    const report=read(REPORT_KEY,{stage:"not run"}),local=read(PREFIX+"migration_1n_local_report",{}),files=read(PREFIX+"files",[]);
  ...
  async function runMigration(show=false){
    if(window.__GOLD1N_MIGRATION_RUNNING__)return window.__GOLD1N_MIGRATION_PROMISE__;
    window.__GOLD1N_MIGRATION_RUNNING__=true;
    sourceMap.clear();collectLocalSources();
    const promise=(async()=>{
      let cloudInfo={summary:null,sourceVersion:"none"},cloudError=null;
      try{const fb=await withTimeout(import("./firebase.js"),20000,"Firebase migration connection timed out");if(fb.db)cloudInfo=await withTimeout(collectCloudSources(fb),30000,"Cloud migration scan timed out");else cloudError="Firebase is not configured"}catch(error){cloudError=error.message;console.warn("Gold 1N cloud migration continued locally",error)}
      ...
      if(cloudTemporarilyUnverified){
        ...
        window.__GOLD1N_MIGRATION_BLOCK_SAVES__=true;window.__GOLD1N_MIGRATION_COMPLETE__=false;
        localStorage.setItem(PREFIX+"cloud_status","migration waiting for cloud verification");
        if(show)window.Gold1N?.notify?.("Cloud migration not verified","Gold 1N kept cloud autosave blocked so older files cannot be overwritten. Reconnect and run the migration scan again.","Migration & Continuity");
        return report;
      }
      report.autosaveUnblockedAt=now();report.autosaveStillBlocked=false;
      write(REPORT_KEY,report);refreshDesktop();
      window.__GOLD1N_MIGRATION_BLOCK_SAVES__=false;window.__GOLD1N_MIGRATION_COMPLETE__=true;
      try{window.__GOLD1N_MIGRATION_COMMITTING__=true;await window.Gold1N?.saveWorkspaceNow?.(false)}catch(error){console.warn("Gold 1N post-migration save deferred",error)}finally{window.__GOLD1N_MIGRATION_COMMITTING__=false}
      const noticeKey=PREFIX+"migration_1n_notice_shown";
      if(show||localStorage.getItem(noticeKey)!=="true"){
        window.Gold1N?.notify?.("Migration complete",`${report.filesAfter||0} files and all available VM categories were merged before cloud autosave was enabled.`,"Migration & Continuity");
        localStorage.setItem(noticeKey,"true");
      }
      return report;
    })().finally(()=>{window.__GOLD1N_MIGRATION_RUNNING__=false});
    window.__GOLD1N_MIGRATION_PROMISE__=promise;return promise;
  }

  function install(){
    registerMigrationApp();
    const api=window.Gold1N||{};api.runMigration=runMigration;api.openMigrationCenter=openMigrationCenter;api.migrationReport=()=>read(REPORT_KEY,null);window.Gold1N=api;
    if(window.Gold50){window.Gold50.openMigrationCenter=openMigrationCenter;window.Gold50.runMigration=runMigration}
    runMigration(false);
    window.addEventListener("online",()=>{if(window.__GOLD1N_MIGRATION_BLOCK_SAVES__===true)setTimeout(()=>runMigration(false),1000)});
    setInterval(()=>{if(window.__GOLD1N_MIGRATION_BLOCK_SAVES__===true&&navigator.onLine)runMigration(false)},60000);
  }
