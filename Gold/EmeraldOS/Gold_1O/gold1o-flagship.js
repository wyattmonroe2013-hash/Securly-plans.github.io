"use strict";
/* EmeraldOS Gold 1O flagship experience.
...
(function EmeraldOSGold1OFlagship(){
  if(window.__EMERALDOS_GOLD_1O_FLAGSHIP__) return;
  window.__EMERALDOS_GOLD_1O_FLAGSHIP__=true;

  const VERSION="1O",FOLDER="Gold_1O",PREFIX="gold1g_";
  const $=id=>document.getElementById(id);
  const api=()=>window.Gold50||window.Gold1O;
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1O write failed",key,error);return false}};
  ...
  function notification(title,body,app="Gold 1O"){api()?.notify?.(title,body,app)}
  ...
  function healthReport(){
    ...
    const migration=read(PREFIX+"migration_1o_cloud_report",read(PREFIX+"migration_1o_local_report",{}));
    const checks=[
      ...
      {name:"Migration protection",pass:window.__GOLD1O_MIGRATION_BLOCK_SAVES__!==true,detail:window.__GOLD1O_MIGRATION_BLOCK_SAVES__===true?"Cloud saves remain blocked until migration is verified.":"Migration gate is open."},
      ...
      {name:"Previous-format compatibility",pass:Boolean(window.Gold1OFileCompatibility),detail:"Gold 1O compatibility registry is active."},
  ...
  function openHome(){
    ...
    const html=`<div class="app-shell gold1o-home"><div class="app-toolbar"><button class="button primary" data-home-action="command">Command Palette</button><button class="button" data-home-action="newdoc">New document</button><button class="button" data-home-action="backup">Back up now</button><button class="button" data-home-action="settings">Settings</button></div><div class="app-body"><section class="gold1o-hero"><div><p class="eyebrow">EMERALDOS GOLD 1O</p><h1>Welcome back, ${esc(user())}</h1><p>Your files, applications, settings, and earlier Gold formats stay together in one E.L.S.U.S. cloud VM.</p></div><div class="gold1o-health-ring" aria-label="${report.passed} of ${report.total} system checks passed"><b>${report.passed}/${report.total}</b><span>checks passed</span></div></section><div class="grid4 gold1o-stat-grid"><button class="card" data-open-app="explorer"><b>Files</b><h2>${files().filter(f=>!f.trash).length}</h2><span>available</span></button><button class="card" data-open-app="workspace"><b>Tasks</b><h2>${tasks().filter(t=>!t.done).length}</h2><span>open</span></button><button class="card" data-open-app="systemhealth"><b>VM storage</b><h2>${fmtBytes(localBytes())}</h2><span>local cache</span></button><button class="card" data-open-app="updateshell"><b>Version</b><h2>Gold ${VERSION}</h2><span>E.L.S.U.S. ready</span></button></div><div class="gold1o-dashboard-grid"><section class="card"><div class="section-heading"><h2>Recent files</h2><button class="link-button" data-open-app="explorer">View all</button></div><div class="gold1o-list">${recent.map(file=>`<button data-home-file="${esc(file.id)}"><span class="gold1o-file-badge">${esc(String(file.type||"file").slice(0,3).toUpperCase())}</span><span><b>${esc(file.name)}</b><small>${esc(file.folder||"Documents")} · ${new Date(file.updated||file.created||Date.now()).toLocaleString()}</small></span></button>`).join("")||'<p class="muted">No recent files yet.</p>'}</div></section><section class="card"><div class="section-heading"><h2>My day</h2><button class="link-button" data-open-app="workspace">Open workspace</button></div><div class="gold1o-list">${openTasks.map(task=>`<button data-home-task="${esc(task.id||"")}"><span class="gold1o-check"></span><span><b>${esc(task.title||task.text||"Task")}</b><small>${task.due?`Due ${new Date(task.due).toLocaleDateString()}`:"No due date"}</small></span></button>`).join("")||'<p class="muted">You are caught up.</p>'}</div></section></div><section class="card gold1o-quick-launch"><div class="section-heading"><h2>Quick launch</h2><span class="muted">Ctrl + Space opens Command Palette</span></div><div class="gold1o-quick-grid">${["office","mail","calendar","stickynotes","focussessions","backupcenter","compatibility","accessibility"].map(id=>{const app=api()?.APPS?.find(a=>a.id===id);return app?`<button data-open-app="${id}"><span>${esc(app.label||app.name.slice(0,2))}</span><b>${esc(app.name)}</b></button>`:""}).join("")}</div></section></div></div>`;
  ...
  function closeCommandPalette(){$("gold1oCommandPalette")?.remove()}
  function openCommandPalette(initial=""){
    closeCommandPalette();
    const overlay=document.createElement("div");overlay.id="gold1oCommandPalette";overlay.className="gold1o-command-overlay";overlay.innerHTML=`<section class="gold1o-command-dialog" role="dialog" aria-modal="true" aria-label="Command Palette"><div class="gold1o-command-input"><span>⌕</span><input id="gold1oCommandInput" value="${esc(initial)}" placeholder="Search apps, files, settings, and actions" autocomplete="off" spellcheck="false"><kbd>Esc</kbd></div><div id="gold1oCommandResults" class="gold1o-command-results" aria-live="polite"></div><footer><span>↑ ↓ select</span><span>Enter open</span><span>Ctrl + Space toggle</span></footer></section>`;document.body.appendChild(overlay);
    const input=$("gold1oCommandInput"),holder=$("gold1oCommandResults");let selected=0,current=[];
  ...
  function clipboardItems(){
    const source=read(PREFIX+"clipboard_history_1o",read(PREFIX+"clipboard_history_1n",[]));
  ...
  function mergeImportedFiles(incoming=[]){
    ...
    incoming.forEach(raw=>{if(!raw||typeof raw!=="object")return;const file={...raw,id:String(raw.id||uid("import")),name:String(raw.name||"Imported File"),folder:String(raw.folder||"Documents"),type:String(raw.type||"text"),content:String(raw.content??""),updated:raw.updated||now(),created:raw.created||now(),trash:Boolean(raw.trash)};const signature=`${file.name.toLowerCase()}|${file.folder.toLowerCase()}|${file.type}|${file.content}`;if(signatures.has(signature))return;if(ids.has(file.id))file.id=`${file.id}_1o_${Math.random().toString(36).slice(2,7)}`;ids.add(file.id);signatures.add(signature);merged.push(file)});return merged;
  }
  ...
  function openBackupCenter(){
    ...
    const html=`<div class="app-shell gold1o-backup"><div class="app-toolbar"><button class="button primary" id="backupCreate">Create restore point</button><button class="button" id="backupDownload">Download full backup</button><button class="button" id="backupImport">Import and merge backup</button><button class="button" id="backupCloud">Save cloud VM now</button></div><div class="app-body"><div class="gold1o-page-title"><div><h1>Backup &amp; Sync</h1><p>Create recovery points and merge backups without deleting newer or conflicting files.</p></div><span class="gold1o-status-pill good">Migration-safe</span></div><div class="grid3"><div class="card"><b>Files protected</b><h2>${files().length}</h2></div><div class="card"><b>Restore points</b><h2>${list.length}</h2></div><div class="card"><b>Local VM cache</b><h2>${fmtBytes(localBytes())}</h2></div></div><h2>Restore points</h2><div class="gold1o-snapshot-list">${list.map(item=>`<article class="card"><div><b>${esc(item.name||"Restore point")}</b><small>${new Date(item.createdAt).toLocaleString()} · ${item.fileCount||0} files</small></div><div class="button-row"><button class="button" data-snapshot-download="${esc(item.id)}">Download</button><button class="button primary" data-snapshot-merge="${esc(item.id)}">Merge restore point</button><button class="button danger" data-snapshot-delete="${esc(item.id)}">Delete</button></div></article>`).join("")||'<p class="muted">No restore points have been created.</p>'}</div><section class="card gold1o-safety-note"><h3>Safe restore policy</h3><p>Gold 1O merges files and settings. It does not delete current files simply because they are absent from an older backup.</p></section></div></div>`;
  ...
  function openCompatibilityCenter(){
    ...
    const html=`<div class="app-shell gold1o-compat"><div class="app-toolbar"><button class="button primary" id="compatScan">Scan again</button><button class="button" id="compatRepair">Repair safe metadata</button><button class="button" id="compatExport">Export report</button></div><div class="app-body"><div class="gold1o-page-title"><div><h1>File Compatibility Center</h1><p>Checks files from previous EmeraldOS Gold versions without changing their contents.</p></div><span class="gold1o-status-pill ${scan.review?"review":"good"}">${scan.compatible}/${scan.total} compatible</span></div><div class="gold1o-table-wrap"><table><thead><tr><th>File</th><th>Folder</th><th>Type</th><th>Compatibility</th><th></th></tr></thead><tbody>${scan.results.map(row=>`<tr><td><b>${esc(row.file.name||"Unnamed file")}</b><small>${esc(row.ext||"no extension")}</small></td><td>${esc(row.file.folder||"Documents")}</td><td>${esc(row.file.type||"unknown")}</td><td><span class="gold1o-status-pill ${row.issues.length?"review":"good"}">${esc(row.status)}</span>${row.issues.length?`<small>${esc(row.issues.join("; "))}</small>`:""}</td><td><button class="button" data-compat-open="${esc(row.file.id)}">Open</button></td></tr>`).join("")||'<tr><td colspan="5">No files were found.</td></tr>'}</tbody></table></div><section class="card"><h3>Backward-compatibility policy</h3><p>Gold 1O preserves earlier file identifiers, content, folders, metadata, and conflicting copies. Safe repair only adds missing metadata; it does not rewrite file content.</p></section></div></div>`;
    ...
    win.querySelector("#compatExport").onclick=()=>download("EmeraldOS-Gold-1O-File-Compatibility.json",compatibilityScan());
  ...
  window.Gold1OFileCompatibility={scan:compatibilityScan,extensionTypes};

  function openSystemHealth(){
    const report=healthReport();
    const html=`<div class="app-shell gold1o-health"><div class="app-toolbar"><button class="button primary" id="healthRun">Run checks again</button><button class="button" id="healthSave">Save VM now</button><button class="button" id="healthExport">Export diagnostics</button><button class="button danger" id="healthCleanup">Clear safe caches</button></div><div class="app-body"><div class="gold1o-page-title"><div><h1>System Health</h1><p>Clear status and repair guidance for your Gold virtual machine.</p></div><span class="gold1o-status-pill ${report.passed===report.total?"good":"review"}">${report.passed}/${report.total} passed</span></div><div class="grid4"><div class="card"><b>Files</b><h2>${report.files}</h2></div><div class="card"><b>Applications</b><h2>${report.apps}</h2></div><div class="card"><b>Local storage</b><h2>${fmtBytes(report.storageBytes)}</h2></div><div class="card"><b>Migration</b><h2>${window.__GOLD1O_MIGRATION_BLOCK_SAVES__===true?"Protected":"Ready"}</h2></div></div><div class="gold1o-health-list">${report.checks.map(check=>`<article class="card ${check.pass?"pass":"review"}"><span class="gold1o-health-icon">${check.pass?"✓":"!"}</span><div><b>${esc(check.name)}</b><p>${esc(check.detail)}</p></div></article>`).join("")}</div></div></div>`;
    ...
    win.querySelector("#healthRun").onclick=refresh;win.querySelector("#healthSave").onclick=()=>api()?.saveWorkspaceNow?.(true);win.querySelector("#healthExport").onclick=()=>download("EmeraldOS-Gold-1O-Diagnostics.json",healthReport());
  ...
  function accessibility(){return {...{highContrast:false,reduceMotion:false,largeText:false,underlineLinks:false,focusRings:true,largeCursor:false,compact:false},...read(PREFIX+"accessibility_1o",{})}}
  function applyAccessibility(settings=accessibility()){
    document.body.classList.toggle("gold1o-high-contrast",settings.highContrast);document.body.classList.toggle("gold1o-reduce-motion",settings.reduceMotion);document.body.classList.toggle("gold1o-large-text",settings.largeText);document.body.classList.toggle("gold1o-underline-links",settings.underlineLinks);document.body.classList.toggle("gold1o-strong-focus",settings.focusRings);document.body.classList.toggle("gold1o-large-cursor",settings.largeCursor);document.body.classList.toggle("gold1o-compact",settings.compact);
  }
  function saveAccessibility(settings){write(PREFIX+"accessibility_1o",settings);applyAccessibility(settings);api()?.setPrefs?.({accessibility1O:settings});api()?.saveWorkspaceNow?.(false)}
  function openAccessibility(){
    ...
    const html=`<div class="app-shell gold1o-accessibility"><div class="app-toolbar"><button class="button" id="accessReset">Restore defaults</button><button class="button" id="accessSettings">Open Windows-style Settings</button></div><div class="app-body"><div class="gold1o-page-title"><div><h1>Accessibility Center</h1><p>Adjust visibility, motion, pointer, and keyboard behavior with live previews.</p></div><span class="gold1o-status-pill good">Applied instantly</span></div><div class="gold1o-setting-list">${options.map(([id,title,detail])=>`<label class="card gold1o-setting-row"><span><b>${esc(title)}</b><small>${esc(detail)}</small></span><input type="checkbox" data-accessibility="${id}" ${settings[id]?"checked":""}></label>`).join("")}</div><section class="card gold1o-access-preview"><h2>Preview</h2><p>This sample shows readable text, <a href="#" onclick="return false">an interactive link</a>, and keyboard focus.</p><button class="button primary">Sample action</button> <input class="field" value="Sample text field"></section></div></div>`;
  ...
  function openWorkspace(){
    ...
    const html=`<div class="app-shell gold1o-workspace"><div class="app-toolbar"><button class="button primary" id="workspaceAddTask">Add task</button><button class="button" data-workspace-open="calendar">Calendar</button><button class="button" data-workspace-open="focussessions">Focus session</button><button class="button" data-workspace-open="office">New document</button></div><div class="app-body"><div class="gold1o-page-title"><div><h1>My Workspace</h1><p>Tasks, upcoming events, and recent work in one focused view.</p></div><span class="gold1o-status-pill good">${list.filter(t=>!t.done).length} open tasks</span></div><div class="gold1o-dashboard-grid"><section class="card"><h2>Tasks</h2><div class="gold1o-task-list">${list.map(task=>`<label class="gold1o-task ${task.done?"done":""}"><input type="checkbox" data-task-toggle="${esc(task.id)}" ${task.done?"checked":""}><span><b>${esc(task.title||task.text||"Task")}</b><small>${task.due?new Date(task.due).toLocaleDateString():"No due date"}</small></span><button data-task-delete="${esc(task.id)}" aria-label="Delete task">×</button></label>`).join("")||'<p class="muted">No tasks yet.</p>'}</div></section><section class="card"><h2>Upcoming</h2><div class="gold1o-list">${(Array.isArray(events)?events:[]).sort((a,b)=>Date.parse(a.date||a.start||0)-Date.parse(b.date||b.start||0)).slice(0,6).map(event=>`<button data-workspace-open="calendar"><span class="gold1o-date-badge">${new Date(event.date||event.start||Date.now()).getDate()}</span><span><b>${esc(event.title||"Event")}</b><small>${new Date(event.date||event.start||Date.now()).toLocaleString()}</small></span></button>`).join("")||'<p class="muted">No upcoming events.</p>'}</div></section></div><section class="card"><h2>Recent work</h2><div class="gold1o-recent-strip">${recent.map(file=>`<button data-workspace-file="${esc(file.id)}"><span>${esc(String(file.type||"F").slice(0,2).toUpperCase())}</span><b>${esc(file.name)}</b><small>${esc(file.folder)}</small></button>`).join("")||'<p class="muted">No recent files.</p>'}</div></section></div></div>`;
  ...
  function routines(){const value=read(PREFIX+"routines_1o",[]);return Array.isArray(value)?value:[]}
  function saveRoutines(value){write(PREFIX+"routines_1o",value);api()?.saveWorkspaceNow?.(false)}
  ...
  function patchOpenApp(){
    const core=api();if(!core||core.__gold1oFlagshipPatch)return;core.__gold1oFlagshipPatch=true;const original=core.openApp.bind(core),handlers={home:openHome,commandpalette:openCommandPalette,workspace:openWorkspace,clipboardmanager:openClipboardManager,backupcenter:openBackupCenter,compatibility:openCompatibilityCenter,systemhealth:openSystemHealth,accessibility:openAccessibility,routines:openRoutines};core.openApp=(id,options={})=>handlers[id]?handlers[id](options):original(id,options);window.Gold50=core;window.Gold1O=window.Gold1O||core;Object.assign(window.Gold1O,{openHome,openCommandPalette,openWorkspace,openClipboardManager,openBackupCenter,openCompatibilityCenter,openSystemHealth,openAccessibility,openRoutines,healthReport,fullVMExport});
  }
  function installShortcuts(){document.addEventListener("keydown",event=>{if((event.ctrlKey||event.metaKey)&&event.code==="Space"){event.preventDefault();$("gold1oCommandPalette")?closeCommandPalette():openCommandPalette()}if(event.key==="Escape"&&$("gold1oCommandPalette"))closeCommandPalette()});}
  function installShellPolish(){
    document.body.dataset.flagship="1O";const update=()=>{document.body.classList.toggle("gold1o-narrow",innerWidth<820);document.body.classList.toggle("gold1o-medium",innerWidth<1120)};update();window.addEventListener("resize",update,{passive:true});
    if(!$("gold1oCommandTaskBtn")){const button=document.createElement("button");button.id="gold1oCommandTaskBtn";button.className="task-btn gold1o-command-task";button.title="Command Palette (Ctrl + Space)";button.setAttribute("aria-label","Open Command Palette");button.textContent="⌕";button.onclick=()=>openCommandPalette();$("taskViewBtn")?.after(button)}
  }
  function runLoginRoutines(){if(sessionStorage.getItem("gold1o_login_routines_ran")==="true")return;sessionStorage.setItem("gold1o_login_routines_ran","true");setTimeout(()=>routines().filter(item=>item.trigger==="login"&&item.enabled!==false).forEach(runRoutine),2600)}
  function ensureDefaultHomePreference(){if(localStorage.getItem(PREFIX+"home_1o_at_login")===null)write(PREFIX+"home_1o_at_login",true)}
  function maybeOpenHome(){if(read(PREFIX+"home_1o_at_login",true)!==true)return;if(sessionStorage.getItem("gold1o_home_opened")==="true")return;sessionStorage.setItem("gold1o_home_opened","true");setTimeout(()=>api()?.openApp?.("home"),2100)}
  function init(){
    ...
    notification("EmeraldOS Gold 1O ready","Gold Home, Command Palette, compatibility, backup, accessibility, and system health tools are available.","System");
  }
