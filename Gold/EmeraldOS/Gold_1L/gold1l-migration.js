...
/* EmeraldOS Gold 1L local migration bridge.
...
(function EmeraldOSGold1LLocalMigration(){
  if(window.__EMERALDOS_GOLD_1L_LOCAL_MIGRATION__) return;
  window.__EMERALDOS_GOLD_1L_LOCAL_MIGRATION__=true;

  const CANON_PREFIX="gold1g_";
  const REPORT_KEY=CANON_PREFIX+"migration_1l_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1L migration write failed",key,error);return false}};
  ...
  function signature(file){
    return [String(file.name||"").toLowerCase(),String(file.folder||"").toLowerCase(),String(file.type||""),String(file.content||"")].join("|");
  }
  function mergeFiles(...groups){
    ...
    groups.flat().filter(Boolean).forEach(raw=>{
      ...
      const sig=signature(f),sameId=byId.get(f.id),sameContent=bySig.get(sig);
      if(sameContent){
        const newer=Date.parse(f.updated||0)>Date.parse(sameContent.updated||0)?f:sameContent;
        Object.assign(sameContent,{...sameContent,...newer});return;
      }
      if(sameId){
        // A reused legacy ID must not erase a different file. Keep both and
        // assign a migration-safe ID to the conflicting item.
        f.id=`${f.id}_1l_${Math.random().toString(36).slice(2,8)}`;
      }
  ...
  const candidateKeys=[
    "gold1g_files","gold1k_files","gold1j_files","gold1i_files","gold1h_files","gold1l_files","gold1f_files","gold1e_files","gold1d_files","gold1c_files","gold1b_files","gold1a_files",
    "gold1g_workspace","gold1k_workspace","gold1j_workspace","gold1i_workspace","gold1h_workspace","gold1l_workspace","emerald_session","emerald_workspace","emeraldos_workspace","fileSystem","filesystem","drive"
  ];
  const fileGroups=[];
  ...
  const categoryNames=["prefs","mail","notes","tasks","events","contacts","tickets","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","folders","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings"];
  const legacyPrefixes=["gold1l_","gold1k_","gold1j_","gold1i_","gold1h_","gold1g_","gold1f_","gold1e_","gold1d_","gold1c_","gold1b_","gold1a_"];
  ...
  const keyMigrations={
    "gold1g_registry_1k":"gold1g_registry_1l",
    "gold1g_registry_1k_history":"gold1g_registry_1l_history",
    "gold1g_clipboard_history_1k":"gold1g_clipboard_history_1l",
    "gold1g_startup_apps_1k":"gold1g_startup_apps_1l",
    "gold1g_workspace_meta_1k":"gold1g_workspace_meta_1l",
    "gold1g_registry_1j":"gold1g_registry_1l",
    "gold1g_registry_1j_history":"gold1g_registry_1l_history",
    "gold1g_clipboard_history_1j":"gold1g_clipboard_history_1l",
    "gold1g_startup_apps_1j":"gold1g_startup_apps_1l",
    "gold1g_workspace_meta_1j":"gold1g_workspace_meta_1l",
    "gold1g_registry_1i":"gold1g_registry_1l",
    "gold1g_registry_1i_history":"gold1g_registry_1l_history",
    "gold1g_clipboard_history_1i":"gold1g_clipboard_history_1l",
    "gold1g_startup_apps_1i":"gold1g_startup_apps_1l",
    "gold1g_workspace_meta_1i":"gold1g_workspace_meta_1l",
    "gold1g_registry_1h":"gold1g_registry_1l",
    "gold1g_registry_1h_history":"gold1g_registry_1l_history",
    "gold1g_clipboard_history_1h":"gold1g_clipboard_history_1l",
    "gold1g_startup_apps_1h":"gold1g_startup_apps_1l",
    "gold1g_workspace_meta_1h":"gold1g_workspace_meta_1l"
  };
