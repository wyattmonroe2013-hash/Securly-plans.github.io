...
========================================================= */
(function EmeraldOSGold1LFeatures(){
  if(window.__EMERALDOS_GOLD_1L_FEATURES__) return;
  window.__EMERALDOS_GOLD_1L_FEATURES__=true;
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1L local write failed",key,error);return false}};
  ...
  const api=()=>window.Gold50||window.Gold1L;
  const notify=(title,body,app="EmeraldOS Gold 1L")=>api()?.notify?.(title,body,app)||console.info(`[${app}] ${title}: ${body}`);
  ...
  function staffSession(){
    for(const key of [PREFIX+"staff_session","gold1l_staff_session","gold1j_staff_session"]){
  ...
  function setCustomLogo(appId,source){
    ...
    window.Gold1L?.saveWorkspaceNow?.(false);
  }
  ...
  function logoMarkup(app,size=32){
    ...
    return `<span class="gold1l-logo-wrap" style="width:${size}px;height:${size}px"><img class="gold1l-custom-logo" src="${esc(logoFor(app))}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="fallback-icon" style="display:none">${fallback}</span></span>`;
  }
  ...
  function sandboxUserApp(app,{preview=false,hashVerified=false}={}){
    ...
    const html=`<div class="gold1l-user-app-host"><iframe id="${frameId}" sandbox="allow-scripts allow-forms allow-modals allow-downloads" title="${esc(app.name)}"></iframe><div class="gold1l-sandbox-note">${preview?"Developer preview":"Staff-verified sandbox"} · ${esc(app.owner)}</div></div>`;
  ...
  async function loadLatestManifest(){
    ...
    }catch(error){console.debug("Gold 1L update lookup used local cache",error?.message||error)}
    return cached||{latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",releaseTitle:"EmeraldOS Gold 1L",summary:"Current release"};
  }
  ...
  async function requestVersionChange(manifest){
    await window.Gold1L?.saveWorkspaceNow?.(false);
  ...
  async function openEnhancedUpdateCenter(){
    ...
    win.querySelector("#updateSave").onclick=()=>window.Gold1L?.saveWorkspaceNow?.(true);
    ...
    win.querySelector("#updatePublisher")?.addEventListener("click",()=>window.Gold1L?.openPublisherManager?.());
    return win;
  }
  function openELSUSStatus(){
    const latest=elsusState.latestManifest||read("emeraldGoldShell_latest",null);
    const html=`<div class="app-body"><h1>E.L.S.U.S.</h1><p>Gold 1L uses the original E.L.S.U.S. implementation from Gold 1I.</p><div class="gold1l-settings-cards"><div class="card"><h3>Routing</h3><p>Save VM → write pending manifest → open root <code>gold-shell.html</code></p></div><div class="card"><h3>Cloud VM</h3><p><code>emeraldOSUsers/${esc(userName())}/goldVM/current</code></p></div><div class="card"><h3>Latest manifest</h3><p>${esc(latest?.releaseTitle||"Not checked yet")}</p></div><div class="card"><h3>Publishing</h3><p>Manual Staff Edition and publisher-PIN workflow only.</p></div></div><div class="actions"><button id="elsusCheck" class="button primary">Check for updates</button><button id="elsusSave" class="button">Save VM</button><button id="elsusRoot" class="button">Open root E.L.S.U.S.</button></div><p class="muted">Gold 1L uses the direct, original E.L.S.U.S. route and requires no additional shell patch.</p></div>`;
    ...
    win.querySelector("#elsusSave").onclick=()=>window.Gold1L?.saveWorkspaceNow?.(true);
  ...
  function applyLiveControl(value,{source="cloud"}={}){
    ...
    document.body.classList.toggle("gold1l-maintenance",liveControl.maintenanceMode);
    document.body.classList.toggle("gold1l-support-only",liveControl.supportOnlyMode);
    document.body.classList.toggle("gold1l-live-focus",liveControl.forceFocusAssist);
  ...
  async function listenLiveStaffControl(){
    ...
    }catch(error){console.debug("Gold 1L live control is using local fallback",error)}
  }
  ...
  async function renderStaffCenter(win,tab="overview"){
    ...
    content.innerHTML='<div class="gold1l-loading">Loading Staff Center…</div>';
  ...
  function wireStaffContent(win,tab,data){
    ...
    if(tab==="elsus"){
      ...
      win.querySelector("#staffPublisherOpen")?.addEventListener("click",()=>window.Gold1L?.openPublisherManager?.());
      win.querySelector("#staffSaveVM")?.addEventListener("click",()=>window.Gold1L?.saveWorkspaceNow?.(true));
    }
  }
  ...
  function openAppInspection(app){
    const risks=analyzeCode(app.code);return openWindow(`inspect_${app.id}`,`Inspect: ${app.name}`,`<div class="app-body"><h1>${esc(app.name)}</h1><p>${esc(app.description)}</p><div class="gold1l-settings-cards"><div class="card"><b>Owner</b><p>${esc(app.owner)}</p></div><div class="card"><b>Status</b><p>${esc(statusLabel(app.verificationStatus))}</p></div><div class="card"><b>Capabilities</b><p>${esc(app.requestedPermissions.join(", ")||"None")}</p></div><div class="card"><b>Review indicators</b><p>${esc(risks.join(", ")||"None")}</p></div></div><h2>Source code</h2><pre class="gold1l-source-view">${esc(app.code)}</pre></div>`,{width:980,height:700,singleton:false});
  }
  ...
  async function sendSessionHeartbeat(){
    ...
    try{const fb=await import("./firebase.js");if(fb.db)await fb.setDoc(fb.doc(fb.db,SESSION_COLLECTION,normalizeUser(userName())),{...record,lastSeen:fb.serverTimestamp()},{merge:true})}catch(error){console.debug("Gold 1L session heartbeat cloud write skipped",error.message)}
  }
  ...
  const SETTINGS_SECTIONS=[
    ...
    ["shell","E.L.S.U.S.","Original E.L.S.U.S. update routing"]
  ];
  ...
  function settingsContent(section){
    ...
    if(section==="network")return `<h1>Network & Internet</h1><div class="gold1l-settings-cards"><div class="card"><h3>Status</h3><p>${navigator.onLine?"Connected":"Offline"}</p></div><div class="card"><h3>E.L.S.U.S.</h3><p>Original Gold 1I routing</p></div></div>${settingToggle("setMetered","Metered connection","Reduce optional cloud refresh operations.",Boolean(p.meteredConnection))}${settingToggle("setOfflineFallback","Offline fallback","Keep local VM features available when cloud services are offline.",p.offlineFallback!==false)}<div class="actions"><button class="button" data-open-tool="elsus">E.L.S.U.S. status</button><button class="button" id="networkTest">Run connection test</button></div>`;
  ...
  function openEnhancedSettings(section="system"){
    ...
    const html=`<div class="settings-layout gold1l-settings"><aside class="settings-nav"><h2>Settings</h2><input id="settingsSearch" class="field" placeholder="Find a setting">${sections.map(([id,label,description])=>`<button class="${section===id?"active":""}" data-settings-section="${id}" data-setting-words="${esc((label+" "+description).toLowerCase())}"><b>${esc(label)}</b><small>${esc(description)}</small></button>`).join("")}</aside><main class="settings-content">${content}</main></div>`;
  ...
  function wireEnhancedSettings(win,section){
    ...
    win.querySelectorAll("[data-open-tool]").forEach(button=>button.onclick=()=>({clipboard:window.Gold1L?.openClipboardHistory,elsus:openELSUSStatus}[button.dataset.openTool]?.()));
  ...
  function applyExtendedPrefs(){
    ...
    body.classList.toggle("gold1l-night-light",Boolean(p.nightLight));body.classList.toggle("gold1l-no-transparency",p.transparency===false);body.classList.toggle("gold1l-touch",Boolean(p.touchMode));body.classList.toggle("gold1l-game-mode",Boolean(p.gameMode));body.classList.toggle("gold1l-large-targets",Boolean(p.largePointerTargets));body.classList.toggle("no-motion",Boolean(p.reducedMotion||p.gameMode&&p.disableAnimationsInGameMode));body.classList.toggle("taskbar-top",p.taskbarPosition==="top");
  ...
  function applyVirtualDesktopVisibility(){
    qa(".window").forEach(win=>{const desktop=Number(win.dataset.virtualDesktop||1);win.classList.toggle("virtual-hidden",desktop!==currentDesktop);const task=document.querySelector(`[data-window="${CSS.escape(win.id)}"],#gold1l_task_${CSS.escape(win.id)}`);if(task)task.hidden=desktop!==currentDesktop;});
  ...
  function enhanceTaskViewDesktops(){
    ...
    let bar=panel.querySelector(".gold1l-desktop-bar");if(!bar){bar=document.createElement("div");bar.className="gold1l-desktop-bar";panel.prepend(bar)}
  ...
  function saveStickyNotes(notes){write(STICKY_KEY,notes);window.Gold1L?.saveWorkspaceNow?.(false)}
  function openStickyNotes(){
    const notes=stickyNotes();const html=`<div class="app-shell"><div class="app-toolbar"><button id="stickyNew" class="button primary">New note</button></div><div class="app-body"><div class="gold1l-sticky-grid">${notes.map(note=>`<article class="gold1l-sticky" style="--note-color:${esc(note.color||"#fff2ab")}"><textarea data-sticky-text="${esc(note.id)}">${esc(note.text)}</textarea><div><input type="color" data-sticky-color="${esc(note.id)}" value="${esc(note.color||"#fff2ab")}"><button data-sticky-delete="${esc(note.id)}">Delete</button></div></article>`).join("")||'<p>No sticky notes yet.</p>'}</div></div></div>`;
    ...
    win.querySelectorAll("[data-sticky-color]").forEach(input=>input.oninput=()=>{const list=stickyNotes(),item=list.find(note=>note.id===input.dataset.stickyColor);if(item){item.color=input.value;saveStickyNotes(list);input.closest(".gold1l-sticky").style.setProperty("--note-color",input.value)}});
  ...
  function patchOpenApp(){
    ...
    window.Gold1L=window.Gold1L||core;
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1L","E.L.S.U.S., Staff Center, verified user apps, custom logos, and Windows 10 features are ready.","System");
  }
