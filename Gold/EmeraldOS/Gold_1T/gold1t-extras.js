...
========================================================= */
(function EmeraldOSGold1TExtras(){
  if(window.__EMERALDOS_GOLD_1T_EXTRAS__) return;
  window.__EMERALDOS_GOLD_1T_EXTRAS__=true;

  const VERSION="1T";
  const FOLDER="Gold_1T";
  const UPDATE_SERVICE="Original E.L.S.U.S.";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1t";
  const HISTORY_KEY=PREFIX+"update_history_1t";
  const SNOOZE_KEY=PREFIX+"update_snooze_1t";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1t";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1t";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1t";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1t";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1T update check used local manifest cache",error?.message||error);
    }
  ...
  async function beginUserUpdate(manifest){
    ...
    try{
      if(window.Gold1T?.requestVersionChange){await window.Gold1T.requestVersionChange(manifest);return true}
      if(window.Gold1T?.saveWorkspaceNow)await window.Gold1T.saveWorkspaceNow(false);
      window.Gold1T?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1T live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1T live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1T?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1T?.openUpdateCenter?.(currentManifest);
  ...
  function patchPublicAPI(){
    window.Gold1T=window.Gold1T||api()||{};
    Object.assign(window.Gold1T,{elsusMode:"original",checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1T","Update checks, Focus Sessions, and Quick Settings loaded.","System");
  }
