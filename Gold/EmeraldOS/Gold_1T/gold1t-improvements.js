"use strict";
/* EmeraldOS Gold 1T user-experience, layout, and E.L.S.U.S. Shell V2 test tools. */
(function EmeraldOSGold1TImprovements(){
  if(window.__EMERALDOS_GOLD_1T_IMPROVEMENTS__) return;
  window.__EMERALDOS_GOLD_1T_IMPROVEMENTS__=true;
  ...
  function migrationStatus(){
    const report=read(PREFIX+"migration_1t_cloud_report",null)||read(PREFIX+"migration_1t_local_report",null)||{};
    return {
      complete:window.__GOLD1T_MIGRATION_COMPLETE__===true||report.completed===true,
      blocked:window.__GOLD1T_MIGRATION_BLOCK_SAVES__===true,
  ...
  function runShellV2Tests(){
    ...
    const checks=[
      ...
      {name:"Version login retained",pass:localStorage.getItem("loggedIn")==="true"||localStorage.getItem(PREFIX+"loggedIn")==="true",detail:"Gold 1T performed its own login after shell routing."},
      {name:"Cloud route metadata available",pass:Boolean(userRoute?.folder||read(PREFIX+"workspace_meta_1t",{})?.activeFolder||window.EMERALDOS_GOLD_FOLDER),detail:userRoute?.folder?`Cached Shell V2 route: ${userRoute.folder}.`:`Gold route metadata reports ${FOLDER}.`},
      {name:"Migration gate",pass:window.__GOLD1T_MIGRATION_BLOCK_SAVES__!==true,detail:window.__GOLD1T_MIGRATION_BLOCK_SAVES__===true?"Cloud autosave is correctly blocked until migration validation completes.":"Migration is complete and cloud saving is available."},
  ...
  function openShellV2Test(){
    ...
    const html=`<div class="app-shell gold1p-shell-test"><div class="app-toolbar"><button id="shellTestRun" class="button primary">Run tests again</button><button id="shellTestSave" class="button">Save VM route</button><button id="shellTestExport" class="button">Export report</button><button id="shellTestReturn" class="button">Return to Shell V2</button></div><div class="app-body"><div class="gold1p-page-heading"><div><h1>E.L.S.U.S. Shell V2 Compatibility</h1><p>Tests the optional Shell V2 routing layer while keeping Gold 1T compatible with the original E.L.S.U.S. shell.</p></div><span class="gold1p-status-pill ${result.allPassed?"good":"review"}">${result.passed}/${result.total} passed</span></div><div class="grid3"><div class="card"><b>Launch mode</b><h2>${shell.managed?"Shell V2":"Direct / original shell"}</h2></div><div class="card"><b>Gold route</b><h2>${VERSION}</h2><p>${FOLDER}</p></div><div class="card"><b>Migration gate</b><h2>${window.__GOLD1T_MIGRATION_BLOCK_SAVES__===true?"Protected":"Ready"}</h2></div></div><div class="gold1p-table-wrap"><table><thead><tr><th>Test</th><th>Result</th><th>Details</th></tr></thead><tbody>${rows}</tbody></table></div><section class="card"><h2>What Shell V2 is allowed to do</h2><p>Shell V2 verifies the EmeraldOS account only to locate the user’s current version. It then opens <code>${FOLDER}/index.html</code>, where Gold 1T performs its own normal login. Shell V2 does not replace files, settings, or VM data.</p></section><p class="muted">A review result can be expected when Gold 1T was opened directly instead of through Shell V2. It does not mean the operating system failed.</p></div></div>`;
    ...
    win.querySelector("#shellTestExport").onclick=()=>downloadJSON("EmeraldOS-Gold-1T-Shell-V2-Test.json",runShellV2Tests());
  ...
  function openFeedbackHub(){
    ...
    win.querySelector("#feedbackExport").onclick=()=>downloadJSON("EmeraldOS-Gold-1T-Feedback.json",feedbackItems());
  ...
  function registerApps(){
    ...
    add({id:"welcome",name:"Welcome Center",label:"WC",color:"#0078d7",group:"System",desc:"Quick access, recent files, and Gold 1T status.",open:openWelcomeCenter});
  ...
  function patchOpenApp(){
    ...
    window.Gold50=core;window.Gold1T=window.Gold1T||core;
    Object.assign(window.Gold1T,{openWelcomeCenter,openShellV2Test,runShellV2Tests,openFeedbackHub,shellV2Parameters:shellParameters});
  }
  ...
  function maybeOpenWelcome(){
    if(read(PREFIX+"home_1t_at_login",true)!==true)return;
  ...
  function init(){
    ...
    api().notify?.("EmeraldOS Gold 1T","User improvements, application layout fixes, and Shell V2 compatibility testing are ready.","System");
  }
