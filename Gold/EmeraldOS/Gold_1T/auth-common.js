...
window.GoldAuth = (() => {
  ...
  const ACCOUNT_HISTORY_KEY = "gold1g_account_history_1t";
  ...
  async function findCloudUser(username) {
    ...
    // Gold 1T compatible with both layouts.
    for (const collectionName of collections) {
  ...
  async function beginSession(user) {
    ...
    const userId = String(user?.userId || user?.__id || username).trim();

    await window.Gold1TUserIsolation?.switchToUser?.(userId, username);
    localStorage.setItem("emeraldGoldAuthenticatedUserId", userId.toLowerCase().replace(/[^a-z0-9._-]/g, "_"));
    localStorage.setItem("emeraldGoldAuthenticatedUsername", username);
    localStorage.setItem("loggedIn", "true");
    localStorage.setItem("gold1g_loggedIn", "true");
    localStorage.setItem("gold1t_loggedIn", "true");
    localStorage.setItem("username", username);
    localStorage.setItem("gold1g_username", username);
    localStorage.setItem("gold1t_username", username);
    localStorage.setItem("role", role);
    localStorage.setItem("role2", role2);
    localStorage.setItem("gold1g_role", role);
    localStorage.setItem("gold1g_role2", role2);
    localStorage.setItem("userId", userId);
    localStorage.setItem("gold1t_last_login", new Date().toISOString());
  ...
  return {
    ...
    recordAccountEvent,
    changePassword,
    localIsolationStatus: () => window.Gold1TUserIsolation?.status?.() || null
  };
})();
