...
/* =========================================================
   EMERALDOS GOLD 1K
   SHELL BRIDGE + STAFF CENTER + VERIFIED USER APPS
   + CUSTOM LOGOS + WINDOWS 10 FEATURE PACK
========================================================= */
(function EmeraldOSGold1KFeatures(){
  if(window.__EMERALDOS_GOLD_1K_FEATURES__) return;
  window.__EMERALDOS_GOLD_1K_FEATURES__=true;
  ...
  const SHELL_STATE_KEY=PREFIX+"elsus_shell_state";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1K local write failed",key,error);return false}};
  ...
  const api=()=>window.Gold50||window.Gold1K;
  const notify=(title,body,app="EmeraldOS Gold 1K")=>api()?.notify?.(title,body,app)||console.info(`[${app}] ${title}: ${body}`);
  ...
  let shellState={connected:false,managed:false,mode:"standalone",lastHandshake:null,lastMessage:null,shellVersion:null,latestManifest:null};
  ...
  function staffSession(){
    for(const key of [PREFIX+"staff_session","gold1k_staff_session","gold1j_staff_session"]){
  ...
  function setCustomLogo(appId,source){
    ...
    window.Gold1K?.saveWorkspaceNow?.(false);
  }
  ...
  function logoMarkup(app,size=32){
    ...
    return `<span class="gold1k-logo-wrap" style="width:${size}px;height:${size}px"><img class="gold1k-custom-logo" src="${esc(logoFor(app))}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="fallback-icon" style="display:none">${fallback}</span></span>`;
  }
  ...
  function sandboxUserApp(app,{preview=false,hashVerified=false}={}){
    ...
    const html=`<div class="gold1k-user-app-host"><iframe id="${frameId}" sandbox="allow-scripts allow-forms allow-modals allow-downloads" title="${esc(app.name)}"></iframe><div class="gold1k-sandbox-note">${preview?"Developer preview":"Staff-verified sandbox"} · ${esc(app.owner)}</div></div>`;
  ...
  /* ------------------------- E.L.S.U.S. shell bridge ------------------------- */
  function shellTarget(){return window.parent!==window?window.parent:window.opener&&!window.opener.closed?window.opener:null}
  function sendShell(type,payload={}){
    const target=shellTarget();if(!target)return false;
    try{target.postMessage({source:"emeraldos-gold",protocol:"ELSUS-1",type,version:VERSION,folder:FOLDER,user:userName(),payload},"*");return true}catch{return false}
  }
  function saveShellState(){write(SHELL_STATE_KEY,{...shellState,savedAt:now()})}
  function updateShellButton(){
    const button=$("shellStatusBtn");if(!button)return;
    button.textContent=shellState.connected?"E.L.S.U.S. ✓":shellState.managed?"E.L.S.U.S. …":"E.L.S.U.S.";
    button.classList.toggle("connected",shellState.connected);button.classList.toggle("waiting",shellState.managed&&!shellState.connected);
  }
  function initShellBridge(){
    const params=new URLSearchParams(location.search);
    shellState={...shellState,...read(SHELL_STATE_KEY,{}),managed:window.parent!==window||params.has("goldShell")||params.has("elsus"),mode:window.parent!==window?"embedded":params.has("goldShell")?"shell-routed":"standalone"};
    window.addEventListener("message",async event=>{
      const message=event.data;if(!message||typeof message!=="object")return;
      if(!["emerald-gold-shell","elsus-shell","emeraldos-shell"].includes(message.source)&&!String(message.type||"").startsWith("elsus:"))return;
      shellState.connected=true;shellState.lastMessage=now();shellState.shellVersion=message.shellVersion||message.payload?.shellVersion||shellState.shellVersion;
      const type=String(message.type||"").toLowerCase(),payload=message.payload||{};
      if(type.includes("ack")||type.includes("pong")){shellState.lastHandshake=now();}
      if(type.includes("latest")&&payload.manifest){shellState.latestManifest=payload.manifest;write("emeraldGoldShell_latest",payload.manifest);}
      if(type.includes("open-app")&&payload.id)guardedOpenApp(payload.id,{fromShell:true});
      if(type.includes("save")){await window.Gold1K?.saveWorkspaceNow?.(false);sendShell("elsus:save-complete",{savedAt:now()});}
      if(type.includes("theme")&&payload.theme)api()?.setPrefs?.({theme:payload.theme});
      if(type.includes("staff-control")&&payload.control)applyLiveControl({...liveControl,...payload.control},{source:"shell"});
      if(type.includes("logout"))api()?.logoutGold?.();
      if(type.includes("navigate")&&payload.hash)location.hash=String(payload.hash);
      saveShellState();updateShellButton();
    });
    const hello=()=>sendShell("elsus:ready",{title:document.title,entry:"OS.html",managed:shellState.managed,capabilities:["open-app","cloud-save","live-control","version-change","session","theme","staff-center"]});
    hello();setTimeout(hello,800);setInterval(()=>{sendShell("elsus:heartbeat",{openWindows:qa(".window").length,online:navigator.onLine,staff:isStaff(),controlRevision:liveControl.revision||0});},15000);
    $("shellStatusBtn")?.addEventListener("click",openShellIntegration);
    updateShellButton();saveShellState();
    window.EmeraldELSUS={send:sendShell,state:()=>({...shellState}),requestVersionChange,openShellIntegration};
  }
  async function loadLatestManifest(){
    const cached=shellState.latestManifest||read("emeraldGoldShell_latest",null);
    try{
      const fb=await import("./firebase.js");
      if(fb.db){const snap=await fb.getDoc(fb.doc(fb.db,"system","emeraldGoldLatest"));if(snap.exists()){const manifest=snap.data();shellState.latestManifest=manifest;write("emeraldGoldShell_latest",manifest);sendShell("elsus:latest-manifest",{manifest});return manifest;}}
    }catch(error){console.debug("Gold 1K update lookup used shell/local cache",error)}
    return cached||{latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",releaseTitle:"EmeraldOS Gold 1K",summary:"Current release"};
  }
  ...
  async function requestVersionChange(manifest){
    await window.Gold1K?.saveWorkspaceNow?.(false);
    write(PREFIX+"pending_elsus_update",{manifest,requestedAt:now(),fromVersion:VERSION,fromFolder:FOLDER});
    const sent=sendShell("elsus:request-version-change",{manifest,from:{version:VERSION,folder:FOLDER,entry:"OS.html"},preserveVM:true});
    if(sent&&shellState.managed){notify("Update handed to E.L.S.U.S.","The shell is switching versions while preserving this VM.","System Update");return true;}
    const targetFolder=manifest.folder||`Gold_${manifest.latestVersion||manifest.build}`;const entry=manifest.entry||"OS.html";
    if(confirm(`Open ${manifest.releaseTitle||targetFolder} now? Your Gold VM has been saved.`))location.href=`../${encodeURIComponent(targetFolder).replaceAll("%2F","/")}/${encodeURIComponent(entry).replaceAll("%2F","/")}?goldShell=1&from=${encodeURIComponent(FOLDER)}`;
    return true;
  }
  async function openEnhancedUpdateCenter(){
    ...
    const html=`<div class="app-body"><div class="gold1k-page-title"><div><h1>System Update</h1><p>E.L.S.U.S. preserves the version-independent Gold VM and never publishes a version automatically.</p></div><button id="updateCheckAgain" class="button primary">Check again</button></div><div class="gold1k-settings-cards"><div class="card"><h3>This VM</h3><p><b>EmeraldOS Gold 1K</b><br>Gold_1K / OS.html</p></div><div class="card"><h3>Configured release</h3><p><b>${esc(latest.releaseTitle||`EmeraldOS Gold ${latestVersion}`)}</b><br>${esc(latest.folder||"")} / ${esc(latest.entry||"OS.html")}</p></div><div class="card"><h3>Shell connection</h3><p>${shellState.connected?"Connected to the E.L.S.U.S. shell":shellState.managed?"Waiting for shell handshake":"Running standalone"}</p></div><div class="card"><h3>Status</h3><p>${newer?"A newer version is available.":"This VM is on the latest configured version."}</p></div></div><div class="card"><h3>Release information</h3><p>${esc(latest.summary||"No release notes were supplied.")}</p></div><div class="actions"><button id="updateSave" class="button">Save cloud VM</button><button id="updateApply" class="button primary" ${newer?"":"disabled"}>Update this VM now</button><button id="updateOpenShell" class="button">Shell integration</button>${isStaff()?'<button id="updatePublisher" class="button">Publisher Manager</button>':""}</div><p class="muted">Reading <code>system/emeraldGoldLatest</code> does not change it. Only the guarded Publish this Version action writes that document.</p></div>`;
    ...
    win.querySelector("#updateSave").onclick=()=>window.Gold1K?.saveWorkspaceNow?.(true);
    win.querySelector("#updateApply").onclick=()=>requestVersionChange(latest);
    win.querySelector("#updateOpenShell").onclick=openShellIntegration;
    win.querySelector("#updatePublisher")?.addEventListener("click",()=>window.Gold1K?.openPublisherManager?.());
    return win;
  }
  function openShellIntegration(){
    const state={...shellState};
    const html=`<div class="app-body"><h1>E.L.S.U.S. Service Pack One</h1><div class="gold1k-settings-cards"><div class="card"><h3>Mode</h3><p>${esc(state.mode)}</p></div><div class="card"><h3>Connection</h3><p>${state.connected?"Connected":"Not connected"}</p></div><div class="card"><h3>Shell version</h3><p>${esc(state.shellVersion||"Not reported")}</p></div><div class="card"><h3>Last handshake</h3><p>${esc(state.lastHandshake?new Date(state.lastHandshake).toLocaleString():"None")}</p></div></div><h2>Supported shell commands</h2><p>Open app, save VM, deliver latest-version metadata, apply live staff control, change theme, navigate to an OS section, and request a version switch.</p><div class="actions"><button id="shellHandshake" class="button primary">Send handshake</button><button id="shellSave" class="button">Save through shell</button><button id="shellExit" class="button">Open root shell</button></div><pre>${esc(JSON.stringify(state,null,2))}</pre></div>`;
    const win=openWindow("shellintegration","E.L.S.U.S. Shell Integration",html,{width:820,height:620});
    win.querySelector("#shellHandshake").onclick=()=>{sendShell("elsus:ready",{manual:true,capabilities:["open-app","cloud-save","live-control","version-change"]});notify("Handshake sent","Waiting for the E.L.S.U.S. shell to acknowledge.","E.L.S.U.S.")};
    win.querySelector("#shellSave").onclick=()=>sendShell("elsus:save-request",{requestedAt:now()});
    win.querySelector("#shellExit").onclick=()=>location.href="../gold-shell.html?force=1&from=Gold_1K";
    return win;
  }
  ...
  function applyLiveControl(value,{source="cloud"}={}){
    ...
    document.body.classList.toggle("gold1k-maintenance",liveControl.maintenanceMode);
    document.body.classList.toggle("gold1k-support-only",liveControl.supportOnlyMode);
    document.body.classList.toggle("gold1k-live-focus",liveControl.forceFocusAssist);
    ...
    sendShell("elsus:staff-control-applied",{control:liveControl,source});
    window.dispatchEvent(new CustomEvent("emeraldGoldLiveControl",{detail:liveControl}));
  }
  function appBlockedReason(id){
    if(isStaff())return "";
    if(liveControl.maintenanceMode&&!['support','updateshell','settings','systeminfo','help','shellintegration'].includes(id))return "EmeraldOS Gold is in maintenance mode.";
  ...
  async function listenLiveStaffControl(){
    ...
    }catch(error){console.debug("Gold 1K live control is using local fallback",error)}
  }
  ...
  async function renderStaffCenter(win,tab="overview"){
    ...
    content.innerHTML='<div class="gold1k-loading">Loading Staff Center…</div>';
  ...
  function wireStaffContent(win,tab,data){
    ...
    if(tab==="elsus"){
      win.querySelector("#staffShellOpen")?.addEventListener("click",openShellIntegration);
      win.querySelector("#staffPublisherOpen")?.addEventListener("click",()=>window.Gold1K?.openPublisherManager?.());
      win.querySelector("#staffSaveVM")?.addEventListener("click",()=>window.Gold1K?.saveWorkspaceNow?.(true));
    }
  }
  ...
  function openAppInspection(app){
    const risks=analyzeCode(app.code);return openWindow(`inspect_${app.id}`,`Inspect: ${app.name}`,`<div class="app-body"><h1>${esc(app.name)}</h1><p>${esc(app.description)}</p><div class="gold1k-settings-cards"><div class="card"><b>Owner</b><p>${esc(app.owner)}</p></div><div class="card"><b>Status</b><p>${esc(statusLabel(app.verificationStatus))}</p></div><div class="card"><b>Capabilities</b><p>${esc(app.requestedPermissions.join(", ")||"None")}</p></div><div class="card"><b>Review indicators</b><p>${esc(risks.join(", ")||"None")}</p></div></div><h2>Source code</h2><pre class="gold1k-source-view">${esc(app.code)}</pre></div>`,{width:980,height:700,singleton:false});
  }
  ...
  async function sendSessionHeartbeat(){
    const record={username:userName(),normalizedUser:normalizeUser(userName()),version:VERSION,folder:FOLDER,entry:"OS.html",shellManaged:shellState.managed,shellConnected:shellState.connected,staff:isStaff(),online:navigator.onLine,lastSeenISO:now(),openWindows:qa(".window").length,userAgent:navigator.userAgent.slice(0,300)};
    write(PREFIX+"session_heartbeat",record);sendShell("elsus:session",record);
    try{const fb=await import("./firebase.js");if(fb.db)await fb.setDoc(fb.doc(fb.db,SESSION_COLLECTION,normalizeUser(userName())),{...record,lastSeen:fb.serverTimestamp()},{merge:true})}catch(error){console.debug("Gold 1K session heartbeat cloud write skipped",error.message)}
  }
  ...
  const SETTINGS_SECTIONS=[
    ...
    ["search","Search","Search behavior and history"],["privacy","Privacy","Clipboard and diagnostics"],["update","Update & Security","E.L.S.U.S. and recovery"],
    ["shell","E.L.S.U.S. Service Pack One","In-OS shell integration"]
  ];
  ...
  function settingsContent(section){
    ...
    if(section==="network")return `<h1>Network & Internet</h1><div class="gold1k-settings-cards"><div class="card"><h3>Status</h3><p>${navigator.onLine?"Connected":"Offline"}</p></div><div class="card"><h3>E.L.S.U.S. shell</h3><p>${shellState.connected?"Connected":"Not connected"}</p></div></div>${settingToggle("setMetered","Metered connection","Reduce optional cloud refresh operations.",Boolean(p.meteredConnection))}${settingToggle("setOfflineFallback","Offline fallback","Keep local VM features available when cloud services are offline.",p.offlineFallback!==false)}<div class="actions"><button class="button" data-open-tool="shell">Shell status</button><button class="button" id="networkTest">Run connection test</button></div>`;
    ...
    if(section==="update")return `<h1>Update & Security</h1><div class="gold1k-settings-cards"><div class="card"><h3>System Update</h3><p>Check the configured E.L.S.U.S. release without changing the Firestore pointer.</p><button class="button primary" data-open-app="updateshell">Open System Update</button></div><div class="card"><h3>Security Center</h3><button class="button" data-open-app="security">Open Security Center</button></div><div class="card"><h3>Restore</h3><button class="button" data-open-app="restore">Open Restore Center</button></div></div>`;
    if(section==="shell")return `<h1>E.L.S.U.S. Service Pack One</h1><div class="gold1k-settings-cards"><div class="card"><h3>Connection</h3><p>${shellState.connected?"Connected":"Not connected"}</p></div><div class="card"><h3>Mode</h3><p>${esc(shellState.mode)}</p></div><div class="card"><h3>Latest metadata</h3><p>${esc(shellState.latestManifest?.releaseTitle||"Not supplied by shell")}</p></div></div>${settingToggle("setShellCommands","Allow shell commands","Accept safe open, save, theme, navigation, and update commands.",p.shellCommands!==false)}${settingToggle("setShellHeartbeat","Shell heartbeat","Report VM status to the parent shell.",p.shellHeartbeat!==false)}<div class="actions"><button class="button" data-open-tool="shell">Open integration status</button><button id="sendShellHandshake" class="button">Send handshake</button><button id="openRootShell" class="button">Open root shell</button></div>`;
  ...
  function openEnhancedSettings(section="system"){
    ...
    const html=`<div class="settings-layout gold1k-settings"><aside class="settings-nav"><h2>Settings</h2><input id="settingsSearch" class="field" placeholder="Find a setting">${sections.map(([id,label,description])=>`<button class="${section===id?"active":""}" data-settings-section="${id}" data-setting-words="${esc((label+" "+description).toLowerCase())}"><b>${esc(label)}</b><small>${esc(description)}</small></button>`).join("")}</aside><main class="settings-content">${content}</main></div>`;
  ...
  function wireEnhancedSettings(win,section){
    ...
    win.querySelectorAll("[data-open-tool]").forEach(button=>button.onclick=()=>({clipboard:window.Gold1K?.openClipboardHistory,shell:openShellIntegration}[button.dataset.openTool]?.()));
    ...
    [["setNotifications","notifications"],["setFocusAssist","focusAssist"],["setNightLight","nightLight"],["setSingleClick","singleClickOpen"],["setTouchMode","touchMode"],["setKeyboardHints","keyboardHints"],["setMetered","meteredConnection"],["setOfflineFallback","offlineFallback"],["setTransparency","transparency"],["setLabels","showLabels"],["setSmallTaskbar","smallTaskbar"],["setClockSeconds","clockSeconds"],["setGameMode","gameMode"],["setDisableAnimationsGame","disableAnimationsInGameMode"],["setCaptureHints","captureHints"],["setReducedMotion","reducedMotion"],["setHighContrastAuto","highContrastShortcut"],["setLargePointer","largePointerTargets"],["setSearchApps","searchApps"],["setSearchFiles","searchFiles"],["setSearchSettings","searchSettings"],["setSearchHistory","searchHistory"],["setClipboardHistory","clipboardHistory"],["setDiagnostics","optionalDiagnostics"],["setSessionHeartbeat","sessionHeartbeat"],["setShellCommands","shellCommands"],["setShellHeartbeat","shellHeartbeat"]].forEach(([id,key])=>bind(id,key));
    ...
    win.querySelector("#sendShellHandshake")?.addEventListener("click",()=>sendShell("elsus:ready",{manual:true}));
    win.querySelector("#openRootShell")?.addEventListener("click",()=>location.href="../gold-shell.html?force=1&from=Gold_1K");
  }
  function applyExtendedPrefs(){
    const p=prefs(),body=document.body,taskbar=$("taskbar"),search=$("searchBtn");
    body.classList.toggle("gold1k-night-light",Boolean(p.nightLight));body.classList.toggle("gold1k-no-transparency",p.transparency===false);body.classList.toggle("gold1k-touch",Boolean(p.touchMode));body.classList.toggle("gold1k-game-mode",Boolean(p.gameMode));body.classList.toggle("gold1k-large-targets",Boolean(p.largePointerTargets));body.classList.toggle("no-motion",Boolean(p.reducedMotion||p.gameMode&&p.disableAnimationsInGameMode));body.classList.toggle("taskbar-top",p.taskbarPosition==="top");
  ...
  function applyVirtualDesktopVisibility(){
    qa(".window").forEach(win=>{const desktop=Number(win.dataset.virtualDesktop||1);win.classList.toggle("virtual-hidden",desktop!==currentDesktop);const task=document.querySelector(`[data-window="${CSS.escape(win.id)}"],#gold1k_task_${CSS.escape(win.id)}`);if(task)task.hidden=desktop!==currentDesktop;});
  ...
  function enhanceTaskViewDesktops(){
    ...
    let bar=panel.querySelector(".gold1k-desktop-bar");if(!bar){bar=document.createElement("div");bar.className="gold1k-desktop-bar";panel.prepend(bar)}
  ...
  function saveStickyNotes(notes){write(STICKY_KEY,notes);window.Gold1K?.saveWorkspaceNow?.(false)}
  function openStickyNotes(){
    const notes=stickyNotes();const html=`<div class="app-shell"><div class="app-toolbar"><button id="stickyNew" class="button primary">New note</button></div><div class="app-body"><div class="gold1k-sticky-grid">${notes.map(note=>`<article class="gold1k-sticky" style="--note-color:${esc(note.color||"#fff2ab")}"><textarea data-sticky-text="${esc(note.id)}">${esc(note.text)}</textarea><div><input type="color" data-sticky-color="${esc(note.id)}" value="${esc(note.color||"#fff2ab")}"><button data-sticky-delete="${esc(note.id)}">Delete</button></div></article>`).join("")||'<p>No sticky notes yet.</p>'}</div></div></div>`;
    ...
    win.querySelectorAll("[data-sticky-color]").forEach(input=>input.oninput=()=>{const list=stickyNotes(),item=list.find(note=>note.id===input.dataset.stickyColor);if(item){item.color=input.value;saveStickyNotes(list);input.closest(".gold1k-sticky").style.setProperty("--note-color",input.value)}});
  ...
  function installApps(){
    ...
    ensureApp({id:"shellintegration",name:"E.L.S.U.S. Shell",label:"ES",color:"#0078d7",group:"System",desc:"View and test in-OS shell compatibility.",open:openShellIntegration});
    if(isStaff()){
  ...
  function patchOpenApp(){
    ...
    Object.assign(window.Gold1K||{}, {
      ...
      openUpdateCenter:openEnhancedUpdateCenter,openShellIntegration,requestVersionChange,setCustomLogo,customLogos,liveControl:()=>({...liveControl}),shellState:()=>({...shellState}),
    ...
    window.Gold1K=window.Gold1K||core;
  }
  function patchDeepLinks(){
    const hash=location.hash.slice(1).toLowerCase();
    const map={staff:"staffcenter",staffcenter:"staffcenter",livecontrol:"livecontrol",appverification:"appverification",shell:"shellintegration",elsus:"shellintegration",stickynotes:"stickynotes",snipping:"snipping",voicerecorder:"voicerecorder"};
  ...
  function init(){
    ...
    patchOpenApp();installApps();applyExtendedPrefs();initShellBridge();listenLiveStaffControl();listenUserAppReviewUpdates();initVirtualDesktops();patchTaskView();patchDeepLinks();
    ...
    window.addEventListener("beforeunload",()=>{saveShellState();saveVirtualDesktops();controlUnsubscribers.forEach(unsub=>{try{unsub()}catch{}});if(appVerificationUnsubscribe)try{appVerificationUnsubscribe()}catch{}});
    notify("EmeraldOS Gold 1K","E.L.S.U.S. Service Pack One, Staff Center, verified user apps, custom logos, and Windows 10 features are ready.","System");
  }
