...
========================================================= */
(function EmeraldOSGold1K(){
  if(window.__EMERALDOS_GOLD_1K_LOADED__) return;
  window.__EMERALDOS_GOLD_1K_LOADED__=true;

  const VERSION="1K";
  const FOLDER="Gold_1K";
  const PREFIX="gold1g_";
  const PUBLISHER_PIN="093013";
  const REGISTRY_KEY=PREFIX+"registry_1k";
  const REGISTRY_HISTORY_KEY=PREFIX+"registry_1k_history";
  const CLIPBOARD_KEY=PREFIX+"clipboard_history_1k";
  const STARTUP_KEY=PREFIX+"startup_apps_1k";
  const CLOUD_CATEGORIES=["prefs","files","tickets","mail","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","registry_1k","clipboard_history_1k","startup_apps_1k","custom_app_logos","app_verification_cache","elsus_shell_state","sticky_notes","voice_recordings","update_preferences_1k","update_history_1k","focus_sessions_1k"];
  ...
  const writeJSON=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1K local write failed",key,error);return false}};
  ...
  function notify(title,body,app="EmeraldOS Gold 1K"){
    try{
  ...
  function isStaff(){
    /* Gold 1K protected tools require a Staff Edition session that passed
       both EmeraldOS role verification and Emerald Mail verification. */
    for(const key of [PREFIX+"staff_session","gold1k_staff_session"]){
  ...
  function focusAnyWindow(win){
    ...
    const button=$("gold1k_task_"+win.id);
    qa(".gold1k-task-button").forEach(b=>b.classList.remove("active"));
    if(button) button.classList.add("active");
  }

  function taskButtonFor(win){
    const taskApps=$("taskApps");
    if(!taskApps || $("gold1k_task_"+win.id)) return;
    const button=document.createElement("button");
    button.id="gold1k_task_"+win.id;
    button.className="gold1k-task-button active";
    button.title=win.dataset.title||"EmeraldOS Gold 1K";
  ...
  function saveCustomWindowState(){
    const state=qa(".gold1k-window").map(w=>({
      id:w.id,title:w.dataset.title,left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,
      minimized:w.classList.contains("minimized"),maximized:w.classList.contains("gold1k-maximized")
    }));
  ...
  function open1IWindow(id,title,html,options={}){
    ...
    win.className="window gold1k-window";
  ...
  function wireCustomWindow(win){
    ...
    title?.addEventListener("pointerdown",e=>{
      if(e.target.closest("button")||win.classList.contains("gold1k-maximized")) return;
    ...
    win.querySelector("[data-g1h-resize]")?.addEventListener("pointerdown",e=>{
      if(win.classList.contains("gold1k-maximized")) return;
  ...
  function minimizeWindow(id){
    ...
    $("gold1k_task_"+id)?.classList.remove("active");
    saveCustomWindowState();
  }

  function maximizeWindow(id){
    const win=$(id);if(!win)return;
    if(win.classList.contains("gold1k-maximized")){
      const restore=readJSON(PREFIX+"restore_"+id,null);
      win.classList.remove("gold1k-maximized","gold1k-snapped");
      ...
      win.classList.add("gold1k-maximized");
    }
  ...
  function closeWindow(id){
    const win=$(id);if(!win)return;
    $("gold1k_task_"+id)?.remove();
    customWindows.delete(id);win.remove();saveCustomWindowState();
  }

  function snapWindow(win,zone){
    if(!win)return;
    if(win.classList.contains("gold1k-maximized")) win.classList.remove("gold1k-maximized");
    ...
    win.classList.add("gold1k-snapped");
    const map={
  ...
  function showSnapPreview(x,y){
    ...
    let preview=$("gold1kSnapPreview");
    if(!zone){preview?.remove();return}
    if(!preview){preview=document.createElement("div");preview.id="gold1kSnapPreview";preview.className="gold1k-snap-preview";document.body.appendChild(preview)}
  ...
  function hideSnapPreview(){$("gold1kSnapPreview")?.remove()}
  ...
  function registryTreeHTML(node,path=[],depth=0){
    return registryChildren(node).map(name=>{
      ...
      return `<div class="gold1k-tree-row ${selected?"selected":""}" data-reg-path="${esc(pathToken(next))}" style="padding-left:${depth*15}px">
        <span class="gold1k-tree-caret" data-reg-toggle="${esc(pathToken(next))}">${hasChildren?(expanded?"▼":"▶"):""}</span>
        <span class="gold1k-tree-folder">▰</span><span>${esc(name)}</span>
  ...
  function registryHTML(){
    ...
    return `<div class="gold1k-app gold1k-registry">
      <div class="gold1k-commandbar">
        ...
        <input id="gold1kRegistryPath" class="gold1k-field gold1k-reg-path gold1k-code" value="${esc(pathText(registrySelection))}" aria-label="Registry path">
        ${locked?'<span class="gold1k-reg-lock">Read-only without Staff Edition</span>':""}
      </div>
      <div class="gold1k-reg-body">
        <div class="gold1k-reg-tree">${registryTreeHTML(data)}</div>
        <div class="gold1k-reg-values">
          <table class="gold1k-reg-table"><thead><tr><th style="width:32%">Name</th><th style="width:19%">Type</th><th>Data</th></tr></thead>
          <tbody>${Object.entries(values).map(([name,value])=>`<tr data-reg-value="${esc(encodeURIComponent(name))}" class="${registrySelectedValue===name?"selected":""}"><td>${esc(name)}</td><td>${esc(value.type||"REG_SZ")}</td><td>${esc(registryValueDisplay(value))}</td></tr>`).join("")||'<tr><td colspan="3" class="gold1k-empty">This key has no values.</td></tr>'}</tbody></table>
        </div>
      </div>
      <div class="gold1k-statusbar"><span>${esc(pathText(registrySelection))}</span><span>${Object.keys(values).length} value(s)</span><span>${locked?"Protected hive":"Editable"}</span></div>
    </div>`;
  }
  ...
  function wireRegistryUI(win){
    ...
    const pathInput=$("gold1kRegistryPath");pathInput?.addEventListener("keydown",e=>{if(e.key==="Enter")navigateRegistry(pathInput.value)});
  }
  ...
  function applyRegistryEffects(data=registryData()){
    ...
    document.body.classList.toggle("gold1k-no-transparency",registryValueAt(data,personal,"Transparency",true)===false);
  ...
  function taskManagerHTML(tab="processes"){
    return `<div class="gold1k-app gold1k-taskmgr">
      <div class="gold1k-tabs"><button class="${tab==="processes"?"active":""}" data-task-tab="processes">Processes</button><button class="${tab==="performance"?"active":""}" data-task-tab="performance">Performance</button><button class="${tab==="startup"?"active":""}" data-task-tab="startup">Startup</button><button class="${tab==="details"?"active":""}" data-task-tab="details">Details</button></div>
      <div class="gold1k-taskmgr-content">${taskManagerPage(tab)}</div>
      <div class="gold1k-statusbar"><span>${processRows().length} apps and windows</span><span>EmeraldOS Gold 1K</span><span class="gold1k-muted">Virtual process manager</span></div>
    </div>`;
  }
  function taskManagerPage(tab){
    if(tab==="performance"){
      ...
      return `<div class="gold1k-performance-grid"><div class="gold1k-performance-card"><h3>CPU</h3><h1>${Math.min(94,8+processRows().length*4)}%</h1><div class="gold1k-meter"><span style="width:${Math.min(94,8+processRows().length*4)}%"></span></div><p class="gold1k-muted">Browser-hosted virtual CPU activity estimate.</p></div><div class="gold1k-performance-card"><h3>Memory</h3><h1>${percent}%</h1><div class="gold1k-meter"><span style="width:${percent}%"></span></div><p>${(localBytes/1024/1024).toFixed(2)} MB local VM storage in use.</p></div><div class="gold1k-performance-card"><h3>Cloud VM</h3><h2>${esc(localStorage.getItem(PREFIX+"cloud_status")||"Ready")}</h2><p>Path: emeraldOSUsers/${esc(userName())}/goldVM/current</p></div><div class="gold1k-performance-card"><h3>Uptime</h3><h2>${formatUptime(performance.now())}</h2><p>${navigator.onLine?"Network connected":"Offline"}</p></div></div>`;
    }
    if(tab==="startup"){
      ...
      return `<table class="gold1k-process-table"><thead><tr><th>Name</th><th>Status</th><th>Startup impact</th><th></th></tr></thead><tbody>${startup.map((a,i)=>`<tr><td>${esc(a.name)}</td><td>${a.enabled?"Enabled":"Disabled"}</td><td>${esc(a.impact||"Not measured")}</td><td><button class="gold1k-button" data-startup-toggle="${i}">${a.enabled?"Disable":"Enable"}</button></td></tr>`).join("")}</tbody></table>`;
    }
    const rows=processRows();
    return `<table class="gold1k-process-table"><thead><tr><th>Name</th><th>Status</th><th>App ID</th><th>Memory</th><th></th></tr></thead><tbody>${rows.map(p=>`<tr><td>${esc(p.title)}</td><td>${esc(p.status)}</td><td class="gold1k-code">${esc(p.app)}</td><td>${p.memory} MB</td><td><button class="gold1k-button" data-end-task="${esc(p.id)}">End task</button></td></tr>`).join("")||'<tr><td colspan="5" class="gold1k-empty">No applications are running.</td></tr>'}</tbody></table>`;
  }
  ...
  function controlPanelHTML(){return `<div class="gold1k-app gold1k-control-shell"><div class="gold1k-control-header"><h2>Control Panel</h2><p class="gold1k-muted">Adjust your computer's settings.</p><input id="gold1kControlSearch" class="gold1k-field" style="width:min(420px,100%)" placeholder="Search Control Panel"></div><div class="gold1k-control-grid">${CONTROL_ITEMS.map(item=>`<button class="gold1k-control-item" data-control="${item[3]}" data-control-text="${esc((item[0]+" "+item[1]).toLowerCase())}"><span class="icon">${item[2]}</span><span><b>${esc(item[0])}</b><small>${esc(item[1])}</small></span></button>`).join("")}</div></div>`}
  ...
  function wireControlPanel(win){
    ...
    $("gold1kControlSearch")?.addEventListener("input",e=>{const q=e.target.value.toLowerCase();win.querySelectorAll("[data-control]").forEach(b=>b.style.display=b.dataset.controlText.includes(q)?"":"none")});
  }
  function openControlItem(id){
    ...
    if(id==="network")return openSimplePanel("Network and Internet",`<h2>Network status</h2><p>${navigator.onLine?"Connected to the Internet":"Offline"}</p><div class="gold1k-control-grid"><button class="gold1k-control-item" onclick="Gold1K.runCommand('settings')"><span class="icon">⌁</span><span><b>Network settings</b><small>Open EmeraldOS Settings.</small></span></button></div>`);
    if(id==="hardware")return base("settings");
  }
  function openSimplePanel(id,html){return open1IWindow(id.toLowerCase().replace(/\W/g,"_"),id,`<div class="gold1k-app" style="padding:18px;overflow:auto">${html}</div>`,{width:760,height:520,icon:"⚙",refresh:true})}
  function openSystemInformation(){
    ...
    return openSimplePanel("System",`<h1>EmeraldOS Gold 1K</h1><p>Emerald Systems virtual operating environment</p><hr><div class="gold1k-control-grid"><div class="gold1k-performance-card"><b>Edition</b><p>EmeraldOS Gold 1K</p></div><div class="gold1k-performance-card"><b>User</b><p>${esc(userName())}</p></div><div class="gold1k-performance-card"><b>Browser platform</b><p>${esc(navigator.platform||"Web platform")}</p></div><div class="gold1k-performance-card"><b>Display</b><p>${innerWidth} × ${innerHeight} @ ${devicePixelRatio||1}x</p></div><div class="gold1k-performance-card"><b>Local VM storage</b><p>${(storage/1024/1024).toFixed(2)} MB</p></div><div class="gold1k-performance-card"><b>Cloud VM</b><p>emeraldOSUsers/${esc(userName())}/goldVM/current</p></div></div>`);
  }

  /* ------------------------- Run and Clipboard ------------------------- */
  function openRunDialog(){
    const html=`<div class="gold1k-app"><div class="gold1k-run"><div class="gold1k-run-icon">▦</div><div><p>Type the name of a program, folder, document, or EmeraldOS command, and Gold will open it for you.</p><div class="gold1k-run-row"><label for="gold1kRunInput">Open:</label><input id="gold1kRunInput" class="gold1k-field" autocomplete="off"></div></div></div><div class="gold1k-run-buttons"><button class="gold1k-button primary" id="gold1kRunOK">OK</button><button class="gold1k-button" onclick="Gold1K.closeWin('win_g1h_run')">Cancel</button><button class="gold1k-button" onclick="Gold1K.runCommand('explorer')">Browse...</button></div></div>`;
    const win=open1IWindow("run","Run",html,{width:520,height:255,icon:"▦",refresh:true});
    const input=$("gold1kRunInput");setTimeout(()=>input?.focus(),0);input?.addEventListener("keydown",e=>{if(e.key==="Enter")runCommand(input.value)});$("gold1kRunOK")?.addEventListener("click",()=>runCommand(input?.value||""));return win;
  }
  ...
  function openClipboardHistory(){
    ...
    const html=`<div class="gold1k-app"><div class="gold1k-commandbar"><b>Clipboard</b><span class="gold1k-muted">Win + V</span><span style="flex:1"></span><button id="gold1kClipboardClear">Clear all</button></div><div class="gold1k-clipboard-list">${items.map(item=>`<div class="gold1k-clipboard-item" data-clip-id="${esc(item.id)}" title="Click to copy">${esc(item.text)}</div>`).join("")||'<div class="gold1k-empty"><h2>Your clipboard is empty</h2><p>Copy text to add it to clipboard history.</p></div>'}</div></div>`;
    ...
    $("gold1kClipboardClear")?.addEventListener("click",()=>{writeJSON(CLIPBOARD_KEY,[]);openClipboardHistory()});return win;
  }
  ...
  function calendarHTML(date=new Date()){
    ...
    const labels=["Su","Mo","Tu","We","Th","Fr","Sa"].map(d=>`<span class="gold1k-muted">${d}</span>`);
    ...
    return `<div class="gold1k-calendar-head"><b>${date.toLocaleString(undefined,{month:"long",year:"numeric"})}</b><span>Today</span></div><div class="gold1k-calendar-grid">${labels.join("")}${days.join("")}</div>`;
  }
  function toggleClockFlyout(){
    const existing=$("gold1kClockFlyout");if(existing){existing.remove();return}
    const date=new Date(),fly=document.createElement("section");fly.id="gold1kClockFlyout";fly.className="gold1k-clock-flyout";
    fly.innerHTML=`<div class="gold1k-clock-time">${date.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div><div class="gold1k-clock-date">${date.toLocaleDateString([],{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</div>${calendarHTML(date)}`;
  ...
  async function fetchLatestManifest(){
    ...
    }catch(error){console.warn("Gold 1K update check used local manifest",error);return fallback}
  }
  async function openUpdateCenter(latestManifest=null){
    ...
    const html=`<div class="gold1k-app" style="padding:20px;overflow:auto"><div style="display:flex;align-items:start;justify-content:space-between;gap:12px"><div><h1>System Update</h1><p class="gold1k-muted">E.L.S.U.S. changes this VM only after you choose to update.</p></div><button id="gold1kCheckUpdates" class="gold1k-button primary">Check for updates</button></div><div class="gold1k-performance-grid"><div class="gold1k-performance-card"><h3>This VM</h3><p><b>EmeraldOS Gold 1K</b></p><p>Gold_1K / OS.html</p></div><div class="gold1k-performance-card"><h3>Latest configured release</h3><p><b>${esc(latest.releaseTitle||`EmeraldOS Gold ${latestVersion}`)}</b></p><p>${esc(latest.folder||"")} / ${esc(latest.entry||"OS.html")}</p></div><div class="gold1k-performance-card"><h3>Status</h3><p>${newer?"A newer release is available.":"This VM is on the latest configured release."}</p></div><div class="gold1k-performance-card"><h3>VM continuity</h3><p>Files, apps, settings, registry, and workspace data remain in the version-independent Gold VM.</p></div></div><h3>Release information</h3><p>${esc(latest.summary||"No release information is available.")}</p><div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:18px"><button id="gold1kSaveBeforeUpdate" class="gold1k-button">Save cloud VM</button><button id="gold1kApplyUpdate" class="gold1k-button primary" ${newer?"":"disabled"}>Update this VM now</button><button class="gold1k-button" onclick="Gold1K.openPublisherManager()">Update Publisher Manager</button></div></div>`;
    const win=open1IWindow("updateshell","System Update",html,{width:980,height:680,icon:"↻",refresh:true});
    $("gold1kCheckUpdates")?.addEventListener("click",async()=>openUpdateCenter(await fetchLatestManifest()));
    $("gold1kSaveBeforeUpdate")?.addEventListener("click",()=>saveWorkspaceNow1I(true));
    $("gold1kApplyUpdate")?.addEventListener("click",()=>applyConfiguredUpdate(latest));return win;
  }
  async function applyConfiguredUpdate(manifest){
    const latestVersion=manifest?.latestVersion||manifest?.build||"";
    if(versionRank(latestVersion)<=versionRank(VERSION))return notify("No newer update","EmeraldOS Gold 1K is already current.","System Update");
  ...
  /* ------------------------- Publisher ------------------------- */
  function releaseManifest(){return {product:"EmeraldOS Gold",latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",channel:"stable",status:"stable",required:false,enabled:true,setupMode:"manualUpdateSetup",releaseTitle:"EmeraldOS Gold 1K",servicePack:"E.L.S.U.S. Service Pack One",servicePackVersion:"1",activeUpdateNotifications:true,summary:"EmeraldOS Gold 1K restores active update notifications that ask before updating, improves update history and update preferences, adds Focus Sessions and Quick Settings, and includes minor Windows 10-style UI refinements while retaining E.L.S.U.S. Service Pack One from Gold 1J.",migrationFrom:["1J","1I","1H","1G","1F","1E","1D","1C","1B","1A"],migrationId:"gold-1j-to-1k-active-update-service-migration",minShellVersion:"1.1",requiredServicePack:"E.L.S.U.S. Service Pack One",rollbackFolder:"Gold_1J",rollbackVersion:"1J",releasedAt:"2026-07-13T00:00:00.000Z"}}
  ...
  function openPublisherManager(){
    if(!isStaff()){
      ...
      return openSimplePanel("Staff verification required",`<h2>Update Publisher Manager</h2><p>Publishing is available only from a verified Gold Staff Edition session.</p><button class="gold1k-button primary" onclick="location.href='staff.html'">Open Gold Staff Edition</button>`);
    }
    const unlocked=publisherUnlocked();
    const html=`<div class="gold1k-app" style="padding:20px;overflow:auto"><h1>Update Publisher Manager</h1><p>Publish <b>EmeraldOS Gold 1K</b> to <code>system/emeraldGoldLatest</code>.</p><div class="gold1k-performance-card"><b>Security status</b><p>${unlocked?"Publisher PIN accepted for this staff session.":"Locked. Enter the publisher PIN."}</p>${unlocked?"":'<label>Publisher PIN <input id="gold1kPublisherPin" type="password" class="gold1k-field"></label> <button id="gold1kUnlockPublisher" class="gold1k-button">Unlock</button>'}</div><div class="gold1k-performance-card"><b>Release target</b><p>Gold_1K / OS.html</p><p>Release title: EmeraldOS Gold 1K</p></div><div style="display:flex;gap:8px;margin-top:18px"><button id="gold1kPublishVersion" class="gold1k-button primary" ${unlocked?"":"disabled"}>Publish this Version</button><button id="gold1kExportManifest" class="gold1k-button">Export manifest</button></div><p class="gold1k-muted">Gold 1K never publishes from a query string or during first boot.</p></div>`;
    const win=open1IWindow("publisher","Update Publisher Manager",html,{width:720,height:540,icon:"UP",refresh:true});
    $("gold1kUnlockPublisher")?.addEventListener("click",()=>unlockPublisher($("gold1kPublisherPin")?.value||""));
    $("gold1kPublishVersion")?.addEventListener("click",()=>{publisherClickAuthorizedUntil=Date.now()+3000;publishLatest()});
    $("gold1kExportManifest")?.addEventListener("click",()=>saveText("FIREBASE_EMERALDGOLDLATEST_1K.json",JSON.stringify(releaseManifest(),null,2),"application/json"));return win;
  }
  ...
  async function publishLatest(){
    ...
    if(Date.now()>publisherClickAuthorizedUntil){
      notify("Publish button required","Open Update Publisher Manager and click Publish this Version. Gold 1K cannot publish from startup, a URL, or a console call.","System Update");
      return false;
    }
    publisherClickAuthorizedUntil=0;
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"system","emeraldGoldLatest"),releaseManifest(),{merge:true});writeJSON("emeraldGoldShell_latest",releaseManifest());localStorage.setItem(PREFIX+"latest_manifest_published_1k","true");notify("E.L.S.U.S. updated","system/emeraldGoldLatest now points to Gold_1K.","System Update");return true}catch(error){console.warn(error);notify("Publish failed",error.message,"System Update");return false}
  }

  /* ------------------------- Split Cloud VM ------------------------- */
  function localCategoryValue(category){
    if(category==="registry_1k")return registryData();
    if(category==="clipboard_history_1k")return clipboardHistory();
    if(category==="startup_apps_1k")return readJSON(STARTUP_KEY,[]);
  ...
  function currentWindowState(){return qa(".window").map(w=>({id:w.id,title:w.dataset.title||"",left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,minimized:w.classList.contains("minimized"),maximized:w.classList.contains("max")||w.classList.contains("gold1k-maximized")})).slice(0,80)}
  async function saveWorkspaceNow1I(show=true){
    const summary={product:"EmeraldOS Gold",version:VERSION,build:VERSION,activeVersion:VERSION,activeFolder:FOLDER,schema:"gold1k-split-v2",user:userName(),savedAt:now(),categories:CLOUD_CATEGORIES,openWindows:currentWindowState(),cloudPath:"emeraldOSUsers/{username}/goldVM/current"};
    writeJSON(PREFIX+"workspace_meta_1k",summary);
    try{
      const fbm=await import("./firebase.js");
      ...
      if(show)notify("Cloud VM saved","Gold 1K saved the complete split, quota-safe VM state to the cloud.","Gold VM");return summary;
    }catch(error){
      console.warn("Gold 1K split cloud save failed",error);localStorage.setItem(PREFIX+"cloud_status","save failed: "+error.message);if(show)notify("Saved locally","Cloud sync will retry when Firebase is available.","Gold VM");return summary;
    }
  }
  function canonicalCloudCategory(category){
    return ({
      registry_1h:"registry_1k",registry_1i:"registry_1k",registry_1j:"registry_1k",registry_1k:"registry_1k",
      clipboard_history_1h:"clipboard_history_1k",clipboard_history_1i:"clipboard_history_1k",clipboard_history_1j:"clipboard_history_1k",clipboard_history_1k:"clipboard_history_1k",
      startup_apps_1h:"startup_apps_1k",startup_apps_1i:"startup_apps_1k",startup_apps_1j:"startup_apps_1k",startup_apps_1k:"startup_apps_1k"
    })[category]||category;
  }
  ...
  function mergeMigratedFiles(...groups){
    ...
    groups.flat().filter(Boolean).forEach(raw=>{
      ...
      if(sameId)f.id=`${f.id}_1k_${Math.random().toString(36).slice(2,8)}`;
  ...
  async function readLegacyCloudFiles(fbm){
    ...
    for(const path of paths){try{const snap=await fbm.getDoc(fbm.doc(fbm.db,...path));if(snap.exists())groups.push(collectMigratedFiles(snap.data()))}catch(error){console.debug("Gold 1K legacy cloud source skipped",path.join("/"),error?.message)}}
    return groups.flat();
  }
  async function restoreWorkspace1I(show=true){
    try{
      ...
      if(summary){
        const splitSchemas=new Set(["gold1h-split-v1","gold1i-split-v1","gold1i-split-v2","gold1j-split-v1","gold1j-split-v2","gold1k-split-v1","gold1k-split-v2"]);
        if(splitSchemas.has(summary.schema)){
          for(const sourceCategory of summary.categories||CLOUD_CATEGORIES){
            ...
            if(category==="files")fileGroups.push(collectMigratedFiles(value));
              ...
              const key=category==="registry_1k"?REGISTRY_KEY:category==="clipboard_history_1k"?CLIPBOARD_KEY:category==="startup_apps_1k"?STARTUP_KEY:PREFIX+category;
              writeJSON(key,value);
            }
          }
        }else{
          fileGroups.push(collectMigratedFiles(summary));
          const legacyMap={prefs:"prefs",mail:"mail",notes:"notes",tasks:"tasks",events:"events",contacts:"contacts",tickets:"tickets",notifications:"notifications",user_apps:"user_apps",remote_sessions:"remote_sessions",emergency_logs:"emergency_logs",vm_snapshots:"vm_snapshots",registry_1h:"registry_1k",registry_1i:"registry_1k",registry_1j:"registry_1k",registry_1k:"registry_1k",clipboard_history_1h:"clipboard_history_1k",clipboard_history_1i:"clipboard_history_1k",clipboard_history_1j:"clipboard_history_1k",clipboard_history_1k:"clipboard_history_1k",startup_apps_1h:"startup_apps_1k",startup_apps_1i:"startup_apps_1k",startup_apps_1j:"startup_apps_1k",startup_apps_1k:"startup_apps_1k"};
          Object.entries(legacyMap).forEach(([source,category])=>{if(summary[source]===undefined)return;const key=category==="registry_1k"?REGISTRY_KEY:category==="clipboard_history_1k"?CLIPBOARD_KEY:category==="startup_apps_1k"?STARTUP_KEY:PREFIX+category;writeJSON(key,summary[source])});
        }
      }
      ...
      writeJSON(PREFIX+"migration_1k_cloud_report",{completed:true,completedAt:now(),sourceSchema:summary?.schema||"none",filesBefore:Array.isArray(localFiles)?localFiles.length:0,filesAfter:mergedFiles.length,firestoreReleasePointerChanged:false});
      try{window.Gold50?.setPrefs?.(readJSON(PREFIX+"prefs",{}));window.Gold50?.saveFiles?.(mergedFiles)}catch{}
      if(show)notify("Cloud VM migration complete",`${mergedFiles.length} files are available in Gold 1K. The E.L.S.U.S. release pointer was not changed.`,"Gold VM");
      return summary||{schema:"legacy-migration",files:mergedFiles.length};
    }catch(error){console.warn("Gold 1K cloud restore failed",error);if(show)notify("Restore failed",error.message,"Gold VM");return null}
  }
  ...
  function customToolButtons(){return `
    <button class="gold1k-start-tile" onclick="Gold1K.openRegistryEditor()"><span>▥</span><b>Registry Editor</b></button>
    <button class="gold1k-start-tile" onclick="Gold1K.openTaskManager()"><span>▤</span><b>Task Manager</b></button>
    <button class="gold1k-start-tile" onclick="Gold1K.openControlPanel()"><span>⚙</span><b>Control Panel</b></button>
    <button class="gold1k-start-tile" onclick="Gold1K.openRun()"><span>▦</span><b>Run</b></button>
    <button class="gold1k-start-tile" onclick="Gold1K.openClipboardHistory()"><span>▧</span><b>Clipboard</b></button>
    <button class="gold1k-start-tile" onclick="Gold50.openApp('converter')"><span>⇄</span><b>Converter</b></button>
    <button class="gold1k-start-tile" onclick="Gold50.openApp('storage')"><span>◫</span><b>Storage</b></button>
    ${isStaff()?'<button class="gold1k-start-tile" onclick="Gold1K.openPublisherManager()"><span>UP</span><b>Publisher</b></button>':''}` }
  function enhanceStartMenu(){
    ...
    if(!tileColumn||tileColumn.querySelector(".gold1k-start-tools-inline"))return;
    startEnhancing=true;
    const section=document.createElement("section");
    section.className="gold1k-start-tools-inline";
    section.innerHTML=`<h3>System tools</h3><div class="gold1k-start-tool-grid">${customToolButtons()}</div>`;
    tileColumn.appendChild(section);
    startEnhancing=false;
  }
  function enhanceActionCenter(){
    const panel=$("actionCenter");if(!panel||actionEnhancing||panel.querySelector(".gold1k-quick-actions"))return;
    actionEnhancing=true;
    panel.insertAdjacentHTML("afterbegin",`<div class="gold1k-quick-actions"><button class="gold1k-quick-action" data-quick="focus"><span>☾</span>Focus assist</button><button class="gold1k-quick-action" data-quick="night"><span>◐</span>Night light</button><button class="gold1k-quick-action" data-quick="settings"><span>⚙</span>All settings</button><button class="gold1k-quick-action" data-quick="save"><span>☁</span>Cloud save</button><button class="gold1k-quick-action" data-quick="safe"><span>!</span>Safe mode</button><button class="gold1k-quick-action" data-quick="registry"><span>▥</span>Registry</button><button class="gold1k-quick-action" data-quick="task"><span>▤</span>Task Manager</button><button class="gold1k-quick-action" data-quick="control"><span>▦</span>Control Panel</button></div>`);
  ...
  function quickAction(action,button){
    if(action==="focus"){document.body.classList.toggle("gold1k-focus");button.classList.toggle("active");return}
    if(action==="night"){document.body.classList.toggle("gold1k-night");button.classList.toggle("active");document.body.style.filter=document.body.classList.contains("gold1k-night")?"sepia(.22) brightness(.92)":"";return}
    ...
    if(action==="safe"&&confirm("Restart EmeraldOS Gold 1K in Safe Mode?")){localStorage.setItem(PREFIX+"safemode","true");location.reload()}
  }
  function enhanceSearch(){
    const panel=$("searchPanel");if(!panel||panel.querySelector(".gold1k-search-tools"))return;
    const box=document.createElement("div");box.className="gold1k-search-tools";box.innerHTML=`<h3>System tools</h3><div class="gold1k-start-tool-grid">${customToolButtons()}</div>`;panel.appendChild(box);
  }
  function enhanceDesktop(){
    ...
    tools.forEach(tool=>{if(desktop.querySelector(`[data-gold1k-shortcut="${tool.id}"]`))return;const button=document.createElement("button");button.className="desktop-icon gold1k-system-shortcut";button.dataset.gold1kShortcut=tool.id;button.innerHTML=`<span class="app-icon">${tool.icon}</span><span>${esc(tool.name)}</span>`;button.addEventListener("dblclick",tool.open);button.addEventListener("click",()=>{qa(".desktop-icon").forEach(x=>x.classList.remove("selected"));button.classList.add("selected")});desktop.appendChild(button)});
  }
  function showPowerMenu(){
    open1IWindow("power","Power",`<div class="gold1k-app" style="padding:20px"><h2>Power</h2><div class="gold1k-control-grid"><button class="gold1k-control-item" onclick="location.reload()"><span class="icon">↻</span><span><b>Restart</b><small>Restart EmeraldOS Gold 1K.</small></span></button><button class="gold1k-control-item" onclick="Gold1K.saveWorkspaceNow(true)"><span class="icon">☁</span><span><b>Save cloud VM</b><small>Save before leaving.</small></span></button><button class="gold1k-control-item" onclick="Gold50.logoutGold?.()"><span class="icon">⇥</span><span><b>Sign out</b><small>Close this user session.</small></span></button></div></div>`,{width:540,height:400,icon:"⏻",refresh:true});
  }
  function showDesktop(){
    const windows=qa(".window");const anyVisible=windows.some(w=>!w.classList.contains("minimized"));windows.forEach(w=>w.classList.toggle("minimized",anyVisible));qa(".gold1k-task-button").forEach(b=>b.classList.toggle("active",!anyVisible));
  }
  ...
  function showAltTab(){
    ...
    let overlay=$("gold1kAltTab");if(!overlay){overlay=document.createElement("div");overlay.id="gold1kAltTab";overlay.className="gold1k-alt-tab";document.body.appendChild(overlay)}
    overlay.innerHTML=`<div class="gold1k-alt-tab-inner">${altTabWindows.map((w,i)=>`<div class="gold1k-alt-tab-item ${i===altTabIndex?"active":""}"><span>${esc(w.dataset.icon||"▦")}</span><b>${esc(w.dataset.title||"Application")}</b><small>${w.classList.contains("minimized")?"Minimized":"Running"}</small></div>`).join("")}</div>`;
  }
  function commitAltTab(){
    $("gold1kAltTab")?.remove();const win=altTabWindows[altTabIndex];if(win){win.classList.remove("minimized");focusAnyWindow(win)}
  }
  ...
  async function finishUpdateSetup(){
    ...
    $("gold1kUpdateSetup")?.remove();
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"emeraldOSUsers",userName(),"goldVM","setup"),{firstBootCompleted:true,firstBootScope:"global-elsus-user",lastCompletedVersion:VERSION,lastUpdateSetupVersion:VERSION,lastSetupType:"update",licenseAccepted:true,licenseVersion:"Gold_1K",folder:FOLDER,updatedAt:now()},{merge:true})}catch(error){console.warn("Gold 1K update setup cloud save failed",error)}
    notify("EmeraldOS Gold 1K is ready","Your existing cloud VM data and preferences were preserved.","Setup");
  }
  function maybeShowUpdateSetup(){
    ...
    const overlay=document.createElement("section");overlay.id="gold1kUpdateSetup";overlay.className="gold1k-update-setup";overlay.innerHTML=`<div class="gold1k-update-card"><h1>Welcome to EmeraldOS Gold 1K</h1><p>This update keeps your existing E.L.S.U.S. cloud VM and introduces a more familiar, user-friendly desktop.</p><ul><li>Active update notifications that ask before changing versions</li><li>E.L.S.U.S. Service Pack One shell and Staff Center integration</li><li>Focus Sessions and Windows 10-style Quick Settings</li><li>Minor Start, taskbar, window, panel, and accessibility refinements</li><li>Merge-based, quota-safe cloud VM migration</li></ul><p>Migration merges files from earlier local workspaces and cloud layouts. No files or applications are intentionally removed.</p><div class="actions"><button class="gold1k-button" onclick="document.getElementById('gold1kUpdateSetup').remove()">Remind me later</button><button class="gold1k-button primary" onclick="Gold1K.finishUpdateSetup()">Continue</button></div></div>`;document.body.appendChild(overlay);
  }
  ...
  function patchGoldAPI(){
    ...
    if(!api.__gold1kOriginalOpenApp)api.__gold1kOriginalOpenApp=api.openApp?.bind(api);
    api.openApp=function gold1IOpenApp(id){
      ...
      if(custom[id])return custom[id]();return api.__gold1kOriginalOpenApp?.(id);
    };
    api.saveWorkspaceNow=saveWorkspaceNow1I;api.restoreWorkspace=restoreWorkspace1I;
    Object.assign(api,{openRegistryEditor1I:openRegistryEditor,openTaskManager1I:openTaskManager,openControlPanel1I:openControlPanel,openRun1I:openRunDialog,openClipboardHistory1I:openClipboardHistory,openUpdatePublisherManager1I:openPublisherManager,openRegistryEditor1K:openRegistryEditor,openTaskManager1K:openTaskManager,openControlPanel1K:openControlPanel,openRun1K:openRunDialog,openClipboardHistory1K:openClipboardHistory,openUpdatePublisherManager1K:openPublisherManager,gold1kPublishLatest:publishLatest,gold1kManifest:releaseManifest});
    window.Gold1G=api;window.Gold1K=publicAPI;return true;
  }
  ...
  function installObserver(){
    const observer=new MutationObserver(()=>{clearTimeout(window.__gold1kRefreshTimer);window.__gold1kRefreshTimer=setTimeout(refreshEnhancements,40)});observer.observe(document.body,{childList:true,subtree:true});
  }
  function branding(){
    document.title="EmeraldOS Gold 1K";document.body.classList.add("gold1k","gold1k-shell");document.body.dataset.build=VERSION;
    qa("h1,h2,p,span").forEach(el=>{if(el.children.length===0&&el.textContent?.includes("EmeraldOS Gold 1G"))el.textContent=el.textContent.replaceAll("EmeraldOS Gold 1G","EmeraldOS Gold 1K")});
  }
  ...
  function init(){
    ...
    window.addEventListener("beforeunload",()=>{writeJSON(PREFIX+"workspace_meta_1k",{version:VERSION,folder:FOLDER,savedAt:now(),openWindows:currentWindowState()})});
