...
/* =========================================================
   EMERALDOS GOLD 1K
   E.L.S.U.S. SERVICE PACK ONE + ACTIVE UPDATE SERVICE
...
========================================================= */
(function EmeraldOSGold1KServicePack(){
  if(window.__EMERALDOS_GOLD_1K_SERVICE_PACK__) return;
  window.__EMERALDOS_GOLD_1K_SERVICE_PACK__=true;

  const VERSION="1K";
  const FOLDER="Gold_1K";
  const SERVICE_PACK="E.L.S.U.S. Service Pack One";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1k";
  const HISTORY_KEY=PREFIX+"update_history_1k";
  const SNOOZE_KEY=PREFIX+"update_snooze_1k";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1k";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1k";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1k";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1k";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1K update check used shell/local cache",error?.message||error);
    }
  ...
  function updateButton(){
    let button=$("gold1kUpdateStatusBtn");
    if(button)return button;
    const taskbar=$("taskbar"),anchor=$("quickSettingsBtn")||$("shellStatusBtn");
    if(!taskbar||!anchor)return null;
    button=document.createElement("button");
    button.id="gold1kUpdateStatusBtn";
    button.className="task-btn gold1k-update-status hidden";
    button.title="System Update";
    button.setAttribute("aria-label","System Update");
    button.innerHTML='<span aria-hidden="true">↻</span><span class="gold1k-update-dot" aria-hidden="true"></span>';
  ...
  function removeUpdatePrompt(){
    $("gold1kActiveUpdatePrompt")?.remove();
    document.body.classList.remove("gold1k-update-prompt-open");
  }
  function showUpdatePrompt(manifest,force=false){
    ...
    const firstSetup=$("setupWizard"),updateSetup=$("gold1kUpdateSetup");
    ...
    prompt.id="gold1kActiveUpdatePrompt";
    prompt.className=`gold1k-active-update-prompt${manifest.required?" required":""}`;
    prompt.setAttribute("role","dialog");
    prompt.setAttribute("aria-labelledby","gold1kUpdatePromptTitle");
    const version=manifestVersion(manifest),title=manifest.releaseTitle||`EmeraldOS Gold ${version}`;
    prompt.innerHTML=`
      <div class="gold1k-update-prompt-head">
        <span class="gold1k-update-prompt-icon" aria-hidden="true">↻</span>
        <div><b id="gold1kUpdatePromptTitle">${esc(title)} is ready</b><small>${esc(manifest.servicePack||SERVICE_PACK)}</small></div>
        <button id="gold1kUpdatePromptClose" aria-label="Close update notification">×</button>
      </div>
      ...
      <div class="gold1k-update-prompt-meta"><span>Current: ${VERSION}</span><span>Available: ${esc(version)}</span><span>${manifest.required?"Required update":"Optional update"}</span></div>
      <div class="gold1k-update-prompt-actions">
        <button id="gold1kUpdateNow" class="button primary">Update now</button>
        <button id="gold1kUpdateDetails" class="button">View details</button>
        <button id="gold1kUpdateLater" class="button">Later</button>
      </div>`;
    document.body.appendChild(prompt);
    document.body.classList.add("gold1k-update-prompt-open");
    localStorage.setItem(LAST_PROMPT_KEY,now());
    addHistory("prompt",manifest,"The active update prompt was displayed.");
    $("gold1kUpdateNow").onclick=()=>beginUserUpdate(manifest);
    $("gold1kUpdateDetails").onclick=()=>{removeUpdatePrompt();openUpdateStatus(manifest)};
    $("gold1kUpdateLater").onclick=()=>{snooze();removeUpdatePrompt();addHistory("snoozed",manifest,`Snoozed for ${updatePrefs().snoozeHours} hours.`);notify("Update reminder snoozed",`Gold will remind you about ${title} later.`,"System Update")};
    $("gold1kUpdatePromptClose").onclick=()=>{if(!manifest.required)snooze(1);removeUpdatePrompt()};
    return true;
  }
  ...
  async function beginUserUpdate(manifest){
    ...
    notify("Preparing update","Saving the cloud VM before handing the update to E.L.S.U.S.","System Update");
    try{
      if(window.Gold1K?.requestVersionChange){await window.Gold1K.requestVersionChange(manifest);return true}
      if(window.Gold1K?.saveWorkspaceNow)await window.Gold1K.saveWorkspaceNow(false);
      window.Gold1K?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function checkForUpdates(options={}){
    ...
    if(isNewer(manifest)){
      ...
      if(!alreadyNotified||options.forcePrompt){
        notify("Update available",`${manifest.releaseTitle||`EmeraldOS Gold ${version}`} is ready. Choose when to update.`,"System Update");
        localStorage.setItem(LAST_NOTIFIED_VERSION_KEY,version);
      }
      showUpdatePrompt(manifest,Boolean(options.forcePrompt));
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1K live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1K live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1K?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1K?.openUpdateCenter?.(currentManifest);
  ...
  function quickSettingsPanel(){
    let panel=$("gold1kQuickSettingsPanel");
    ...
    panel=document.createElement("section");panel.id="gold1kQuickSettingsPanel";panel.className="gold1k-quick-settings";
    panel.innerHTML=`<header><b>Quick Settings</b><small>${esc(SERVICE_PACK)}</small></header><div class="gold1k-quick-grid"><button data-quick="focus" class="${prefs.focusAssist?"active":""}"><span>☾</span>Focus assist</button><button data-quick="night" class="${prefs.nightLight?"active":""}"><span>◐</span>Night light</button><button data-quick="dark" class="${prefs.theme==="dark"?"active":""}"><span>◑</span>Dark mode</button><button data-quick="transparent" class="${prefs.transparency!==false?"active":""}"><span>▧</span>Transparency</button><button data-quick="updates" class="${updatePrefs().activeNotifications?"active":""}"><span>↻</span>Update alerts</button><button data-quick="settings"><span>⚙</span>All settings</button></div><footer><span>${navigator.onLine?"Connected":"Offline"}</span><button id="quickOpenUpdate">${updateAvailable?"Update available":"Up to date"}</button></footer>`;
    document.body.appendChild(panel);
    panel.querySelector('[data-quick="focus"]').onclick=()=>{api()?.setPrefs?.({focusAssist:!prefs.focusAssist});panel.remove();quickSettingsPanel()};
    panel.querySelector('[data-quick="night"]').onclick=()=>{api()?.setPrefs?.({nightLight:!prefs.nightLight});panel.remove();quickSettingsPanel()};
    panel.querySelector('[data-quick="dark"]').onclick=()=>{api()?.setPrefs?.({theme:prefs.theme==="dark"?"light":"dark"});panel.remove();quickSettingsPanel()};
    panel.querySelector('[data-quick="transparent"]').onclick=()=>{api()?.setPrefs?.({transparency:prefs.transparency===false});panel.remove();quickSettingsPanel()};
    panel.querySelector('[data-quick="updates"]').onclick=()=>{setUpdatePrefs({activeNotifications:!updatePrefs().activeNotifications});panel.remove();quickSettingsPanel()};
    panel.querySelector('[data-quick="settings"]').onclick=()=>{panel.remove();api()?.openApp?.("settings")};
  ...
  function openFocusSessions(){
    ...
    const html=`<div class="app-shell"><div class="app-toolbar"><b>Focus Sessions</b><span style="flex:1"></span><button id="focusSettings" class="button">Settings</button></div><div class="app-body gold1k-focus-app"><div class="gold1k-focus-clock" data-focus-time>${formatFocusTime(focusRemaining)}</div><h2 data-focus-state>${focusRunning?"Focusing":"Ready"}</h2><p>Use a distraction-free timer with Focus Assist and Gold notifications.</p><div class="gold1k-focus-controls"><button id="focusStart" class="button primary">${focusRunning?"Pause":"Start"}</button><button id="focusReset" class="button">Reset</button></div><div class="gold1k-focus-presets"><button data-focus-minutes="15">15 minutes</button><button data-focus-minutes="25">25 minutes</button><button data-focus-minutes="45">45 minutes</button><button data-focus-minutes="60">60 minutes</button></div><label class="gold1k-focus-option"><input id="focusAssistDuringSession" type="checkbox" checked> Turn on Focus Assist while the timer is running</label></div></div>`;
  ...
  function addServicePackBranding(){
    document.body.dataset.elsusServicePack="one";
    const apply=()=>{
      const menu=$("startMenu");
      if(menu&&!menu.querySelector(".gold1k-sp1-start-badge")){
        const badge=document.createElement("div");badge.className="gold1k-sp1-start-badge";badge.innerHTML=`<span>E.L.S.U.S.</span><b>Service Pack One</b>`;menu.prepend(badge);
      }
    };
    apply();
    const observer=new MutationObserver(()=>apply());
    if($("startMenu"))observer.observe($("startMenu"),{childList:true});
  }
  ...
  function installEvents(){
    ...
    document.addEventListener("click",event=>{if(!event.target.closest("#gold1kQuickSettingsPanel")&&!event.target.closest("#quickSettingsBtn"))$("gold1kQuickSettingsPanel")?.remove()});
    ...
    window.addEventListener("message",event=>{
      ...
      if(type!=="elsus:latest-manifest")return;
      const manifest=message.payload?.manifest||message.manifest;if(!manifest)return;
      latestManifest=manifest;write(MANIFEST_CACHE_KEY,manifest);updateAvailable=isNewer(manifest);updateTaskbarStatus();
      if(updateAvailable){addHistory("shell-detected",manifest,"A newer release was delivered by the E.L.S.U.S. shell.");showUpdatePrompt(manifest,false)}
    });
  ...
  function patchPublicAPI(){
    window.Gold1K=window.Gold1K||api()||{};
    Object.assign(window.Gold1K,{servicePack:SERVICE_PACK,checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }

  function init(){
    if(!api()?.APPS){setTimeout(init,80);return}
    patchPublicAPI();registerApps();addServicePackBranding();installEvents();updateButton();scheduleChecks();startLiveListener();if(focusRunning){focusRemaining=Math.max(0,Math.ceil((focusEndsAt-Date.now())/1000));focusTimer=setInterval(tickFocusSession,250)}
    setTimeout(()=>checkForUpdates({source:"startup check"}),5500);
    notify("EmeraldOS Gold 1K",`${SERVICE_PACK} and active update notifications are ready.`,"System");
  }
