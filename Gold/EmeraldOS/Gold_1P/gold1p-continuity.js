...
/* EmeraldOS Gold 1P cloud continuity and migration gate.
   ...
   Gold 1P to save over the shared VM. This module never writes the shared
   system/emeraldGoldLatest release pointer. */
(function EmeraldOSGold1PContinuity(){
  if(window.__EMERALDOS_GOLD_1P_CONTINUITY__)return;
  window.__EMERALDOS_GOLD_1P_CONTINUITY__=true;

  const VERSION="1P",PREFIX="gold1g_",REPORT_KEY=PREFIX+"migration_1p_cloud_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1P continuity local write failed",key,error);return false}};
  const helper=()=>window.Gold1PLocalMigration||{};
  ...
  const knownSchemas=new Set([
    ...
    "gold1k-split-v1","gold1k-split-v2","gold1l-split-v1","gold1l-split-v2","gold1m-split-v1","gold1m-split-v2","gold1n-split-v1","gold1n-split-v2","gold1o-split-v1","gold1o-split-v2","gold1p-split-v1","gold1p-split-v2"
  ]);
  const baseCategories=["prefs","files","folders","tickets","mail","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings","feedback_1p","routines_1p","accessibility_1p","home_1p_at_login","notification_preferences_1p","session_state_1p"];
  const targetCategories=[...baseCategories,"registry_1p","registry_1p_history","clipboard_history_1p","startup_apps_1p","update_preferences_1p","update_history_1p","focus_sessions_1p"];
  ...
  function localKeyFor(category){
    if(category==="registry_1p")return PREFIX+"registry_1p";
    if(category==="registry_1p_history")return PREFIX+"registry_1p_history";
    if(category==="clipboard_history_1p")return PREFIX+"clipboard_history_1p";
    if(category==="startup_apps_1p")return PREFIX+"startup_apps_1p";
    if(category==="update_preferences_1p")return PREFIX+"update_preferences_1p";
    if(category==="update_history_1p")return PREFIX+"update_history_1p";
    if(category==="focus_sessions_1p")return PREFIX+"focus_sessions_1p";
    if(category==="notification_preferences_1p")return PREFIX+"notification_preferences_1p";
    if(category==="session_state_1p")return PREFIX+"session_state_1p";
  ...
  function mergeCategory(category,sources){
    ...
    const arrayCategory=["tickets","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","sticky_notes","voice_recordings","registry_1p_history","clipboard_history_1p","startup_apps_1p","update_history_1p"].includes(category);
  ...
  async function collectOrphanedChunks(fb,vmUser=currentUser()){
    ...
    }catch(error){console.debug("Gold 1P orphaned chunk recovery unavailable",error?.message)}
  }
  ...
  async function archiveSourceCategory(fb,version,category,value,vmUser=currentUser()){
    ...
    const id=`${safeVersion}__${category}`,json=JSON.stringify(value??null),base={version:safeVersion,category,updatedAt:now(),source:"Gold 1P pre-migration archive"};
  ...
  async function collectCloudSources(fb){
    ...
    for(const vmUser of userCandidates()){
      ...
      }catch(error){console.debug("Gold 1P VM summary source skipped",vmUser,error?.message)}
      ...
      if(summary){
        if(summary.openWindows)addSource("session_state_1p",{version:candidateVersion,updatedAt:summary.savedAt||now(),windows:summary.openWindows,openApps:[]},`goldVM/current window summary at ${vmUser}`);
        if(knownSchemas.has(summary.schema)){
          for(const category of summary.categories||[]){
            ...
            }catch(error){console.warn("Gold 1P category migration skipped",category,error)}
          }
        }else{
      ...
      }catch(error){console.debug("Gold 1P category scan unavailable",vmUser,error?.message)}
      ...
      }catch(error){console.debug("Gold 1P version backup scan unavailable",vmUser,error?.message)}
  ...
  function refreshDesktop(){
    try{window.Gold50?.setPrefs?.(read(PREFIX+"prefs",{}));window.Gold50?.saveFiles?.(read(PREFIX+"files",[]));window.Gold50?.renderDesktop?.();window.Gold50?.renderStartMenu?.();window.Gold50?.renderNotificationBadge?.()}catch(error){console.debug("Gold 1P UI refresh after migration skipped",error?.message)}
  }

  function openMigrationCenter(){
    const report=read(REPORT_KEY,{stage:"not run"}),local=read(PREFIX+"migration_1p_local_report",{}),files=read(PREFIX+"files",[]);
  ...
  async function runMigration(show=false){
    if(window.__GOLD1P_MIGRATION_RUNNING__)return window.__GOLD1P_MIGRATION_PROMISE__;
    window.__GOLD1P_MIGRATION_RUNNING__=true;
    sourceMap.clear();collectLocalSources();
    const promise=(async()=>{
      let cloudInfo={summary:null,sourceVersion:"none"},cloudError=null;
      try{const fb=await withTimeout(import("./firebase.js"),20000,"Firebase migration connection timed out");if(fb.db)cloudInfo=await withTimeout(collectCloudSources(fb),30000,"Cloud migration scan timed out");else cloudError="Firebase is not configured"}catch(error){cloudError=error.message;console.warn("Gold 1P cloud migration continued locally",error)}
      ...
      if(cloudTemporarilyUnverified){
        ...
        window.__GOLD1P_MIGRATION_BLOCK_SAVES__=true;window.__GOLD1P_MIGRATION_COMPLETE__=false;
        localStorage.setItem(PREFIX+"cloud_status","migration waiting for cloud verification");
        if(show)window.Gold1P?.notify?.("Cloud migration not verified","Gold 1P kept cloud autosave blocked so older files cannot be overwritten. Reconnect and run the migration scan again.","Migration & Continuity");
        return report;
      }
      report.autosaveUnblockedAt=now();report.autosaveStillBlocked=false;
      write(REPORT_KEY,report);refreshDesktop();
      window.__GOLD1P_MIGRATION_BLOCK_SAVES__=false;window.__GOLD1P_MIGRATION_COMPLETE__=true;
      try{window.__GOLD1P_MIGRATION_COMMITTING__=true;await window.Gold1P?.saveWorkspaceNow?.(false)}catch(error){console.warn("Gold 1P post-migration save deferred",error)}finally{window.__GOLD1P_MIGRATION_COMMITTING__=false}
      const noticeKey=PREFIX+"migration_1p_notice_shown";
      if(show||localStorage.getItem(noticeKey)!=="true"){
        window.Gold1P?.notify?.("Migration complete",`${report.filesAfter||0} files and all available VM categories were merged before cloud autosave was enabled.`,"Migration & Continuity");
        localStorage.setItem(noticeKey,"true");
      }
      return report;
    })().finally(()=>{window.__GOLD1P_MIGRATION_RUNNING__=false});
    window.__GOLD1P_MIGRATION_PROMISE__=promise;return promise;
  }

  function install(){
    registerMigrationApp();
    const api=window.Gold1P||{};api.runMigration=runMigration;api.openMigrationCenter=openMigrationCenter;api.migrationReport=()=>read(REPORT_KEY,null);window.Gold1P=api;
    if(window.Gold50){window.Gold50.openMigrationCenter=openMigrationCenter;window.Gold50.runMigration=runMigration}
    runMigration(false);
    window.addEventListener("online",()=>{if(window.__GOLD1P_MIGRATION_BLOCK_SAVES__===true)setTimeout(()=>runMigration(false),1000)});
    setInterval(()=>{if(window.__GOLD1P_MIGRATION_BLOCK_SAVES__===true&&navigator.onLine)runMigration(false)},60000);
  }
