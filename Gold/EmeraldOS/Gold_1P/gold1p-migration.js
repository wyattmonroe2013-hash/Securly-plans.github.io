...
/* EmeraldOS Gold 1P local continuity migration.
   ...
   asynchronous cloud migration in gold1p-continuity.js has completed. */
(function EmeraldOSGold1PLocalMigration(){
  if(window.__EMERALDOS_GOLD_1P_LOCAL_MIGRATION__) return;
  window.__EMERALDOS_GOLD_1P_LOCAL_MIGRATION__=true;
  window.__GOLD1P_MIGRATION_BLOCK_SAVES__=true;

  const PREFIX="gold1g_";
  const REPORT_KEY=PREFIX+"migration_1p_local_report";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1P local migration write failed",key,error);return false}};
  ...
  function normalizeFile(raw,folderHint="Documents"){
    ...
    return {...raw,id:String(raw.id||raw.fileId||uid()),name,type,folder,content:String(content??""),created:raw.created||raw.createdAt||now(),updated:raw.updated||raw.updatedAt||now(),trash:Boolean(raw.trash||raw.deleted),migratedTo:"1P"};
  }
  ...
  function mergeFiles(...groups){
    ...
    groups.flat(Infinity).filter(Boolean).forEach(raw=>{
      ...
      if(sameId)file.id=`${file.id}_1p_${Math.random().toString(36).slice(2,8)}`;
  ...
  const fileKeys=[
    "gold1g_files","gold1h_files","gold1i_files","gold1j_files","gold1k_files","gold1l_files","gold1m_files","gold1n_files","gold1o_files","gold1p_files",
    "gold1g_workspace","gold1h_workspace","gold1i_workspace","gold1j_workspace","gold1k_workspace","gold1l_workspace","gold1m_workspace","gold1n_workspace","gold1o_workspace","gold1p_workspace",
  ...
  const previousSession=readMaybe(PREFIX+"session_state_1p")||readMaybe(PREFIX+"session_state_1o");
  const previousMeta=readMaybe(PREFIX+"workspace_meta_1o")||readMaybe("gold1o_workspace_meta");
  if(previousSession!==undefined)write(PREFIX+"session_state_1p",previousSession);
  else if(previousMeta?.openWindows)write(PREFIX+"session_state_1p",{version:"1O",updatedAt:previousMeta.savedAt||now(),windows:previousMeta.openWindows,openApps:[]});
  const previousNotificationPrefs=readMaybe(PREFIX+"notification_preferences_1p")||readMaybe(PREFIX+"notification_preferences_1o");
  if(previousNotificationPrefs!==undefined)write(PREFIX+"notification_preferences_1p",previousNotificationPrefs);
  const previousHomePreference=readMaybe(PREFIX+"home_1p_at_login")??readMaybe(PREFIX+"home_1o_at_login")??readMaybe(PREFIX+"welcome_1n_at_login");
  if(previousHomePreference!==undefined)write(PREFIX+"home_1p_at_login",previousHomePreference);
  ...
  const prefixes=["gold1a_","gold1b_","gold1c_","gold1d_","gold1e_","gold1f_","gold1h_","gold1i_","gold1j_","gold1k_","gold1l_","gold1m_","gold1n_","gold1o_","gold1p_","gold1g_"];
  ...
  const sourceVersions=["1h","1i","1j","1k","1l","1m","1n","1o","1p"];
  versionedFamilies.forEach(family=>{
    ...
    if(merged!==undefined)write(`${PREFIX}${family}_1p`,merged);
  });
  ...
  window.Gold1PLocalMigration={report,collectFiles,mergeFiles,deepMerge,mergeArray,normalizeFile};
})();
