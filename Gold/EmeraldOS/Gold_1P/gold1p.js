...
========================================================= */
(function EmeraldOSGold1P(){
  if(window.__EMERALDOS_GOLD_1P_LOADED__) return;
  window.__EMERALDOS_GOLD_1P_LOADED__=true;

  const VERSION="1P";
  const FOLDER="Gold_1P";
  const PREFIX="gold1g_";
  const PUBLISHER_PIN="093013";
  const REGISTRY_KEY=PREFIX+"registry_1p";
  const REGISTRY_HISTORY_KEY=PREFIX+"registry_1p_history";
  const CLIPBOARD_KEY=PREFIX+"clipboard_history_1p";
  const STARTUP_KEY=PREFIX+"startup_apps_1p";
  const CLOUD_CATEGORIES=["prefs","files","folders","tickets","mail","notes","tasks","events","contacts","notifications","user_apps","remote_sessions","emergency_logs","vm_snapshots","registry_1p","registry_1p_history","clipboard_history_1p","startup_apps_1p","custom_app_logos","app_verification_cache","elsus_original_state","elsus_shell_state","sticky_notes","voice_recordings","feedback_1p","routines_1p","accessibility_1p","home_1p_at_login","update_preferences_1p","update_history_1p","focus_sessions_1p","notification_preferences_1p","session_state_1p"];
  ...
  const writeJSON=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1P local write failed",key,error);return false}};
  ...
  function notify(title,body,app="EmeraldOS Gold 1P"){
    try{
  ...
  function isStaff(){
    /* Gold 1P protected tools require a Staff Edition session that passed
       both EmeraldOS role verification and Emerald Mail verification. */
    for(const key of [PREFIX+"staff_session","gold1p_staff_session"]){
  ...
  function taskButtonFor(win){
    ...
    button.title=win.dataset.title||"EmeraldOS Gold 1P";
  ...
  function taskManagerHTML(tab="processes"){
    return `<div class="gold1p-app gold1p-taskmgr">
      ...
      <div class="gold1p-statusbar"><span>${processRows().length} apps and windows</span><span>EmeraldOS Gold 1P</span><span class="gold1p-muted">Virtual process manager</span></div>
    </div>`;
  }
  ...
  function openControlItem(id){
    ...
    if(id==="network")return openSimplePanel("Network and Internet",`<h2>Network status</h2><p>${navigator.onLine?"Connected to the Internet":"Offline"}</p><div class="gold1p-control-grid"><button class="gold1p-control-item" onclick="Gold1P.runCommand('settings')"><span class="icon">⌁</span><span><b>Network settings</b><small>Open EmeraldOS Settings.</small></span></button></div>`);
  ...
  async function fetchLatestManifest(){
    ...
    }catch(error){console.warn("Gold 1P update check used local manifest",error);return fallback}
  }
  ...
  async function applyConfiguredUpdate(manifest){
    ...
    if(versionRank(latestVersion)<=versionRank(VERSION))return notify("No newer update","EmeraldOS Gold 1P is already current.","System Update");
  ...
  /* ------------------------- Publisher ------------------------- */
  function releaseManifest(){return {product:"EmeraldOS Gold",latestVersion:VERSION,build:VERSION,folder:FOLDER,entry:"OS.html",channel:"stable",status:"stable",required:false,enabled:true,setupMode:"manualUpdateSetup",releaseTitle:"EmeraldOS Gold 1P",activeUpdateNotifications:true,elsusMode:"original",shellV2Compatible:true,summary:"EmeraldOS Gold 1P improves the cloud pick-up-where-you-left-off VM, reduces startup and update notification noise, refines Quick Settings, preserves every supported Gold file format, and fixes remaining application layout collisions.",migrationFrom:["1O","1N","1M","1L","1K","1J","1I","1H","1G","1F","1E","1D","1C","1B","1A"],migrationId:"gold-1o-to-1p-cloud-vm-continuity",minShellVersion:"1.0",rollbackFolder:"Gold_1O",rollbackVersion:"1O",releasedAt:"2026-07-14T00:00:00.000Z"}}
  ...
  function openPublisherManager(){
    ...
    const html=`<div class="gold1p-app" style="padding:20px;overflow:auto"><h1>Update Publisher Manager</h1><p>Publish <b>EmeraldOS Gold 1P</b> to <code>system/emeraldGoldLatest</code>.</p><div class="gold1p-performance-card"><b>Security status</b><p>${unlocked?"Publisher PIN accepted for this staff session.":"Locked. Enter the publisher PIN."}</p>${unlocked?"":'<label>Publisher PIN <input id="gold1pPublisherPin" type="password" class="gold1p-field"></label> <button id="gold1pUnlockPublisher" class="gold1p-button">Unlock</button>'}</div><div class="gold1p-performance-card"><b>Release target</b><p>Gold_1P / OS.html</p><p>Release title: EmeraldOS Gold 1P</p></div><div style="display:flex;gap:8px;margin-top:18px"><button id="gold1pPublishVersion" class="gold1p-button primary" ${unlocked?"":"disabled"}>Publish this Version</button><button id="gold1pExportManifest" class="gold1p-button">Export manifest</button></div><p class="gold1p-muted">Gold 1P never publishes from a query string or during first boot.</p></div>`;
    ...
    $("gold1pExportManifest")?.addEventListener("click",()=>saveText("FIREBASE_EMERALDGOLDLATEST_1P.json",JSON.stringify(releaseManifest(),null,2),"application/json"));return win;
  }
  ...
  async function publishLatest(){
    ...
    if(Date.now()>publisherClickAuthorizedUntil){
      notify("Publish button required","Open Update Publisher Manager and click Publish this Version. Gold 1P cannot publish from startup, a URL, or a console call.","System Update");
      return false;
    }
    publisherClickAuthorizedUntil=0;
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"system","emeraldGoldLatest"),releaseManifest(),{merge:true});writeJSON("emeraldGoldShell_latest",releaseManifest());localStorage.setItem(PREFIX+"latest_manifest_published_1p","true");notify("E.L.S.U.S. updated","system/emeraldGoldLatest now points to Gold_1P.","System Update");return true}catch(error){console.warn(error);notify("Publish failed",error.message,"System Update");return false}
  }

  /* ------------------------- Split Cloud VM ------------------------- */
  function localCategoryValue(category){
    if(category==="registry_1p")return registryData();
    if(category==="registry_1p_history")return readJSON(REGISTRY_HISTORY_KEY,[]);
    if(category==="clipboard_history_1p")return clipboardHistory();
    if(category==="startup_apps_1p")return readJSON(STARTUP_KEY,[]);
    const objectCategories=new Set(["prefs","folders","mail","app_verification_cache","elsus_original_state","elsus_shell_state","accessibility_1p","home_1p_at_login","update_preferences_1p","focus_sessions_1p","notification_preferences_1p","session_state_1p"]);
  ...
  async function saveWorkspaceNow1I(show=true){
    const summary={product:"EmeraldOS Gold",version:VERSION,build:VERSION,activeVersion:VERSION,activeFolder:FOLDER,schema:"gold1p-split-v2",user:userName(),savedAt:now(),categories:CLOUD_CATEGORIES,openWindows:currentWindowState(),cloudPath:"emeraldOSUsers/{username}/goldVM/current"};
    if(window.__GOLD1P_MIGRATION_BLOCK_SAVES__===true && window.__GOLD1P_MIGRATION_COMMITTING__!==true){
      writeJSON(PREFIX+"workspace_meta_1p",{...summary,saveDeferred:true,deferredReason:"migration-in-progress"});
      if(show)notify("Save deferred","Gold 1P is still merging data from previous versions. Cloud save will run automatically after migration completes.","Migration & Continuity");
      return summary;
    }
    writeJSON(PREFIX+"workspace_meta_1p",summary);
    try{
      const fbm=await import("./firebase.js");
      ...
      await fbm.setDoc(fbm.doc(fbm.db,"emeraldOSUsers",vmUserId(),"goldVM","current"),{...summary,categoryIndex:results,migrationValidated:true,migrationReportKey:"migration_1p_cloud_report"},{merge:true});
      ...
      if(show)notify("Cloud VM saved","Gold 1P saved the complete split, quota-safe VM state to the cloud.","Gold VM");return summary;
    }catch(error){
      console.warn("Gold 1P split cloud save failed",error);localStorage.setItem(PREFIX+"cloud_status","save failed: "+error.message);if(show)notify("Saved locally","Cloud sync will retry when Firebase is available.","Gold VM");return summary;
    }
  }
  function canonicalCloudCategory(category){
    return ({
      registry_1h:"registry_1p",registry_1i:"registry_1p",registry_1j:"registry_1p",registry_1k:"registry_1p",registry_1l:"registry_1p",registry_1m:"registry_1p",registry_1n:"registry_1p",registry_1o:"registry_1p",registry_1p:"registry_1p",
      registry_1h_history:"registry_1p_history",registry_1i_history:"registry_1p_history",registry_1j_history:"registry_1p_history",registry_1k_history:"registry_1p_history",registry_1l_history:"registry_1p_history",registry_1m_history:"registry_1p_history",registry_1n_history:"registry_1p_history",registry_1o_history:"registry_1p_history",registry_1p_history:"registry_1p_history",
      clipboard_history_1h:"clipboard_history_1p",clipboard_history_1i:"clipboard_history_1p",clipboard_history_1j:"clipboard_history_1p",clipboard_history_1k:"clipboard_history_1p",clipboard_history_1l:"clipboard_history_1p",clipboard_history_1m:"clipboard_history_1p",clipboard_history_1n:"clipboard_history_1p",clipboard_history_1o:"clipboard_history_1p",clipboard_history_1p:"clipboard_history_1p",
      startup_apps_1h:"startup_apps_1p",startup_apps_1i:"startup_apps_1p",startup_apps_1j:"startup_apps_1p",startup_apps_1k:"startup_apps_1p",startup_apps_1l:"startup_apps_1p",startup_apps_1m:"startup_apps_1p",startup_apps_1n:"startup_apps_1p",startup_apps_1o:"startup_apps_1p",startup_apps_1p:"startup_apps_1p",
      update_preferences_1k:"update_preferences_1p",update_preferences_1l:"update_preferences_1p",update_preferences_1m:"update_preferences_1p",update_preferences_1n:"update_preferences_1p",update_preferences_1o:"update_preferences_1p",update_preferences_1p:"update_preferences_1p",
      update_history_1k:"update_history_1p",update_history_1l:"update_history_1p",update_history_1m:"update_history_1p",update_history_1n:"update_history_1p",update_history_1o:"update_history_1p",update_history_1p:"update_history_1p",
      focus_sessions_1k:"focus_sessions_1p",focus_sessions_1l:"focus_sessions_1p",focus_sessions_1m:"focus_sessions_1p",focus_sessions_1n:"focus_sessions_1p",focus_sessions_1o:"focus_sessions_1p",focus_sessions_1p:"focus_sessions_1p",
      feedback_1n:"feedback_1p",feedback_1o:"feedback_1p",feedback_1p:"feedback_1p",
      routines_1n:"routines_1p",routines_1o:"routines_1p",routines_1p:"routines_1p",
      accessibility_1n:"accessibility_1p",accessibility_1o:"accessibility_1p",accessibility_1p:"accessibility_1p",
      welcome_1n_at_login:"home_1p_at_login",home_1o_at_login:"home_1p_at_login",home_1p_at_login:"home_1p_at_login",
      notification_preferences_1o:"notification_preferences_1p",notification_preferences_1p:"notification_preferences_1p",
      session_state_1o:"session_state_1p",session_state_1p:"session_state_1p"
    })[category]||category;
  }
  ...
  function mergeMigratedFiles(...groups){
    ...
    groups.flat().filter(Boolean).forEach(raw=>{
      ...
      if(sameId)f.id=`${f.id}_1p_${Math.random().toString(36).slice(2,8)}`;
  ...
  async function readLegacyCloudFiles(fbm){
    ...
    for(const path of paths){try{const snap=await fbm.getDoc(fbm.doc(fbm.db,...path));if(snap.exists())groups.push(collectMigratedFiles(snap.data()))}catch(error){console.debug("Gold 1P legacy cloud source skipped",path.join("/"),error?.message)}}
    return groups.flat();
  }
  async function restoreWorkspace1I(show=true){
    try{
      ...
      if(summary){
        const splitSchemas=new Set(["gold1h-split-v1","gold1h-split-v2","gold1i-split-v1","gold1i-split-v2","gold1j-split-v1","gold1j-split-v2","gold1k-split-v1","gold1k-split-v2","gold1l-split-v1","gold1l-split-v2","gold1m-split-v1","gold1m-split-v2","gold1n-split-v1","gold1n-split-v2","gold1o-split-v1","gold1o-split-v2","gold1p-split-v1","gold1p-split-v2"]);
        if(splitSchemas.has(summary.schema)){
          for(const sourceCategory of summary.categories||CLOUD_CATEGORIES){
            ...
            if(category==="files")fileGroups.push(collectMigratedFiles(value));
              ...
              const key=category==="registry_1p"?REGISTRY_KEY:category==="clipboard_history_1p"?CLIPBOARD_KEY:category==="startup_apps_1p"?STARTUP_KEY:PREFIX+category;
              writeJSON(key,value);
            }
          }
        }else{
          fileGroups.push(collectMigratedFiles(summary));
          const legacyMap={"prefs":"prefs","files":"files","folders":"folders","mail":"mail","notes":"notes","tasks":"tasks","events":"events","contacts":"contacts","tickets":"tickets","notifications":"notifications","user_apps":"user_apps","remote_sessions":"remote_sessions","emergency_logs":"emergency_logs","vm_snapshots":"vm_snapshots","custom_app_logos":"custom_app_logos","app_verification_cache":"app_verification_cache","elsus_original_state":"elsus_original_state","elsus_shell_state":"elsus_shell_state","sticky_notes":"sticky_notes","voice_recordings":"voice_recordings","registry_1h":"registry_1p","registry_1h_history":"registry_1p_history","clipboard_history_1h":"clipboard_history_1p","startup_apps_1h":"startup_apps_1p","registry_1i":"registry_1p","registry_1i_history":"registry_1p_history","clipboard_history_1i":"clipboard_history_1p","startup_apps_1i":"startup_apps_1p","registry_1j":"registry_1p","registry_1j_history":"registry_1p_history","clipboard_history_1j":"clipboard_history_1p","startup_apps_1j":"startup_apps_1p","registry_1k":"registry_1p","registry_1k_history":"registry_1p_history","clipboard_history_1k":"clipboard_history_1p","startup_apps_1k":"startup_apps_1p","registry_1l":"registry_1p","registry_1l_history":"registry_1p_history","clipboard_history_1l":"clipboard_history_1p","startup_apps_1l":"startup_apps_1p","registry_1m":"registry_1p","registry_1m_history":"registry_1p_history","clipboard_history_1m":"clipboard_history_1p","startup_apps_1m":"startup_apps_1p","registry_1n":"registry_1p","registry_1n_history":"registry_1p_history","clipboard_history_1n":"clipboard_history_1p","startup_apps_1n":"startup_apps_1p","registry_1o":"registry_1p","registry_1o_history":"registry_1p_history","clipboard_history_1o":"clipboard_history_1p","startup_apps_1o":"startup_apps_1p","registry_1p":"registry_1p","registry_1p_history":"registry_1p_history","clipboard_history_1p":"clipboard_history_1p","startup_apps_1p":"startup_apps_1p","update_preferences_1k":"update_preferences_1p","update_history_1k":"update_history_1p","focus_sessions_1k":"focus_sessions_1p","update_preferences_1l":"update_preferences_1p","update_history_1l":"update_history_1p","focus_sessions_1l":"focus_sessions_1p","update_preferences_1m":"update_preferences_1p","update_history_1m":"update_history_1p","focus_sessions_1m":"focus_sessions_1p","update_preferences_1n":"update_preferences_1p","update_history_1n":"update_history_1p","focus_sessions_1n":"focus_sessions_1p","update_preferences_1o":"update_preferences_1p","update_history_1o":"update_history_1p","focus_sessions_1o":"focus_sessions_1p","update_preferences_1p":"update_preferences_1p","update_history_1p":"update_history_1p","focus_sessions_1p":"focus_sessions_1p","feedback_1n":"feedback_1p","routines_1n":"routines_1p","accessibility_1n":"accessibility_1p","feedback_1o":"feedback_1p","routines_1o":"routines_1p","accessibility_1o":"accessibility_1p","feedback_1p":"feedback_1p","routines_1p":"routines_1p","accessibility_1p":"accessibility_1p","welcome_1n_at_login":"home_1p_at_login","home_1o_at_login":"home_1p_at_login","home_1p_at_login":"home_1p_at_login","notification_preferences_1o":"notification_preferences_1p","notification_preferences_1p":"notification_preferences_1p","session_state_1o":"session_state_1p","session_state_1p":"session_state_1p"};
          Object.entries(legacyMap).forEach(([source,category])=>{if(summary[source]===undefined)return;const key=category==="registry_1p"?REGISTRY_KEY:category==="clipboard_history_1p"?CLIPBOARD_KEY:category==="startup_apps_1p"?STARTUP_KEY:PREFIX+category;writeJSON(key,summary[source])});
        }
      }
      ...
      writeJSON(PREFIX+"migration_1p_cloud_report",{completed:true,completedAt:now(),sourceSchema:summary?.schema||"none",filesBefore:Array.isArray(localFiles)?localFiles.length:0,filesAfter:mergedFiles.length,firestoreReleasePointerChanged:false});
      try{window.Gold50?.setPrefs?.(readJSON(PREFIX+"prefs",{}));window.Gold50?.saveFiles?.(mergedFiles)}catch{}
      if(show)notify("Cloud VM migration complete",`${mergedFiles.length} files are available in Gold 1P. The E.L.S.U.S. release pointer was not changed.`,"Gold VM");
      return summary||{schema:"legacy-migration",files:mergedFiles.length};
    }catch(error){console.warn("Gold 1P cloud restore failed",error);if(show)notify("Restore failed",error.message,"Gold VM");return null}
  }
  ...
  function customToolButtons(){return `
    <button class="gold1p-start-tile" onclick="Gold1P.openRegistryEditor()"><span>▥</span><b>Registry Editor</b></button>
    <button class="gold1p-start-tile" onclick="Gold1P.openTaskManager()"><span>▤</span><b>Task Manager</b></button>
    <button class="gold1p-start-tile" onclick="Gold1P.openControlPanel()"><span>⚙</span><b>Control Panel</b></button>
    <button class="gold1p-start-tile" onclick="Gold1P.openRun()"><span>▦</span><b>Run</b></button>
    <button class="gold1p-start-tile" onclick="Gold1P.openClipboardHistory()"><span>▧</span><b>Clipboard</b></button>
    ...
    ${isStaff()?'<button class="gold1p-start-tile" onclick="Gold1P.openPublisherManager()"><span>UP</span><b>Publisher</b></button>':''}` }
  ...
  function quickAction(action,button){
    ...
    if(action==="safe"&&confirm("Restart EmeraldOS Gold 1P in Safe Mode?")){localStorage.setItem(PREFIX+"safemode","true");location.reload()}
  }
  ...
  /* ------------------------- Update setup ------------------------- */
  function updateSetupKey(){return `emeraldos_gold_1p_update_setup_done_${normalizeUser(userName())}`}
  ...
  async function finishUpdateSetup(){
    ...
    try{const fbm=await import("./firebase.js");await fbm.setDoc(fbm.doc(fbm.db,"emeraldOSUsers",vmUserId(),"goldVM","setup"),{firstBootCompleted:true,firstBootScope:"global-elsus-user",lastCompletedVersion:VERSION,lastUpdateSetupVersion:VERSION,lastSetupType:"update",licenseAccepted:true,licenseVersion:"Gold_1P",folder:FOLDER,updatedAt:now()},{merge:true})}catch(error){console.warn("Gold 1P update setup cloud save failed",error)}
    notify("EmeraldOS Gold 1P is ready","Your existing cloud VM data and preferences were preserved.","Setup");
  }
  function maybeShowUpdateSetup(){
    ...
    const overlay=document.createElement("section");overlay.id="gold1pUpdateSetup";overlay.className="gold1p-update-setup";overlay.innerHTML=`<div class="gold1p-update-card"><h1>Welcome to EmeraldOS Gold 1P</h1><p>Your existing E.L.S.U.S. cloud VM is being preserved while Gold 1P restores your previous workspace quietly and improves everyday usability.</p><ul><li>Quiet startup with one consolidated status banner</li><li>Cloud workspace resume and safer session continuity</li><li>Backup &amp; Sync with safe merge restore</li><li>File Compatibility Center for previous Gold file versions</li><li>System Health and Accessibility Center</li><li>Original E.L.S.U.S. and Shell V2 compatibility</li><li>Staff Center, verified user apps, custom logos, and less intrusive update prompts</li><li>Settings, toolbar, table, card, and narrow-window collision fixes</li></ul><p>Files, folders, settings, preferences, Registry data, applications, mail, and all available VM categories are merged and validated before cloud autosave can begin.</p><div class="actions"><button class="gold1p-button" onclick="document.getElementById('gold1pUpdateSetup').remove()">Remind me later</button><button class="gold1p-button primary" onclick="Gold1P.finishUpdateSetup()">Continue</button></div></div>`;document.body.appendChild(overlay);
  }
  ...
  function branding(){
    document.title="EmeraldOS Gold 1P";document.body.classList.add("gold1p","gold1p-shell");document.body.dataset.build=VERSION;
    qa("h1,h2,p,span").forEach(el=>{if(el.children.length===0&&el.textContent?.includes("EmeraldOS Gold 1G"))el.textContent=el.textContent.replaceAll("EmeraldOS Gold 1G","EmeraldOS Gold 1P")});
  }
  ...
  function init(){
    ...
    setInterval(()=>{if(window.__GOLD1P_MIGRATION_BLOCK_SAVES__!==true)saveWorkspaceNow1I(false)},60000);
    window.addEventListener("beforeunload",()=>{writeJSON(PREFIX+"workspace_meta_1p",{version:VERSION,folder:FOLDER,savedAt:now(),openWindows:currentWindowState()})});
