...
========================================================= */
(function EmeraldOSGold1PPolish(){
  if(window.__EMERALDOS_GOLD_1P_POLISH__)return;
  window.__EMERALDOS_GOLD_1P_POLISH__=true;

  const VERSION="1P",PREFIX="gold1g_";
  const SESSION_KEY=PREFIX+"session_state_1p";
  const NOTIFICATION_PREFS_KEY=PREFIX+"notification_preferences_1p";
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1P continuity cache write failed",key,error);return false}};
  ...
  function captureSessionState(){
    ...
    const state={
      ...
      windows,
      virtualDesktop:read(PREFIX+"virtual_desktop_1p",read(PREFIX+"virtual_desktop_1o",1)),
  ...
  function previousSessionState(){
    ...
    const meta=read(PREFIX+"workspace_meta_1o",null)||read(PREFIX+"workspace_meta_1p",null);
  ...
  async function waitForMigration(){
    ...
    while(window.__GOLD1P_MIGRATION_BLOCK_SAVES__===true&&Date.now()-started<maximumWait)await new Promise(resolve=>setTimeout(resolve,300));
  }
  ...
  async function restorePreviousWorkspace(){
    ...
    for(const id of apps){
      ...
      try{api()?.openApp?.(id)}catch(error){console.debug("Gold 1P skipped workspace app",id,error?.message)}
  ...
  function openStartupContinuity(){
    ...
    win.querySelector("#continuitySave").onclick=async()=>{save();await window.Gold1P?.saveWorkspaceNow?.(true)};
  ...
  function patchAPI(){
    ...
    const originalSave=window.Gold1P?.saveWorkspaceNow?.bind(window.Gold1P);
    if(originalSave&&!window.Gold1P.__gold1pSessionSavePatched){window.Gold1P.__gold1pSessionSavePatched=true;window.Gold1P.saveWorkspaceNow=async show=>{captureSessionState();return originalSave(show)}}
    Object.assign(window.Gold1P||core,{openStartupContinuity,captureSessionState,restorePreviousWorkspace,notificationPreferences,setNotificationPreferences});
  ...
  async function finishStartup(){
    ...
    if(prefs.showStartupSummary!==false&&!$("gold1pActiveUpdatePrompt")&&!$("gold1pUpdateSetup")&&(restored||suppressed)){
      api()?.notify?.("Welcome back",`${restored?`${restored} application${restored===1?"":"s"} restored. `:""}${suppressed?`${suppressed} startup notice${suppressed===1?" was":"s were"} kept in Action Center.`:"Your cloud VM is ready."}`,"EmeraldOS Gold 1P");
    }
    captureSessionState();
  }
