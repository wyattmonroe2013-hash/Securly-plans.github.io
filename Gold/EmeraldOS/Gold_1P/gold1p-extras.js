...
========================================================= */
(function EmeraldOSGold1PExtras(){
  if(window.__EMERALDOS_GOLD_1P_EXTRAS__) return;
  window.__EMERALDOS_GOLD_1P_EXTRAS__=true;

  const VERSION="1P";
  const FOLDER="Gold_1P";
  const UPDATE_SERVICE="Original E.L.S.U.S.";
  const PREFIX="gold1g_";
  const PREFS_KEY=PREFIX+"update_preferences_1p";
  const HISTORY_KEY=PREFIX+"update_history_1p";
  const SNOOZE_KEY=PREFIX+"update_snooze_1p";
  const LAST_PROMPT_KEY=PREFIX+"update_last_prompt_1p";
  const LAST_CHECK_KEY=PREFIX+"update_last_check_1p";
  const FOCUS_STATE_KEY=PREFIX+"focus_sessions_1p";
  const LAST_NOTIFIED_VERSION_KEY=PREFIX+"update_last_notified_version_1p";
  ...
  async function fetchLatestManifest(source="manual"){
    ...
    try{
      ...
      console.debug("Gold 1P update check used local manifest cache",error?.message||error);
    }
  ...
  async function beginUserUpdate(manifest){
    ...
    try{
      if(window.Gold1P?.requestVersionChange){await window.Gold1P.requestVersionChange(manifest);return true}
      if(window.Gold1P?.saveWorkspaceNow)await window.Gold1P.saveWorkspaceNow(false);
      window.Gold1P?.openUpdateCenter?.();
      return true;
    }catch(error){
  ...
  async function startLiveListener(){
    ...
    try{
      ...
      },error=>console.debug("Gold 1P live update listener unavailable",error?.message||error));
    }catch(error){console.debug("Gold 1P live update listener skipped",error?.message||error)}
  }

  function openUpdateStatus(manifest=latestManifest){
    const core=api();if(!core?.openWindow)return window.Gold1P?.openUpdateCenter?.(manifest);
    ...
    win.querySelector("#spOpenUpdateCenter").onclick=()=>window.Gold1P?.openUpdateCenter?.(currentManifest);
  ...
  function patchPublicAPI(){
    window.Gold1P=window.Gold1P||api()||{};
    Object.assign(window.Gold1P,{elsusMode:"original",checkForUpdates,showUpdatePrompt,openUpdateStatus,openUpdatePreferences:openUpdateStatus,openFocusSessions,quickSettings:quickSettingsPanel,latestManifest:()=>latestManifest,updatePreferences:updatePrefs});
  }
  ...
  function init(){
    ...
    notify("EmeraldOS Gold 1P","Update checks, Focus Sessions, and Quick Settings loaded.","System");
  }
