"use strict";
/* EmeraldOS Gold 1P flagship experience.
...
(function EmeraldOSGold1PFlagship(){
  if(window.__EMERALDOS_GOLD_1P_FLAGSHIP__) return;
  window.__EMERALDOS_GOLD_1P_FLAGSHIP__=true;
  ...
  const write=(key,value)=>{try{localStorage.setItem(key,JSON.stringify(value));return true}catch(error){console.warn("Gold 1P write failed",key,error);return false}};
  ...
  function notification(title,body,app="Gold 1P"){api()?.notify?.(title,body,app)}
  ...
  function healthReport(){
    ...
    const migration=read(PREFIX+"migration_1p_cloud_report",read(PREFIX+"migration_1p_local_report",{}));
    const checks=[
      ...
      {name:"Migration protection",pass:window.__GOLD1P_MIGRATION_BLOCK_SAVES__!==true,detail:window.__GOLD1P_MIGRATION_BLOCK_SAVES__===true?"Cloud saves remain blocked until migration is verified.":"Migration gate is open."},
      ...
      {name:"Previous-format compatibility",pass:Boolean(window.Gold1PFileCompatibility),detail:"Gold 1P compatibility registry is active."},
  ...
  function clipboardItems(){
    const source=read(PREFIX+"clipboard_history_1p",read(PREFIX+"clipboard_history_1n",[]));
  ...
  function saveClipboard(items){write(PREFIX+"clipboard_history_1p",items.slice(0,100));api()?.saveWorkspaceNow?.(false)}
  ...
  function mergeImportedFiles(incoming=[]){
    ...
    incoming.forEach(raw=>{if(!raw||typeof raw!=="object")return;const file={...raw,id:String(raw.id||uid("import")),name:String(raw.name||"Imported File"),folder:String(raw.folder||"Documents"),type:String(raw.type||"text"),content:String(raw.content??""),updated:raw.updated||now(),created:raw.created||now(),trash:Boolean(raw.trash)};const signature=`${file.name.toLowerCase()}|${file.folder.toLowerCase()}|${file.type}|${file.content}`;if(signatures.has(signature))return;if(ids.has(file.id))file.id=`${file.id}_1p_${Math.random().toString(36).slice(2,7)}`;ids.add(file.id);signatures.add(signature);merged.push(file)});return merged;
  }
  ...
  function openCompatibilityCenter(){
    ...
    win.querySelector("#compatExport").onclick=()=>download("EmeraldOS-Gold-1P-File-Compatibility.json",compatibilityScan());
  ...
  window.Gold1PFileCompatibility={scan:compatibilityScan,extensionTypes};

  function openSystemHealth(){
    const report=healthReport();
    const html=`<div class="app-shell gold1p-health"><div class="app-toolbar"><button class="button primary" id="healthRun">Run checks again</button><button class="button" id="healthSave">Save VM now</button><button class="button" id="healthExport">Export diagnostics</button><button class="button danger" id="healthCleanup">Clear safe caches</button></div><div class="app-body"><div class="gold1p-page-title"><div><h1>System Health</h1><p>Clear status and repair guidance for your Gold virtual machine.</p></div><span class="gold1p-status-pill ${report.passed===report.total?"good":"review"}">${report.passed}/${report.total} passed</span></div><div class="grid4"><div class="card"><b>Files</b><h2>${report.files}</h2></div><div class="card"><b>Applications</b><h2>${report.apps}</h2></div><div class="card"><b>Local storage</b><h2>${fmtBytes(report.storageBytes)}</h2></div><div class="card"><b>Migration</b><h2>${window.__GOLD1P_MIGRATION_BLOCK_SAVES__===true?"Protected":"Ready"}</h2></div></div><div class="gold1p-health-list">${report.checks.map(check=>`<article class="card ${check.pass?"pass":"review"}"><span class="gold1p-health-icon">${check.pass?"✓":"!"}</span><div><b>${esc(check.name)}</b><p>${esc(check.detail)}</p></div></article>`).join("")}</div></div></div>`;
    ...
    win.querySelector("#healthRun").onclick=refresh;win.querySelector("#healthSave").onclick=()=>api()?.saveWorkspaceNow?.(true);win.querySelector("#healthExport").onclick=()=>download("EmeraldOS-Gold-1P-Diagnostics.json",healthReport());
  ...
  function accessibility(){return {...{highContrast:false,reduceMotion:false,largeText:false,underlineLinks:false,focusRings:true,largeCursor:false,compact:false},...read(PREFIX+"accessibility_1p",{})}}
  ...
  function saveAccessibility(settings){write(PREFIX+"accessibility_1p",settings);applyAccessibility(settings);api()?.setPrefs?.({accessibility1P:settings});api()?.saveWorkspaceNow?.(false)}
  ...
  function routines(){const value=read(PREFIX+"routines_1p",[]);return Array.isArray(value)?value:[]}
  function saveRoutines(value){write(PREFIX+"routines_1p",value);api()?.saveWorkspaceNow?.(false)}
  ...
  function installShellPolish(){
    document.body.dataset.flagship="1P";const update=()=>{document.body.classList.toggle("gold1p-narrow",innerWidth<820);document.body.classList.toggle("gold1p-medium",innerWidth<1120)};update();window.addEventListener("resize",update,{passive:true});
  ...
  function ensureDefaultHomePreference(){if(localStorage.getItem(PREFIX+"home_1p_at_login")===null)write(PREFIX+"home_1p_at_login",true)}
  function maybeOpenHome(){if(read(PREFIX+"home_1p_at_login",true)!==true)return;if(sessionStorage.getItem("gold1p_home_opened")==="true")return;sessionStorage.setItem("gold1p_home_opened","true");setTimeout(()=>api()?.openApp?.("home"),2100)}
  function init(){
    ...
    notification("EmeraldOS Gold 1P ready","Gold Home, Command Palette, compatibility, backup, accessibility, and system health tools are available.","System");
  }
