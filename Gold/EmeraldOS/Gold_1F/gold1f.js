"use strict";
/* EmeraldOS Gold 1F - independent logos, staff-only tools, live remote desktop, EmeraldOS Gold UX polish */
(function(){
const PREFIX="gold1f_";
const BUILD={name:"EmeraldOS Gold 1F",version:"1F",cloudPath:"goldVM/current",bios:"Emerald BIOS A9",shellManaged:true,folder:"Gold_1F"};

/* Gold 1F E.L.S.U.S. compatibility helpers.
...
function migrateGold1ELocalData(){
  try{
    if(localStorage.getItem('gold1f_migrated_from_gold1e')==='true') return;
    const oldPrefix='gold1e_', newPrefix='gold1f_';
    ...
    if(localStorage.getItem('gold1e_setup_done')==='true'){
      localStorage.setItem('gold1f_updated_from','1E');
      setElsusFirstSetupDone();
    }
    // Update Setup must be run once for Gold 1F even if older data copied over.
    localStorage.removeItem('gold1f_update_setup_done');
    localStorage.removeItem('gold1f_license_agreed_update');
    localStorage.setItem('gold1f_migrated_from_gold1e','true');
  }catch(e){console.warn('Gold 1F migration from Gold 1E skipped',e)}
}
function migrateELSUSFirstBootFlags1F(){
  try{
  ...
  }catch(e){console.warn('Gold 1F E.L.S.U.S. setup flag migration skipped',e)}
}
function gold1fShouldRunFirstBootSetup(){return !elsusFirstSetupDone()}
function gold1fShouldRunUpdateSetup(){
  if(gold1fShouldRunFirstBootSetup()) return false;
  if(localStorage.getItem(PREFIX+'update_setup_done')==='true') return false;
  const oldKnown = !!(localStorage.getItem(PREFIX+'updated_from') || localStorage.getItem('gold1f_migrated_from_gold1e')==='true' || elsusLastVersion() && elsusLastVersion()!==BUILD.version);
  return oldKnown;
}
function gold1fBootSetupGate(){
  if(gold1fShouldRunFirstBootSetup()) { setTimeout(openSetup,350); return; }
  localStorage.setItem(PREFIX+'setup_done','true');
  if(gold1fShouldRunUpdateSetup()) setTimeout(openUpdateSetup1F,900);
}
function gold1fStaffGateNotice(){
...
/* EmeraldOS Gold 1F local data migration from Gold 1A.
   Cloud VM data remains version-independent under emeraldOSUsers/{username}/goldVM/current. */
function migrateGold1ALocalData(){
  try{
    if(localStorage.getItem('gold1f_migrated_from_gold1a')==='true') return;
    const oldPrefix='gold1a_', newPrefix='gold1f_';
    Object.keys(localStorage).forEach(k=>{
      if(k.startsWith(oldPrefix)){
        const nk=newPrefix+k.slice(oldPrefix.length);
        if(localStorage.getItem(nk)===null) localStorage.setItem(nk, localStorage.getItem(k));
    ...
    localStorage.setItem('gold1f_migrated_from_gold1a','true');
  }catch(e){console.warn('Gold 1F migration skipped', e)}
}
...
async function initFirebase(){try{fb=await import("./firebase.js");return fb}catch(e){console.warn("Gold 1F Firebase unavailable",e);return null}}
...
function defaultFiles(){return[
 {id:uid(),name:"Welcome to EmeraldOS Gold 1F.edoc",type:"doc",folder:"Documents",star:true,content:"<h1>Welcome to EmeraldOS Gold 1F</h1><p>This version improves setup, support, Staff Edition, Emerald BIOS, apps, logos, settings, and EmeraldOS Gold usability.</p>",created:now(),updated:now(),trash:false},
...
function mail(){return read(PREFIX+"mail",{inbox:[{id:uid(),from:"system@gold.mail",to:mailAddress(),subject:"Welcome to Gold Mail",body:"Gold Mail is ready in EmeraldOS Gold 1F.",read:false,time:now()}],sent:[],drafts:[],trash:[]})}function saveMail(m){write(PREFIX+"mail",m);renderNotifBadge();saveWorkspaceDebounced()}
function notes(){return read(PREFIX+"notes",[])}function saveNotes(n){write(PREFIX+"notes",n);saveWorkspaceDebounced()}
function tasks(){return read(PREFIX+"tasks",[{id:uid(),text:"Explore EmeraldOS Gold 1F",done:false,priority:"Normal",due:""}])}function saveTasks(t){write(PREFIX+"tasks",t);saveWorkspaceDebounced()}
...
APPS.push(
 {id:"help",name:"Get Help",label:"GH",color:"blue",group:"Support",desc:"Gold 1F help, tips, guides, and shortcuts.",open:openHelp},
...
function staffPreview(){notify("Staff login required","Open Staff Edition and sign in with a verified staff account.","Staff Edition");gold1fStaffGateNotice()}
...
function staffViewRemoteDesktop(sessionId){if(!isStaff()){gold1fStaffGateNotice();return}const w=openWindow('remoteview_'+sessionId,'Live Remote Desktop',remoteDesktopPreviewHTML(sessionId),{width:980,height:720,singleton:false});const int=setInterval(()=>{if(!document.body.contains(w)){clearInterval(int);return}const c=w.querySelector('.win-content');if(c)c.innerHTML=remoteDesktopPreviewHTML(sessionId)},1500)}
...
function openBIOSOptions(){openWindow("bios","BIOS Options",`<h2>Emerald BIOS A9</h2><div class="bios-embed"><p>Emerald BIOS options are available at startup using F12.</p><div class="toolbar"><button class="btn primary" onclick="location.href='bios.html'">Open full BIOS</button><button class="btn" onclick="localStorage.setItem('gold1f_safemode','true');location.reload()">Boot Safe Mode</button><button class="btn" onclick="localStorage.removeItem('gold1f_setup_done');location.reload()">Run Setup</button><button class="btn" onclick="location.href='staff.html'">Staff Edition</button></div></div>`,{width:720,height:420})}
...
function openSetup(){
  ...
  function setupHero(){return `<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>Welcome to EmeraldOS Gold 1F</h2><p class="muted">Set up your Gold virtual machine, personalize your desktop, choose your apps, and connect support tools.</p></div></div><div class="setup-checklist"><span>Account protected</span><span>Workspace continuity</span><span>Emerald support ready</span></div></div>`}
...
function finishSetup(){localStorage.setItem(PREFIX+'setup_done','true');$("setupWizard")?.classList.add('hidden');notify('Setup complete','EmeraldOS Gold 1F is ready.','Setup')}
...
function openNotepad(){openWindow('notepad','Notepad',`<h2>Notepad</h2><textarea id="np" class="field" rows="14">${esc(read(PREFIX+'notepad',''))}</textarea><button class="btn primary" onclick="localStorage.setItem('gold1f_notepad',JSON.stringify(np.value));Gold50.notify('Saved','Notepad saved','Notepad')">Save</button><button class="btn" onclick="Gold50.saveText('notepad.txt',np.value)">Export</button>`,{width:620,height:520})}
...
function openCalendar(){openWindow('calendar','Calendar',`<h2>Calendar</h2><p>${new Date().toDateString()}</p><textarea class="field" rows="8" onchange="localStorage.setItem('gold1f_calendar',JSON.stringify(this.value))">${esc(read(PREFIX+'calendar',''))}</textarea>`,{width:550,height:420})}
...
function openSecurity(){openWindow('security','Security Center',`<h2>Security Center</h2><div class="grid3"><div class="card"><h3>Safe Mode</h3><p>${localStorage.getItem(PREFIX+'safemode')==='true'?'Enabled':'Disabled'}</p><button class="btn" onclick="localStorage.setItem('gold1f_safemode','true');location.reload()">Enable</button><button class="btn" onclick="localStorage.removeItem('gold1f_safemode');location.reload()">Disable</button></div><div class="card"><h3>Blocked Users</h3><p>${read(PREFIX+'blocked',[]).length}</p></div><div class="card"><h3>Staff Session</h3><p>${isStaff()?'Active':'Not active'}</p></div></div>`,{width:760,height:480})}
...
function officeNew(type){
  ...
  if(type==='sheet')content=JSON.stringify([['Item','Amount','Notes'],['Apps','30','Gold 1F'],['Tickets','5','Support'],['Total','=SUM(B2:B3)','']]);
...
let saveTimer=null;function saveWorkspaceDebounced(){clearTimeout(saveTimer);saveTimer=setTimeout(()=>saveWorkspaceNow(false),700)}async function saveWorkspaceNow(show=true){const data={prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs(),savedAt:now(),build:BUILD.name};write(PREFIX+'workspace',data);if(fb?.setDoc&&fb?.doc&&fb?.db){try{await fb.setDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'),data)}catch(e){console.warn('cloud save failed',e)}}if(show)notify('Workspace saved','Gold 1F workspace saved locally and attempted cloud sync.','Restore Center')}
async function restoreWorkspace(show=true){let data=read(PREFIX+'workspace',null);if(fb?.getDoc&&fb?.doc&&fb?.db){try{const snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'));if(snap.exists())data=snap.data()}catch(e){console.warn('cloud restore failed',e)}}if(data){['prefs','files','tickets','mail','notes','tasks','contacts','notifications'].forEach(k=>{if(data[k])write(PREFIX+k,data[k])});applyPrefs();renderDesktop();renderNotifBadge();if(show)notify('Workspace restored','Gold 1F workspace restored.','Restore Center')}}function exportWorkspace(){saveText('emeraldos-gold-8.0-workspace.json',JSON.stringify({prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs()},null,2),'application/json')}function importWorkspacePrompt(){const txt=prompt('Paste workspace JSON');if(!txt)return;try{const data=JSON.parse(txt);Object.keys(data).forEach(k=>write(PREFIX+k,data[k]));location.reload()}catch{alert('Invalid JSON')}}function resetGold(){if(confirm('Reset local EmeraldOS Gold 1F data?')){Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)).forEach(k=>localStorage.removeItem(k));location.reload()}}
function openApp(id){closeOverlays();const app=appById(id);if(app.staffOnly&&!isStaff()){gold1fStaffGateNotice();return}(app.open||openExplorer)()}function notifyPublic(t,b,a){return notify(t,b,a)}
...
========================================================= */
Object.assign(BUILD,{name:"EmeraldOS Gold 1F",version:"1F",cloudPath:"goldVM/current",bios:"Emerald BIOS A9"});
...
function applyPrefs(){const p=prefs();document.body.classList.toggle("dark",p.theme==="dark");document.body.classList.toggle("highcontrast",p.theme==="highcontrast");document.body.classList.toggle("bigtext",!!p.bigText);document.body.classList.toggle("no-motion",!!p.reducedMotion);document.body.classList.toggle("gold1f-solid-bg",!!p.useBackgroundColor);document.body.classList.toggle("gold1f-readable",!!p.readableFont);document.body.classList.toggle("gold1f-focus-ring",!!p.focusRings);document.body.classList.toggle("gold1f-large-cursor",!!p.largeCursor);document.body.classList.toggle("gold1f-underlined",!!p.underlineControls);document.body.classList.toggle("gold1f-extra-contrast",!!p.highContrastBorders);document.body.classList.toggle("gold1f-taskbar-top",p.taskbarPosition==="top");document.body.classList.toggle("gold1f-taskbar-compact",p.taskbarSize==="compact");document.body.classList.remove("bg-gold-bloom","bg-blue-window","bg-emerald-field","bg-slate-pro","custom-bg","gradient-aurora","gradient-skyline","gradient-emerald-pro","gradient-midnight","gradient-sunrise","gradient-ribbon","gradient-calm");if(p.customBackground){document.body.classList.add("custom-bg");document.body.style.setProperty("--customBg",`url(${p.customBackground})`);document.body.style.setProperty("--bgMode",p.bgMode||"cover");document.body.style.setProperty("--bgPosition",p.bgPosition||"center");document.body.style.setProperty("--bgRepeat",p.bgRepeat||"no-repeat")}else if(p.useBackgroundColor){document.body.style.setProperty("--gold1fBackground",p.backgroundColor||"#e9f3ff")}else{document.body.classList.add("gradient-"+(p.gradient||"aurora"));document.body.style.removeProperty("--customBg")}document.documentElement.style.setProperty("--accent",p.accent||"#0078d4");document.documentElement.style.setProperty("--accent2",p.secondaryAccent||"#0f7b4d");document.documentElement.style.setProperty("--gold70Secondary",p.secondaryAccent||"#0f7b4d");document.documentElement.style.setProperty("--winRadius",p.rounding==="none"?"0":p.rounding==="large"?"14px":p.rounding==="pill"?"20px":"6px");document.documentElement.style.setProperty("--gold1fTaskOpacity",String((Number(p.taskbarOpacity)||92)/100));document.documentElement.style.setProperty("--gold1fPanelOpacity",String((Number(p.panelOpacity)||94)/100));document.documentElement.style.setProperty("--gold1fMica",(Number(p.micaBlur)||0)+"px");document.documentElement.style.setProperty("--gold1fStartWidth",(Number(p.startWidth)||860)+"px");document.documentElement.style.setProperty("--gold1fTextScale",String(Number(p.textScale)||1));document.documentElement.style.setProperty("--gold1fBackground",p.backgroundColor||"#e9f3ff");document.documentElement.style.setProperty("--font",p.fontFamily||"Segoe UI");document.documentElement.style.setProperty("--gold1fIconScale",p.iconSize==="small"?".86":p.iconSize==="large"?"1.16":p.iconSize==="xl"?"1.32":"1");document.documentElement.style.setProperty("--gold1fDesktopGap",p.density==="compact"?"8px":p.density==="spacious"?"24px":"16px");const search=$("searchBtn"),widgets=$("widgetsBtn"),staff=$("staffBtn");if(search)search.style.display=p.taskbarSearch?"":"none";if(widgets)widgets.style.display=p.taskbarWidgets?"":"none";if(staff)staff.style.display=p.taskbarStaff?"":"none";}

function gold1fLocalState(){const localKeys={};Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)||["loggedIn","username","role","role2","userId"].includes(k)).forEach(k=>localKeys[k]=localStorage.getItem(k));return{build:BUILD.name,version:BUILD.version,savedAt:now(),user:username(),prefs:prefs(),files:files(),tickets:tickets(),mail:mail(),notes:notes(),tasks:tasks(),contacts:contacts(),notifications:notifs(),userApps:read(PREFIX+'user_apps',[]),remoteSessions:read(PREFIX+'remote_sessions',[]),emergencyLogs:read(PREFIX+'emergency_logs',[]),snapshots:read(PREFIX+'vm_snapshots',[]),openWindows:[...document.querySelectorAll('.window')].map(w=>({id:w.id,title:w.dataset.title,left:w.style.left,top:w.style.top,width:w.style.width,height:w.style.height,max:w.classList.contains('max'),min:w.classList.contains('minimized')})),localKeys};}
async function gold1fCloudWrite(data){if(!prefs().cloudSync)return false;if(!fb)await initFirebase();if(fb?.setDoc&&fb?.doc&&fb?.db){try{await fb.setDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'),data);await fb.setDoc(fb.doc(fb.db,'emeraldOSGoldVMStates',username()),data);localStorage.setItem(PREFIX+'cloud_last_save',now());localStorage.setItem(PREFIX+'cloud_status','saved');return true}catch(e){console.warn('Gold 8 cloud save failed',e);localStorage.setItem(PREFIX+'cloud_status','save failed: '+(e.message||e));return false}}localStorage.setItem(PREFIX+'cloud_status','firebase unavailable');return false;}
async function gold1fCloudRead(){if(!fb)await initFirebase();if(fb?.getDoc&&fb?.doc&&fb?.db){try{let snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSUsers',username(),'gold8','current'));if(snap.exists()){localStorage.setItem(PREFIX+'cloud_status','restored');return snap.data()}snap=await fb.getDoc(fb.doc(fb.db,'emeraldOSGoldVMStates',username()));if(snap.exists()){localStorage.setItem(PREFIX+'cloud_status','restored');return snap.data()}}catch(e){console.warn('Gold 8 cloud restore failed',e);localStorage.setItem(PREFIX+'cloud_status','restore failed: '+(e.message||e))}}return null;}
...
async function saveWorkspaceNow(show=true){const data=gold1fLocalState();write(PREFIX+'workspace',data);const cloud=await gold1fCloudWrite(data);if(show)notify('Gold VM saved',cloud?'Everything was saved to the cloud VM state.':'Saved locally. Cloud sync will retry when Firebase is available.','Gold VM');return data;}
async function restoreWorkspace(show=true){let data=null;if(prefs().restoreOnLogin!==false)data=await gold1fCloudRead();if(!data)data=read(PREFIX+'workspace',null);if(data){if(data.localKeys)Object.entries(data.localKeys).forEach(([k,v])=>{if(v!==null&&v!==undefined)localStorage.setItem(k,String(v))});['prefs','files','tickets','mail','notes','tasks','contacts','notifications'].forEach(k=>{if(data[k])write(PREFIX+k,data[k])});if(data.userApps)write(PREFIX+'user_apps',data.userApps);if(data.remoteSessions)write(PREFIX+'remote_sessions',data.remoteSessions);if(data.emergencyLogs)write(PREFIX+'emergency_logs',data.emergencyLogs);if(data.snapshots)write(PREFIX+'vm_snapshots',data.snapshots);applyPrefs();renderDesktop();renderNotifBadge();if(show)notify('Gold VM restored','Your apps, files, settings, support history, and customizations were restored.','Gold VM')}else if(show)notify('No VM state found','Use setup or restore a backup to begin.','Gold VM')}
function exportWorkspace(){saveText('emeraldos-gold-8.0-cloud-vm-backup.json',JSON.stringify(gold1fLocalState(),null,2),'application/json')}
function resetGold(){if(confirm('Reset this EmeraldOS Gold 1F VM on this browser? Cloud data is not deleted.')){Object.keys(localStorage).filter(k=>k.startsWith(PREFIX)).forEach(k=>localStorage.removeItem(k));location.reload()}}
function gold1fCloudStatusHTML(){return `<div class="grid4"><div class="card quick-card"><h3>Cloud sync</h3><p>${esc(localStorage.getItem(PREFIX+'cloud_status')||'ready')}</p></div><div class="card quick-card"><h3>Last save</h3><p>${esc(localStorage.getItem(PREFIX+'cloud_last_save')||'not yet')}</p></div><div class="card quick-card"><h3>VM path</h3><p>emeraldOSUsers/${esc(username())}/goldVM/current</p></div><div class="card quick-card"><h3>Tracked data</h3><p>apps, files, settings, tickets, mail, and workspace</p></div></div>`}
...
/* Gold 1F boot fix: these Gold 8 handlers are scoped inside seedGold50, so expose and install them when seedGold50 runs. */
  try{
    ...
    document.body.classList.add('gold1f','gold1f-shell');
    installGold80Apps();
    applyPrefs();
    if(!window.__gold1fAutoSaveInstalled){
      window.__gold1fAutoSaveInstalled=true;
      setInterval(()=>{const n=Number(prefs().autosaveInterval)||15;if(prefs().cloudSync!==false)saveWorkspaceNow(false)},15000);
    }
  }catch(e){console.error('Gold 1F handler install failed',e)}



/* EmeraldOS Gold 1F shell-managed update integration */
function gold1fShellManifest(){return read('emeraldGoldShell_latest',{latestVersion:'1F',folder:'Gold_1F',entry:'OS.html',releaseTitle:'EmeraldOS Gold 1F'});}
function gold1fVMState(){
  let cloudState={};
  try{cloudState=gold1fLocalState?gold1fLocalState():{};}catch(e){cloudState={};}
  return {
...
function gold1fPostState(reason='heartbeat'){
  try{
    if(window.parent&&window.parent!==window){
      window.parent.postMessage({type:'emeraldos-gold-live-state',reason,state:gold1fVMState()},'*');
    }
  }catch(e){console.warn('Gold 1F shell state post failed',e);}
}
function gold1fAskShellUpdateCheck(){try{window.parent?.postMessage({type:'emeraldos-gold-check-update',from:'Gold_1F',version:BUILD.version},'*');notify('Update check requested','The EmeraldOS Gold Shell is checking Firebase for the latest build.','Gold Update Center')}catch(e){notify('Update check failed',e.message,'Gold Update Center')}}
function openGold1AUpdateCenter(){
  const m=gold1fShellManifest();
  const active=localStorage.getItem('emeraldGoldShell_activeVersion')||BUILD.version;
  openWindow('updateshell','Gold Update Center',`<div class="goldshell-header"><div><h2>Gold Update Center</h2><p class="muted">Shell-managed updates keep your Gold VM data separate from version folders.</p></div><button class="btn primary" onclick="Gold50.gold1fAskShellUpdateCheck()">Check for updates</button></div><div class="grid2"><div class="card"><h3>Running build</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1F</p></div><div class="card"><h3>Latest Firebase pointer</h3><p>${esc(m.latestVersion||'Unknown')}</p><p class="muted">${esc(m.folder||'No folder')} / ${esc(m.entry||'OS.html')}</p></div><div class="card"><h3>Update behavior</h3><p>Gold Shell reads <code>system/emeraldGoldLatest</code>, saves a VM snapshot, then loads the folder Firebase points to.</p></div><div class="card"><h3>VM data</h3><p>Preferences, files, tickets, mail, apps, support history, and workspace state stay in the cloud VM profile.</p></div></div><h3>Release note</h3><p>${esc(m.summary||m.releaseTitle||'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.saveWorkspaceNow(true);Gold50.gold1fPostState('manual-save')">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html'">Return to Gold Shell</button></div>`,{width:980,height:680});
}
function installGold100ShellFeatures(){
  ...
  window.addEventListener('message',ev=>{const d=ev.data||{};if(d.type==='emeraldos-gold-latest'){write('emeraldGoldShell_latest',d.latest||{});notify('Update pointer refreshed',`Latest: ${(d.latest&&d.latest.latestVersion)||'unknown'}`,'Gold Update Center')}if(d.type==='emeraldos-gold-shell-save-request'){gold1fPostState('shell-save-request')}});
  window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{gold1fPostState('boot');setInterval(()=>gold1fPostState('heartbeat'),8000)},1700));
}
...
function applyGold50Visuals(){
  const p=prefs();
  document.body.classList.toggle('gold1f-solid-bg',!!p.useBackgroundColor);
  document.body.classList.toggle('gold1f-readable',!!p.readableFont);
  document.body.classList.toggle('gold1f-focus-ring',!!p.focusRing);
  document.body.classList.toggle('gold1f-large-cursor',!!p.largeCursor);
  document.body.classList.toggle('gold1f-links-underlined',!!p.underlineLinks);
  document.body.classList.toggle('gold1f-extra-contrast',!!p.extraContrast);
  document.body.classList.toggle('gold1f-less-blur',p.windowTransparency===false);
  document.body.classList.toggle('gold1f-compact-start',p.startStyle==='compact');
  document.body.classList.remove('gold1f-filter-none','gold1f-filter-warm','gold1f-filter-cool','gold1f-filter-gray');
  document.body.classList.add('gold1f-filter-'+(p.colorFilter||'none'));
  document.documentElement.style.setProperty('--gold1fBackgroundColor',p.backgroundColor||'#edf2f8');
  document.documentElement.style.setProperty('--gold1fTextScale',((Number(p.textScale)||100)/100).toString());
  document.documentElement.style.setProperty('--gold1fTaskbarHeight',p.taskbarSize==='large'?'56px':p.taskbarSize==='compact'?'40px':'48px');
}
...
function settings50Page(tab){const p=prefs();
 ...
 if(tab==='update')return `<h2>Update & Security</h2><div class="grid2"><div class="card"><h3>EmeraldOS Gold 1F</h3><p>Development, accessibility, personalization, and EmeraldOS Gold user-experience update.</p></div><div class="card"><h3>Safe Mode</h3><button class="btn" onclick="Gold50.openApp('safemode')">Open Safe Mode</button></div></div>`;
...
function getGold50Packages(){return gold1fUserApps()}
...
window.addEventListener('message',e=>{const d=e.data||{};if(d.source!=='gold1f-package')return;if(d.type==='notify')notify(d.title||'Package App',d.message||'',d.app||'Package App');if(d.type==='export')saveText(d.name||'package-export.txt',d.text||'');if(d.type==='open')openApp(d.id||'settings')});
...
function applyGold70Visuals(){
  const p=prefs();
  document.body.classList.add('gold1f');
  ...
  document.documentElement.style.setProperty('--gold1fSecondary',p.secondaryAccent||'#0f7b4d');
  document.documentElement.style.setProperty('--gold1fTaskOpacity',String((Number(p.taskbarOpacity)||92)/100));
  document.documentElement.style.setProperty('--gold1fStartWidth',(Number(p.startWidth)||860)+'px');
  document.documentElement.style.setProperty('--gold1fIconScale',p.iconSize==='large'?'1.14':p.iconSize==='small'?'.88':'1');
  document.documentElement.style.setProperty('--gold1fDesktopGap',p.density==='compact'?'9px':p.density==='spacious'?'24px':'16px');
  document.documentElement.style.setProperty('--gold1fMica',(Number(p.micaBlur)||18)+'px');
}
...
function settings60Page(tab){const p=prefs();
  ...
  if(tab==='privacy')return `<h2>Privacy</h2><label class="setting-line"><span>Focus Assist</span><input type="checkbox" ${p.focusAssist?'checked':''} onchange="Gold50.setGold70Pref('focusAssist',this.checked)"></label><label class="setting-line"><span>Notifications</span><input type="checkbox" ${p.notifications!==false?'checked':''} onchange="Gold50.setGold70Pref('notifications',this.checked)"></label><label class="setting-line"><span>Remote assistance requires visible user consent</span><input type="checkbox" checked disabled></label><button class="btn" onclick="localStorage.removeItem('${PREFIX}search_history');Gold50.notify('Privacy','Search history cleared','Settings')">Clear local search history</button>`;
  if(tab==='update')return `<h2>Update & Security</h2><div class="grid2"><div class="card"><h3>EmeraldOS Gold 1F</h3><p>Personalization, Staff resources, Support, BIOS A9, and EmeraldOS Gold-style user friendliness.</p></div><div class="card"><h3>Safe Mode</h3><button class="btn" onclick="Gold50.openApp('safemode')">Open Safe Mode</button></div></div>`;
...
function openStaffCenter60(){if(!isStaff()){gold1fStaffGateNotice();return}openWindow('staff','Gold Staff Center',staffDashboard60(),{width:1120,height:760})}
...
function openStaffKnowledge60(){if(!isStaff()){gold1fStaffGateNotice();return}const q='';openWindow('staffkb','Staff Knowledge Base',staffKnowledgeHTML(q),{width:980,height:700});window.Gold50.searchStaffKB60=(v)=>{const b=document.querySelector('#win_staffkb .win-content');if(b)b.innerHTML=staffKnowledgeHTML(v)};}
...
function openStaffReports60(){if(!isStaff()){gold1fStaffGateNotice();return}const open=tickets().filter(t=>t.status!=='Resolved').length,res=tickets().filter(t=>t.status==='Resolved').length,remote=remoteSessions().length;openWindow('staffreports','Staff Reports',`<h2>Staff Reports</h2><div class="grid4"><div class="card"><h3>Open tickets</h3><p>${open}</p></div><div class="card"><h3>Resolved</h3><p>${res}</p></div><div class="card"><h3>Remote sessions</h3><p>${remote}</p></div><div class="card"><h3>Staff logs</h3><p>${read(PREFIX+'staff_logs',[]).length}</p></div></div><div class="toolbar"><button class="btn primary" onclick="Gold50.saveText('gold1f-tickets.json',JSON.stringify(Gold50.tickets?Gold50.tickets():[],null,2),'application/json')">Export Tickets</button><button class="btn" onclick="Gold50.staffLogs()">Open Staff Logs</button></div><h3>Recent logs</h3>${read(PREFIX+'staff_logs',[]).slice(0,8).map(l=>`<div class="card"><b>${esc(l.type)}</b><p>${esc(l.note)}</p><small>${new Date(l.time).toLocaleString()}</small></div>`).join('')||'<p class="muted">No logs.</p>'}`,{width:920,height:680})}
function openStaffBroadcast60(){if(!isStaff()){gold1fStaffGateNotice();return}openWindow('staffbroadcast','Staff Broadcast',`<h2>Staff Broadcast</h2><p class="muted">Send a visible local notification. Future versions can sync broadcasts through Firestore.</p><label>Title<input id="bc_title" class="field" value="Staff Broadcast"></label><label>Message<textarea id="bc_body" class="field" rows="6"></textarea></label><button class="btn primary" onclick="Gold50.sendStaffBroadcast60()">Send Broadcast</button>`,{width:700,height:520})}
...
function applyGold70SupportVisuals(){document.body.classList.add('gold1f-goldshell');const p=prefs();document.documentElement.style.setProperty('--gold1fTileOpacity',p.tileOpacity||'.94');document.documentElement.style.setProperty('--gold1fPanelRadius',p.goldshellRadius||'8px')}
...
/* EmeraldOS Gold 1F rebuilt update-system fallback.
   Some Gold 1F update helpers are created inside the seeded boot scope.
...
function gold1fRebuiltSnapshot(){
  return {
    product:'EmeraldOS Gold',
    version:BUILD.version,
    folder:'Gold_1F',
...
function gold1fRebuiltLocalManifest(){
  return read('emeraldGoldShell_latest',{latestVersion:'1F',build:'1F',folder:'Gold_1F',entry:'OS.html',releaseTitle:'EmeraldOS Gold 1F',summary:'EmeraldOS Gold update-system build.'});
}
async function gold1fRebuiltFetchLatest(){
  try{
    const fbm = await import('./firebase.js');
    const snap = await fbm.getDoc(fbm.doc(fbm.db,'system','emeraldGoldLatest'));
    const latest = snap.exists()?Object.assign(gold1fRebuiltLocalManifest(),snap.data()):gold1fRebuiltLocalManifest();
    write('emeraldGoldShell_latest',latest);
    return latest;
  }catch(e){
    console.warn('Gold 1F update check Firebase unavailable',e);
    return gold1fRebuiltLocalManifest();
  }
}
function gold1fRebuiltSaveBeforeUpdate(){
  try{saveWorkspaceNow(false)}catch(e){console.warn(e)}
  try{write('emeraldGoldShell_liveState',gold1fRebuiltSnapshot())}catch(e){console.warn(e)}
}
async function gold1fRebuiltCheckForUpdates(){
  const latest = await gold1fRebuiltFetchLatest();
  const current = BUILD.version;
  const folder = latest.folder || 'Gold_1F';
  const version = latest.latestVersion || latest.build || '1B';
  const message = version===current && folder==='Gold_1F'
...
function gold1fRebuiltApplyUpdate(){
  gold1fRebuiltSaveBeforeUpdate();
  localStorage.setItem('emeraldGoldShell_forceCheck','true');
  localStorage.setItem('emeraldGoldShell_returnedFromVersion','Gold_1F');
  location.href='../gold-shell.html?force=1';
}
function openGold1AUpdateCenterRebuilt(manifest){
  const m=manifest||gold1fRebuiltLocalManifest();
  const version=m.latestVersion||m.build||'1B';
  const folder=m.folder||'Gold_1F';
  const entry=m.entry||'OS.html';
  const currentFolder='Gold_1F';
  const isDifferent=(version!==BUILD.version)||(folder!==currentFolder);
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">EmeraldOS Gold uses a root updater that checks Firebase, saves the VM, and boots the configured version folder. The updater is not visible after boot.</p></div><button class="btn primary" onclick="Gold50.gold1fCheckForUpdates()">Check now</button></div><div class="grid2"><div class="card"><h3>Current VM</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: ${currentFolder}</p></div><div class="card"><h3>Configured latest</h3><p>${esc(version)}</p><p class="muted">${esc(folder)} / ${esc(entry)}</p></div><div class="card"><h3>Status</h3><p>${isDifferent?'An update or folder change is available.':'No newer folder is currently configured.'}</p></div><div class="card"><h3>VM continuity</h3><p>Before applying an update, Gold saves a workspace snapshot so your files, settings, apps, tickets, and desktop state can continue.</p></div></div><h3>Release note</h3><p>${esc(m.summary||m.releaseTitle||'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.gold1fSaveUpdateSnapshot()">Save VM snapshot</button><button class="btn primary" onclick="Gold50.gold1fApplyUpdate()">Restart through System Update</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button></div>`,{width:980,height:680});
}
Object.assign(window.Gold50,{
  openGold1AUpdateCenter:(typeof openGold1AUpdateCenter==='function'?openGold1AUpdateCenter:openGold1AUpdateCenterRebuilt),
  gold1fAskShellUpdateCheck:(typeof gold1fAskShellUpdateCheck==='function'?gold1fAskShellUpdateCheck:gold1fRebuiltCheckForUpdates),
  gold1fCheckForUpdates:gold1fRebuiltCheckForUpdates,
  gold1fApplyUpdate:gold1fRebuiltApplyUpdate,
  gold1fSaveUpdateSnapshot:()=>{gold1fRebuiltSaveBeforeUpdate();notify('VM snapshot saved','EmeraldOS Gold saved a local update snapshot.','System Update')},
  gold1fPostState:(typeof gold1fPostState==='function'?gold1fPostState:()=>write('emeraldGoldShell_liveState',gold1fRebuiltSnapshot())),
  gold1fVMState:(typeof gold1fVMState==='function'?gold1fVMState:gold1fRebuiltSnapshot),
...
try{const updateApp=APPS.find(a=>a.id==='updateshell');const updateDef={id:'updateshell',name:'System Update',label:'UP',color:'blue',group:'System',desc:'Check Firebase for the latest EmeraldOS Gold folder and restart through the updater.',open:window.Gold50.openGold1AUpdateCenter};if(updateApp)Object.assign(updateApp,updateDef);else APPS.push(updateDef);}catch(e){console.warn('Gold 1F update app install failed',e)}
try{enhanceSettings70()}catch(e){console.warn('Gold 1F settings enhancer failed',e)}


/* Gold 1F boot fix: final exposure is installed inside seedGold50 so boot never references scoped handlers too early. */
function gold1fInstallLiveHandlers(){return true;}
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{if(window.Gold50?.openCloudSyncCenter80){document.body.classList.add('gold1f','gold1f-shell');}}catch(e){console.warn(e)}},120));


/* EmeraldOS Gold 1F first-run Firestore rules installer.
...
function gold1fRulesText(){return `rules_version = '2';
...
`;}
function gold1fRulesStatus(){return read(PREFIX+'rules_status',{ran:false,status:'not_run',message:'Rules installer has not run yet.'});}
function gold1fWriteRulesStatus(status){write(PREFIX+'rules_status',status);return status;}
async function gold1fStageFirestoreRules(){
  const status={ran:true,status:'staging',version:'1F',time:now(),message:'Preparing EmeraldOS Gold 1F rules.'};
  gold1fWriteRulesStatus(status);
  let fbm=null;
  try{fbm=await import('./firebase.js');}catch(e){status.status='local_only';status.message='Firebase module unavailable. Rules saved locally only.';status.error=e.message;gold1fWriteRulesStatus(status);return status;}
  const payload={product:'EmeraldOS Gold',version:'1F',rulesVersion:'gold10-default',rules:gold1fRulesText(),createdBy:username(),createdAt:now(),status:'pending_deployment',note:'Generated automatically by EmeraldOS Gold 1F first boot. Deploy from Firebase Console/CLI or a secured deployer endpoint.'};
  try{
...
async function gold1fTrySecureRulesDeploy(){
  ...
  const body={product:'EmeraldOS Gold',version:'1F',rules:gold1fRulesText(),requestedBy:username(),time:now()};
...
async function gold1fAutoRulesInstall(silent=false){
  if(localStorage.getItem(PREFIX+'rules_install_ran')==='true') return gold1fRulesStatus();
  localStorage.setItem(PREFIX+'rules_install_ran','true');
  const staged=await gold1fStageFirestoreRules();
  try{
    const deployed=await gold1fTrySecureRulesDeploy();
    if(deployed && (deployed.ok||deployed.deployed)){
      staged.status='deployed';staged.message='Rules were deployed through the configured secure deployer.';staged.deployerResult=deployed;gold1fWriteRulesStatus(staged);
    }
  }catch(e){staged.deployerError=e.message;gold1fWriteRulesStatus(staged);}
  if(!silent){notify('Rules installer finished',gold1fRulesStatus().message,'Security Rules');}
  return gold1fRulesStatus();
}
function openGoldRulesInstaller100(){
  const s=gold1fRulesStatus();
  openWindow('rulesinstaller','Security Rules Installer',`<div class="goldshell-header"><div><h2>Security Rules Installer</h2><p class="muted">Gold 1F prepares the recommended Firestore rules on first boot. Direct deployment requires Firebase Console/CLI or a secure deployer endpoint.</p></div><button class="btn primary" onclick="Gold50.gold1fAutoRulesInstall(false)">Run installer</button></div><div class="grid2"><div class="card"><h3>Status</h3><p>${esc(s.status)}</p><p class="muted">${esc(s.message||'')}</p></div><div class="card"><h3>Secure deployer</h3><p>Optional. Set <code>emeraldGoldRulesDeployerUrl</code> and <code>emeraldGoldRulesDeployerKey</code> in localStorage for automatic deployment through your own backend.</p></div></div><h3>Rules</h3><textarea class="field mono" rows="18">${esc(gold1fRulesText())}</textarea><div class="toolbar"><button class="btn" onclick="navigator.clipboard?.writeText(Gold50.gold1fRulesText())">Copy rules</button><button class="btn" onclick="location.href='rules-installer.html'">Open full rules page</button></div>`,{width:1080,height:760});
}
function installGold100RulesInstaller(){
  try{
    const old=APPS.find(a=>a.id==='rulesinstaller');
    const app={id:'rulesinstaller',name:'Security Rules Installer',label:'RI',color:'red',group:'System',desc:'Stages Gold 1F Firestore rules and deploys through an optional secure backend.',staffOnly:true,open:openGoldRulesInstaller100};
  ...
  }catch(e){console.warn('Gold 1F rules app install failed',e)}
  /* Rules installer is manual in redesigned Gold 1F. */
}
installGold100RulesInstaller();
Object.assign(window.Gold50,{openGoldRulesInstaller100,gold1fAutoRulesInstall,gold1fRulesText,gold1fRulesStatus,gold1fStageFirestoreRules,gold1fTrySecureRulesDeploy});



/* EmeraldOS Gold 1F redesigned manual update architecture.
   The root shell checks Firebase, but each user chooses when to move their VM to a newer folder.
   Gold_1F can publish its own latest pointer when manually booted once by staff. */
const GOLD1A_LATEST_MANIFEST = {
  ...
  enabled: true,
  setupMode: "continue",
  releaseTitle: "EmeraldOS Gold 1F",
...
function gold1fCurrentManifest(){ return {...GOLD1A_LATEST_MANIFEST}; }
function gold1fLatestCache(){ return read('emeraldGoldShell_latest', GOLD1A_LATEST_MANIFEST); }
function gold1fIsNewerManifest(latest){
  if(!latest) return false;
  const latestVersion = String(latest.latestVersion || latest.build || '');
  const latestFolder = String(latest.folder || '');
  return latestVersion !== BUILD.version || latestFolder !== 'Gold_1F';
}
async function gold1fFetchLatestManifest(){
  try{
    const fbm = await import('./firebase.js');
    const snap = await fbm.getDoc(fbm.doc(fbm.db, 'system', 'emeraldGoldLatest'));
    ...
    console.warn('Gold 1F could not read update manifest', error);
    return gold1fLatestCache();
  }
}
function gold1fUpdateSnapshot(){
  const data = {
    product:'EmeraldOS Gold',
    version:BUILD.version,
    folder:'Gold_1F',
...
async function gold1fCloudWrite(path, value){
  try{ const fbm = await import('./firebase.js'); await fbm.setDoc(fbm.doc(fbm.db, ...path), value, {merge:true}); return true; }
  catch(error){ console.warn('Gold 1F cloud write failed', path, error); return false; }
}
async function gold1fSaveManualUpdateSnapshot(){
  const snap = gold1fUpdateSnapshot();
  const user = username();
  await gold1fCloudWrite(['emeraldOSUsers', user, 'goldVM', 'current'], {
    activeVersion: BUILD.version,
    activeFolder: 'Gold_1F',
    entry: 'OS.html',
    lastSave: now(),
    cloudSync: true
  });
  await gold1fCloudWrite(['emeraldOSUsers', user, 'goldVMSnapshots', `manual_update_${Date.now()}`], snap);
  notify('VM snapshot saved','Your EmeraldOS Gold VM state was saved before update.','System Update');
  return snap;
}
async function gold1fApplyManualUpdate(){
  const latest = await gold1fFetchLatestManifest();
  if(!latest || !latest.folder){ alert('No update target is configured.'); return; }
  await gold1fSaveManualUpdateSnapshot();
...
function gold1fSkipUpdate(version){
...
function gold1fRemindLater(version){
...
function gold1fUpdatePromptHTML(latest){
  const version = esc(latest.latestVersion || latest.build || 'Unknown');
  const folder = esc(latest.folder || '');
  return `<div class="update-prompt"><h2>EmeraldOS Gold update available</h2><p>A newer EmeraldOS Gold version is available for your cloud VM.</p><div class="grid2"><div class="card"><h3>Current</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1F</p></div><div class="card"><h3>Available</h3><p>EmeraldOS Gold ${version}</p><p class="muted">Folder: ${folder}</p></div></div><h3>Release note</h3><p>${esc(latest.summary || latest.releaseTitle || 'No release note was provided.')}</p><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1fApplyManualUpdate()">Update this VM now</button><button class="btn" onclick="Gold50.gold1fRemindLater('${version}')">Remind me later</button><button class="btn" onclick="Gold50.gold1fSkipUpdate('${version}')">Skip this version</button><button class="btn" onclick="Gold50.openGold1AUpdateCenter()">Open System Update</button></div><p class="muted">Updates are manual per user. Your VM keeps running this version until you choose to update.</p></div>`;
}
function gold1fOpenUpdatePrompt(latest){
  openWindow('updateprompt','EmeraldOS Gold Update Available',gold1fUpdatePromptHTML(latest),{width:860,height:590});
}
async function gold1fCheckLoginUpdateNotice(){
  const latest = await gold1fFetchLatestManifest();
  const latestVersion = String(latest.latestVersion || latest.build || '');
  ...
  const newer = gold1fIsNewerManifest(latest);
  write('emeraldGold_updateAvailable', {available:newer, latest, active:gold1fCurrentManifest(), checkedAt:now()});
  if(newer && skipped !== latestVersion && !(laterVersion === latestVersion && Date.now() < laterUntil)){
    notify('EmeraldOS Gold update available',`Version ${latestVersion} is ready. You can update now or stay on Gold 1F.`, 'System Update');
    setTimeout(()=>gold1fOpenUpdatePrompt(latest), 700);
  }
}
function openGold1AUpdateCenter(latestManifest){
  const latest = latestManifest || gold1fLatestCache();
  const newer = gold1fIsNewerManifest(latest);
  const version = latest.latestVersion || latest.build || 'Unknown';
  const folder = latest.folder || '';
  const active = read('emeraldGoldShell_activeManifest', gold1fCurrentManifest());
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">EmeraldOS Gold checks Firebase for the newest approved version. Updates are manual for each user, so your VM only changes when you choose Update.</p></div><button class="btn primary" onclick="Gold50.gold1fCheckForUpdatesManual()">Check now</button></div><div class="grid2"><div class="card"><h3>This VM is running</h3><p>${esc(BUILD.name)}</p><p class="muted">Folder: Gold_1F</p></div><div class="card"><h3>Latest configured build</h3><p>EmeraldOS Gold ${esc(version)}</p><p class="muted">${esc(folder)} / ${esc(latest.entry || 'OS.html')}</p></div><div class="card"><h3>User update mode</h3><p>${newer?'A newer version is available.':'This VM is on the latest configured version.'}</p><p class="muted">The shell will not move you to a new folder until you accept the update.</p></div><div class="card"><h3>Release publisher</h3><p>When a new version folder is uploaded, boot that version manually once to publish its Firebase pointer.</p><button class="btn" onclick="Gold50.gold1fPublishLatestManifest(true)">Publish Gold 1F as latest</button></div></div><h3>Release note</h3><p>${esc(latest.summary || latest.releaseTitle || 'No release note available.')}</p><div class="toolbar"><button class="btn" onclick="Gold50.gold1fSaveManualUpdateSnapshot()">Save VM snapshot</button><button class="btn primary" ${newer?'':'disabled'} onclick="Gold50.gold1fApplyManualUpdate()">Update this VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="Gold50.gold1fResetUpdateChoice()">Reset update choice</button></div>`,{width:1020,height:720});
}
async function gold1fCheckForUpdatesManual(){
  const latest = await gold1fFetchLatestManifest();
  openGold1AUpdateCenter(latest);
  if(gold1fIsNewerManifest(latest)) gold1fOpenUpdatePrompt(latest);
  else notify('System Update','This VM is running the latest configured EmeraldOS Gold version.','System Update');
}
function gold1fResetUpdateChoice(){
...
async function gold1fPublishLatestManifest(showResult=false){
  ...
  try{
    ...
    if(showResult || manualBoot) notify('Release pointer published','system/emeraldGoldLatest now points to Gold_1F.','System Update');
    return {ok:true};
  }catch(error){
    console.warn('Could not publish Gold 1F latest pointer', error);
    write(PREFIX+'latest_manifest_publish_error', {time:now(), error:error.message, manifest:GOLD1A_LATEST_MANIFEST});
    if(showResult || manualBoot) notify('Release pointer not published', 'Firebase rules may require staff write access. Use Gold_1F/OS.html?publishLatest=1 or staff credentials.', 'System Update');
    return {ok:false, error:error.message};
  }
}
function gold1fInstallManualUpdateSystem(){
  try{
    ...
    if(old) Object.assign(old, app); else APPS.push(app);
  }catch(error){ console.warn('Gold 1F manual update app install failed', error); }
}
gold1fInstallManualUpdateSystem();
Object.assign(window.Gold50, {
  openGold1AUpdateCenter,
  gold1fCheckForUpdatesManual,
  gold1fApplyManualUpdate,
  gold1fSaveManualUpdateSnapshot,
  gold1fResetUpdateChoice,
  gold1fSkipUpdate,
  gold1fRemindLater,
  gold1fPublishLatestManifest,
  gold1fFetchLatestManifest,
  gold1fCurrentManifest
});
window.addEventListener('DOMContentLoaded',()=>{
  setTimeout(()=>gold1fPublishLatestManifest(false), 1800);
  setTimeout(()=>gold1fCheckLoginUpdateNotice(), 2600);
});
...
/* EmeraldOS Gold 1F OS-folder-only update helpers */
function gold1fLatestManifest(){
  return {
    product:'EmeraldOS Gold', latestVersion:'1F', build:'1F', folder:'Gold_1F', entry:'OS.html',
...
async function gold1fPublishLatest(showResult=true){
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
    const {doc,setDoc}=api;
    const manifest=gold1fLatestManifest();
    await setDoc(doc(api.db,'system','emeraldGoldLatest'), manifest, {merge:true});
    localStorage.setItem('gold1f_published_latest','true');
    if(showResult) notify('System Update','Gold 1F was published as the latest available build.','System Update');
    return true;
  }catch(e){
    console.warn('Gold 1F publish latest failed', e);
    if(showResult) notify('System Update','Could not publish latest build. Check Firebase rules/access.','System Update');
    return false;
  }
}
function openGold1AUpdateCenter(){
  openWindow('updateshell','System Update',`<div class="goldshell-header"><div><h2>System Update</h2><p class="muted">Gold 1F is an OS folder only. The shell stays outside this folder and only routes updates when users choose to update.</p></div><button class="btn primary" onclick="Gold50.gold1fCheckForUpdatesManual?.()||Gold50.gold1fCheckForUpdates?.()">Check Firebase</button></div><div class="grid2"><div class="card"><h3>This folder</h3><p>EmeraldOS Gold 1F</p><p class="muted">Gold_1F / OS.html</p></div><div class="card"><h3>Publish flow</h3><p>Boot Gold_1F manually once with <code>?publishLatest=1</code>, or press publish here, to set <code>system/emeraldGoldLatest</code>.</p></div><div class="card"><h3>User updates</h3><p>Each user updates manually. The shell notifies them on login and only changes their active folder after they accept.</p></div><div class="card"><h3>Continuity</h3><p>VM data remains stored under <code>goldVM/current</code>, independent of the version folder.</p></div></div><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1fPublishLatest(true)">Publish Gold 1F as latest</button><button class="btn" onclick="Gold50.saveWorkspaceNow(true)">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html?applyUpdate=1'">Restart through shell</button></div>`,{width:1040,height:700});
}
function gold1fInstallUpdateApp(){
  try{
  ...
  }catch(e){console.warn('Gold 1F update app install failed',e)}
}
gold1fInstallUpdateApp();
Object.assign(window.Gold50||{}, {gold1fLatestManifest,gold1fPublishLatest,openGold1AUpdateCenter});
window.Gold1A=window.Gold50; window.Gold1F=window.Gold50;
if(new URLSearchParams(location.search).has('publishLatest')) setTimeout(()=>gold1fPublishLatest(true), 1200);


/* =========================================================
   EMERALDOS GOLD 1F - E.L.S.U.S. COMPATIBILITY + UX PATCH
...
const GOLD1A_RELEASE_MANIFEST={
  ...
  channel:'stable', status:'stable', required:false, enabled:true, setupMode:'continue',
  releaseTitle:'EmeraldOS Gold 1F',
  summary:'Gold 1F is an OS-folder-only E.L.S.U.S.-compatible release built from Gold 11.0. It improves the EmeraldOS Gold desktop experience, adds a curated non-duplicate app catalog, improves customization safety, and keeps VM data version-independent.',
  migrationFrom:['11.0','10.0','9.0','8.0','8.0.1'], migrationId:'gold11-to-gold1f-elsus',
  minShellVersion:'1.0', rollbackFolder:'Gold_11.0', rollbackVersion:'11.0', releasedAt:new Date().toISOString()
};
function gold1fClamp(n,min,max,fall){n=Number(n);if(!Number.isFinite(n))n=fall;return Math.max(min,Math.min(max,n));}
function gold1fCSSColor(v,fall){v=String(v||'').trim();return /^#[0-9a-f]{3,8}$/i.test(v)?v:fall;}
function gold1fCurrentManifest(){return {...GOLD1A_RELEASE_MANIFEST};}
async function gold1fPublishLatest(showResult=true){
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
...
function gold1fInstallCuratedApps(){
  ...
  unique.forEach(gold1fAddOrUpdateApp);
  ...
  pref.backgroundColor=pref.backgroundColor||'#eaf3ff'; pref.panelOpacity=gold1fClamp(pref.panelOpacity,70,100,94); pref.taskbarOpacity=gold1fClamp(pref.taskbarOpacity,60,100,92);
  pref.startWidth=gold1fClamp(pref.startWidth,560,1100,880); pref.micaBlur=gold1fClamp(pref.micaBlur,0,30,18);
  write(PREFIX+'prefs',pref);
}
function gold1fApplyCustomizationSafeCss(){
  const p=prefs(); const root=document.documentElement;
  document.body.classList.add('gold1f','gold1f-elsus');
  document.body.classList.toggle('gold1f-solid-bg',!!p.useBackgroundColor && !p.customBackground);
  document.body.classList.toggle('gold1f-taskbar-top',p.taskbarPosition==='top');
  root.style.setProperty('--gold1fBgColor',gold1fCSSColor(p.backgroundColor,'#eaf3ff'));
  root.style.setProperty('--gold1fAccent',gold1fCSSColor(p.accent,'#0078d4'));
  root.style.setProperty('--gold1fAccent2',gold1fCSSColor(p.secondaryAccent,'#0f7b4d'));
  root.style.setProperty('--accent',gold1fCSSColor(p.accent,'#0078d4'));
  root.style.setProperty('--accent2',gold1fCSSColor(p.secondaryAccent,'#0f7b4d'));
  root.style.setProperty('--gold1fPanelOpacity',String(gold1fClamp(p.panelOpacity,70,100,94)/100));
  root.style.setProperty('--gold1fTaskOpacity',String(gold1fClamp(p.taskbarOpacity,60,100,92)/100));
  root.style.setProperty('--gold1fMica',gold1fClamp(p.micaBlur,0,30,18)+'px');
  root.style.setProperty('--gold1fStartWidth',gold1fClamp(p.startWidth,560,1100,880)+'px');
  root.style.setProperty('--gold1fIconScale',p.iconSize==='xl'?'1.28':p.iconSize==='large'?'1.14':p.iconSize==='small'?'.88':'1');
  root.style.setProperty('--gold1fDesktopGap',p.density==='compact'?'9px':p.density==='spacious'?'24px':'16px');
  root.style.setProperty('--winRadius',p.rounding==='none'?'0':p.rounding==='large'?'14px':p.rounding==='pill'?'18px':'8px');
}
function gold1fWireCustomizationSafety(){
  const oldSetGold70=window.Gold50?.setGold70Pref;
  if(oldSetGold70&&!oldSetGold70.gold1fWrapped){
    const wrapped=function(k,v){oldSetGold70(k,v);gold1fApplyCustomizationSafeCss();try{renderDesktop()}catch{}};wrapped.gold1fWrapped=true;window.Gold50.setGold70Pref=wrapped;
  }
  const oldSetPrefs=window.Gold50?.setPrefs;
  if(oldSetPrefs&&!oldSetPrefs.gold1fWrapped){
    const wrapped=function(p){oldSetPrefs(p);gold1fApplyCustomizationSafeCss();try{renderDesktop()}catch{}};wrapped.gold1fWrapped=true;window.Gold50.setPrefs=wrapped;
  }
}
function openGold1AWelcome(){
  openWindow('emeraldbrand','EmeraldOS Gold 1F',`<div class="welcome70"><div class="gold1f-hero"><div class="gold-logo-xl">G</div><div><h1>EmeraldOS Gold 1F</h1><p>OS-folder-only E.L.S.U.S. release with a cleaner EmeraldOS Gold desktop, curated app catalog, safer customization, and cloud VM continuity.</p></div></div><div class="grid3"><div class="card"><h3>Customize everything</h3><p>Theme Studio controls background color, wallpaper, accents, taskbar, panels, blur, rounding, desktop spacing, and accessibility.</p><button class="btn primary" onclick="Gold50.openApp('themestudio')">Open Theme Studio</button></div><div class="card"><h3>Cloud VM</h3><p>Your files, settings, apps, tickets, mail, notes, tasks, and workspace stay separate from the version folder.</p><button class="btn" onclick="Gold50.openApp('cloudsync')">Open Cloud Sync</button></div><div class="card"><h3>E.L.S.U.S.</h3><p>The updater shell prompts users and only changes their active version when they choose to update.</p><button class="btn" onclick="Gold50.openApp('updateshell')">System Update</button></div></div></div>`,{width:940,height:620});
}
function gold1fInstallBootPolish(){
  gold1fInstallCuratedApps();
  gold1fApplyCustomizationSafeCss();
  gold1fWireCustomizationSafety();
  gold1fAddOrUpdateApp({id:'emeraldbrand',name:'EmeraldOS Gold Welcome',label:'EG',color:'gold',group:'Core',purpose:'Welcome',desc:'Overview of Gold 1F, E.L.S.U.S., customization, and cloud VM continuity.',open:openGold1AWelcome});
...
window.Gold1A=window.Gold50; window.Gold1F=window.Gold50;
window.addEventListener('DOMContentLoaded',()=>setTimeout(gold1fInstallBootPolish,120));
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{ if(new URLSearchParams(location.search).has('publishLatest')) { if(isStaff()) openUpdatePublisherManager1F(); else gold1fStaffGateNotice(); } },1500));


/* =========================================================
   EMERALDOS GOLD 1F UPDATE SETUP + CLASSIC RIGHT-CLICK MENU
========================================================= */
const GOLD1F_ESSENTIAL_DESKTOP=['explorer','office','mail','support','settings','updateshell'];
function gold1fDesktopPins(){return read(PREFIX+'desktopFilePins',[])}
function gold1fSaveDesktopPins(arr){write(PREFIX+'desktopFilePins',[...new Set(arr)].filter(Boolean));saveWorkspaceDebounced();try{renderDesktop()}catch{}}
function gold1fStartPins(){return read(PREFIX+'startPins',['explorer','office','support','settings','updateshell'])}
function gold1fTaskbarPins(){return read(PREFIX+'taskbarPins',['explorer','office','support'])}
function gold1fSetStartPins(arr){write(PREFIX+'startPins',[...new Set(arr)].filter(Boolean));try{renderStartMenu()}catch{};saveWorkspaceDebounced()}
function gold1fSetTaskbarPins(arr){write(PREFIX+'taskbarPins',[...new Set(arr)].filter(Boolean));try{renderTaskApps()}catch{};saveWorkspaceDebounced()}
function gold1fPin(arr,id){return [...new Set([...(arr||[]),id])].filter(Boolean)}
function gold1fUnpin(arr,id){return (arr||[]).filter(x=>x!==id)}
function gold1fApplyEssentialDesktop(){const p=prefs();p.desktopApps=[...GOLD1F_ESSENTIAL_DESKTOP];write(PREFIX+'prefs',p);applyPrefs();renderDesktop();renderStartMenu();notify('Desktop cleaned','Only essential apps are pinned to the desktop. All apps are still in Start and Search.','Setup')}
function gold1fPinAppToDesktop(id){const p=prefs();p.desktopApps=gold1fPin(p.desktopApps||[],id);write(PREFIX+'prefs',p);applyPrefs();renderDesktop();notify('Pinned to desktop',appById(id)?.name||id,'Desktop')}
function gold1fRemoveAppFromDesktop(id){const p=prefs();p.desktopApps=gold1fUnpin(p.desktopApps||[],id);write(PREFIX+'prefs',p);applyPrefs();renderDesktop();notify('Removed from desktop',appById(id)?.name||id,'Desktop')}
function gold1fPinAppToStart(id){gold1fSetStartPins(gold1fPin(gold1fStartPins(),id));notify('Pinned to Start',appById(id)?.name||id,'Start')}
function gold1fUnpinAppFromStart(id){gold1fSetStartPins(gold1fUnpin(gold1fStartPins(),id));notify('Unpinned from Start',appById(id)?.name||id,'Start')}
function gold1fPinAppToTaskbar(id){gold1fSetTaskbarPins(gold1fPin(gold1fTaskbarPins(),id));notify('Pinned to taskbar',appById(id)?.name||id,'Taskbar')}
function gold1fUnpinAppFromTaskbar(id){gold1fSetTaskbarPins(gold1fUnpin(gold1fTaskbarPins(),id));notify('Unpinned from taskbar',appById(id)?.name||id,'Taskbar')}
function gold1fPinFileToDesktop(fid){if(!files().some(f=>f.id===fid)){notify('File not found','That file could not be pinned.','Desktop');return}gold1fSaveDesktopPins(gold1fPin(gold1fDesktopPins(),fid));notify('Pinned file to desktop',files().find(f=>f.id===fid)?.name||fid,'Gold Explorer')}
function gold1fUnpinFileFromDesktop(fid){gold1fSaveDesktopPins(gold1fUnpin(gold1fDesktopPins(),fid));notify('Unpinned file from desktop',files().find(f=>f.id===fid)?.name||fid,'Desktop')}
function gold1fFileDesktopIcon(f){return `<button class="desktop-icon desktop-file ${prefs().showLabels?'':'no-label'}" data-fileid="${esc(f.id)}" ondblclick="Gold50.openFile('${esc(f.id)}')" title="${esc(f.name)}"><div class="file-desktop-icon">${esc(fileIcon(f.type))}</div><span>${esc(f.name)}</span></button>`}
try{
  const gold1fOriginalRenderDesktop=renderDesktop;
  renderDesktop=function(){
    ...
    const pinIds=gold1fDesktopPins();
    const fileHTML=pinIds.map(fid=>files().find(f=>f.id===fid&&!f.trash)).filter(Boolean).map(gold1fFileDesktopIcon).join('');
    d.innerHTML=appHTML+fileHTML;
  };
}catch(e){console.warn('Gold 1F desktop override skipped',e)}
try{
  const gold1fOriginalRenderTaskApps=renderTaskApps;
  renderTaskApps=function(){
    const bar=$('taskApps'); if(!bar) return;
    const pinHTML=gold1fTaskbarPins().map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff()).map(a=>`<button class="taskbar-app pinned" data-appid="${esc(a.id)}" title="Pinned: ${esc(a.name)}" onclick="Gold50.openApp('${esc(a.id)}')">${appIcon(a,true)}<span>${esc(a.name)}</span></button>`).join('');
...
}catch(e){console.warn('Gold 1F taskbar override skipped',e)}
try{
  renderStartMenu=function(){
    const sm=$('startMenu'); if(!sm) return;
    const apps=visibleApps(); const groups=[...new Set(apps.map(a=>a.group))];
    const pins=gold1fStartPins().map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff());
...
}catch(e){console.warn('Gold 1F Start override skipped',e)}
function gold1fContextMenu(items,x,y){
...
function gold1fDesktopContext(e){
  ...
  if(fileEl){
    ...
    return gold1fContextMenu([
      {id:'open',label:'Open',action:()=>openFile(fid)},
      {id:'unpin',label:'Unpin file from desktop',action:()=>gold1fUnpinFileFromDesktop(fid)},
  ...
  if(appEl){
    ...
    return gold1fContextMenu([
      ...
      {id:'start',label:gold1fStartPins().includes(id)?'Unpin from Start':'Pin to Start',action:()=>gold1fStartPins().includes(id)?gold1fUnpinAppFromStart(id):gold1fPinAppToStart(id)},
      {id:'taskbar',label:gold1fTaskbarPins().includes(id)?'Unpin from taskbar':'Pin to taskbar',action:()=>gold1fTaskbarPins().includes(id)?gold1fUnpinAppFromTaskbar(id):gold1fPinAppToTaskbar(id)},
      {id:'desktop',label:(p.desktopApps||[]).includes(id)?'Remove from desktop':'Pin to desktop',action:()=>((prefs().desktopApps||[]).includes(id)?gold1fRemoveAppFromDesktop(id):gold1fPinAppToDesktop(id))},
      'sep',
  ...
  return gold1fContextMenu([
    ...
    'sep',
    {id:'clean',label:'Use essential desktop layout',action:()=>gold1fApplyEssentialDesktop()},
    {id:'settings',label:'Settings',action:()=>openSettings()}
  ],e.clientX,e.clientY);
}
function openDesktopCustomizer1B(){
  const p=prefs(); const pinnedFiles=gold1fDesktopPins(); const sPins=gold1fStartPins(); const tPins=gold1fTaskbarPins();
  openWindow('desktopcustomizer','Desktop Customizer',`<div class="goldshell-header"><div><h2>Desktop Customizer</h2><p class="muted">Classic EmeraldOS right-click tools for desktop apps, Start pins, taskbar pins, and pinned files.</p></div><button class="btn primary" onclick="Gold50.gold1fApplyEssentialDesktop()">Essential desktop</button></div><div class="grid2"><div class="card"><h3>Apps</h3><div class="purpose-map compact">${visibleApps().map(a=>`<div class="card mini"><h4>${appIcon(a,true)} ${esc(a.name)}</h4><div class="toolbar"><button class="btn small" onclick="Gold50.${(p.desktopApps||[]).includes(a.id)?'gold1fRemoveAppFromDesktop':'gold1fPinAppToDesktop'}('${esc(a.id)}')">${(p.desktopApps||[]).includes(a.id)?'Remove desktop':'Desktop'}</button><button class="btn small" onclick="Gold50.${sPins.includes(a.id)?'gold1fUnpinAppFromStart':'gold1fPinAppToStart'}('${esc(a.id)}')">${sPins.includes(a.id)?'Unpin Start':'Start'}</button><button class="btn small" onclick="Gold50.${tPins.includes(a.id)?'gold1fUnpinAppFromTaskbar':'gold1fPinAppToTaskbar'}('${esc(a.id)}')">${tPins.includes(a.id)?'Unpin taskbar':'Taskbar'}</button></div></div>`).join('')}</div></div><div class="card"><h3>Files pinned to desktop</h3><p class="muted">Pin important files from your Gold VM to the desktop without moving them.</p>${files().filter(f=>!f.trash).slice(0,80).map(f=>`<div class="settings-row"><div><b>${esc(f.name)}</b><p class="muted">${esc(f.folder)} · ${esc(f.type)}</p></div><button class="btn small" onclick="Gold50.${pinnedFiles.includes(f.id)?'gold1fUnpinFileFromDesktop':'gold1fPinFileToDesktop'}('${esc(f.id)}')">${pinnedFiles.includes(f.id)?'Unpin':'Pin file'}</button></div>`).join('')||'<p class="muted">No files yet.</p>'}</div></div>`,{width:1100,height:720})
}
function openUpdateSetup1B(){
  const box=$('setupWizard'); if(!box) return;
  let step=0;
  const steps=[
    ()=>`<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>EmeraldOS Gold 1F Update Setup</h2><p class="muted">Your VM was updated through E.L.S.U.S. This short setup keeps your files and settings, then helps you choose the new desktop layout.</p></div></div><div class="setup-checklist"><span>VM preserved</span><span>Desktop controls added</span><span>Classic right-click returned</span></div></div>`,
    ()=>`<h2>Choose your updated desktop</h2><p class="muted">Gold 1F keeps only essential apps on the desktop by default. Everything else stays available in Start and Search.</p><div class="grid3"><button class="tile-button" onclick="Gold50.gold1fApplyEssentialDesktop()">Use essential desktop</button><button class="tile-button" onclick="Gold50.openDesktopCustomizer1B()">Customize pins</button><button class="tile-button" onclick="Gold50.openApp('themestudio')">Customize style</button></div><h3>Essential desktop apps</h3><div class="grid3">${GOLD1F_ESSENTIAL_DESKTOP.map(id=>appById(id)).filter(Boolean).map(a=>`<div class="card">${appIcon(a,true)} <b>${esc(a.name)}</b><p class="muted">${esc(a.desc)}</p></div>`).join('')}</div>`,
    ...
    ()=>`<h2>You're ready</h2><p class="muted">Gold 1F is ready. Your VM data is still managed by E.L.S.U.S. and your existing data has been kept.</p><div class="grid3"><button class="tile-button" onclick="Gold50.openApp('updateshell')">System Update</button><button class="tile-button" onclick="Gold50.openApp('desktopcustomizer')">Desktop Customizer</button><button class="tile-button" onclick="Gold50.openApp('support')">Support Center</button></div>`
  ];
  function render(){box.classList.remove('hidden');box.innerHTML=`<div class="setup-card setup-card-rebuilt update-setup"><div class="setup-titlebar"><b>Gold 1F Update Setup</b><button class="btn small" onclick="Gold50.finishUpdateSetup1B()">Skip</button></div><div class="setup-steps">${steps.map((_,i)=>`<div class="setup-dot ${i<=step?'active':''}"></div>`).join('')}</div><div class="setup-body">${steps[step]()}</div><div class="toolbar setup-actions"><button class="btn" ${step===0?'disabled':''} onclick="Gold50.updateSetupStep1B(-1)">Back</button><span class="muted">Step ${step+1} of ${steps.length}</span><button class="btn primary" onclick="Gold50.updateSetupStep1B(1)">${step===steps.length-1?'Finish':'Next'}</button></div></div>`}
...
function finishUpdateSetup1B(){localStorage.setItem(PREFIX+'update_setup_done','true');$('setupWizard')?.classList.add('hidden');notify('Update setup complete','EmeraldOS Gold 1F is ready.','Setup')}
function gold1fInstallUpdateSetup(){
  if(gold1fShouldRunUpdateSetup()) setTimeout(openUpdateSetup1F,700);
}
function gold1fInstallContextMenus(){document.addEventListener('contextmenu',gold1fDesktopContext);document.addEventListener('keydown',e=>{if(e.shiftKey&&e.key==='F10'){e.preventDefault();const d=$('desktop');const r=d?.getBoundingClientRect();gold1fContextMenu([{id:'customize',label:'Open Desktop Customizer',action:()=>openDesktopCustomizer1B()},{id:'refresh',label:'Refresh desktop',action:()=>renderDesktop()}],(r?.left||20)+80,(r?.top||20)+80)}})}
gold1fAddOrUpdateApp({id:'desktopcustomizer',name:'Desktop Customizer',label:'DC',color:'blue',group:'Customization',purpose:'Desktop pin management',desc:'Classic desktop right-click tools for app pins, taskbar pins, Start pins, and pinned files.',open:openDesktopCustomizer1B});
Object.assign(window.Gold50||{}, {gold1fApplyEssentialDesktop,gold1fPinAppToDesktop,gold1fRemoveAppFromDesktop,gold1fPinAppToStart,gold1fUnpinAppFromStart,gold1fPinAppToTaskbar,gold1fUnpinAppFromTaskbar,gold1fPinFileToDesktop,gold1fUnpinFileFromDesktop,openDesktopCustomizer1B,openUpdateSetup1B,finishUpdateSetup1B});
window.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{try{gold1fInstallContextMenus();renderDesktop();renderStartMenu();renderTaskApps();gold1fInstallUpdateSetup();}catch(e){console.warn('Gold 1F polish failed',e)}},650)});


/* =========================================================
   EMERALDOS GOLD 1F - ADVANCED DESKTOP, ROLLBACK, CRASH UX,
   AND STAFF-ONLY E.L.S.U.S. PUBLISHER MANAGER
========================================================= */
const GOLD1F_PUBLISHER_PIN = "093013";
const GOLD1F_ROLLBACK_TARGETS = [
...
function gold1fValidColor(value, fallback){
  value=String(value||"").trim();
  return /^#[0-9a-f]{3,8}$/i.test(value)?value:fallback;
}
function gold1fNumber(value,min,max,fallback){
  value=Number(value);
...
try{
  const gold1fBaseDefaultPrefs = defaultPrefs;
  defaultPrefs = function(){
    const base = gold1fBaseDefaultPrefs();
    return {
      ...base,
...
}catch(e){console.warn("Gold 1F default preference extension skipped",e)}

try{
  const gold1fBaseApplyPrefs = applyPrefs;
  applyPrefs = function(){
    gold1fBaseApplyPrefs();
    const p=prefs();
    const root=document.documentElement;
    root.style.setProperty("--gold1fTaskbarColor", gold1fValidColor(p.taskbarColor,"#f6f9fd"));
    root.style.setProperty("--gold1fTaskbarText", gold1fValidColor(p.taskbarTextColor,"#111827"));
    root.style.setProperty("--gold1fStartColor", gold1fValidColor(p.startMenuColor,"#ffffff"));
    root.style.setProperty("--gold1fGridSize", gold1fNumber(p.desktopGridSize,64,150,92)+"px");
    document.body.classList.toggle("gold1f-free-desktop", p.desktopObjectMode!=="grid");
    document.body.classList.toggle("gold1f-grid-desktop", p.desktopObjectMode==="grid");
    document.body.classList.toggle("gold1f-trash-hidden", p.trashOnDesktop===false);
  };
}catch(e){console.warn("Gold 1F visual preference extension skipped",e)}

function gold1fDesktopPositions(){return read(PREFIX+"desktopPositions",{})}
function gold1fSaveDesktopPositions(map){write(PREFIX+"desktopPositions",map||{});saveWorkspaceDebounced()}
function gold1fDesktopItemKey(type,id){return type+":"+id}
function gold1fDefaultPosition(i){
  const p=prefs(), grid=gold1fNumber(p.desktopGridSize,64,150,92);
  const cols=Math.max(1,Math.floor((window.innerWidth-40)/grid));
  return {x:18+(i%cols)*grid, y:18+Math.floor(i/cols)*(grid+18)};
}
function gold1fPositionStyle(key,index){
  const pos=gold1fDesktopPositions()[key] || gold1fDefaultPosition(index);
  const x=gold1fNumber(pos.x,0,Math.max(0,innerWidth-92),18);
  const y=gold1fNumber(pos.y,0,Math.max(0,innerHeight-120),18);
  return `left:${x}px;top:${y}px;`;
}
function gold1fSnap(v){
  const p=prefs();
  if(!p.desktopSnapToGrid) return v;
  const grid=gold1fNumber(p.desktopGridSize,64,150,92);
  return Math.round(v/grid)*grid+18;
}
function gold1fTrashIconHTML(index){
  const key=gold1fDesktopItemKey('system','trash');
  return `<button class="desktop-icon desktop-trash ${prefs().showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-trash="true" style="${gold1fPositionStyle(key,index)}" ondblclick="Gold50.openTrashCan1F()" title="Trash"><div class="trash-can-icon">♻</div><span>Trash</span></button>`;
}
function openTrashCan1F(){
  try{ openExplorer(); setTimeout(()=>Gold50.explorerView('trash'),80); }
  catch(e){ gold1fFileOrAppFailure('Trash','Trash failed to load.',e,'trash'); }
}
...
try{
  renderDesktop=function(){
    const d=$('desktop'); if(!d) return;
    const p=prefs();
    d.classList.add('gold1f-movable-desktop');
    d.style.gap=p.density==='compact'?'8px':p.density==='spacious'?'22px':'16px';
    ...
    const appHTML=(p.desktopApps||[]).map(id=>appById(id)).filter(Boolean).filter(a=>!a.staffOnly||isStaff()).map(a=>{
      const key=gold1fDesktopItemKey('app',a.id);
      const html=`<button class="desktop-icon ${p.showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-appid="${esc(a.id)}" style="width:${width};${gold1fPositionStyle(key,i++)}" ondblclick="Gold50.openApp('${esc(a.id)}')" title="${esc(a.desc)}">${appIcon(a)}<span>${esc(a.name)}</span></button>`;
      return html;
    }).join('');
    const fileHTML=gold1fDesktopPins().map(fid=>files().find(f=>f.id===fid&&!f.trash)).filter(Boolean).map(f=>{
      const key=gold1fDesktopItemKey('file',f.id);
      return `<button class="desktop-icon desktop-file ${p.showLabels?'':'no-label'}" data-deskitem="${esc(key)}" data-fileid="${esc(f.id)}" style="width:${width};${gold1fPositionStyle(key,i++)}" ondblclick="Gold50.openFile('${esc(f.id)}')" title="${esc(f.name)}"><div class="file-desktop-icon">${esc(fileIcon(f.type))}</div><span>${esc(f.name)}</span></button>`;
    }).join('');
    const trashHTML=p.trashOnDesktop===false?'':gold1fTrashIconHTML(i++);
    d.innerHTML=appHTML+fileHTML+trashHTML;
  };
}catch(e){console.warn('Gold 1F desktop renderer failed to install',e)}

function gold1fInstallDesktopDrag(){
  let state=null;
  ...
  document.addEventListener('pointerup',e=>{
    ...
    state.icon.style.left=x+'px'; state.icon.style.top=y+'px';
    const map=gold1fDesktopPositions(); map[state.key]={x,y,updatedAt:now()}; gold1fSaveDesktopPositions(map);
...
function gold1fCenterPopup(title,message,level='warning',actions=[]){
  if(prefs().centerFailurePopups===false) return notify(title,message,'EmeraldOS Gold');
  document.querySelectorAll('.gold1f-center-popup').forEach(x=>x.remove());
  const d=document.createElement('div');
  d.className='gold1f-center-popup '+level;
...
function gold1fFileOrAppFailure(name,message,error,kind='app'){
  console.warn('Gold 1F load failure',name,error);
  notify('Load problem',`${name} refused to load.`,'EmeraldOS Gold');
  gold1fCenterPopup(`${kind==='file'?'File':'Application'} refused to load`,`${message} ${error?.message||''}`.trim(),'warning',[
...
function gold1fBSOD(reason='EmeraldOS Gold stopped responding.',code='GOLD_RUNTIME_FAULT'){
  if(document.querySelector('.gold1f-bsod')) return;
  const d=document.createElement('div');
  d.className='gold1f-bsod';
  d.innerHTML=`<div><h1>:(</h1><h2>EmeraldOS Gold ran into a problem.</h2><p>${esc(reason)}</p><p class="bsod-code">Stop code: ${esc(code)}</p><div class="bsod-progress"><span></span></div><div class="toolbar"><button onclick="location.reload()">Restart EmeraldOS Gold</button><button onclick="localStorage.setItem('${PREFIX}safemode','true');location.reload()">Restart in Safe Mode</button><button onclick="this.closest('.gold1f-bsod').remove();Gold50.openApp('support')">Open Support</button></div></div>`;
  document.body.appendChild(d);
}

try{
  const gold1fOriginalOpenApp = window.Gold50?.openApp || openApp;
  function gold1fSafeOpenApp(id){
    try{
      const app=appById(id);
      if(!app) throw new Error('The app is not installed or is hidden.');
      return gold1fOriginalOpenApp(id);
    }catch(e){gold1fFileOrAppFailure(id,'EmeraldOS Gold could not start this app.',e,'app');return false;}
  }
  window.Gold50.openApp=gold1fSafeOpenApp;
  openApp=gold1fSafeOpenApp;
}catch(e){console.warn('Gold 1F app failure wrapper skipped',e)}
try{
  const gold1fOriginalOpenFile = window.Gold50?.openFile || openFile;
  function gold1fSafeOpenFile(fid){
    try{
      ...
      return gold1fOriginalOpenFile(fid);
    }catch(e){gold1fFileOrAppFailure(fid,'EmeraldOS Gold could not open this file.',e,'file');return false;}
  }
  window.Gold50.openFile=gold1fSafeOpenFile;
  openFile=gold1fSafeOpenFile;
}catch(e){console.warn('Gold 1F file failure wrapper skipped',e)}

function gold1fReleaseManifest(){
  return {
    product:'EmeraldOS Gold', latestVersion:'1F', build:'1F', folder:'Gold_1F', entry:'OS.html',
    channel:'stable', status:'stable', required:false, enabled:true, setupMode:'migrationIntro',
    releaseTitle:'EmeraldOS Gold 1F',
    summary:'Gold 1F adds deeper customization, movable desktop objects, Trash, rollback tools, centered load-failure popups, crash recovery screens, and staff/PIN-gated E.L.S.U.S. publishing.',
    migrationFrom:['1B','1A','11.0'], migrationId:'gold1b-to-gold1f-desktop-rollback',
    minShellVersion:'1.0', rollbackFolder:'Gold_1B', rollbackVersion:'1B', releasedAt:new Date().toISOString()
  };
}
function gold1fPublisherUnlocked(){return localStorage.getItem(PREFIX+'publisher_pin_unlocked')==='true'}
async function gold1fRequirePublisherAccess(show=true){
  if(!isStaff()){
    if(show) gold1fCenterPopup('Staff verification required','Update publishing is only available after signing in through Gold Staff Edition.','danger',[{label:'Open Staff Edition',primary:true,action:()=>location.href='staff.html'}]);
    return false;
  }
  if(gold1fPublisherUnlocked()) return true;
  const pin=prompt('Update Publisher Manager PIN');
  if(pin!==GOLD1F_PUBLISHER_PIN){
    if(show) gold1fCenterPopup('Publisher PIN rejected','The Update Publisher Manager PIN was incorrect.','danger');
...
async function gold1fPublishLatest(showResult=true){
  if(!(await gold1fRequirePublisherAccess(showResult))) return false;
  try{
    const api=fb||await initFirebase();
    if(!api) throw new Error('Firebase module did not load.');
    const manifest=gold1fReleaseManifest();
    ...
    if(showResult) gold1fCenterPopup('E.L.S.U.S. published','system/emeraldGoldLatest now points to Gold_1F. Users will still update manually when prompted.','success');
    return true;
  }catch(e){
    console.warn('Gold 1F publish failed',e);
    if(showResult) gold1fCenterPopup('Publish failed','Firebase rejected the publish request. Check Firestore rules and staff write permissions.','danger',[{label:'Open Diagnostics',action:()=>Gold50.openApp('diagnostics')}]);
    return false;
  }
}

function openELSUSUpdateCenter1F(){
  const latest=read('emeraldGoldShell_latest',gold1fReleaseManifest());
  const update=read('emeraldGold_updateAvailable',{});
  openWindow('updateshell','E.L.S.U.S. System Update',`<div class="goldshell-header"><div><h2>E.L.S.U.S. System Update</h2><p class="muted">The update shell stays separate. This app shows update status and lets users restart through E.L.S.U.S. Publishing is staff-only.</p></div><button class="btn primary" onclick="location.href='../gold-shell.html?manualCheck=1'">Check through shell</button></div><div class="grid2"><div class="card"><h3>This VM folder</h3><p>EmeraldOS Gold 1F</p><p class="muted">Gold_1F / OS.html</p></div><div class="card"><h3>Latest known folder</h3><p>${esc(latest.latestVersion||'Unknown')}</p><p class="muted">${esc(latest.folder||'No folder')} / ${esc(latest.entry||'OS.html')}</p></div><div class="card"><h3>User-controlled updates</h3><p>Each user updates manually. Their active folder changes only after accepting the update prompt.</p></div><div class="card"><h3>Rollback</h3><p>Rollback can help recover from a bad update, but may cause app or file instability.</p><button class="btn" onclick="Gold50.openApp('rollback')">Open Rollback</button></div></div><h3>Actions</h3><div class="toolbar"><button class="btn primary" onclick="Gold50.saveWorkspaceNow(true)">Save VM now</button><button class="btn" onclick="Gold50.exportWorkspace()">Export backup</button><button class="btn" onclick="location.href='../gold-shell.html?manualCheck=1'">Restart through E.L.S.U.S.</button>${isStaff()?'<button class="btn danger" onclick="Gold50.openUpdatePublisherManager1F()">Update Publisher Manager</button>':''}</div><p class="muted">Last shell check: ${esc(update.checkedAt||'Not checked in this browser')}</p>`,{width:1080,height:740});
}
function openUpdatePublisherManager1F(){
  if(!isStaff()){gold1fStaffGateNotice();return}
  gold1fRequirePublisherAccess(true).then(ok=>{
    if(!ok)return;
    const hist=read(PREFIX+'publish_history',[]);
    openWindow('updatepublisher','Update Publisher Manager',`<div class="goldshell-header"><div><h2>Update Publisher Manager</h2><p class="muted">Staff-only E.L.S.U.S. publishing. A publisher PIN is required every session.</p></div><button class="btn danger" onclick="localStorage.removeItem('${PREFIX}publisher_pin_unlocked');Gold50.notify('Publisher locked','PIN access was cleared.','E.L.S.U.S.')">Lock publisher</button></div><div class="grid2"><div class="card"><h3>Publish Gold 1F</h3><p>Writes <code>system/emeraldGoldLatest</code> to point users to <code>Gold_1F/OS.html</code>. Users must still accept the update manually.</p><button class="btn primary" onclick="Gold50.gold1fPublishLatest(true)">Publish Gold 1F as latest</button></div><div class="card"><h3>Rollback target</h3><p>Default rollback: Gold 1B. Changing the public latest version should only be done if the current latest build breaks boot.</p><button class="btn" onclick="Gold50.gold1fPublishRollbackLatest('Gold_1B','1B')">Publish Gold 1B as emergency latest</button></div><div class="card"><h3>PIN status</h3><p>${gold1fPublisherUnlocked()?'Unlocked':'Locked'}</p></div><div class="card"><h3>Staff session</h3><p>${esc(username())}</p><p class="muted">Role: ${esc(currentStaff()?.role||localStorage.getItem('role')||'staff')}</p></div></div><h3>Recent publishing</h3>${hist.map(h=>`<div class="card"><b>${esc(h.manifest?.releaseTitle||'Publish')}</b><p class="muted">${esc(h.time)} · ${esc(h.user)}</p></div>`).join('')||'<p class="muted">No publish history on this device.</p>'}`,{width:1080,height:760});
  });
}
async function gold1fPublishRollbackLatest(folder='Gold_1B',version='1B'){
  if(!(await gold1fRequirePublisherAccess(true)))return false;
  ...
  try{
    ...
    const manifest={...gold1fReleaseManifest(), latestVersion:version, build:version, folder, releaseTitle:`EmeraldOS Gold ${version} Emergency Rollback`, summary:'Emergency rollback published by staff through Gold 1F Update Publisher Manager.', rollbackFolder:'Gold_1F', rollbackVersion:'1F', releasedAt:new Date().toISOString()};
    await api.setDoc(api.doc(api.db,'system','emeraldGoldLatest'),manifest,{merge:true});
    gold1fCenterPopup('Emergency rollback published',`E.L.S.U.S. now points to ${folder}.`,'success');
    return true;
  }catch(e){gold1fCenterPopup('Rollback publish failed',e.message||'Firebase rejected the rollback publish.','danger');return false;}
}

function openRollbackCenter1F(){
  openWindow('rollback','Version Rollback',`<div class="goldshell-header"><div><h2>Version Rollback</h2><p class="muted">Rollback changes only this user's selected E.L.S.U.S. folder unless staff publishes an emergency rollback.</p></div><button class="btn primary" onclick="Gold50.saveVMSnapshot()">Create snapshot</button></div><div class="danger-card"><h3>Rollback warning</h3><p>Rolling back can cause instability with newer applications, files, settings, or VM data. Create a snapshot first and only rollback if a new version is not working.</p></div><div class="grid2">${GOLD1F_ROLLBACK_TARGETS.map(t=>`<div class="card"><h3>${esc(t.title)}</h3><p class="muted">${esc(t.folder)} / ${esc(t.entry)}</p><button class="btn" onclick="Gold50.gold1fUserRollback('${esc(t.folder)}','${esc(t.version)}')">Rollback my VM to ${esc(t.version)}</button></div>`).join('')}</div>`,{width:1000,height:720});
}
function gold1fUserRollback(folder,version){
  ...
  const manifest={product:'EmeraldOS Gold',latestVersion:version,build:version,folder,entry:'OS.html',channel:'stable',status:'rollback',required:false,enabled:true,setupMode:'continue',releaseTitle:'EmeraldOS Gold '+version,summary:'User selected rollback through Gold 1F.',rollbackFrom:'Gold_1F',rolledBackAt:now()};
  ...
  gold1fCenterPopup('Rollback selected','E.L.S.U.S. will restart into '+folder+'.','warning',[{label:'Restart now',primary:true,action:()=>location.href='../gold-shell.html?rollback=1'}]);
}

function openThemeStudio1F(){
  const p=prefs();
  openWindow('themestudio','Theme Studio',`<h2>Theme Studio</h2><p class="muted">Gold 1F adds safer deep customization with protected CSS variables.</p><div class="grid2"><div class="card"><h3>Colors</h3><label>Accent<input type="color" class="field" value="${esc(p.accent||'#0078d4')}" oninput="Gold50.setPrefs({accent:this.value})"></label><label>Secondary accent<input type="color" class="field" value="${esc(p.secondaryAccent||'#0f7b4d')}" oninput="Gold50.setPrefs({secondaryAccent:this.value})"></label><label>Taskbar color<input type="color" class="field" value="${esc(p.taskbarColor||'#f6f9fd')}" oninput="Gold50.gold1fSetPref('taskbarColor',this.value)"></label><label>Taskbar text<input type="color" class="field" value="${esc(p.taskbarTextColor||'#111827')}" oninput="Gold50.gold1fSetPref('taskbarTextColor',this.value)"></label><label>Start menu color<input type="color" class="field" value="${esc(p.startMenuColor||'#ffffff')}" oninput="Gold50.gold1fSetPref('startMenuColor',this.value)"></label></div><div class="card"><h3>Desktop objects</h3><label>Desktop mode<select class="field" onchange="Gold50.gold1fSetPref('desktopObjectMode',this.value)"><option value="free" ${p.desktopObjectMode!=='grid'?'selected':''}>Free move</option><option value="grid" ${p.desktopObjectMode==='grid'?'selected':''}>Grid layout</option></select></label><label><input type="checkbox" ${p.desktopSnapToGrid!==false?'checked':''} onchange="Gold50.gold1fSetPref('desktopSnapToGrid',this.checked)"> Snap objects to grid</label><label>Grid size<input type="range" min="64" max="150" value="${esc(p.desktopGridSize||92)}" oninput="Gold50.gold1fSetPref('desktopGridSize',this.value)"></label><label><input type="checkbox" ${p.trashOnDesktop!==false?'checked':''} onchange="Gold50.gold1fSetPref('trashOnDesktop',this.checked)"> Show Trash on desktop</label><button class="btn" onclick="Gold50.gold1fResetDesktopPositions()">Reset icon positions</button></div><div class="card"><h3>Background</h3><label>Background color<input type="color" class="field" value="${esc(p.backgroundColor||'#eaf3ff')}" oninput="Gold50.setGold70Pref?.('backgroundColor',this.value);Gold50.setGold70Pref?.('useBackgroundColor',true);Gold50.gold1fSetPref('backgroundColor',this.value)"></label><label>Background URL<input class="field" value="${esc(p.customBackground||'')}" onchange="Gold50.setPrefs({customBackground:this.value})"></label><input type="file" class="field" accept="image/*" onchange="Gold50.loadBackgroundFile(this)"></div><div class="card"><h3>Reliability</h3><label><input type="checkbox" ${p.centerFailurePopups!==false?'checked':''} onchange="Gold50.gold1fSetPref('centerFailurePopups',this.checked)"> Center failure popups</label><label><input type="checkbox" ${p.bsodOnCriticalErrors!==false?'checked':''} onchange="Gold50.gold1fSetPref('bsodOnCriticalErrors',this.checked)"> Crash screen for critical errors</label><button class="btn danger" onclick="Gold50.gold1fBSOD('Manual crash screen test.','MANUAL_TEST')">Test crash screen</button></div></div>`,{width:1080,height:760});
}
function gold1fSetPref(key,value){const p={};p[key]=value;setPrefs(p)}
function gold1fResetDesktopPositions(){localStorage.removeItem(PREFIX+'desktopPositions');renderDesktop();notify('Desktop reset','Desktop object positions were reset.','Desktop')}

function gold1fInstallTrashContext(){
  document.addEventListener('contextmenu',e=>{
    const trash=e.target.closest('[data-trash]'); if(!trash)return;
    e.preventDefault(); e.stopPropagation();
    gold1fContextMenu([
      {id:'open',label:'Open Trash',action:()=>openTrashCan1F()},
      ...
      {id:'hide',label:'Hide Trash icon',action:()=>gold1fSetPref('trashOnDesktop',false)}
    ],e.clientX,e.clientY);
  },true);
}
...
window.addEventListener('error',e=>{
  try{
    ...
    gold1fCenterPopup('EmeraldOS Gold warning',msg,'warning',[{label:'Open Diagnostics',action:()=>Gold50.openApp('diagnostics')}]);
    if(prefs().bsodOnCriticalErrors && /ReferenceError|SyntaxError|not defined|Cannot read|Cannot set/.test(msg)) setTimeout(()=>gold1fBSOD(msg,'GOLD_UNHANDLED_ERROR'),250);
  }catch{}
});
...
function gold1fInstallApps(){
  gold1fAddOrUpdateApp({id:'updateshell',name:'E.L.S.U.S. System Update',label:'UP',color:'blue',group:'System',purpose:'User update status',desc:'Check update status, save the VM, restart through E.L.S.U.S., and open rollback tools.',open:openELSUSUpdateCenter1F});
  gold1fAddOrUpdateApp({id:'updatepublisher',name:'Update Publisher Manager',label:'PM',color:'red',group:'Staff',purpose:'Staff update publishing',desc:'Staff-only and PIN-gated E.L.S.U.S. publishing controls.',staffOnly:true,open:openUpdatePublisherManager1F});
  gold1fAddOrUpdateApp({id:'rollback',name:'Version Rollback',label:'RB',color:'orange',group:'System',purpose:'Rollback',desc:'Rollback this VM to a previous EmeraldOS Gold folder with instability warnings.',open:openRollbackCenter1F});
  gold1fAddOrUpdateApp({id:'crashrecovery',name:'Crash Recovery',label:'CR',color:'blue',group:'Support',purpose:'Reliability',desc:'Test and recover from EmeraldOS Gold crash screens and load failures.',open:()=>openWindow('crashrecovery','Crash Recovery',`<h2>Crash Recovery</h2><p class="muted">Use these tools if an app or file refuses to load.</p><div class="grid3"><button class="tile-button" onclick="Gold50.gold1fBSOD('Manual crash screen test.','MANUAL_TEST')">Test crash screen</button><button class="tile-button" onclick="Gold50.openApp('diagnostics')">Diagnostics</button><button class="tile-button" onclick="localStorage.setItem('${PREFIX}safemode','true');location.reload()">Restart in Safe Mode</button></div>`,{width:780,height:520})});
  try{const theme=APPS.find(a=>a.id==='themestudio'); if(theme) theme.open=openThemeStudio1F;}catch{}
}

Object.assign(window.Gold50||{}, {
  openTrashCan1F,gold1fCenterPopup,gold1fBSOD,gold1fPublishLatest,gold1fRequirePublisherAccess,
  openELSUSUpdateCenter1F,openUpdatePublisherManager1F,gold1fPublishRollbackLatest,openRollbackCenter1F,gold1fUserRollback,
  openThemeStudio1F,gold1fSetPref,gold1fResetDesktopPositions
});
window.Gold1F=window.Gold50;
if(new URLSearchParams(location.search).has('publishLatest')) setTimeout(()=>{ if(isStaff()) openUpdatePublisherManager1F(); else gold1fStaffGateNotice(); }, 1600);
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1fInstallApps();gold1fInstallDesktopDrag();gold1fInstallTrashContext();applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1F feature install failed',e)}},900));


/* =========================================================
   EMERALDOS GOLD 1F FINAL UX, LANGUAGE, ACCESSIBILITY,
   STAFF DIRECT CONTROL, AND SAFE CUSTOMIZATION LAYER
========================================================= */
const gold1fBaseDefaultPrefs = defaultPrefs;
defaultPrefs = function(){
  return {
    ...gold1fBaseDefaultPrefs(),
    language:"en",
    taskbarColor:"#f6f9fd",
...
applyPrefs = function(){
  ...
  const p=prefs();
  const root=document.documentElement;
  root.style.setProperty('--gold1fTaskbarColor',p.taskbarColor||'#f6f9fd');
  root.style.setProperty('--gold1fTaskbarText',p.taskbarTextColor||'#111827');
  root.style.setProperty('--gold1fStartColor',p.startMenuColor||'#ffffff');
  root.style.setProperty('--gold1fBackgroundColor',p.backgroundColor||'#eaf3ff');
  root.style.setProperty('--gold1fPanelOpacity',String(Math.max(.55,Math.min(1,Number(p.panelOpacity||.96)))));
  root.style.setProperty('--gold1fMenuOpacity',String(Math.max(.55,Math.min(1,Number(p.menuOpacity||.96)))));
  document.documentElement.lang=p.language==='es'?'es':'en';
  document.body.dataset.language=p.language==='es'?'es':'en';
  document.body.classList.toggle('gold1f-readable',!!p.readableFont);
  document.body.classList.toggle('gold1f-hit-large',!!p.largerHitTargets);
  document.body.classList.toggle('gold1f-underline-controls',!!p.underlineControls);
  document.body.classList.toggle('gold1f-extra-contrast',!!p.highContrastBorders);
  document.body.classList.remove('gold1f-filter-warm','gold1f-filter-cool','gold1f-filter-gray');
  if(p.colorFilter&&p.colorFilter!=='none') document.body.classList.add('gold1f-filter-'+p.colorFilter);
...
function gold1fMarkProcessed(cmd,session){
...
const gold1fBaseProcessRemoteCommand = processRemoteCommand;
processRemoteCommand = function(cmd,session){
  ...
  if(!directTypes.includes(t)) return gold1fBaseProcessRemoteCommand(cmd,session);
  if(!gold1fMarkProcessed(cmd,session)) return;
  try{
...
const gold1fBaseSnapshot = remoteDesktopSnapshot;
remoteDesktopSnapshot = function(){
  const snap=gold1fBaseSnapshot();
...
function gold1fInstallFinalApps(){
  gold1fAddOrUpdateApp({id:'language',name:'Language & Region',label:'LR',color:'teal',group:'Settings',purpose:'Language',desc:'Switch EmeraldOS Gold between English and Spanish core shell language.',open:openLanguageRegion1F});
  gold1fAddOrUpdateApp({id:'accessibility',name:'Accessibility Center',label:'AC',color:'blue',group:'Settings',purpose:'Accessibility',desc:'Readable text, contrast, reduced motion, focus rings, and language access.',open:openAccessibility1F});
  gold1fAddOrUpdateApp({id:'staffresources',name:'Staff Resources',label:'SR',color:'purple',group:'Staff',purpose:'Staff resources',desc:'Staff-only support procedures, E.L.S.U.S. notes, and remote support policy.',staffOnly:true,open:openStaffResources1F});
  try{const t=APPS.find(a=>a.id==='themestudio'); if(t)t.open=openThemeStudio1FPlus;}catch{}
  try{const s=APPS.find(a=>a.id==='staff'); if(s)s.staffOnly=true;}catch{}
}
Object.assign(window.Gold50||{}, {openLanguageRegion1F,openAccessibility1F,openThemeStudio1FPlus,openStaffResources1F,gold1fT});
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1fInstallFinalApps();applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1F final layer failed',e)}},1050));




/* =========================================================
   EMERALDOS GOLD 1F - LICENSED SETUP, TASK MANAGER,
   STAFF MAIL PORTAL, GAMES, APP-SPECIFIC LOGOS, SAFE UI
========================================================= */
(function gold1fMigrateFrom1D(){
  try{
    if(localStorage.getItem('gold1f_migrated_from_gold1d')==='true') return;
    const oldPrefix='gold1d_', newPrefix='gold1f_';
    Object.keys(localStorage).forEach(k=>{
      if(k.startsWith(oldPrefix)){
        const nk=newPrefix+k.slice(oldPrefix.length);
        if(localStorage.getItem(nk)===null) localStorage.setItem(nk, localStorage.getItem(k));
    ...
    localStorage.setItem('gold1f_migrated_from_gold1d','true');
    if(localStorage.getItem(newPrefix+'setup_done')==='true') localStorage.setItem(newPrefix+'updated_from','1D');
  }catch(e){console.warn('Gold 1F migration from 1D skipped',e)}
})();
const GOLD1F_TOS_URL='https://securly-plans.github.io/TOS.html';
const GOLD1F_PUBLISHER_PIN_FINAL='093013';
const GOLD1F_MANIFEST={product:'EmeraldOS Gold',latestVersion:'1F',build:'1F',folder:'Gold_1F',entry:'OS.html',channel:'stable',status:'stable',required:false,enabled:true,setupMode:'manualUpdateSetup',releaseTitle:'EmeraldOS Gold 1F',summary:'Gold 1F fixes Update Setup loading, normal-user boot routing, shell login enforcement, and makes First-Boot Setup global for E.L.S.U.S. compatible versions.',migrationFrom:['1E','1D','1C','1B','1A'],migrationId:'gold1e-to-gold1f-setup-login-fix',minShellVersion:'1.0',rollbackFolder:'Gold_1E',rollbackVersion:'1E',releasedAt:new Date().toISOString()};
function gold1fUpsertApp(def){try{const i=APPS.findIndex(a=>a.id===def.id); if(i>=0) Object.assign(APPS[i],def); else APPS.push(def);}catch(e){console.warn('Gold 1F app install failed',def?.id,e)}}
function gold1fLicenseText(){return `EmeraldOS Gold 1F Virtual License\n\nProduct: EmeraldOS Gold 1F\nEdition folder: Gold_1F\nUpdate service: EmeraldOS Live Service Update System (E.L.S.U.S.)\nLicense type: virtual machine-style browser workspace license\n\nThis license allows the signed-in EmeraldOS user to access one cloud-restored EmeraldOS Gold virtual workspace through the EmeraldOS shell or this OS folder. EmeraldOS Gold stores preferences, desktop layout, app data, files, support tickets, update selections, and VM snapshots in local browser storage and, where configured, Firebase cloud storage.\n\nBy continuing, you agree that EmeraldOS Gold can save and restore your VM state, run first-boot setup, run update setup after E.L.S.U.S. updates, show support and safety notices, and create recovery snapshots before updates or rollback.\n\nStaff remote support requires user consent, keeps a visible remote-control banner, and can be ended by the user at any time. Staff-only applications and resources require Staff Edition verification.\n\nUser-installed apps and Appstore uploads can affect your VM data. Only install apps you trust. Rollback to previous versions can cause instability with applications, files, settings, and unsupported app data.\n\nYou must read and agree to the Emerald Systems Terms of Service before using EmeraldOS Gold 1F. TOS location: ${GOLD1F_TOS_URL}\n\nThis virtual license is not a guarantee of permanent availability, uninterrupted service, or compatibility with every browser/device. EmeraldOS Gold is provided as a web-based virtual desktop experience for the signed-in account.\n\nAgreement record: EmeraldOS Gold 1F / Gold_1F / ${now()}`}
function gold1fLicenseHTML(kind){return `<div class="license-pane"><h2>${kind==='update'?'EmeraldOS Gold 1F Update Setup':'EmeraldOS Gold 1F First-Boot Setup'}</h2><p class="muted">Before continuing, read the Emerald Systems Terms of Service and accept the EmeraldOS Gold virtual license.</p><div class="license-box"><pre>${esc(gold1fLicenseText())}</pre></div><div class="tos-frame"><div class="toolbar"><a class="btn" target="_blank" href="${GOLD1F_TOS_URL}">Open Emerald Systems TOS</a><button class="btn" onclick="Gold50.gold1fCopyLicense()">Copy virtual license</button></div><iframe title="Emerald Systems TOS" src="${GOLD1F_TOS_URL}"></iframe></div><label class="license-agree"><input id="gold1f_license_${kind}" type="checkbox" onchange="Gold50.gold1fSetLicenseAgreement('${kind}',this.checked)"> I have had a chance to read the Emerald Systems TOS and I agree to the EmeraldOS Gold 1F Virtual License.</label></div>`}
function gold1fCopyLicense(){navigator.clipboard?.writeText(gold1fLicenseText()).then(()=>notify('License copied','The EmeraldOS Gold virtual license was copied.','Setup')).catch(()=>saveText('EmeraldOS-Gold-1F-Virtual-License.txt',gold1fLicenseText()))}
function gold1fSetLicenseAgreement(kind,checked){localStorage.setItem(PREFIX+'license_agreed_'+kind,checked?'true':'false'); const b=document.querySelector('.setup-actions .primary'); if(b)b.disabled=!gold1fCanAdvanceSetup(kind)}
function gold1fCanAdvanceSetup(kind){return localStorage.getItem(PREFIX+'license_agreed_'+kind)==='true'}
function openSetup(){
  const box=$('setupWizard'); if(!box) return;
  ...
  const steps=[
    ()=>`<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>Welcome to EmeraldOS Gold 1F</h2><p class="muted">Set up your EmeraldOS Gold virtual desktop once. Your setup, apps, files, preferences, support history, and VM state can be restored when you return.</p></div></div><div class="setup-checklist"><span>Cloud VM ready</span><span>Login protected</span><span>E.L.S.U.S. compatible</span></div></div>`,
    ()=>gold1fLicenseHTML('first'),
    ...
    ()=>`<h2>Choose essential desktop apps</h2><p class="muted">Only essential apps are placed on the desktop by default. Everything else stays available in Start/Search.</p><div class="setup-app-grid">${['explorer','office','mail','support','settings','updateshell','taskmanager','games'].map(id=>{const a=APPS.find(x=>x.id===id)||{id,name:id,label:id.slice(0,2)};return `<label class="setup-app-choice"><input type="checkbox" ${selected.has(id)?'checked':''} onchange="Gold50.gold1fToggleSetupApp('${id}',this.checked)">${appIcon(a,true)}<span>${esc(a.name)}</span></label>`}).join('')}</div>`,
    ...
    ()=>`<h2>Ready to start</h2><p>EmeraldOS Gold 1F will save this setup to your VM profile.</p><div class="card"><h3>Virtual license</h3><p class="muted">Agreement will be recorded for this VM account and version.</p></div>`
  ];
  function render(){box.classList.remove('hidden'); const kind='first'; box.innerHTML=`<div class="setup-card setup-card-rebuilt gold1f-setup"><div class="setup-titlebar"><b>EmeraldOS Gold 1F First-Boot Setup</b></div><div class="setup-steps">${steps.map((_,i)=>`<div class="setup-dot ${i<=step?'active':''}"></div>`).join('')}</div><div class="setup-body">${steps[step]()}</div><div class="toolbar setup-actions"><button class="btn" ${step===0?'disabled':''} onclick="Gold50.gold1fSetupStep(-1)">Back</button><span class="muted">Step ${step+1} of ${steps.length}</span><button class="btn primary" ${step===1&&!gold1fCanAdvanceSetup(kind)?'disabled':''} onclick="Gold50.gold1fSetupStep(1)">${step===steps.length-1?'Accept and finish':'Next'}</button></div></div>`}
  window.Gold50.gold1fSetupStep=(d)=>{ if(step===1 && d>0 && !gold1fCanAdvanceSetup('first')){notify('Agreement required','You must agree to the Emerald Systems TOS and virtual license to continue.','Setup');return;} if(step+d>=steps.length){finishSetup();return;} step=Math.max(0,Math.min(steps.length-1,step+d));render(); };
  window.Gold50.gold1fToggleSetupApp=(id,on)=>{on?selected.add(id):selected.delete(id);setPrefs({desktopApps:[...selected]})};
  render();
}
const gold1fBaseFinishSetup = typeof finishSetup==='function'?finishSetup:null;
finishSetup=function(){
  ...
  localStorage.setItem(PREFIX+'license_version','Gold_1F');
  localStorage.setItem(PREFIX+'license_time',now());
  $('setupWizard')?.classList.add('hidden');
  notify('Setup complete','EmeraldOS Gold 1F is ready.','Setup');
  saveWorkspaceNow?.(true);
};
function openUpdateSetup1F(){
  const box=$('setupWizard'); if(!box) return; let step=0;
  const steps=[
    ()=>`<div class="setup-hero"><div class="setup-brand"><div class="setup-logo">G</div><div><h2>EmeraldOS Gold 1F Update Setup</h2><p class="muted">Your VM updated through E.L.S.U.S. This short setup preserves your files and preferences while reviewing new terms and features.</p></div></div><div class="setup-checklist"><span>VM preserved</span><span>New app logos</span><span>Task Manager added</span></div></div>`,
    ()=>gold1fLicenseHTML('update'),
    ()=>`<h2>What changed in Gold 1F?</h2><div class="grid2"><div class="card"><h3>Setup and license</h3><p>First-Boot Setup and Update Setup now include the EmeraldOS Gold virtual license and TOS agreement.</p></div><div class="card"><h3>Apps</h3><p>Task Manager, Staff Mail Portal, and Gold Games were added without duplicating existing app purposes.</p></div><div class="card"><h3>Login and staff access</h3><p>Gold still requires normal login. Staff resources and apps require Staff Edition verification.</p></div><div class="card"><h3>Design</h3><p>App logos and shell surfaces were refreshed for a cleaner modern EmeraldOS Gold desktop.</p></div></div>`,
  ...
  function render(){box.classList.remove('hidden');box.innerHTML=`<div class="setup-card setup-card-rebuilt update-setup gold1f-setup"><div class="setup-titlebar"><b>Gold 1F Update Setup</b><button class="btn small" onclick="Gold50.finishUpdateSetup1F()">Skip after agreement</button></div><div class="setup-steps">${steps.map((_,i)=>`<div class="setup-dot ${i<=step?'active':''}"></div>`).join('')}</div><div class="setup-body">${steps[step]()}</div><div class="toolbar setup-actions"><button class="btn" ${step===0?'disabled':''} onclick="Gold50.updateSetupStep1F(-1)">Back</button><span class="muted">Step ${step+1} of ${steps.length}</span><button class="btn primary" ${step===1&&!gold1fCanAdvanceSetup('update')?'disabled':''} onclick="Gold50.updateSetupStep1F(1)">${step===steps.length-1?'Finish update setup':'Next'}</button></div></div>`}
  window.Gold50.updateSetupStep1F=(d)=>{ if(step===1 && d>0 && !gold1fCanAdvanceSetup('update')){notify('Agreement required','You must agree to the TOS and virtual license to continue.','Update Setup');return;} if(step+d>=steps.length){finishUpdateSetup1F();return;} step=Math.max(0,Math.min(steps.length-1,step+d));render(); };
  render();
}
function finishUpdateSetup1F(){
  ...
  localStorage.setItem(PREFIX+'license_time',now());
  $('setupWizard')?.classList.add('hidden');
  notify('Update setup complete','EmeraldOS Gold 1F is ready.','Update Setup');
  saveWorkspaceNow?.(true);
}
function gold1fInstallUpdateSetup(){
  if(gold1fShouldRunUpdateSetup()) setTimeout(openUpdateSetup1F,700);
}
function openTaskManager1F(){
...
function openStaffMailPortal1F(){
  if(!isStaff()){notify('Staff login required','Open Staff Edition and verify your staff account to access Staff Mail Portal.','Staff'); gold1fStaffGateNotice(); return;}
...
function openGoldGames1F(){
  openWindow('games','Gold Games',`<h2>Gold Games</h2><p class="muted">Built-in offline games for EmeraldOS Gold. Scores stay inside your VM profile.</p><div class="games-grid"><div class="game-card"><h3>Gold Clicker</h3><p>Simple score challenge.</p><button class="btn primary" onclick="Gold50.openGameClicker1F()">Play</button></div><div class="game-card"><h3>Memory Match</h3><p>Match Emerald cards.</p><button class="btn primary" onclick="Gold50.openGameMemory1F()">Play</button></div><div class="game-card"><h3>Gold Blocks</h3><p>Mini block counter puzzle.</p><button class="btn primary" onclick="Gold50.openGameBlocks1F()">Play</button></div></div>`,{width:820,height:560});
}
function openGameClicker1F(){let score=Number(localStorage.getItem(PREFIX+'clicker_score')||0);openWindow('game_clicker','Gold Clicker',`<h2>Gold Clicker</h2><div class="clicker-score" id="clicker_score">${score}</div><button class="btn primary big-game-btn" onclick="Gold50.gameClick1F()">Collect Gold</button><button class="btn" onclick="localStorage.setItem('${PREFIX}clicker_score','0');Gold50.openGameClicker1F()">Reset</button>`,{width:520,height:420})}
function gameClick1F(){let s=Number(localStorage.getItem(PREFIX+'clicker_score')||0)+1;localStorage.setItem(PREFIX+'clicker_score',String(s));const el=$('clicker_score');if(el)el.textContent=s;}
function openGameMemory1F(){const symbols=['G','E','★','◆','G','E','★','◆'].sort(()=>Math.random()-.5);write(PREFIX+'memory_board',symbols.map((s,i)=>({i,s,open:false,done:false})));write(PREFIX+'memory_pick',[]);renderMemory1F()}
function renderMemory1F(){const b=read(PREFIX+'memory_board',[]);openWindow('game_memory','Memory Match',`<h2>Memory Match</h2><div class="memory-board">${b.map(c=>`<button class="memory-card ${c.open||c.done?'open':''}" onclick="Gold50.memoryPick1F(${c.i})">${c.open||c.done?esc(c.s):'?'}</button>`).join('')}</div><button class="btn" onclick="Gold50.openGameMemory1F()">New game</button>`,{width:560,height:520})}
function memoryPick1F(i){let b=read(PREFIX+'memory_board',[]),p=read(PREFIX+'memory_pick',[]);const c=b.find(x=>x.i===i);if(!c||c.done||c.open)return;c.open=true;p.push(i);if(p.length===2){const [a,bid]=p.map(id=>b.find(x=>x.i===id));if(a&&bid&&a.s===bid.s){a.done=bid.done=true}else setTimeout(()=>{let nb=read(PREFIX+'memory_board',[]);nb.forEach(x=>{if(!x.done)x.open=false});write(PREFIX+'memory_board',nb);renderMemory1F()},700);p=[]}write(PREFIX+'memory_board',b);write(PREFIX+'memory_pick',p);renderMemory1F()}
function openGameBlocks1F(){const state=read(PREFIX+'blocks_state',{turns:12,score:0});openWindow('game_blocks','Gold Blocks',`<h2>Gold Blocks</h2><p class="muted">Pick blocks. Gold gives points, gray costs a turn.</p><div class="taskmgr-stats"><div><b>${state.score}</b><span>Score</span></div><div><b>${state.turns}</b><span>Turns</span></div></div><div class="blocks-board">${Array.from({length:12},(_,i)=>`<button class="block" onclick="Gold50.blocksPick1F(${i})">■</button>`).join('')}</div><button class="btn" onclick="localStorage.removeItem('${PREFIX}blocks_state');Gold50.openGameBlocks1F()">Reset</button>`,{width:560,height:520})}
function blocksPick1F(i){let s=read(PREFIX+'blocks_state',{turns:12,score:0});if(s.turns<=0)return; s.turns--; s.score+=Math.random()>.35?10:-3; write(PREFIX+'blocks_state',s); if(s.turns<=0)notify('Gold Blocks finished','Final score: '+s.score,'Gold Games'); openGameBlocks1F()}
function openUpdatePublisherManager1F(){
  if(!isStaff()){notify('Staff login required','You must be logged into Staff Edition to access Update Publisher Manager.','System Update'); gold1fStaffGateNotice(); return;}
  const unlocked=localStorage.getItem(PREFIX+'publisher_unlocked')==='true';
  openWindow('updatepublisher','Update Publisher Manager',`<h2>Update Publisher Manager</h2><p class="muted">Staff-only E.L.S.U.S. release publishing. Requires Staff Edition and the publisher PIN.</p>${!unlocked?`<div class="card"><h3>PIN required</h3><input class="field" id="publisher_pin" type="password" placeholder="Publisher PIN"><button class="btn primary" onclick="Gold50.unlockPublisher1F(document.getElementById('publisher_pin').value)">Unlock</button></div>`:`<div class="card success-card"><h3>Publisher unlocked</h3><p>Gold 1F can now publish its Firebase latest pointer for this staff session.</p></div><div class="toolbar"><button class="btn primary" onclick="Gold50.gold1fPublishLatest(true)">Publish Gold 1F as latest</button><button class="btn" onclick="Gold50.gold1fPublishRollback1D()">Emergency rollback to Gold 1D</button></div>`}<div class="card"><h3>Current manifest</h3><pre>${esc(JSON.stringify(GOLD1F_MANIFEST,null,2))}</pre></div>`,{width:940,height:720});
}
function unlockPublisher1F(pin){if(String(pin||'')===GOLD1F_PUBLISHER_PIN_FINAL){localStorage.setItem(PREFIX+'publisher_unlocked','true');notify('Publisher unlocked','Update Publisher Manager is available for this staff session.','System Update');openUpdatePublisherManager1F()}else notify('Incorrect PIN','The Update Publisher Manager PIN was not accepted.','System Update')}
async function gold1fPublishLatest(showResult=true){
  if(!isStaff()||localStorage.getItem(PREFIX+'publisher_unlocked')!=='true') {openUpdatePublisherManager1F();return false;}
  try{const api=fb||await initFirebase(); if(!api) throw new Error('Firebase module did not load.'); await api.setDoc(api.doc(api.db,'system','emeraldGoldLatest'), GOLD1F_MANIFEST,{merge:true}); localStorage.setItem(PREFIX+'latest_manifest_published','true'); write('emeraldGoldShell_latest',GOLD1F_MANIFEST); if(showResult)notify('E.L.S.U.S. updated','system/emeraldGoldLatest now points to Gold_1F.','System Update'); return true;}catch(e){console.warn('Gold 1F publish failed',e); if(showResult)notify('Publish failed',e.message,'System Update'); return false;}
}
async function gold1fPublishRollback1D(){
  if(!isStaff()||localStorage.getItem(PREFIX+'publisher_unlocked')!=='true') return openUpdatePublisherManager1F();
  const rollback={...GOLD1F_MANIFEST,latestVersion:'1D',build:'1D',folder:'Gold_1D',status:'rollback',releaseTitle:'Emergency rollback to EmeraldOS Gold 1D',summary:'Staff emergency rollback from Gold 1F.',rollbackFrom:'Gold_1F',rolledBackAt:now()};
...
function gold1fInstallApps(){
  gold1fUpsertApp({id:'taskmanager',name:'Task Manager',label:'TM',color:'blue',group:'System',purpose:'Process management',desc:'View open windows, local VM storage, session state, and end tasks.',open:openTaskManager1F});
  gold1fUpsertApp({id:'staffmail',name:'Staff Mail Portal',label:'SM',color:'teal',group:'Staff',purpose:'Staff webmail portal',desc:'Staff-only Emerald Mail portal through mail.monroecorp.org.',staffOnly:true,open:openStaffMailPortal1F});
  gold1fUpsertApp({id:'games',name:'Gold Games',label:'GG',color:'gold',group:'Games',purpose:'Built-in games',desc:'Offline EmeraldOS Gold games hub.',open:openGoldGames1F});
  gold1fUpsertApp({id:'updatepublisher',name:'Update Publisher Manager',label:'UP',color:'red',group:'Staff',purpose:'Staff E.L.S.U.S. publishing',desc:'Staff-only update publishing with publisher PIN.',staffOnly:true,open:openUpdatePublisherManager1F});
...
// Tighten publish functions from earlier layers so publishing is staff + PIN only in Gold 1F.
gold1fPublishLatestManifest = gold1fPublishLatest;
gold1fPublishLatest = gold1fPublishLatest;
function gold1fSafeRenderAll(){try{applyPrefs();renderDesktop();renderStartMenu();renderTaskApps();}catch(e){console.warn('Gold 1F render refresh failed',e)}}
Object.assign(window.Gold50||{}, {gold1fLicenseText,gold1fLicenseHTML,gold1fCopyLicense,gold1fSetLicenseAgreement,gold1fSetupStep:null,openUpdateSetup1F,finishUpdateSetup1F,updateSetupStep1F:null,openTaskManager1F,openStaffMailPortal1F,openGoldGames1F,openGameClicker1F,gameClick1F,openGameMemory1F,memoryPick1F,openGameBlocks1F,blocksPick1F,openUpdatePublisherManager1F,unlockPublisher1F,gold1fPublishLatest,gold1fPublishRollback1D});
window.Gold1F=window.Gold50;
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{try{gold1fInstallApps();gold1fInstallUpdateSetup();gold1fSafeRenderAll();}catch(e){console.warn('Gold 1F layer failed',e)}},1300));
if(new URLSearchParams(location.search).has('publishLatest')) setTimeout(()=>{ if(isStaff()) openUpdatePublisherManager1F(); else gold1fStaffGateNotice(); },1700);
