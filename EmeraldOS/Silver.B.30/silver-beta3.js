...
(function () {
    ...
    const BUILD = {
        ...
        displayName: "EmeraldOS Silver Beta 3.0",
        channel: "Beta",
        version: "2.0",
        ...
        sessionCollection: "silverBeta30"
    };
    ...
    function boot() {
        ...
        silverNotify("Silver Beta", "Welcome to EmeraldOS Silver Beta 3.0.", "System", "info");
    }
...
(function () {
    ...
    const BUILD = {
        name: "EmeraldOS Silver Beta 3.0",
        storagePrefix: "silver30_",
        cloudCollection: "emeraldOSUsers",
    ...
    function applyDesktopPrefs() {
        ...
        if (badge) badge.textContent = "Silver Beta 3.0";
    ...
    function defaultDrive() {
        return [
            ...
            { id:"slide_plan", type:"eslide", title:"Project Presentation", updatedAt:new Date().toISOString(), slides:[{title:"Silver Beta 3.0",body:"Custom desktop, responsive UI, and Drive-like Office."}] }
        ];
    }
    ...
    window.openSilverUpdates11 = function () { openWindowSilver("Silver Updates", `<div class="silver-beta-shell"><div class="silver-beta-header">${iconHTML("sync")}<div><h2>Silver Updates</h2><p>Current build: EmeraldOS Silver Beta 3.0</p></div></div><ul><li>Customizable desktop folders.</li><li>Responsive Silver shell.</li><li>Drive-like Silver Office workspace.</li><li>More Silver applications.</li><li>Improved UI polish.</li></ul></div>`, "updates-11"); };
    ...
    function boot() {
        ...
        notify("Silver Beta 3.0", "Custom desktop folders and Silver Drive are ready.", "System", "success");
    }
...
(function(){
    ...
    const KEY = {
        prefs: "silver30_consolidated_desktop_prefs",
        files: "silver30_drive_items",
        view: "silver30_drive_view",
        notices: "silver30_universal_notices"
    };
    ...
    function renderDesktop(){
        const desktop=document.getElementById("desktop"); if(!desktop) return;
        document.body.dataset.theme="silver-beta30";
        desktop.innerHTML="";
        const p=prefs();
    ...
    function addSidebar(){
        ...
        side.innerHTML=`<div class="gadget"><b>Silver Beta 3.0</b><small>No editions in this beta</small></div><div class="gadget"><b>${esc(username())}</b><small>Workspace user</small></div><div class="gadget"><b>Cloud</b><small>Ready when Firebase is available</small></div><div class="gadget"><button onclick="openSilverWorkspace20()">Home</button><button onclick="openSilverDriveHub20()">Drive</button></div>`;
        document.body.appendChild(side);
    }
    function noEditionsNotice(){ return `<div class="silver20-no-editions"><b>Editions are temporarily removed in Silver Beta 3.0.</b><br>All Silver Beta 3.0 apps are available while this product line is being tested. Edition controls can be restored in a later release.</div>`; }
    ...
    window.openSilverDesktopCustomizer20=function(){
        ...
        const html=`<div class="silver20-shell"><div class="silver-beta-header">${logoHTML("settings")}<div><h2>Silver Desktop Customizer</h2><p>Everything on the Silver desktop is customizable. Removing a folder from desktop does not delete it.</p></div></div>${noEditionsNotice()}<div class="silver11-settings-grid"><section><h3>Desktop Items</h3>${checks}</section><section><h3>Layout</h3><label>Icon Size<select id="silver30_icon"><option ${p.iconSize==='compact'?'selected':''}>compact</option><option ${p.iconSize==='normal'?'selected':''}>normal</option><option ${p.iconSize==='large'?'selected':''}>large</option></select></label><label>Density<select id="silver30_density"><option ${p.density==='compact'?'selected':''}>compact</option><option ${p.density==='comfortable'?'selected':''}>comfortable</option><option ${p.density==='spacious'?'selected':''}>spacious</option></select></label><label class="silver11-check"><input id="silver30_sidebar_toggle" type="checkbox" ${p.sidebar?"checked":""}> Show sidebar gadgets</label><button onclick="silver20SaveDesktopCustomizer()">Save Desktop</button><button onclick="silver20ResetDesktop()">Reset Defaults</button></section></div></div>`;
        open("Silver Desktop Customizer",html,"silver-desktop-customizer-20");
    };
    window.silver20SaveDesktopCustomizer=function(){ const p=prefs(); p.desktop=[...document.querySelectorAll('[data-silver20-desk]:checked')].map(i=>i.dataset.silver20Desk); p.iconSize=document.getElementById("silver30_icon")?.value||"normal"; p.density=document.getElementById("silver30_density")?.value||"comfortable"; p.sidebar=!!document.getElementById("silver30_sidebar_toggle")?.checked; savePrefs(p); notify("Desktop", "Silver desktop saved.", "Desktop", "success"); };
    window.silver20ResetDesktop=function(){ write(KEY.prefs, defaultPrefs()); renderDesktop(); notify("Desktop", "Silver Beta 3.0 defaults restored.", "Desktop", "success"); };
    ...
    function patchStart(){
        ...
        const build=document.getElementById("emerald40-build-badge"); if(build) build.textContent="Silver Beta 3.0";
        const results=document.getElementById("start-results");
    ...
    function boot(){
        document.body.dataset.theme="silver-beta30";
        localStorage.setItem("silver30_editions_temporarily_disabled", "true");
        patchStart(); renderDesktop();
        setTimeout(()=>{ patchStart(); renderDesktop(); }, 600);
        setTimeout(()=>{ patchStart(); }, 1400);
        notify("Silver Beta 3.0", "Consolidated workspace, no-editions beta mode, and Vista-like app logos are active.", "System", "success");
    }
    if(document.readyState==="loading") document.addEventListener("DOMContentLoaded", boot); else boot();
})();
...
/* =========================================================
   EMERALDOS SILVER BETA 3.0 PRODUCT-LINE SHELL
   - Separate Silver UX layer
   - Vista-inspired original UI
   - Consolidated Silver hubs
   - Universal sidebar and desktop
========================================================= */
(function(){
    if (window.EmeraldOSSilverBeta30Loaded) return;
    window.EmeraldOSSilverBeta30Loaded = true;
    ...
    const BUILD = {
        ...
        channel: "Beta",
        version: "3.0",
        codename: "Aero Workspace",
        storagePrefix: "silver30_"
    };
    const KEY = {
        prefs: BUILD.storagePrefix + "prefs",
        notices: BUILD.storagePrefix + "notices",
        activity: BUILD.storagePrefix + "activity",
        docs: BUILD.storagePrefix + "docs",
        vm: BUILD.storagePrefix + "vm_state"
    };

    const LOGOS30 = {
        workspace:["S","workspace"], drive:["DR","drive"], office:["OF","office"], docs:["DC","docs"], sheets:["Σ","sheets"], slides:["SL","slides"], forms:["FM","office"], mail:["✉","mail"], chat:["CH","chat"], people:["PE","people"], calendar:["31","office"], notes:["NT","office"], tasks:["✓","office"], journal:["JR","office"], photos:["IMG","media"], media:["▶","media"], creator:["{ }","creator"], store:["ST","creator"], code:["JS","creator"], system:["⚙","system"], settings:["CP","system"], personalize:["PS","system"], sync:["SY","system"], security:["SC","security"], recovery:["RC","system"], help:["?","help"], feedback:["FB","feedback"], network:["NW","system"], bell:["NO","system"], search:["⌕","system"], folder:["▣","drive"], updates:["UP","system"]
    };
    const APPS30 = [
        {id:"workspace", name:"Silver Workspace", logo:"workspace", category:"Core", desc:"The main Silver dashboard with resume, recent work, notifications, and quick actions.", desktop:true, run:"openSilver30Workspace"},
        {id:"driveoffice", name:"Silver Drive + Office", logo:"drive", category:"Drive & Office", desc:"Consolidated Drive, Docs, Sheets, Slides, Forms, templates, and vault.", desktop:true, run:"openSilver30DriveOffice"},
        {id:"communications", name:"Silver Communications", logo:"mail", category:"Communication", desc:"Mail, Chat, People, contacts, blocking, and unread message notifications.", desktop:true, run:"openSilver30Communications"},
        {id:"media", name:"Silver Media Center", logo:"media", category:"Media", desc:"Photos, gallery, media launcher, and presentation-friendly viewing.", desktop:true, run:"openSilver30MediaCenter"},
        {id:"creator", name:"Silver Creator Studio", logo:"creator", category:"Creator", desc:"Application Editor, Code Studio, App Market, API docs, themes, and icons.", desktop:true, run:"openSilver30Creator"},
        {id:"control", name:"Silver Control Panel", logo:"settings", category:"System", desc:"The central control panel for desktop, network, security, personalization, and recovery.", desktop:true, run:"openSilver30ControlPanel"},
        {id:"apps", name:"All Silver Apps", logo:"folder", category:"Core", desc:"Every Silver app and folder remains available here even if removed from the desktop.", desktop:true, run:"openSilver30Apps"},
        {id:"notifications", name:"Notifications", logo:"bell", category:"Core", desc:"Universal unread messages, shared files, mail, sync, appstore, and system alerts.", run:"openSilver30Notifications"},
        {id:"resume", name:"Resume Center", logo:"sync", category:"Core", desc:"Save and restore the cloud workspace across devices.", run:"openSilver30Resume"},
        {id:"drive", name:"Silver Drive", logo:"drive", category:"Drive & Office", desc:"Drive-style file manager with folders, recent, starred, trash, and preview.", run:"openSilverDrive20"},
        {id:"docs", name:"Silver Docs", logo:"docs", category:"Drive & Office", desc:"Word processor with templates, print/export, and Drive save.", run:"silver20NewDoc"},
        {id:"sheets", name:"Silver Sheets", logo:"sheets", category:"Drive & Office", desc:"Spreadsheet workspace for simple tables and totals.", run:"silver20NewSheet"},
        {id:"slides", name:"Silver Slides", logo:"slides", category:"Drive & Office", desc:"Slides and presentation launcher.", run:"silver20NewSlides"},
        {id:"forms", name:"Silver Forms", logo:"forms", category:"Drive & Office", desc:"Basic form builder.", run:"silver20NewForm"},
        {id:"mail", name:"Silver Mail", logo:"mail", category:"Communication", desc:"Internal EmeraldOS mail for Silver users.", run:"openSilverBetaMail"},
        {id:"chat", name:"Silver Chat", logo:"chat", category:"Communication", desc:"Integrated chat and rooms.", run:"openSilverBetaChat"},
        {id:"people", name:"Silver People", logo:"people", category:"Communication", desc:"User directory, profiles, contacts, and blocking.", run:"openSilverBetaPeople"},
        {id:"calendar", name:"Silver Calendar", logo:"calendar", category:"Productivity", desc:"Calendar and schedule.", run:"openSilverBetaCalendar"},
        {id:"notes", name:"Silver Notes", logo:"notes", category:"Productivity", desc:"Silver notes.", run:"openSilverBetaNotes"},
        {id:"tasks", name:"Silver Tasks", logo:"tasks", category:"Productivity", desc:"Tasks and planning.", run:"openSilverBetaTasks"},
        {id:"journal", name:"Silver Journal", logo:"journal", category:"Productivity", desc:"Private journal.", run:"openSilverBetaJournal"},
        {id:"photos", name:"Silver Photo Gallery", logo:"photos", category:"Media", desc:"Photo and image hub.", run:"openSilverBetaGallery"},
        {id:"mediaapp", name:"Silver Media", logo:"media", category:"Media", desc:"Media player launcher.", run:"openSilverBetaMedia"},
        {id:"appmarket", name:"Silver App Market", logo:"store", category:"Creator", desc:"User Appstore with safety warning and install flow.", run:"openSilverBetaAppMarket"},
        {id:"applibrary", name:"Silver App Library", logo:"store", category:"Creator", desc:"Installed user apps and .eapp tools.", run:"openSilverBetaAppLibrary"},
        {id:"codestudio", name:"Silver Code Studio", logo:"code", category:"Creator", desc:"Coding tools, snippets, scanner, and publishing checks.", run:"openSilverBetaCodeStudio"},
        {id:"assistant", name:"Silver Assistant", logo:"help", category:"Creator", desc:"Assistant sidebar, offline tips, and API settings.", run:"openSilverBetaAssistant"},
        {id:"personalization", name:"Silver Personalization", logo:"personalize", category:"System", desc:"Themes, wallpapers, glass effects, icon sizes, and desktop style.", run:"openSilver30Personalization"},
        {id:"network", name:"Network and Sync", logo:"network", category:"System", desc:"Cloud, sync, workspace continuity, and connection status.", run:"openSilver30Network"},
        {id:"security", name:"Silver Security Center", logo:"security", category:"System", desc:"Privacy, blocking, app risk scanning, and safety controls.", run:"openSilverBetaSecurity"},
        {id:"recovery", name:"Silver Recovery", logo:"recovery", category:"System", desc:"Safe Mode, reset tools, and desktop repair.", run:"openSilverBetaRecovery"},
        {id:"updates", name:"Silver Updates", logo:"updates", category:"System", desc:"Product line information and build notes.", run:"openSilver30Updates"},
        {id:"help", name:"Help and Support", logo:"help", category:"Support", desc:"Getting started, shortcuts, and troubleshooting.", run:"openSilver30Help"},
        {id:"feedback", name:"Silver Feedback", logo:"feedback", category:"Support", desc:"Feedback and bug reports.", run:"openSilverBetaFeedback"}
    ];
    const FOLDERS30 = [
        {id:"core", name:"Core Workspace", logo:"workspace", desc:"Dashboard, resume, notifications, and all apps.", apps:["workspace","apps","notifications","resume"]},
        {id:"driveoffice", name:"Drive & Office", logo:"drive", desc:"Drive, Docs, Sheets, Slides, Forms, and templates.", apps:["driveoffice","drive","docs","sheets","slides","forms"]},
        {id:"communication", name:"Communication", logo:"mail", desc:"Mail, Chat, People, contacts, and blocks.", apps:["communications","mail","chat","people"]},
        {id:"productivity", name:"Productivity", logo:"office", desc:"Calendar, Notes, Tasks, and Journal.", apps:["calendar","notes","tasks","journal"]},
        {id:"media", name:"Media", logo:"media", desc:"Photo Gallery and Media Center.", apps:["media","photos","mediaapp"]},
        {id:"creator", name:"Creator Platform", logo:"creator", desc:"App Market, App Library, Code Studio, and Assistant.", apps:["creator","appmarket","applibrary","codestudio","assistant"]},
        {id:"system", name:"System", logo:"system", desc:"Control Panel, Personalization, Network, Security, Recovery, and Updates.", apps:["control","personalization","network","security","recovery","updates"]},
        {id:"support", name:"Support", logo:"help", desc:"Help and feedback.", apps:["help","feedback"]}
    ];

    function esc(s){ return String(s ?? "").replace(/[&<>'"]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c])); }
    function user(){ return localStorage.getItem("username") || localStorage.getItem("40_username") || "Guest"; }
    function read(k,d){ try { return JSON.parse(localStorage.getItem(k)) ?? d; } catch { return d; } }
    function write(k,v){ localStorage.setItem(k, JSON.stringify(v)); }
    function open(title, html, id){ if (typeof window.openWindow === "function") window.openWindow(title, html, id || title.replace(/\W+/g,"-").toLowerCase()); else alert(title); }
    function call(fn){ if (typeof window[fn] === "function") return window[fn](); silver30Notice("Unavailable", fn + " is not loaded in this build.", "System", "warning"); }
    function logo(id, big=false){ const i=LOGOS30[id] || LOGOS30.workspace; return `<span class="silver30-logo silver30-logo-${esc(i[1])}" ${big?'style="width:70px;height:70px;font-size:25px;border-radius:18px"':''}><span>${esc(i[0])}</span></span>`; }
    function card(app){ if(!app) return ""; return `<div class="silver30-card" onclick="${esc(app.run)}()">${logo(app.logo)}<div><b>${esc(app.name)}</b><small>${esc(app.desc)}</small></div></div>`; }
    function hero(id,title,sub){ return `<div class="silver30-shell"><div class="silver30-hero">${logo(id,true)}<div><h2>${esc(title)}</h2><p>${esc(sub)}</p></div></div>`; }
    function end(){ return `</div>`; }
    function categoryGrid(apps){ return `<div class="silver30-grid">${apps.map(card).join("")}</div>`; }
    function app(id){ return APPS30.find(a=>a.id===id); }
    function defaults(){ return { desktop:["core","driveoffice","communication","creator","system","media"], sidebar:true, density:"comfortable", iconSize:"normal" }; }
    function prefs(){ return {...defaults(), ...read(KEY.prefs,{})}; }
    function savePrefs(p){ write(KEY.prefs,p); renderDesktop(); renderSidebar(); }

    window.silver30Notice = function(title,msg,cat="System",type="info"){
        if (typeof window.silverNotify === "function") { try { return window.silverNotify(title,msg,cat,type); } catch{} }
        const arr=read(KEY.notices,[]); arr.unshift({id:Date.now()+Math.random(), title, msg, cat, type, read:false, time:new Date().toISOString()}); write(KEY.notices,arr);
        const bell=document.getElementById("silver-bell"); if(bell){ const unread=arr.filter(n=>!n.read).length; bell.textContent=String(unread); bell.classList.toggle("has-unread",unread>0); }
    };

    function renderDesktop(){
        const desktop=document.getElementById("desktop"); if(!desktop) return;
        const p=prefs(); const items=p.desktop.map(id => FOLDERS30.find(f=>f.id===id) || app(id)).filter(Boolean);
        desktop.innerHTML = items.map(item => `<div class="silver30-desktop-item" tabindex="0" onclick="${FOLDERS30.some(f=>f.id===item.id)?`openSilver30Folder('${esc(item.id)}')`:esc(item.run)+'()'}">${logo(item.logo,true)}<span class="silver30-desktop-label">${esc(item.name)}</span></div>`).join("");
        document.body.dataset.silverDensity=p.density;
        document.body.dataset.silverIconSize=p.iconSize;
    }
    function renderSidebar(){
        let side=document.getElementById("silver30-sidebar"); const p=prefs();
        if(!p.sidebar){ if(side) side.remove(); return; }
        if(!side){ side=document.createElement("aside"); side.id="silver30-sidebar"; document.body.appendChild(side); }
        const notices = read(KEY.notices,[]).filter(n=>!n.read).length;
        const activity = read(KEY.activity,[]).slice(0,4);
        side.innerHTML = `<div class="silver30-gadget"><h3>Silver Clock</h3><div class="silver30-bigclock" id="silver30-clock">--:--</div><p>${new Date().toLocaleDateString()}</p></div>
        <div class="silver30-gadget"><h3>Workspace</h3><p>User: <b>${esc(user())}</b></p><p>Build: <b>Silver Beta 3.0</b></p><p>Mode: Separate Silver shell</p></div>
        <div class="silver30-gadget"><h3>Notifications</h3><p><b>${notices}</b> unread items</p><button onclick="openSilver30Notifications()">Open Notifications</button></div>
        <div class="silver30-gadget"><h3>Recent Activity</h3>${activity.length?activity.map(a=>`<p>${esc(a)}</p>`).join(""):`<p>No recent Silver activity yet.</p>`}</div>`;
        setTimeout(updateClock,50);
    }
    function updateClock(){ const c=document.getElementById("silver30-clock"); if(c) c.textContent=new Date().toLocaleTimeString([], {hour:"2-digit", minute:"2-digit"}); }
    setInterval(updateClock, 1000*30);
    function activity(msg){ const arr=read(KEY.activity,[]); arr.unshift(new Date().toLocaleTimeString([], {hour:"2-digit", minute:"2-digit"})+" — "+msg); write(KEY.activity,arr.slice(0,20)); renderSidebar(); }

    window.openSilver30Folder=function(id){
        const f=FOLDERS30.find(x=>x.id===id); if(!f) return;
        const apps=f.apps.map(app).filter(Boolean);
        open(f.name, hero(f.logo,f.name,f.desc)+categoryGrid(apps)+`<div class="silver30-toolbar"><button onclick="silver30RemoveDesktopItem('${esc(id)}')">Remove Folder from Desktop</button><button onclick="openSilver30DesktopCustomizer()">Customize Desktop</button></div>`+end(), "silver-folder-"+id);
    };
    window.openSilver30Apps=function(){
        const cats=[...new Set(APPS30.map(a=>a.category))];
        const html=hero("folder","All Silver Apps","Application folders and every Silver app remain available here even if removed from the desktop.")+
        `<div class="silver30-toolbar"><button onclick="openSilver30DesktopCustomizer()">Customize Desktop</button><button onclick="openSilver30Search()">Search Apps</button></div>`+
        cats.map(cat=>`<h3>${esc(cat)}</h3>${categoryGrid(APPS30.filter(a=>a.category===cat))}`).join("")+end();
        open("All Silver Apps",html,"silver-all-apps-30");
    };
    window.openSilver30Workspace=function(){
        const html=hero("workspace","Silver Workspace","A separate Silver desktop experience with Vista-inspired glass UI, cloud continuity, and consolidated hubs.")+
        `<div class="silver30-toolbar"><button onclick="openSilver30Resume()">Resume Workspace</button><button onclick="openSilver30DriveOffice()">Open Drive + Office</button><button onclick="openSilver30Notifications()">Notifications</button><button onclick="openSilver30ControlPanel()">Control Panel</button></div>`+
        `<div class="silver30-grid">${["driveoffice","communications","creator","control","media","apps"].map(id=>card(app(id))).join("")}</div>`+end();
        open("Silver Workspace",html,"silver-workspace-30"); activity("Opened Silver Workspace");
    };
    window.openSilver30DriveOffice=function(){
        const html=hero("drive","Silver Drive + Office","A Drive-like workspace for documents, sheets, slides, forms, folders, templates, and saved files.")+
        `<div class="silver30-toolbar"><button onclick="openSilverDrive20()">Open Silver Drive</button><button onclick="silver20NewDoc()">New Doc</button><button onclick="silver20NewSheet()">New Sheet</button><button onclick="silver20NewSlides()">New Slides</button><button onclick="silver20NewForm()">New Form</button><button onclick="openSilver30OfficePolish()">Office Overview</button></div>`+
        categoryGrid([app("drive"),app("docs"),app("sheets"),app("slides"),app("forms"),{name:"Templates",logo:"office",desc:"Open the Silver Office template gallery.",run:"openSilverTemplates11"}])+end();
        open("Silver Drive + Office",html,"silver-drive-office-30"); activity("Opened Silver Drive + Office");
    };
    window.openSilver30OfficePolish=function(){
        open("Silver Office", hero("office","Silver Office","A cleaner Drive-first office suite built into the Silver workspace.")+
        `<div class="silver30-grid"><div class="silver30-card" onclick="silver20NewDoc()">${logo("docs")}<div><b>Silver Docs</b><small>Documents with templates, export, print, and autosave status.</small></div></div><div class="silver30-card" onclick="silver20NewSheet()">${logo("sheets")}<div><b>Silver Sheets</b><small>Spreadsheets with simple tables and CSV-friendly layout.</small></div></div><div class="silver30-card" onclick="silver20NewSlides()">${logo("slides")}<div><b>Silver Slides</b><small>Presentations and slide decks.</small></div></div><div class="silver30-card" onclick="openSilverDrive20()">${logo("drive")}<div><b>Silver Drive</b><small>Every Office item starts and ends in Drive.</small></div></div></div>`+end(), "silver-office-30");
    };
    window.openSilver30Communications=function(){ open("Silver Communications", hero("mail","Silver Communications","Mail, Chat, People, contacts, blocking, and message notifications.")+categoryGrid([app("mail"),app("chat"),app("people"),app("notifications")])+end(), "silver-comms-30"); };
    window.openSilver30MediaCenter=function(){ open("Silver Media Center", hero("media","Silver Media Center","Photo Gallery, media tools, and presentation-friendly viewing.")+categoryGrid([app("photos"),app("mediaapp"),app("slides")])+end(), "silver-media-30"); };
    window.openSilver30Creator=function(){ open("Silver Creator Studio", hero("creator","Silver Creator Studio","Application Editor, Code Studio, App Market, App Library, API docs, and customizations.")+categoryGrid([app("appmarket"),app("applibrary"),app("codestudio"),app("assistant")])+end(), "silver-creator-30"); };
    window.openSilver30ControlPanel=function(){ open("Silver Control Panel", hero("settings","Silver Control Panel","Personalization, system settings, network, sync, security, recovery, and support.")+categoryGrid([app("personalization"),app("network"),app("security"),app("recovery"),app("updates"),app("help")])+end(), "silver-control-30"); };
    window.openSilver30Personalization=function(){ open("Silver Personalization", hero("personalize","Silver Personalization","Tune the Silver desktop, glass effects, icons, sidebar, wallpaper, and layout.")+`<div class="silver30-toolbar"><button onclick="openSilver30DesktopCustomizer()">Desktop Customizer</button><button onclick="silver30SetSidebar(true)">Show Sidebar</button><button onclick="silver30SetSidebar(false)">Hide Sidebar</button><button onclick="silver30ResetDesktop()">Reset Desktop</button></div>`+end(), "silver-personalization-30"); };
    window.openSilver30Network=function(){ open("Network and Sync", hero("network","Network and Sync","Cloud workspace status, Silver Drive sync, and device continuity.")+`<div class="silver30-toolbar"><button onclick="openSilver30Resume()">Resume Center</button><button onclick="openSilverSyncCenter20()">Sync Center</button><button onclick="silverCloudSaveSession?.()">Save Session</button></div>`+end(), "silver-network-30"); };
    window.openSilver30Notifications=function(){ if(typeof window.openSilverNotifications20==="function") return window.openSilverNotifications20(); open("Notifications", hero("bell","Universal Notifications","Unread mail, chat, shared files, sync, appstore, and system notices.")+end(), "silver-notifications-30"); };
    window.openSilver30Resume=function(){ if(typeof window.openSilverResumeCenter20==="function") return window.openSilverResumeCenter20(); open("Resume Center", hero("sync","Resume Center","Save and restore your Silver workspace across devices.")+end(), "silver-resume-30"); };
    window.openSilver30Updates=function(){ open("Silver Updates", hero("updates","EmeraldOS Silver Beta 3.0","A standalone Silver product-line shell with Vista-inspired glass UI, stronger consolidation, and a separate Silver app experience.")+`<ul><li>Separate Silver desktop shell and Start experience.</li><li>Vista-inspired original glass UI, orb button, taskbar, windows, and sidebar gadgets.</li><li>Consolidated hubs replace scattered app clutter.</li><li>All apps have refreshed glass-style app logos.</li><li>Application folders remain available through All Silver Apps.</li><li>Base EmeraldOS features are kept only as compatibility services behind the Silver interface.</li></ul>`+end(), "silver-updates-30"); };
    window.openSilver30Help=function(){ open("Help and Support", hero("help","Silver Help and Support","Guides for using Silver as a cloud workspace.")+`<div class="silver30-grid"><div class="silver30-card">${logo("workspace")}<div><b>Start with Silver Workspace</b><small>Use it as your home screen for recent work and quick actions.</small></div></div><div class="silver30-card">${logo("drive")}<div><b>Use Drive + Office</b><small>Make documents from Drive and keep work organized.</small></div></div><div class="silver30-card">${logo("sync")}<div><b>Resume on another device</b><small>Use Resume Center to save your workspace before switching devices.</small></div></div><div class="silver30-card">${logo("settings")}<div><b>Customize everything</b><small>Open Personalization or Desktop Customizer.</small></div></div></div>`+end(), "silver-help-30"); };
    window.openSilver30Search=function(){
        const html=hero("search","Silver Search","Search Silver apps and consolidated hubs.")+`<input id="silver30_search_box" placeholder="Search apps, files, settings, mail, office..." oninput="silver30SearchApps(this.value)"><div id="silver30_search_results" style="margin-top:10px">${categoryGrid(APPS30.slice(0,8))}</div>`+end();
        open("Silver Search",html,"silver-search-30");
    };
    window.silver30SearchApps=function(q){ const el=document.getElementById("silver30_search_results"); if(!el)return; q=String(q||"").toLowerCase(); const matches=APPS30.filter(a=>(a.name+a.desc+a.category).toLowerCase().includes(q)).slice(0,30); el.innerHTML=categoryGrid(matches.length?matches:APPS30.slice(0,8)); };
    window.openSilver30DesktopCustomizer=function(){
        const p=prefs(); const items=[...FOLDERS30.map(f=>({...f,type:"Folder"})),...APPS30.map(a=>({...a,type:"App"}))];
        const checks=items.map(i=>`<label style="display:flex;gap:8px;align-items:center;margin:7px 0"><input type="checkbox" data-silver30-desk="${esc(i.id)}" ${p.desktop.includes(i.id)?"checked":""}> ${logo(i.logo)} <span><b>${esc(i.name)}</b><small style="display:block;color:#587082">${esc(i.type)} — ${esc(i.desc||"")}</small></span></label>`).join("");
        open("Desktop Customizer", hero("personalize","Desktop Customizer","Show, hide, or restore Silver desktop folders and apps. Removed folders still stay available in All Silver Apps.")+`<div style="display:grid;grid-template-columns:1.2fr .8fr;gap:12px"><section>${checks}</section><section><h3>Layout</h3><label>Icon Size<select id="silver30_icon"><option ${p.iconSize==='normal'?'selected':''}>normal</option><option ${p.iconSize==='compact'?'selected':''}>compact</option><option ${p.iconSize==='large'?'selected':''}>large</option></select></label><br><br><label>Density<select id="silver30_density"><option ${p.density==='comfortable'?'selected':''}>comfortable</option><option ${p.density==='compact'?'selected':''}>compact</option><option ${p.density==='spacious'?'selected':''}>spacious</option></select></label><br><br><label><input type="checkbox" id="silver30_sidebar_toggle" ${p.sidebar?'checked':''}> Show sidebar gadgets</label><div class="silver30-toolbar"><button onclick="silver30SaveDesktopCustomizer()">Save</button><button onclick="silver30ResetDesktop()">Reset</button></div></section></div>`+end(), "silver-desktop-customizer-30");
    };
    window.silver30SaveDesktopCustomizer=function(){ const p=prefs(); p.desktop=[...document.querySelectorAll('[data-silver30-desk]:checked')].map(i=>i.dataset.silver30Desk); p.iconSize=document.getElementById("silver30_icon")?.value||"normal"; p.density=document.getElementById("silver30_density")?.value||"comfortable"; p.sidebar=!!document.getElementById("silver30_sidebar_toggle")?.checked; savePrefs(p); silver30Notice("Desktop", "Silver desktop layout saved.", "Desktop", "success"); };
    window.silver30RemoveDesktopItem=function(id){ const p=prefs(); p.desktop=p.desktop.filter(x=>x!==id); savePrefs(p); silver30Notice("Desktop", "Removed from desktop; still available in All Silver Apps.", "Desktop", "success"); };
    window.silver30ResetDesktop=function(){ write(KEY.prefs, defaults()); renderDesktop(); renderSidebar(); silver30Notice("Desktop", "Silver Beta 3.0 desktop restored.", "Desktop", "success"); };
    window.silver30SetSidebar=function(on){ const p=prefs(); p.sidebar=!!on; savePrefs(p); };

    function patchStart(){
        const btn=document.getElementById("start-btn"); if(btn){ btn.textContent="Silver"; btn.title="EmeraldOS Silver"; }
        const build=document.getElementById("emerald40-build-badge"); if(build) build.textContent="Silver Beta 3.0";
        const edition=document.getElementById("emerald40-edition-badge"); if(edition){ edition.textContent=""; edition.style.display="none"; }
        const results=document.getElementById("start-results");
        if(results) results.innerHTML = `<div class="start-item" onclick="openSilver30Workspace()">Silver Workspace</div><div class="start-item" onclick="openSilver30DriveOffice()">Silver Drive + Office</div><div class="start-item" onclick="openSilver30Communications()">Silver Communications</div><div class="start-item" onclick="openSilver30MediaCenter()">Silver Media Center</div><div class="start-item" onclick="openSilver30Creator()">Silver Creator Studio</div><div class="start-item" onclick="openSilver30ControlPanel()">Silver Control Panel</div><div class="start-item" onclick="openSilver30Apps()">All Silver Apps</div><div class="start-item" onclick="openSilver30Search()">Silver Search</div><hr><div class="start-item" onclick="restartOS?.()">Restart Silver</div><div class="start-item" onclick="logoutUser?.()">Log out</div>`;
    }
    function boot(){
        document.body.dataset.theme="silver-beta30";
        document.title="EmeraldOS Silver Beta 3.0";
        localStorage.setItem("silver_product_line", "EmeraldOS Silver");
        localStorage.setItem("silver30_separate_shell", "true");
        patchStart(); renderDesktop(); renderSidebar(); updateClock();
        setTimeout(()=>{ patchStart(); renderDesktop(); renderSidebar(); }, 700);
        setTimeout(()=>{ patchStart(); renderSidebar(); }, 1600);
        if(!localStorage.getItem("silver30_seen_welcome")){ silver30Notice("Welcome to Silver Beta 3.0", "A separate Vista-inspired Silver workspace is active.", "System", "success"); localStorage.setItem("silver30_seen_welcome","true"); }
    }
    if(document.readyState==="loading") document.addEventListener("DOMContentLoaded", boot); else boot();
})();
