...
/* =========================================================
   EmeraldOS Advanced 10
   Independent Windows 10-inspired EmeraldOS model.
   No dependency on EmeraldOS Silver shell files.
========================================================= */
(function(){
  const BUILD = {
    name: "EmeraldOS Advanced 10",
    version: "Independent Preview 1.0",
    prefix: "adv10_independent_",
    cloudPath: "advanced10Independent"
  };
  ...
  const ICONS = {
    home:["HM","logo-blue"], explorer:["EX","logo-blue"], office:["OF","logo-green"], docs:["DC","logo-green"], sheets:["SH","logo-green"], slides:["SL","logo-orange"], forms:["FM","logo-purple"], mail:["ML","logo-teal"], chat:["CH","logo-purple"], people:["PE","logo-pink"], store:["ST","logo-blue"], creator:["CR","logo-purple"], settings:["SE","logo-gray"], control:["CP","logo-gray"], action:["AC","logo-orange"], taskview:["TV","logo-blue"], search:["SR","logo-blue"], widgets:["WG","logo-teal"], timeline:["TL","logo-blue"], restore:["RS","logo-teal"], update:["UP","logo-blue"], device:["DL","logo-teal"], security:["SC","logo-red"], media:["MD","logo-orange"], help:["?","logo-gray"], profile:["US","logo-pink"], app:["AP","logo-purple"]
  };

  const APPS = [
    {id:"home",name:"Advanced Home",group:"Workspace",icon:"home",desktop:true,tile:true,desc:"Dashboard, quick actions, status, and recent activity.",open:()=>openHome()},
    ...
    {id:"office",name:"Advanced Office",group:"Productivity",icon:"office",desktop:true,tile:true,desc:"Docs, Sheets, Slides, Forms, templates, and Drive-style workspace.",open:()=>openOffice()},
    {id:"mail",name:"Advanced Mail",group:"Communication",icon:"mail",desktop:true,tile:true,desc:"Inbox, sent, drafts, trash, compose, and unread notifications.",open:()=>openMail()},
    {id:"chat",name:"Advanced Chat",group:"Communication",icon:"chat",desktop:true,desc:"Rooms, direct messages, unread alerts, and chat history.",open:()=>openChat()},
    ...
    {id:"search",name:"Advanced Search",group:"System",icon:"search",desc:"Search apps, settings, files, people, mail, and documents.",open:()=>openSearchWindow()},
    ...
    {id:"help",name:"Help and Support",group:"Support",icon:"help",desc:"Guides, keyboard shortcuts, troubleshooting, and about Advanced 10.",open:()=>openHelp()}
  ];
  ...
  function username(){return localStorage.getItem("username") || localStorage.getItem("40_username") || "AdvancedUser";}
  function mailAddress(){return username().toLowerCase().replace(/[^a-z0-9._-]/g,".") + "@advanced10.mail";}
  function now(){return new Date().toISOString();}
  function fmt(iso){try{return new Date(iso).toLocaleString();}catch{return "";}}
  function readJSON(key,fallback){try{return JSON.parse(localStorage.getItem(key) || JSON.stringify(fallback));}catch{return fallback;}}
  function writeJSON(key,val){localStorage.setItem(key,JSON.stringify(val));}

  function prefs(){return readJSON(LS.prefs,{theme:"dark",accent:"#0078d4",desktopIcons:["home","explorer","office","mail","chat","people","store","creator","settings","restore"],iconSize:"normal",density:"normal",wallpaper:"default",taskbarLabels:true,restoreOnBoot:true,notifications:true,focusAssist:false,widgets:true,showSeconds:false});}
  ...
  function seedFiles(){return [
    ...
    {id:id(),name:"Welcome to Advanced 10.txt",type:"text",parent:null,star:true,trash:false,content:"Welcome to EmeraldOS Advanced 10. This is an independent Windows 10-inspired EmeraldOS model with its own shell, settings, apps, restore system, and local/cloud workspace.",created:now(),updated:now()},
    {id:id(),name:"Getting Started.edoc",type:"edoc",parent:null,star:false,trash:false,content:"Advanced 10 Getting Started\n\nOpen Start, Search, Task View, Action Center, File Explorer, Office, Mail, Chat, Store, and Creator Studio.",created:now(),updated:now()}
  ];}
  function seedDocs(){return [{id:id(),name:"Project Notes.edoc",content:"Project Notes\n\nUse Advanced Office to edit documents, sheets, slides, and forms.",updated:now(),star:true},{id:id(),name:"Announcement Draft.edoc",content:"Announcement\n\nEmeraldOS Advanced 10 is ready for testing.",updated:now(),star:false}];}
  function seedSheets(){return [{id:id(),name:"Budget.esheet",cells:[["Item","Cost","Status"],["Hosting","0","Free"],["Storage","0","Testing"]],updated:now()}];}
  function seedSlides(){return [{id:id(),name:"Advanced 10 Overview.eslide",slides:[{title:"EmeraldOS Advanced 10",body:"Independent shell, modern taskbar, Action Center, Task View, Settings, Explorer, Office, Mail, and Restore."}],updated:now()}];}
  ...
  function seedMail(){return [
    {id:id(),folder:"inbox",from:"system@advanced10.mail",to:mailAddress(),subject:"Welcome to Advanced Mail",body:"This internal mail app supports inbox, sent, drafts, trash, reply, unread status, and notifications.",read:false,date:now()},
    {id:id(),folder:"sent",from:mailAddress(),to:"demo@advanced10.mail",subject:"Test Sent Message",body:"This is a sample sent email.",read:true,date:now()}
  ];}
  function seedChat(){return {rooms:[{id:"general",name:"General",messages:[{id:id(),from:"System",body:"Welcome to Advanced Chat.",date:now()}]}],active:"general"};}
  function seedPeople(){return [{id:id(),username:username(),displayName:username(),favorite:true,blocked:false,status:"Available",mail:mailAddress()},{id:id(),username:"system",displayName:"Advanced System",favorite:true,blocked:false,status:"Online",mail:"system@advanced10.mail"},{id:id(),username:"demo",displayName:"Demo User",favorite:false,blocked:false,status:"Away",mail:"demo@advanced10.mail"}];}
  function seedApps(){return [{id:id(),name:"Sample Dashboard",label:"DASH",description:"A sample installed user app.",code:"api.setTitle('Sample Dashboard'); api.write('<h2>Sample Dashboard</h2><p>This is an installed Advanced 10 user app.</p>'); api.button('Notify',()=>api.notify('Sample Dashboard','Custom app notification works.'));",installed:now(),permissions:["notifications","localStorage"]}];}
  ...
  function notify(title,body,source="Advanced 10",level="info"){
  ...
  function renderDesktop(){
    ...
    desk.innerHTML = APPS.filter(a=>p.desktopIcons.includes(a.id)).map(a=>`
      <div tabindex="0" class="desktop-icon" ondblclick="Advanced10.openApp('${a.id}')" onclick="this.blur()" title="${esc(a.desc)}">
        ${logoHTML(a.icon)}
  ...
  function renderStart(query=""){
    ...
    if(startResults) startResults.innerHTML = matches.map(a=>`<div class="start-result" onclick="Advanced10.openApp('${a.id}')">${logoHTML(a.icon)}<div><b>${esc(a.name)}</b><small>${esc(a.group)} — ${esc(a.desc)}</small></div></div>`).join("");
    const tileGrid=$("start-tiles-grid");
    if(tileGrid) tileGrid.innerHTML = APPS.filter(a=>a.tile).map(a=>`<div class="tile" onclick="Advanced10.openApp('${a.id}')">${logoHTML(a.icon)}<b>${esc(a.name)}</b></div>`).join("");
  }
  ...
  function openWindow(appId,title,html,opts={}){
    ...
    el.innerHTML=`
      <div class="titlebar" data-drag="${winId}">${logoHTML(app.icon)}<div class="titlebar-title">${esc(title)}</div><div class="win-controls"><button onclick="Advanced10.minimizeWindow('${winId}')">—</button><button onclick="Advanced10.maximizeWindow('${winId}')">□</button><button class="close" onclick="Advanced10.closeWindow('${winId}')">×</button></div></div>
  ...
  function renderTaskbarApps(){const bar=$("taskbar-apps"); if(!bar) return; bar.innerHTML=state.windows.map(w=>`<button class="taskbar-app ${w.id===state.activeId?'active':''} ${w.min?'minimized':''}" onclick="Advanced10.restoreWindow('${w.id}')" title="${esc(w.title)}">${esc(w.title)}</button>`).join("");}
  ...
  function card(appId,title,body,icon="app"){return `<div class="card" onclick="Advanced10.openApp('${appId}')">${logoHTML(icon)}<h3>${esc(title)}</h3><small>${esc(body)}</small></div>`;}

  function openHome(){
    const recent=session().recentApps || []; const unread=notifications().filter(n=>!n.read).length; const f=files().filter(x=>!x.trash);
    openWindow("home","Advanced Home",`${header("home","Advanced Home","Your independent Advanced 10 cloud desktop.")}
      <div class="grid">
      ...
      <div class="toolbar"><button class="btn primary" onclick="Advanced10.openApp('explorer')">Open Explorer</button><button class="btn" onclick="Advanced10.openApp('office')">Open Office</button><button class="btn" onclick="Advanced10.openApp('mail')">Open Mail</button><button class="btn" onclick="Advanced10.openApp('settings')">Open Settings</button><button class="btn" onclick="Advanced10.saveWorkspaceNow()">Save Workspace</button></div>
  ...
  function openExplorer(filter="all"){
    ...
    openWindow("explorer","File Explorer",`${header("explorer","File Explorer","Independent file workspace with folders, previews, stars, trash, and restore.")}
      <div class="toolbar"><button class="btn primary" onclick="Advanced10.newFile()">New Text File</button><button class="btn" onclick="Advanced10.newFolder()">New Folder</button><button class="btn" onclick="Advanced10.openExplorer('all')">My Files</button><button class="btn" onclick="Advanced10.openExplorer('recent')">Recent</button><button class="btn" onclick="Advanced10.openExplorer('starred')">Starred</button><button class="btn" onclick="Advanced10.openExplorer('trash')">Trash</button><input id="fileSearch" placeholder="Search files" oninput="Advanced10.filterFiles(this.value)"></div>
      <div class="split"><aside class="sidebar"><b>Quick access</b><div class="list-item" onclick="Advanced10.openExplorer('all')">My Files</div><div class="list-item" onclick="Advanced10.openExplorer('starred')">Starred</div><div class="list-item" onclick="Advanced10.openExplorer('trash')">Trash</div><hr><button class="btn" onclick="Advanced10.exportBackup()">Export Backup</button><button class="btn" onclick="Advanced10.openApp('restore')">Restore Center</button></aside><section><div id="fileList">${fileListHTML(visible)}</div></section></div>${end()}`,{width:980,height:640});
  }
  function fileListHTML(list){return list.length?list.map(f=>`<div class="list-item file-row" data-name="${esc(f.name.toLowerCase())}">${logoHTML(f.type==="folder"?"explorer":f.type==="edoc"?"docs":"office")}<div style="flex:1"><b>${esc(f.name)}</b><small>${esc(f.type)} · updated ${fmt(f.updated)}</small></div><button class="btn" onclick="Advanced10.previewFile('${f.id}')">Open</button><button class="btn" onclick="Advanced10.renameFile('${f.id}')">Rename</button><button class="btn" onclick="Advanced10.toggleStar('${f.id}')">${f.star?'Unstar':'Star'}</button>${f.trash?`<button class="btn" onclick="Advanced10.restoreFile('${f.id}')">Restore</button><button class="btn danger" onclick="Advanced10.deleteForever('${f.id}')">Delete Forever</button>`:`<button class="btn danger" onclick="Advanced10.trashFile('${f.id}')">Trash</button>`}</div>`).join(""):`<div class="notice">No files here yet.</div>`;}
  ...
  function openOffice(tab="home"){
    openWindow("office","Advanced Office",`${header("office","Advanced Office","Drive-style Docs, Sheets, Slides, Forms, templates, and vault.")}
      <div class="toolbar"><button class="btn primary" onclick="Advanced10.openDocEditor()">New Doc</button><button class="btn" onclick="Advanced10.openSheets()">Sheets</button><button class="btn" onclick="Advanced10.openSlides()">Slides</button><button class="btn" onclick="Advanced10.openForms()">Forms</button><button class="btn" onclick="Advanced10.openTemplates()">Templates</button><button class="btn" onclick="Advanced10.openExplorer()">Open Drive</button></div>
      <h3>Recent documents</h3><div class="grid">${docs().map(d=>`<div class="card"><h3>${esc(d.name)}</h3><small>Updated ${fmt(d.updated)}</small><div class="toolbar"><button class="btn" onclick="Advanced10.openDocEditor('${d.id}')">Open</button><button class="btn" onclick="Advanced10.copyDocToFiles('${d.id}')">Save to Explorer</button></div></div>`).join("")}</div>
  ...
  function openDocEditor(docId=""){const d=docs().find(x=>x.id===docId)||{id:id(),name:"Untitled Document.edoc",content:"",updated:now()};
    openWindow("doc_"+d.id,d.name,`${header("docs","Advanced Docs","Document editor with autosave, export, templates, print, and word count.")}
      <div class="toolbar"><input id="doc_name" value="${esc(d.name)}"><button class="btn primary" onclick="Advanced10.saveDoc('${d.id}')">Save</button><button class="btn" onclick="Advanced10.insertDocTemplate()">Template</button><button class="btn" onclick="Advanced10.insertDate('doc_content')">Date</button><button class="btn" onclick="Advanced10.downloadText((document.getElementById('doc_name').value||'document')+'.txt',document.getElementById('doc_content').value)">Export TXT</button><button class="btn" onclick="Advanced10.exportDocHTML()">Export HTML</button><button class="btn" onclick="window.print()">Print</button><span class="pill" id="doc_count">0 words</span></div>
      <div class="doc-page"><textarea id="doc_content" class="editor" oninput="Advanced10.docCount()">${esc(d.content)}</textarea></div>${end()}`,{multi:true,width:900,height:690}); setTimeout(docCount,30);}
  function saveDoc(docId){let list=docs(); let d=list.find(x=>x.id===docId); const name=$("doc_name")?.value||"Untitled Document.edoc"; const content=$("doc_content")?.value||""; if(d){d.name=name;d.content=content;d.updated=now();} else {d={id:docId,name,content,updated:now(),star:false}; list.unshift(d);} setDocs(list); notify("Document saved",name,"Advanced Office","success");}
  ...
  function openSheets(){const s=sheets()[0]; openWindow("sheets","Advanced Sheets",`${header("sheets","Advanced Sheets","Editable spreadsheet grid with totals and CSV export.")}
    <div class="toolbar"><input id="sheet_name" value="${esc(s.name)}"><button class="btn" onclick="Advanced10.addSheetRow()">Add Row</button><button class="btn primary" onclick="Advanced10.saveSheet('${s.id}')">Save</button><button class="btn" onclick="Advanced10.sheetTotal()">Total Col B</button><button class="btn" onclick="Advanced10.exportCSV('${s.id}')">Export CSV</button></div><table class="sheet"><tbody id="sheet_body">${sheetRows(s.cells)}</tbody></table>${end()}`,{width:820,height:560});}
  ...
  function openSlides(){const deck=slides()[0]; const sl=deck.slides[0]; openWindow("slides","Advanced Slides",`${header("slides","Advanced Slides","Presentation editor with slide preview and export.")}
    <div class="toolbar"><input id="slide_title" value="${esc(sl.title)}"><button class="btn primary" onclick="Advanced10.saveSlide('${deck.id}')">Save</button><button class="btn" onclick="Advanced10.presentSlide()">Present</button></div><textarea id="slide_body" class="editor">${esc(sl.body)}</textarea><div class="slide-preview"><div><h1>${esc(sl.title)}</h1><p>${esc(sl.body)}</p></div></div>${end()}`,{width:900,height:660});}
  ...
  function openForms(){const f=forms()[0]; openWindow("forms","Advanced Forms",`${header("forms","Advanced Forms","Build simple forms and store responses locally.")}
    <div class="toolbar"><input id="form_name" value="${esc(f.name)}"><button class="btn primary" onclick="Advanced10.saveForm('${f.id}')">Save Form</button><button class="btn" onclick="Advanced10.previewForm('${f.id}')">Preview</button></div><textarea id="form_questions" class="editor">${esc(f.questions.join("\n"))}</textarea>${end()}`);}
  ...
  function previewForm(fid){const f=forms().find(x=>x.id===fid); if(!f) return; openWindow("formprev_"+fid,"Form Preview",`${header("forms",f.name,"Preview form")}${f.questions.map((q,i)=>`<label>${esc(q)}<input class="field" id="form_resp_${i}" style="width:100%;margin:6px 0 12px"></label>`).join("")}<button class="btn primary" onclick="Advanced10.submitForm('${fid}')">Submit response</button>${end()}`,{multi:true});}
  ...
  function openTemplates(){openWindow("templates","Templates",`${header("office","Templates","Start from a formatted document template.")}<div class="grid">${["Letter","Memo","Report","Policy","Meeting Notes","Proposal","Announcement","Resume"].map(t=>`<div class="card" onclick="Advanced10.openDocEditor();setTimeout(()=>{document.getElementById('doc_name').value='${t}.edoc';document.getElementById('doc_content').value='${t}\n\nPrepared by ${username()}\n\n';Advanced10.docCount();},80)"><h3>${t}</h3><small>Create a ${t.toLowerCase()} document.</small></div>`).join("")}</div>${end()}`);}

  function openMail(folder="inbox"){
    const messages=mail().filter(m=>m.folder===folder);
    openWindow("mail","Advanced Mail",`${header("mail","Advanced Mail",`${mailAddress()} · Inbox, sent, drafts, trash, compose, and reply.`)}
      <div class="toolbar"><button class="btn primary" onclick="Advanced10.composeMail()">Compose</button><button class="btn" onclick="Advanced10.openMail('inbox')">Inbox</button><button class="btn" onclick="Advanced10.openMail('sent')">Sent</button><button class="btn" onclick="Advanced10.openMail('drafts')">Drafts</button><button class="btn" onclick="Advanced10.openMail('trash')">Trash</button></div>
      <div class="split"><aside class="sidebar"><b>Folders</b>${["inbox","sent","drafts","trash"].map(f=>`<div class="list-item" onclick="Advanced10.openMail('${f}')">${f[0].toUpperCase()+f.slice(1)} <span class="pill">${mail().filter(m=>m.folder===f&&!m.read).length}</span></div>`).join("")}</aside><section>${messages.map(m=>`<div class="list-item"><div style="flex:1"><b>${m.read?'':'● '} ${esc(m.subject)}</b><small>From ${esc(m.from)} · ${fmt(m.date)}</small></div><button class="btn" onclick="Advanced10.readMail('${m.id}')">Open</button><button class="btn danger" onclick="Advanced10.deleteMail('${m.id}')">Delete</button></div>`).join("") || `<div class="notice">No mail in ${folder}.</div>`}</section></div>${end()}`,{width:930,height:630});
  }
  function composeMail(to="",subject="",body="") {openWindow("compose_"+id(),"Compose Mail",`${header("mail","Compose Mail","Send internal Advanced Mail.")}<div class="toolbar"><input id="mail_to" placeholder="To" value="${esc(to)}"><input id="mail_subject" placeholder="Subject" value="${esc(subject)}"></div><textarea id="mail_body" class="editor">${esc(body)}</textarea><div class="toolbar"><button class="btn primary" onclick="Advanced10.sendMail()">Send</button><button class="btn" onclick="Advanced10.saveDraftMail()">Save Draft</button></div>${end()}`,{multi:true,width:760,height:560});}
  function sendMail(){const list=mail(); const to=$("mail_to")?.value||"user@advanced10.mail"; const subject=$("mail_subject")?.value||"No subject"; const body=$("mail_body")?.value||""; list.unshift({id:id(),folder:"sent",from:mailAddress(),to,subject,body,read:true,date:now()}); setMail(list); notify("Mail sent",subject,"Mail"); openMail("sent");}
  ...
  function readMail(mid){const list=mail(); const m=list.find(x=>x.id===mid); if(!m) return; m.read=true; setMail(list); openWindow("mail_"+mid,m.subject,`${header("mail",m.subject,`From ${m.from} to ${m.to}`)}<p class="muted">${fmt(m.date)}</p><div class="card" style="white-space:pre-wrap">${esc(m.body)}</div><div class="toolbar"><button class="btn primary" onclick="Advanced10.composeMail('${esc(m.from)}','Re: ${esc(m.subject)}','\n\n--- Original ---\n${esc(m.body).replace(/\n/g,"\\n")}')">Reply</button><button class="btn danger" onclick="Advanced10.deleteMail('${mid}')">Delete</button></div>${end()}`,{multi:true}); openMail(m.folder);}
  ...
  function openChat(){const data=chat(); const room=data.rooms.find(r=>r.id===data.active)||data.rooms[0]; openWindow("chat","Advanced Chat",`${header("chat","Advanced Chat","Rooms, direct messages, unread alerts, and chat history.")}
    <div class="split"><aside class="sidebar"><button class="btn primary" onclick="Advanced10.newChatRoom()">New Room</button>${data.rooms.map(r=>`<div class="list-item" onclick="Advanced10.setChatRoom('${r.id}')">${esc(r.name)}</div>`).join("")}</aside><section><h3>${esc(room.name)}</h3><div id="chat_messages" style="height:300px;overflow:auto;border:1px solid #dce5ef;padding:8px;background:#fff">${room.messages.map(m=>`<div class="list-item"><b>${esc(m.from)}</b><span>${esc(m.body)}</span><small>${fmt(m.date)}</small></div>`).join("")}</div><div class="toolbar"><input id="chat_input" placeholder="Message"><button class="btn primary" onclick="Advanced10.sendChat()">Send</button></div></section></div>${end()}`,{width:860,height:590});}
  ...
  function openPeople(){const list=people(); openWindow("people","People",`${header("people","People","Contacts, user directory, favorites, and blocking.")}<div class="toolbar"><button class="btn primary" onclick="Advanced10.addContact()">Add Contact</button><input id="peopleSearch" placeholder="Search people" oninput="Advanced10.filterPeople(this.value)"></div><div>${list.map(p=>`<div class="list-item person-row" data-name="${esc((p.username+p.displayName+p.mail).toLowerCase())}">${logoHTML("people")}<div style="flex:1"><b>${esc(p.displayName)}</b><small>${esc(p.username)} · ${esc(p.status)} · ${esc(p.mail)}</small></div><button class="btn" onclick="Advanced10.composeMail('${esc(p.mail)}')">Mail</button><button class="btn" onclick="Advanced10.toggleFavorite('${p.id}')">${p.favorite?'Unfavorite':'Favorite'}</button><button class="btn danger" onclick="Advanced10.toggleBlock('${p.id}')">${p.blocked?'Unblock':'Block'}</button></div>`).join("")}</div>${end()}`);}
  function addContact(){const u=prompt("Username","newuser"); if(!u) return; const list=people(); list.push({id:id(),username:u,displayName:u,favorite:false,blocked:false,status:"Unknown",mail:u+"@advanced10.mail"}); setPeople(list); openPeople();}
  ...
  function openStore(){
    ...
    if(!accepted) return openWindow("storeWarning","Store Security Notice",`${header("store","Store Security Notice","User applications can be risky.")}<div class="notice"><b>Warning! By using this feature, you expose yourself to risk of infection. Use at your own risk.</b><br><br>User-created applications may contain unsafe or unstable code. Only install apps from users you trust.</div><div class="toolbar"><button class="btn primary" onclick="localStorage.setItem('${BUILD.prefix}storeWarningAccepted','true');Advanced10.openApp('store')">Agree and Continue</button><button class="btn" onclick="Advanced10.closeWindow('storeWarning')">Cancel</button></div>${end()}`);
    const apps=userApps();
    openWindow("store","Store",`${header("store","Store","User apps, installed packages, publishing, and app safety.")}<div class="toolbar"><button class="btn primary" onclick="Advanced10.openApp('creator')">Create App</button><button class="btn" onclick="Advanced10.installDemoApp()">Install Demo App</button><button class="btn" onclick="localStorage.removeItem('${BUILD.prefix}storeWarningAccepted');Advanced10.openApp('store')">Reset Warning</button></div><div class="grid">${apps.map(a=>`<div class="card"><h3>${esc(a.name)}</h3><small>${esc(a.description||"User application")}</small><p><span class="pill">${(a.permissions||[]).join(", ")||"No permissions"}</span></p><button class="btn primary" onclick="Advanced10.runUserApp('${a.id}')">Run</button><button class="btn" onclick="Advanced10.exportUserApp('${a.id}')">Export .eapp</button><button class="btn danger" onclick="Advanced10.deleteUserApp('${a.id}')">Uninstall</button></div>`).join("")}</div>${end()}`,{width:920,height:620});
  }
  function openCreator(){openWindow("creator","Creator Studio",`${header("creator","Creator Studio","Application Editor, Code Studio, templates, API docs, and .eapp packages.")}<div class="toolbar"><input id="app_name" placeholder="Application name" value="My Advanced App"><input id="app_label" placeholder="Icon label" value="APP"><button class="btn" onclick="Advanced10.loadAppTemplate()">Template</button><button class="btn primary" onclick="Advanced10.saveCustomApp()">Save App</button><button class="btn" onclick="Advanced10.previewCustomApp()">Run Preview</button></div><textarea id="app_code" class="editor" style="font-family:Consolas,monospace;min-height:320px">api.setTitle('My Advanced App');\napi.write('<h2>Hello from Advanced 10</h2><p>This app was created in Creator Studio.</p>');\napi.button('Send Notification',()=>api.notify('My Advanced App','Notifications work from custom apps.'));</textarea><details open><summary>Custom App API</summary><pre>api.write(html)\napi.button(label, handler)\napi.notify(title, message)\napi.save(key, data)\napi.load(key)\napi.download(filename, content)\napi.prompt(message)\napi.confirm(message)\napi.getUsername()</pre></details>${end()}`,{width:940,height:720});}
  ...
  function openSettings(){const p=prefs(); openWindow("settings","Settings",`${header("settings","Settings","Modern Advanced 10 settings for system, personalization, apps, and privacy.")}<div class="split"><aside class="sidebar"><b>Settings</b>${["System","Personalization","Apps","Accounts","Time & language","Ease of Access","Privacy","Update & Security"].map(x=>`<div class="list-item">${x}</div>`).join("")}</aside><section><h3>Personalization</h3><div class="toolbar"><label>Theme <select id="set_theme"><option value="dark">Dark</option><option value="light">Light</option><option value="dark-windows">Dark Windows</option><option value="contrast">High Contrast</option></select></label><label>Accent <input id="set_accent" type="color" value="${esc(p.accent)}"></label><button class="btn primary" onclick="Advanced10.saveSettings()">Apply</button></div><h3>Desktop icons</h3><div class="grid">${APPS.map(a=>`<label class="card"><input type="checkbox" class="iconToggle" value="${a.id}" ${p.desktopIcons.includes(a.id)?"checked":""}> ${logoHTML(a.icon)} <b>${esc(a.name)}</b><br><small>${esc(a.group)}</small></label>`).join("")}</div><h3>System</h3><div class="toolbar"><button class="btn" onclick="Advanced10.openApp('restore')">Restore Center</button><button class="btn" onclick="Advanced10.openApp('security')">Security Center</button><button class="btn" onclick="Advanced10.resetDesktop()">Reset Desktop</button></div></section></div>${end()}`,{width:980,height:690}); setTimeout(()=>{$("set_theme").value=p.theme;},10);}
  function saveSettings(){const icons=[...document.querySelectorAll(".iconToggle:checked")].map(x=>x.value); setPrefs({theme:$("set_theme")?.value||"dark",accent:$("set_accent")?.value||"#0078d4",desktopIcons:icons}); notify("Settings applied","Advanced 10 personalization updated.","Settings");}
  function resetDesktop(){if(confirm("Restore default desktop icons?")){setPrefs({desktopIcons:["home","explorer","office","mail","chat","people","store","creator","settings","restore"]});}}
  function openControlPanel(){openWindow("control","Control Panel",`${header("control","Control Panel","Classic Advanced 10 management shortcuts.")}<div class="grid">${["settings","security","restore","update","device","explorer","store","creator"].map(i=>{const a=appById(i);return card(a.id,a.name,a.desc,a.icon)}).join("")}</div>${end()}`);}
  function openActionCenterWindow(){openWindow("action","Action Center",actionHTML());}
  function actionHTML(){const list=notifications(); return `${header("action","Action Center","Universal notifications and quick actions.")}<div class="quick-grid"><button onclick="Advanced10.toggleFocusAssist()">Focus Assist: ${prefs().focusAssist?'On':'Off'}</button><button onclick="Advanced10.openApp('settings')">All Settings</button><button onclick="Advanced10.openApp('restore')">Sync</button><button onclick="Advanced10.openApp('security')">Security</button></div><div class="toolbar"><button class="btn" onclick="Advanced10.markAllRead()">Mark all read</button><button class="btn danger" onclick="Advanced10.clearNotifications()">Clear all</button></div>${list.map(n=>`<div class="list-item"><div style="flex:1"><b>${n.read?'':'● '} ${esc(n.title)}</b><small>${esc(n.source)} · ${fmt(n.date)}</small><p>${esc(n.body)}</p></div><button class="btn" onclick="Advanced10.markNotification('${n.id}')">Read</button></div>`).join("") || `<div class="notice">No notifications.</div>`}${end()}`;}
  ...
  function openSearch(){closeFlyouts("search"); const sp=$("search-panel"); sp.innerHTML=`<div class="panel-title"><h2>Search</h2><button class="panel-btn" onclick="Advanced10.closeFlyouts()">Close</button></div><input class="field" style="width:100%" placeholder="Search apps, files, documents, people, mail" oninput="Advanced10.doGlobalSearch(this.value)"><div id="global_results"></div>`; sp.classList.toggle("hidden");}
  function openSearchWindow(){openWindow("search","Advanced Search",`${header("search","Advanced Search","Search apps, files, documents, people, and mail.")}<input class="field" style="width:100%" placeholder="Search" oninput="Advanced10.doGlobalSearch(this.value,'window_results')"><div id="window_results"></div>${end()}`);}
  function doGlobalSearch(q,target="global_results"){const query=String(q||"").toLowerCase(); const results=[]; APPS.forEach(a=>{if((a.name+a.desc+a.group).toLowerCase().includes(query)) results.push({type:"App",name:a.name,sub:a.desc,action:`Advanced10.openApp('${a.id}')`,icon:a.icon});}); files().forEach(f=>{if((f.name+(f.content||"")).toLowerCase().includes(query)) results.push({type:"File",name:f.name,sub:f.type,action:`Advanced10.previewFile('${f.id}')`,icon:"explorer"});}); docs().forEach(d=>{if((d.name+d.content).toLowerCase().includes(query)) results.push({type:"Document",name:d.name,sub:"Advanced Docs",action:`Advanced10.openDocEditor('${d.id}')`,icon:"docs"});}); people().forEach(p=>{if((p.username+p.displayName+p.mail).toLowerCase().includes(query)) results.push({type:"Person",name:p.displayName,sub:p.mail,action:`Advanced10.openApp('people')`,icon:"people"});}); const el=$(target); if(el) el.innerHTML = query? results.slice(0,40).map(r=>`<div class="list-item" onclick="${r.action}">${logoHTML(r.icon)}<div><b>${esc(r.name)}</b><small>${esc(r.type)} · ${esc(r.sub)}</small></div></div>`).join("") || `<div class="notice">No results.</div>` : `<div class="notice">Start typing to search.</div>`;}
  ...
  function taskViewHTML(){return `<div class="panel-title"><h2>Task View</h2><button class="panel-btn" onclick="Advanced10.closeAllWindows()">Close all</button></div><h3>Open windows</h3>${state.windows.map(w=>`<div class="list-item"><div style="flex:1"><b>${esc(w.title)}</b><small>${w.min?'Minimized':'Open'}</small></div><button class="btn" onclick="Advanced10.restoreWindow('${w.id}')">Open</button><button class="btn danger" onclick="Advanced10.closeWindow('${w.id}')">Close</button></div>`).join("") || `<div class="notice">No open windows.</div>`}<h3>Virtual desktops</h3><div class="grid"><div class="card"><h3>Desktop 1</h3><small>Main workspace</small></div><div class="card"><h3>Desktop 2</h3><small>Planned workspace</small></div></div>`;}
  ...
  function openRestore(){const s=session(); openWindow("restore","Restore Center",`${header("restore","Restore Center","Save, restore, export, import, and repair your Advanced 10 workspace.")}<div class="grid"><div class="card"><h3>Last saved</h3><small>${s.lastSaved?fmt(s.lastSaved):"Not saved yet"}</small></div><div class="card"><h3>Last restored</h3><small>${s.lastRestored?fmt(s.lastRestored):"Not restored yet"}</small></div><div class="card"><h3>Cloud path</h3><small>emeraldOSUsers/${esc(username())}/${BUILD.cloudPath}/current</small></div></div><div class="toolbar"><button class="btn primary" onclick="Advanced10.saveWorkspaceNow()">Save Workspace Now</button><button class="btn" onclick="Advanced10.loadCloudWorkspace()">Restore from Cloud</button><button class="btn" onclick="Advanced10.loadLocalWorkspace()">Restore Local</button><button class="btn" onclick="Advanced10.exportBackup()">Export Backup</button><button class="btn" onclick="Advanced10.importBackupPrompt()">Import Backup</button><button class="btn danger" onclick="Advanced10.resetAllAdvanced10()">Factory Reset</button></div>${end()}`,{width:870,height:560});}
  function exportBackup(){downloadText("emeraldos-advanced10-backup.json",JSON.stringify(collectWorkspace(),null,2));}
  ...
  function resetAllAdvanced10(){if(!confirm("Reset all Advanced 10 local data?")) return; Object.values(LS).forEach(k=>localStorage.removeItem(k)); location.reload();}
  function openUpdateCenter(){openWindow("update","Update Center",`${header("update","Update Center","Maintenance, version notes, and update checks.")}<div class="card"><h3>${BUILD.name}</h3><small>${BUILD.version}</small><p>Independent Advanced 10 shell with modern desktop, settings, Explorer, Office, Mail, Chat, Store, Creator Studio, Action Center, Task View, Widgets, Restore, and cloud workspace.</p></div><button class="btn primary" onclick="Advanced10.notify('System up to date','No updates available in this local package.','Update Center')">Check for updates</button>${end()}`);}
  ...
  function openProfile(){openWindow("profile","User Profile",`${header("profile","User Profile","Account identity and Advanced 10 status.")}<div class="grid"><div class="card"><h3>${esc(username())}</h3><small>Username</small></div><div class="card"><h3>${esc(mailAddress())}</h3><small>Advanced Mail address</small></div><div class="card"><h3>${esc(BUILD.name)}</h3><small>${esc(BUILD.version)}</small></div></div>${end()}`);}
  function openHelp(){openWindow("help","Help and Support",`${header("help","Help and Support","Guides, shortcuts, troubleshooting, and about Advanced 10.")}<div class="grid"><div class="card"><h3>Start</h3><small>Use Start to open apps, settings, and power tools.</small></div><div class="card"><h3>Explorer</h3><small>Create, rename, preview, star, trash, restore, and export files.</small></div><div class="card"><h3>Restore</h3><small>Save workspace locally or to Firebase and resume across devices.</small></div><div class="card"><h3>Shortcuts</h3><small>Ctrl+Space Search · Ctrl+Shift+P Start · Alt+Tab Task View</small></div></div><p class="notice">This is a Windows 10-inspired original EmeraldOS model. It does not use Microsoft assets.</p>${end()}`);}
