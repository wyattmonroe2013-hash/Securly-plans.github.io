(() => {
"use strict";
const GOLD_VERSION = "T.U.2";
const GOLD_FOLDER = "Gold_T.U.2";
const IS_TU2 = true;
...
let vm = {owner:shellUser.username, activeVersion:GOLD_VERSION, setup:{complete:false}, preferences:{accent:"#1677c8", backgroundColor:"#e7edf4", iconSize:84, labels:true}, files:[], notes:[], tickets:[], userApps:[], windows:[], versionHistory:[]};
let z = 100;
let openWins = [];
...
function iconSvg(name){
  ...
  const text=map[name]||"◆";
  return `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" fill="#fff"/><rect x="5" y="5" width="54" height="54" rx="12" fill="#f8fbff" stroke="#1677c8" stroke-width="3"/><circle cx="48" cy="16" r="7" fill="#c99a2e"/><text x="32" y="39" text-anchor="middle" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="#12344d">${text}</text></svg>`)}`;
}
...
function applyPrefs(){
  ...
  document.documentElement.style.setProperty("--accent", p.accent || "#1677c8");
...
function openSettings(){
  const p=vm.preferences||{};
  const w=openWindow("Settings", `<h2>Settings</h2><div class="grid"><label class="field">Accent color<input id="setAccent" type="color" value="${p.accent||'#1677c8'}"></label><label class="field">Background color<input id="setBg" type="color" value="${p.backgroundColor||'#e7edf4'}"></label><label class="field">Background image URL<input id="setBgUrl" value="${p.backgroundUrl||''}" placeholder="https://..."></label><label class="field">Icon size<input id="setIcon" type="range" min="70" max="120" value="${p.iconSize||84}"></label></div><div class="toolbar"><button id="saveSettings">Apply settings</button><button id="resetSetup">Run setup again</button></div>`, {width:640});
...
function finishSetup(){
  const setup=$("setup");
  const accent=setup.querySelector("#setupAccent")?.value || "#1677c8";
